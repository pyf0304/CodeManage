...
namespace GetAccessFromDataTable
{
    ...
    public partial class clsGetAccessFromDataTable
    {
            ...
            string strSheetName)
        {
            ...
            {
                throw new Exception(string.Format("导出初始文件：{0}不存在，请与管理员联系！(errid:access0003)", SavePathAndName));
            }
            ...
            if (intRowNum == 0)
            {
                throw new Exception("需要导出的记录数为0，不需要导出！(errid:access0001)");
            }
            ...
            using (OleDbConnection con = new OleDbConnection(connString))
            {
                ...
                catch (Exception objException)
                {
                    ...
                    sbMsg.AppendFormat("(errid:BlEx000036)打开连接出错。(clsGetAccessFromDataTable:GetAccessFromDataTableByArr4OdbcAccess2007)连接串：{0}，出错信息：{1}。",
                        connString,
                ...
                catch (Exception objException)
                {
                    ...
                    StringBuilder sbMsg = new StringBuilder();
                    sbMsg.AppendFormat("(errid:BlEx000037)导出Access出错。(clsGetAccessFromDataTable:GetAccessFromDataTableByArr4OdbcAccess2007)导出SQL语句：{0}，出错信息：{1}。",
                     strSQL.ToString(),
                     objException.Message);
                    throw new Exception(sbMsg.ToString(), objException);
                }
