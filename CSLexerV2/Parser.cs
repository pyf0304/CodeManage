...
namespace DDW
{
    ...
    {
        public TextPostion(int intLine, int intCol, string strID)
        {
            this.Line = intLine;
            this.Col = intCol;
            this.ID = strID;
        }
        public TextPostion(TextPostion objTP)
        {
            this.Line = objTP.Line;
            this.Col = objTP.Col;
            this.ID = objTP.ID;
        }
    ...
    public class Parser
    {
        ...
        //private TextPostion currStartPos4Comment = null;
        //private TextPostion currEndPos4Comment = null;
        ...
        public CompilationUnitNode Parse(TokenCollection tokens, List<string> strings)
        {
            ...
            {
                string strMsg =string.Format("文件分析时出错。文件名：{0}, 错误:{1}, Crashed for token: {2} at line:{3}, col:{4}. (In Parse)",
                    this.currentFileName, objException.Message, curtok, curtok.Line , curtok.Col);
        ...
        private BaseNode ParseCtor(TypeNode type)
        {
            ...
            //curAttributes.Clear();
            node.Modifiers = curmods;
            string strFindType = curmods.ToString().Split(',')[0];
            SetStartToken(nextTokNode, node, strFindType);
            ...
            SetEndToken(nextTokNode, node, TokenID.RCurly);
            return node;
        }
        ...
        private void ParseComment(LinkedListNode<Token> methodStartToken, MethodNode node)
        {
            var prevTokNode4CurrMethod = methodStartToken.Previous;
            var currTokNode = methodStartToken.Previous;
            var prevTokNode = currTokNode.Previous;
            int intCount = 0;
            while (currTokNode == null || intCount <= 50)
            {
                switch(currTokNode.Value.ID)
                {
                    case TokenID.DocComment:
                        if (node.StartPos4Comment == null)
                        {
                            node.StartPos4Comment = new TextPostion(currTokNode.Value.Line, currTokNode.Value.Col, "");
                        }
                        else
                        {
                            node.StartPos4Comment.Line = currTokNode.Value.Line;
                            node.StartPos4Comment.Col = currTokNode.Value.Col;
                        }
                        if (node.EndPos4Comment == null)
                        {
                            node.EndPos4Comment = new TextPostion(prevTokNode4CurrMethod.Value.Line, prevTokNode4CurrMethod.Value.Col, "");
                        }
                        else
                        {
                            node.EndPos4Comment.Line = prevTokNode4CurrMethod.Value.Line;
                            node.EndPos4Comment.Col = prevTokNode4CurrMethod.Value.Col;
                        }
                ...
                //string strPrev = prevTokNode.Value.ToString();
                //string strCurmods = curmods.ToString();
                
                intCount++;
                prevTokNode = currTokNode;
                currTokNode = currTokNode.Previous;
            }
            if (intCount >= 50)
            {
                string ss = prevTokNode.Value.ToString();
            }
            else
            {
                if (node.StartPos4Comment == null)
        ...
        private BaseNode ParseMethod(IType type, QualifiedIdentifierExpression name)
        {
            ...
            node.Modifiers = curmods;
            string strFindType = curmods.ToString().Split(',')[0];
            var prevTokNode = nextTokNode.Previous;
            int intCount = 0;
            while(prevTokNode == null || intCount<30)
            {
                string strPrev = prevTokNode.Value.ToString();
                string strCurmods = curmods.ToString();
                if (strPrev == strFindType)
                {
                    break;
                }
                intCount++;
                prevTokNode = prevTokNode.Previous;
            }
            if (intCount==30)
            {
                string ss = prevTokNode.Value.ToString();
            ...
            {
                string ss = prevTokNode.Value.ToString();
                node.StartToken = prevTokNode.Value;
                var prevprevTokNode = prevTokNode.Previous;
               
            }
            ParseComment(prevTokNode, node);
            curmods = Modifier.Empty;
            ...
            prevTokNode = nextTokNode.Previous;
            intCount = 0;
            while (prevTokNode == null || intCount < 30)
            {
                string strPrev = prevTokNode.Value.ToString();
                //string strRight = "}";
                if (prevTokNode.Value.ID == TokenID.RCurly)
                {
                    break;
                }
                intCount++;
            ...
            {
                string ss = prevTokNode.Value.ToString();
                if (prevTokNode.Next.Value.ID == TokenID.Newline)
                {
                    node.EndToken = prevTokNode.Next.Value;
                }
                else
                {
                    node.EndToken = prevTokNode.Value;
                }
            }

            return node;
        }
        private void SetEndToken(LinkedListNode<Token> nextTokNode, MemberNode node, TokenID findTokenId)
        {
            var           prevTokNode = nextTokNode.Previous;
            int intCount = 0;
            while (prevTokNode == null || intCount < 30)
            {
                string strPrev = prevTokNode.Value.ToString();
                //string strRight = "}";
                if (prevTokNode.Value.ID == findTokenId)
                {
                    break;
                }
                intCount++;
            ...
            else
            {
                ...
                else if (prevTokNode.Next.Value.ID == TokenID.SingleComment)
                {
                    var findTokNode = prevTokNode.Next;
                    while(findTokNode.Value.ID != TokenID.Newline)
                    {
                        findTokNode = findTokNode.Next;
                    }
                    node.EndToken = findTokNode.Value;
                }
                else
                {
                    node.EndToken = prevTokNode.Value;
        ...
        private void SetStartToken(LinkedListNode<Token> nextTokNode, MemberNode node, string findType)
        {
            var prevTokNode = nextTokNode.Previous;
            int intCount = 0;
            while (prevTokNode == null || intCount < 10)
            {
                string strPrev = prevTokNode.Value.ToString();
                //string strfindTokenId = findTokenId.ToString();
                if (strPrev == findType)
                {
                    break;
                }
        ...
        private BaseNode ParseField(IType type, QualifiedIdentifierExpression name)
        {
            ...
            SetStartToken(nextTokNode, node, fieldMods.ToString());
            if ((node.Modifiers & Modifier.Unsafe) != Modifier.Empty)
            {
                //unsafe modifier -> unsafe type.
                isUnsafe++;
            ...
            AssertAndAdvance(TokenID.Semi);
            SetEndToken(nextTokNode, node, TokenID.Semi);
            return node;
        }
