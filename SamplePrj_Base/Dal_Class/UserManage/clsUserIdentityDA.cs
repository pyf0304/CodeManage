...
//以下是属性变量
 ...
 get
 {
 return clsUserIdentityEN._CurrTabName_S;
 }
 }
...
objUserIdentityEN.Memo = objRow[conUserIdentity.Memo].ToString().Trim(); //备注
}
 catch(Exception objException)
{
...
try
{
 ...
 objUserIdentityEN.Memo = objDT.Rows[0][conUserIdentity.Memo].ToString().Trim(); //备注(字段类型:varchar,字段长度:1000,是否可空:True)
}
 catch(Exception objException)
{
...
try
{
 ...
 objUserIdentityEN.Memo = objRow[conUserIdentity.Memo] == DBNull.Value ? null : objRow[conUserIdentity.Memo].ToString().Trim(); //备注(字段类型:varchar,字段长度:1000,是否可空:True)
}
 catch(Exception objException)
{
...
Memo = objRow[conUserIdentity.Memo].ToString().Trim() //备注
};
objUserIdentityEN.ClearUpdateState();//清除修改状态,即清除脏字段信息
return objUserIdentityEN;
}
...
//获取连接对象
objSQL = clsUserIdentityDA.GetSpecSQLObj();
string strMaxValue = objSQL.GetMaxStrId(clsUserIdentityEN._CurrTabName_S, conUserIdentity.IdentityID, 2, "");
return strMaxValue;
}
...
//获取连接对象
objSQL = clsUserIdentityDA.GetSpecSQLObj();
string strMaxValue = objSQL.GetMaxStrId(clsUserIdentityEN._CurrTabName_S, conUserIdentity.IdentityID, 2, strPrefix);
return strMaxValue;
}
...
objRow[conUserIdentity.IdentityDesc] = objUserIdentityEN.IdentityDesc; //身份描述
 ...
 {
objRow[conUserIdentity.Memo] = objUserIdentityEN.Memo; //备注
 }
objDS.Tables[clsUserIdentityEN._CurrTabName_S].Rows.Add(objRow);
try
{
objDA.Update(objDS, clsUserIdentityEN._CurrTabName_S);
}
catch (Exception objException)
{
clsGeneralTab2. LogErrorS(objException, "");
...
foreach(System.Data.DataRow oRow in oDT.Rows)
{
objRow = objDS.Tables[clsUserIdentityEN._CurrTabName_S ].NewRow();
...
objRow[conUserIdentity.Memo] = oRow[conUserIdentity.Memo].ToString().Trim(); //备注
 objDS.Tables[clsUserIdentityEN._CurrTabName_S].Rows.Add(objRow);
}
try
{
...
objDA = new System.Data.SqlClient.SqlDataAdapter(strSQL, objSQL.SQLConnect);
objCB = new System.Data.SqlClient.SqlCommandBuilder(objDA);
objDA.Fill(objDS, clsUserIdentityEN._CurrTabName_S);
if (objDS.Tables[clsUserIdentityEN._CurrTabName_S].Rows.Count  ==  0)
{
//MsgBox("没有相应的ID号:IdentityID = " + "'"+ objUserIdentityEN.IdentityID+"'");
return false;
}
objRow = objDS.Tables[clsUserIdentityEN._CurrTabName_S].Rows[0];
...
sbSQL.AppendFormat("Update UserIdentity Set ");
 ...
 {
 objUserIdentityEN.Memo = objUserIdentityEN.Memo.Replace("'", "''"); //转换值串中的单撇"'",使之成为双撇"''"
 sbSQL.AppendFormat("{1} = '{0}',", objUserIdentityEN.Memo, conUserIdentity.Memo); //备注
 }
 else
 {
 sbSQL.AppendFormat("{0} = null,",conUserIdentity.Memo); //备注
 }
 }
...
sbSQL.AppendFormat("Update UserIdentity Set ");
 ...
 {
 objUserIdentityEN.Memo = objUserIdentityEN.Memo.Replace("'", "''"); //转换值串中的单撇"'",使之成为双撇"''"
 sbSQL.AppendFormat(" Memo = '{0}',", objUserIdentityEN.Memo); //备注
 }
 else
 {
...
return objDT;
 ...
 /// </summary>
 /// <param name = "strIdentityID">身份编号(主键)</param>
 /// <returns></returns>
public bool CheckUserIdentityUniqueness(string strIdentityID )
{
...
return true;
 ...
 /// </summary>
 /// <returns></returns>
public bool CheckUserIdentityUniqueness()
{
StringBuilder sbCondition = new StringBuilder();
 ...
 public static int GetRecCount()
{
 ...
 //获取连接对象
 objSQL = clsUserIdentityDA.GetSpecSQLObj();
int intRecCount = objSQL.GetRecCount(clsUserIdentityEN._CurrTabName_S);
return intRecCount;
}
 ...
 public static int GetRecCountByCond(string strCondition)
{
 ...
 //获取连接对象
 objSQL = clsUserIdentityDA.GetSpecSQLObj();
int intRecCount = objSQL.GetRecCount(clsUserIdentityEN._CurrTabName_S, strCondition);
return intRecCount;
}
