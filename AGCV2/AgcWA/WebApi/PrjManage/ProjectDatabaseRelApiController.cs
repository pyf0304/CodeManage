...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
List<clsProjectDatabaseRelEN> arrProjectDatabaseRelObjLst = clsProjectDatabaseRelBL.GetObjLstCache();
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrProjectDatabaseRelObjLst });
 }
 catch (Exception objException)
 {
...
List<long> lstMId = lstMId_Str.Select(x=>long.Parse(x)).ToList();
 try
 {
IEnumerable<clsProjectDatabaseRelEN> arrProjectDatabaseRelObjLst = clsProjectDatabaseRelBL.GetObjLstByMIdLstCache(lstMId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrProjectDatabaseRelObjLst });
 }
 catch (Exception objException)
 {
