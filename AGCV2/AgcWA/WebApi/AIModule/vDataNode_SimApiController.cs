...
public ActionResult GetObjByDataNodeId(string strDataNodeId)
{
...
List<string> lstDataNodeId = new(arrDataNodeId);
 try
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 strDataNodeId = clsvDataNode_SimBL.GetFirstID_S(strWhereCond);
...
[HttpGet("IsExist")]
public ActionResult IsExist(string strDataNodeId)
{
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new()
{
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
bool bolIsExist = clsvDataNode_SimBL.IsExist(strDataNodeId);
return Ok(new { errorId = 0, errorMsg = "", returnBool = bolIsExist });
 }
 catch (Exception objException)
 {
