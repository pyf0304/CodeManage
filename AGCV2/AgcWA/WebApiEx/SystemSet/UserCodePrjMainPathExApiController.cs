...
namespace AGC.WebApi
{
    ...
    /// </summary>
    public class UserCodePrjMainPathExApiController : UserCodePrjMainPathApiController
    {
        ...
        /// <returns>返回是否存在?</returns>
        [HttpGet("DelRecordEx")]
        public ActionResult DelRecordEx(string strUserCodePrjMainPathId, string strCmPrjId)
        {
            string strFunctionName = clsStackTrace.GetCurrFunction();
            Dictionary<string, string> dictParam = new Dictionary<string, string>();
            ...
            try
            {
                var varResult = clsUserCodePrjMainPathBLEx.DelRecordEx(strUserCodePrjMainPathId, strCmPrjId);
                return Ok(new { errorId = 0, errorMsg = "", returnBool = varResult });
            }
            catch (Exception objException)
            {
