...
{
 /// <summary>
 /// vPrjObjects(vPrjObjects)
 /// (AutoGCLib.AutoGC6Cs_M:A_GenEntityLayerCode)
 /// </summary>
[Serializable]
public class clsvPrjObjectsEN : clsEntityBase2
{
public const string CurrTabName_S = "vPrjObjects"; //当前表名,与该类相关的表名
public const string CurrTabKeyFldName_S = "ObjId"; //当前表中的关键字名称,与该类相关的表中关键字名
protected const int mintAttributeCount = 20;
public static string[] AttributeName = new string[] {"ObjId", "PrjId", "PrjName", "ObjName", "ObjFunction", "ObjTypeId", "FuncModuleAgcId", "FuncModuleName", "UserName", "ObjectType", "IsSynchWithTab", "UserId", "UpdDate", "Memo", "FldNum", "TabCnName", "TabId", "TabName", "UseStateId", "UseStateName"};
//以下是属性变量
...
/// </summary>
 public clsvPrjObjectsEN()
 {
 SetInit();
 mbolIsCheckProperty = false;
 _CurrTabName = "vPrjObjects";
...
public clsvPrjObjectsEN(string strObjId)
 {
strObjId = strObjId.Replace("'", "''");
if (strObjId.Length > 8)
{
throw new Exception("在表:vPrjObjects中,关键字长度不正确!");
}
if (string.IsNullOrEmpty(strObjId)  ==  true)
{
throw new Exception("在表:vPrjObjects中,关键字不能为空 或 null!");
}
try
{
