...
clsFunctionTemplateRelaWApi.ReFreshCache(objFunctionTemplateRelaEN.FunctionTemplateId);
return bolResult;
}
...
List<clsFunctionTemplateRelaEN> arrFunctionTemplateRelaObjLstCache = GetObjLstCache(strFunctionTemplateId);
...
public static IEnumerable<clsFunctionTemplateRelaEN> GetObjLstByMIdLstCache(List<long> arrMId, string strFunctionTemplateId)
{
//初始化列表缓存
string strKey = string.Format("{0}_{1}", clsFunctionTemplateRelaEN._CurrTabName, strFunctionTemplateId);
List<clsFunctionTemplateRelaEN> arrFunctionTemplateRelaObjLstCache = GetObjLstCache(strFunctionTemplateId);
...
if ((int)jobjReturn0["errorId"] == 0)
{
 clsFunctionTemplateRelaEN objFunctionTemplateRelaEN = clsFunctionTemplateRelaWApi.GetObjBymId(long.Parse(arrmId[0]));
clsFunctionTemplateRelaWApi.ReFreshCache(objFunctionTemplateRelaEN.FunctionTemplateId);
var intReturnInt = (int)jobjReturn0["returnInt"];
return intReturnInt;
}
...
if (clsFunctionTemplateRelaEN.AttributeName.Length == 0)
{
result = dataTable;
return result;
}
...
{
//Columns
foreach (string strAttrName in clsFunctionTemplateRelaEN.AttributeName)
{
...
if (string.IsNullOrEmpty(clsFunctionTemplateRelaEN._WhereFormat) == false)
{
strWhereCond =string.Format(clsFunctionTemplateRelaEN._WhereFormat, strFunctionTemplateId);
}
else
{
strWhereCond = string.Format("{0}='{1}'",conFunctionTemplateRela.FunctionTemplateId, strFunctionTemplateId);
}
var strKey = string.Format("{0}_{1}", clsFunctionTemplateRelaEN._CurrTabName, strFunctionTemplateId);
...
return arrFunctionTemplateRelaObjLstCache;
 ...
 /// </summary>
 /// <returns>从缓存中获取的所有对象列表</returns>
public static List<clsFunctionTemplateRelaEN> GetObjLstCacheFromObjLst(string strFunctionTemplateId,List<clsFunctionTemplateRelaEN> arrObjLst_P)
{
var strKey = string.Format("{0}_{1}", clsFunctionTemplateRelaEN._CurrTabName, strFunctionTemplateId);
List<clsFunctionTemplateRelaEN> arrFunctionTemplateRelaObjLstCache = null;
if (CacheHelper.Exsits(strKey) == true)
{
arrFunctionTemplateRelaObjLstCache = CacheHelper.Get<List<clsFunctionTemplateRelaEN>>(strKey);
}
else
{
