...
try
{
objUserIdentityEN.sfUpdFldSetStr = objUserIdentityEN.getsfUpdFldSetStr();
...
throw new Exception(strMsg); 
 ...
 /// <returns></returns>
public static string GetUniConditionStr_IdentityDesc(this clsUserIdentityEN objUserIdentityEN)
{
//检测记录是否存在
...
try
{
 if (string.IsNullOrEmpty(objUserIdentityEN.IdentityID) == true || clsUserIdentityWApi.IsExist(objUserIdentityEN.IdentityID) == true)
 {
     objUserIdentityEN.IdentityID = clsUserIdentityWApi.GetMaxStrId();
 }
clsUserIdentityWApi.CheckPropertyNew(objUserIdentityEN); 
...
{
if (string.IsNullOrEmpty(objUserIdentityEN.sfUpdFldSetStr) == true)
{
...
string strKey = string.Format("{0}", clsUserIdentityEN._CurrTabName);
CacheHelper.Remove(strKey);
clsUserIdentityWApi.objCommFun4WApi.ReFreshCache();
}
}
