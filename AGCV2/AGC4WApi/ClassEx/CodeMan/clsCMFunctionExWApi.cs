...
/*-- -- -- -- -- -- -- -- -- -- --
类名:clsCMFunctionExWApi
表名:CMFunction(00050502)
生成代码版本:2020.01.01.1
生成日期:2020/01/02 12:09:35
生成者:
...
namespace AGC4WApi
{
    ...
    /// </summary>
    public class clsCMFunctionExWApi
    {
        //private static readonly string mstrApiControllerName = "CMFunctionExApi";
        /// <summary>
        /// 静态的对象列表,用于缓存,针对记录较少,作为参数表可以使用
        /// (AutoGCLib.WA_AccessEx4CSharp:GeneCode)
        /// </summary>
        public static List<clsCMFunctionEN> arrCMFunctionObjLst_Cache = null;
        /// <summary>
        /// 从缓存中查找失败的次数
        /// (AutoGCLib.WA_AccessEx4CSharp:GeneCode)
        /// </summary>
        ...
        public static clsCMFunctionEN AddCMFunction(clsCMFunctionEN objCMFunction)
        {
            ...
            bool bolIsExist = clsCMFunctionWApi.IsExistRecord(strCondition);

            if (bolIsExist)
            {
                objCMFunction.CmFunctionId = clsCMFunctionWApi.GetFirstID(strCondition);
                objCMFunction.UpdateWithCondition(strCondition);
            }
            else
            {

                objCMFunction.SetCmFunctionId(clsCMFunctionWApi.GetMaxStrId());
