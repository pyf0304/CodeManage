...
namespace AGC4WApi
{
    ...
    /// </summary>
    public class clsUserLogExWApi
    {
        //private static readonly string mstrApiControllerName = "UserLogExApi";
        /// <summary>
        /// 静态的对象列表,用于缓存,针对记录较少,作为参数表可以使用
        /// (AutoGCLib.WA_AccessEx4CSharp:GeneCode)
        /// </summary>
        public static List<clsUserLogEN> arrUserLogObjLst_Cache = null;
        /// <summary>
        /// 从缓存中查找失败的次数
        /// (AutoGCLib.WA_AccessEx4CSharp:GeneCode)
        /// </summary>
        ...
        public static bool SetUsedLog(string strUserId, string strObjectTable, string strValue)
        {
            ...
            bool bolIsExist = clsUserLogWApi.IsExistRecord(sbCondition.ToString());
            if (bolIsExist == true)
            {
                objUserLogEN = clsUserLogWApi.GetFirstObj(sbCondition.ToString());
                objUserLogEN.UserId = strUserId;
                objUserLogEN.ObjectTable = strObjectTable;
                objUserLogEN.UsedValue = strValue;
                objUserLogEN.UpdDate = clsDateTime.getTodayDateTimeStr(0);
                ...
                return clsUserLogWApi.UpdateRecord(objUserLogEN);
            }
            else
            {
                ...
                //objUserLogEN.UpdTime = clsDateTime.getCurrTime(0);
                //clsUserLogWApi.CheckPropertyNew(objUserLogEN);
                return clsUserLogWApi.AddNewRecord(objUserLogEN);
            }
        }
