...
namespace AGC4WApi
{
    public static class clsTabMainTypeExWApi_Static
    {
        ...
        public static bool EditRecordEx(this clsTabMainTypeEN objTabMainTypeEN)
        {
            try
            {
                objTabMainTypeEN.sf_UpdFldSetStr = objTabMainTypeEN.getsf_UpdFldSetStr();
    ...
    public class clsTabMainTypeExWApi
    {
        ...
        {
            if (string.IsNullOrEmpty(objTabMainTypeEN.sf_UpdFldSetStr) == true)
            {
        ...
        public static void BindCbo_TabMainTypeIdByAppId_Cache(System.Windows.Forms.ComboBox objComboBox, int intApplicationTypeId)
        {
            //为数据源为表的下拉框设置内容
            ...
            IEnumerable<clsAppCodeTypeRelaEN> arrAppCodeTypeRelaObjLst = clsAppCodeTypeRelaWApi.GetObjLst_Cache()
            ...
            arrTabMainTypeObjLst = clsTabMainTypeWApi.GetObjLst_Cache()
                .Where(x => arrTabMainTypeId.Contains(x.TabMainTypeId))
                .ToList();
            //GetAppCodeTypeRelaObjLstByApplicationTypeId_Cache(strApplicationTypeId);
            if (arrTabMainTypeObjLst == null)
            {
                string strMsg = string.Format("应用:[{0}]没有相应的代码类型，请联系管理员！", 
                    clsApplicationTypeWApi.GetNameByApplicationTypeId_Cache(intApplicationTypeId));
                throw new Exception(strMsg);
            }
