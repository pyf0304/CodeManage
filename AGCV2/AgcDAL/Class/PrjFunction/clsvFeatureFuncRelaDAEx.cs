...
namespace AGC.DAL
{
    ...
    /// </summary>
    public class clsvFeatureFuncRelaDAEx : clsvFeatureFuncRelaDA
    {
        ...
        public System.Data.DataTable GetDataTableByAppId(string strCondition, int intApplicationTypeId)
        {
            ...
            strSQL = string.Format("Select * from dbo.fn_getvFeatureFuncRela4GC({0}) where {1}", 
        ...
        /// <param name = "intApplicationTypeId">应用类型Id</param>
        /// <returns>返回数据表DataTable</returns>
        public System.Data.DataTable GetDataTableByAppId4Code(string strCondition, int intApplicationTypeId)
        {
            try
            {
                clsCheckSql.CheckStrSQL_Weak(strCondition);
            ...
            //获取连接对象
            objSQL = clsvFeatureFuncRelaDA.GetSpecSQLObj();
            strSQL = string.Format("Select * from dbo.fn_getvFeatureFuncRela4Code({0}) where {1}",
                intApplicationTypeId,
                strCondition);
            objDT = objSQL.GetDataTable(strSQL);
            return objDT;
