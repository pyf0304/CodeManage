...
namespace AGC_CSWin
{
    ...
    public partial class frmUserPrjGrant_UI : System.Windows.Forms.Form
    {
        ...
        private void ShowData(long lngmId)
        {
            ...
            ///2、检查该关键字的记录是否存在，如果不存在就返回不显示；
            if (clsUserPrjGrantWApi.IsExist(lngmId) == false)		//检查该关键字的记录是否存在
            {
        ...
        private void AddUserPrjGrantRecordSave()
        {
            ...
            {
                ///2、初始化相关类对象
                objUserPrjGrant = new clsUserPrjGrantEN();	//初始化新对象
                ///3、判断是否有相同的关键字,如果主键是标识递增型就不需要判断是否存在
                ///4、把值从界面层传到逻辑层或数据实体层
                PutDataToUserPrjGrantClass(objUserPrjGrant);		//把界面的值传到
                ///5、检查传进去的对象属性是否合法
                try
                {
