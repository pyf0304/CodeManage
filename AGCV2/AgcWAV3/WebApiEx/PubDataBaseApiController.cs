...
namespace AGC.WebApiEx
{
    [ApiController]
    [Route("[controller]")]
    public class PubDataBaseApiController : ControllerBase
    {
        ...
        public ActionResult GetDataTable(string strCurrPrjDataBaseId, string strTabName, int intRecNum, string strWhereCond, string strOrderBy)
        {
        ...
        /// <summary>
        /// 根据条件获取对象列表
        /// 调用方法: GET /api/SqlApi/GetFieldValue?strFldName=v1&strTabName=v2&strWhere=v3
        /// </summary>
        /// <param name = "strWhereCond">给定条件</param>
        /// <param name = "strTabName">给定表名</param>       
        /// <param name = "strFldName">给定表名</param>
        /// <returns>返回对象列表</returns>
        [HttpGet("GetFieldValue")]
        public ActionResult GetFieldValue( string strTabName, string strFldName, string strWhere)
        {
            ...
            if (string.IsNullOrEmpty(strTabName) == true)
            {
                string strMsg = string.Format("根据条件获取表字段值列表，strTabName不能为空!({0})", clsStackTrace.GetCurrClassFunction());
                return Ok(new { errorId = 1, errorMsg = strMsg });
            }
            if (string.IsNullOrEmpty(strFldName) == true)
            {
                string strMsg = string.Format("根据条件获取表字段值列表，strFldName不能为空!({0})", clsStackTrace.GetCurrClassFunction());
                return Ok(new { errorId = 1, errorMsg = strMsg });
            }
            if (string.IsNullOrEmpty(strWhere) == true)
            {
                string strMsg = string.Format("根据条件获取表字段值列表，strWhere不能为空!({0})", clsStackTrace.GetCurrClassFunction());
                return Ok(new { errorId = 1, errorMsg = strMsg });
            }
            try
            {
                //                DataTable objDT = clsCMClassBL.GetDataTable(strWhereCond, strTabName);
                var arrFldValue = clsPubDataBaseBLEx.GetFieldValue(strTabName, strFldName, strWhere);                
                return Ok(new { errorId = 0, errorMsg = "", returnStrLst = arrFldValue });
            }
            catch (Exception objException)
            {
                clsGeneralTab.LogErrorS(objException, clsStackTrace.GetCurrClassFunction());
                string strMsg = string.Format("根据条件获取表字段值列表（GetColumnObjList）时出错！表名：{0},出错信息：{1}。({2})",
                       strTabName, objException.Message,
                        clsStackTrace.GetCurrClassFunction());
                clsPubVar_WebApi.objLog.WriteDebugLog(strMsg);
                return Ok(new { errorId = 1, errorMsg = strMsg });
