...
var clsvPrjTabWS = function () {
    ...
    this.init = function () {
        ...
        this.objvPrjTabEN = new clsvPrjTabEN();
        ResponseData = new tzResponseData();
    };
    this.init = function (strMethod, mapParam, pobjvPrjTabEN, pReCallFunc) {
        ...
        this.WEB_SERVICE_URL = String.format("{0}/{1}/Table_Field/vPrjTabService.asmx", CurrIPAddressAndPort, CurrPrx);
    };
    ...
    this.main = function () {
        switch (this.MethodName) {
            case "GetJSONObjByTabId":
                ResponseData.what = 1;
                try {
                    this.GetJSONObjByTabId(this.Param);
                }
                catch (err) {
            ...
            case "AddNewRecordByJSON":
                ResponseData.what = 3;
                try {
                    this.AddNewRecordByJSON(this.Param, this.objvPrjTabEN);
                }
                catch (err) {
                    myShowErrorMsg(err.message);
                }
            ...
            case "UpdateRecordByJSON":
                ResponseData.what = 4;
                try {
                    this.UpdateRecordByJSON(this.Param, this.objvPrjTabEN);
                }
                catch (err) {
                    myShowErrorMsg(err.message);
                }
    ...
    /// 根据关键字获取相应的记录的对象
    /// mapParam样例:strTabId = "01"
    ///调用样例:行1:clsvPrjTabWS objvPrjTabWS = new clsvPrjTabWS(mHandler, "GetJSONObjByTabId", values, null);
    ///调用样例:行2:Thread mThread = new Thread(objvPrjTabWS);
    ///调用样例:行3:mThread.start();
    /// (AutoGCLib.AutoGC6Cs_WS:Gen_4Ws_GetRecordJSONObjByKey_S_JavaScript)
    /// </summary>
    /// <param name = "mapParam">参数列表</param>
    /// <returns>获取相应的记录的对象</returns>
    this.GetJSONObjByTabId = function (mapParam) {
        var strMethodName = "GetJSONObjByTabId";
        try {
    ...
    ///调用样例:行1:clsvPrjTabWS objvPrjTabWS = new clsvPrjTabWS(mHandler, "GetJSONObjLst", values, null);
    ///调用样例:行2:Thread mThread = new Thread(objvPrjTabWS);
    ///调用样例:行3:mThread.start();
    ...
    /// 根据条件获取相应的记录对象列表
    /// mapParam样例: mapParam.put("strWhereCond", "1 = 1");
    ///调用样例:行1:clsvPrjTabWS objvPrjTabWS = new clsvPrjTabWS(mHandler, "GetTopJSONObjLst", values, null);
    ///调用样例:行2:Thread mThread = new Thread(objvPrjTabWS);
    ///调用样例:行3:mThread.start();
    ...
    /// 根据条件获取相应的记录对象列表
    /// mapParam样例: mapParam.put("strWhereCond", "1 = 1");
    ///调用样例:行1:clsvPrjTabWS objvPrjTabWS = new clsvPrjTabWS(mHandler, "GetJSONObjLstByPager", values, null);
    ///调用样例:行2:Thread mThread = new Thread(objvPrjTabWS);
    ///调用样例:行3:mThread.start();
    ...
    this.GetMaxStrId = function (mapParam) {
        var strMethodName = "vPrjTab_GetMaxStrId";
        try {
            this.CallWebService(strMethodName, mapParam, this.WEB_SERVICE_URL);
        }
        catch (objException) {
    ...
    this.GetMaxStrIdByPrefix = function (mapParam) {
        var strMethodName = "vPrjTab_GetMaxStrIdByPrefix";
        try {
            this.CallWebService(strMethodName, mapParam, this.WEB_SERVICE_URL);
        }
        catch (objException) {
    ...
    /// <summary>
    /// 根据条件获取相应的记录对象列表
    /// mapParam样例: mapParam.put("strWhereCond", "TabId = '01'");
    ///调用样例:行1:clsvPrjTabWS objvPrjTabWS = new clsvPrjTabWS(mHandler, "GetJSONObjLst", values, null);
    ...
    this.IsExistRecord = function (mapParam) {
        var strMethodName = "vPrjTab_IsExistRecord";
        try {
            this.CallWebService(strMethodName, mapParam, this.WEB_SERVICE_URL);
        }
        catch (objException) {
    ...
    this.funGetRecCountByCond = function (mapParam) {
        var strMethodName = "vPrjTab_funGetRecCountByCond";
        try {
            this.CallWebService(strMethodName, mapParam, this.WEB_SERVICE_URL);
        }
        catch (objException) {
