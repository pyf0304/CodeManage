<%@ Control Language="c#" AutoEventWireup="True" CodeBehind="wucEditRegion4PreviewV2.ascx.cs"
    Inherits="AGC.Webform.wucEditRegion4PreviewV2" %>
...
<script>
    ...
    function ReSelectCtrl() {
        ...
        if (arrDiv.length == 0) {
             ...
             alert('在当前界面中没有找到Div:pnlControlLst. (In ReSelectCtrl of wucEditRegion4PreviewV2.ascx) ');
    ...
    function txt_Click(strCtrlId, event) {
        ...
        if (bolIsPressShift === true) {
            ...
            if (SingleSelectGvCheck(strCtrlId,'gvEditRegionFlds') == true) {
                thisChk.className = "form-control  border-danger";
                Update(strCtrlId);
            }
            else {
...
<div id="divEditRegion4Preview" runat="server" class="border border-danger" >

    <div>
        <asp:Label ID="lblTitle" runat="server" Text="可视化" Width="400px" CssClass="text-warning"></asp:Label>
        <asp:Label ID="lblTitleOutUse" runat="server" Width="150px" Text="编辑区域不使用" CssClass="text-warning"></asp:Label>
    </div>
    <div>
