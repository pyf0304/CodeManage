...
{
    /// <summary>
    /// wfmCMFeature_QUDI 的摘要说明。其中Q代表查询,U代表修改
    /// (AutoGCLib.AutoGC_Cs_CWeb_PubFun:GenQueryUpdDelInsRecControlCode)
    /// </summary>
    public partial class wfmCMFeature_QUDI : CommWebPageBase
    {
        ...
        protected string CombinevCMFeatureCondition()
        {
            ...
            //例如 1 = 1 && UserName = '张三'
            string strWhereCond = string.Format(" {0} = '{1}' ", convCMFeature.PrjId, clsPubVar.CurrSelPrjId);
        ...
        protected void btnCloneNewRec4Gv_Click(object sender, EventArgs e)
        {
            ...
            clsCMFeatureEN obj = new clsCMFeatureEN(strCmFeatureId);
            clsCMFeatureBL.GetCMFeature(ref obj);
            obj.CmFeatureId = clsCMFeatureBL.GetMaxStrIdByPrefix_S(clsPubVar.CurrSelPrjId);
        ...
        {
            if (ddlCmFeatureTypeIdq.SelectedIndex <= 0)
            {
                lblMsgList.Text = "请输入CmFeatureTypeId!";
                clsCommonJsFunc.Alert(this, lblMsgList.Text);
                ddlCmFeatureTypeIdq.Focus();
                return false;
            }
            return true;
        }
