...
namespace AGC.Webform
{
	...
	/// </summary>
	public partial class wfmViewRegion_QUDI : CommWebPageBase
    {
		//生成有关的变量定义代码
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)	///如果本界面是初次被调用，而不是单击按钮事件所调用
			{
				...
				wucViewRegion1.SetDdl_OUTSQLDSTypeId();
				wucViewRegion1.SetDdl_OUTRelaTabId(clsPubVar.CurrSelPrjId);
				wucViewRegion1.SetDdl_INSQLDSTypeId();
				wucViewRegion1.SetDdl_INRelaTabId(clsPubVar.CurrSelPrjId);
...
//lblViewName.Text = new clsViewInfoEN(ViewId, true).ViewCnName + new clsViewInfoEN(ViewId, true).ViewName;
		...
		private void ShowData(long lngRegionId)
		{
			//操作步骤：
			//1、检查关键字是否为空；
			...
			//1、检查关键字是否为空；
			if (lngRegionId == 0) return ;		//如果关键字为空就返回退出
			//2、检查该关键字的记录是否存在，如果不存在就返回不显示；
			...
			if (objViewRegionEN.RegionTypeId == "0001")//区域类型：查询区域
			{
				...
				//5、把该对象的所有属性显示在界面上，在这里显示在表控件中
				wucViewRegion1.ColNum = objQryRegion.ColNum;
			...
			else if (objViewRegionEN.RegionTypeId == "0003")//区域类型：编辑区域
			{
				...
				//5、把该对象的所有属性显示在界面上，在这里显示在表控件中
				wucViewRegion1.ColNum = objEditRegion.ColNum;
		...
		private void GetDataFromViewRegionClass(clsViewRegionEN pobjViewRegion)
		{
			...
			wucViewRegion1.OUTSQLDSTypeId = pobjViewRegion.OUTSQLDSTypeId;

			wucViewRegion1.OUTRelaTabId = pobjViewRegion.OUTRelaTabId;
			wucViewRegion1.INSQLDSTypeId = pobjViewRegion.INSQLDSTypeId;
			wucViewRegion1.INRelaTabId = pobjViewRegion.INRelaTabId;
		...
		protected void btnOKUpd_Click(object sender, System.EventArgs e)
		{
			...
			switch(strCommandText)
			{
				...
				case "确认添加":
                    ...
                    if (clsViewRegionBL.IsExistRecord("ViewId = '" + objViewRegionEN.ViewId + "' and RegionName = '" + objViewRegionEN.RegionName + "'") == true)
					{
					...
					{
						objViewRegionEN.RegionId = clsViewRegionBL.GetFirstID_S("ViewId = '" + objViewRegionEN.ViewId + "' and RegionName = '" + objViewRegionEN.RegionName + "'");
						//存储相关区域的具体代码
						if (objViewRegionEN.RegionTypeId == "0001")//区域类型：查询区域
						{
		...
		private void PutDataToViewRegionClass(clsViewRegionEN pobjViewRegion)
		{
			...
			pobjViewRegion.ViewId = ViewId;
			pobjViewRegion.RegionTypeId = wucViewRegion1.RegionTypeId;
			pobjViewRegion.RegionFunction = wucViewRegion1.RegionFunction;
			pobjViewRegion.OUTSQLDSTypeId = wucViewRegion1.OUTSQLDSTypeId;
			pobjViewRegion.OUTRelaTabId = wucViewRegion1.OUTRelaTabId;
			pobjViewRegion.INSQLDSTypeId = wucViewRegion1.INSQLDSTypeId;
			pobjViewRegion.INRelaTabId = wucViewRegion1.INRelaTabId;
		...
		protected void btnEditRegionFld_Click(object sender, System.EventArgs e)
		{
			...
			else if (strRegionTypeId == clsRegionTypeENEx.DETAIL_REGION)		//树形区域
			{
  ...
  //          }
		...
		protected void btnStartGene_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				{
					clsViewInfoBLEx.ImportRegionAndFlds(ViewId, //视图ID(ViewId)
						clsRegionTypeENEx.DETAIL_REGION,				//区域类型
		...
		/// </summary>
		/// <param name="lngRegionId">表记录的关键字，显示该表关键字的内容</param>
		private void ShowSpecialData(long lngRegionId)
		{
            //操作步骤：
            //1、检查关键字是否为空；
            //2、检查该关键字的记录是否存在，如果不存在就返回不显示；
			...
			switch(objViewRegionEN.RegionTypeId)
			{
				...
				case clsRegionTypeENEx.EXCELEXPORT_REGION:	//EXCEL导出区域
					...
					break;
				case clsRegionTypeENEx.DETAIL_REGION:	//详细区域
					break;
...
//			BindDg_DGRegion();
		...
		private void UpdateRegionRecordSave(long lngRegionId)
		{
			//这是一个单表的修改的代码，由于逻辑层太简单，
			//就把逻辑层合并到控制层,
			//操作步骤：
