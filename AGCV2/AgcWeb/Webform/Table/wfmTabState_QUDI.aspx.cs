...
{
	/// <summary>
	/// wfmTabState_QUDI 的摘要说明。其中Q代表查询，U代表修改
	/// </summary>
	public partial class wfmTabState_QUDI : CommWebPageBase
    {
		...
		private void BindDg_TabState()
		{
			...
			objDV.Sort = strSortTabStateBy;     ///注意这一行代码有可能需要注释掉，目前这行仅仅在单独的查询界面中起作用
		...
		///　　　     在这里是把值传到表控件中
		/// </summary>
		/// <param name="strTabStateId">表记录的关键字，显示该表关键字的内容</param>
		private void ShowData(string strTabStateId)
		{
			...
			//1、检查关键字是否为空；
			if (strTabStateId == "") return ;		//如果关键字为空就返回退出
			//2、检查该关键字的记录是否存在，如果不存在就返回不显示；
			if (clsTabStateBL.IsExist(strTabStateId) == false)		//检查该关键字的记录是否存在
			{
				string ss = "关键字为：" + strTabStateId + "的不存在!";
		...
		/// </summary>
		/// <param name="pobjTabState">表实体类对象</param>
		...
		protected void btnOKUpd_Click(object sender, System.EventArgs e)
		{
			...
			switch(strCommandText)
			{
				...
				case "确认添加":
					...
					//2、定义对象并初始化对象
					//clsTabStateEN objTabState;	//定义对象
					objTabState = new clsTabStateEN(wucTabState1.TabStateId);	//初始化新对象
					//3、判断是否有相同的关键字,如果主键是标识递增型就不需要判断是否存在
					if (clsTabStateBL.IsExist(wucTabState1.TabStateId))	//判断是否有相同的关键字
					{
						strMsg = "关键字字段已有相同的值";
					...
					//4、把值从界面层传到逻辑层或数据实体层
					PutDataToTabStateClass(objTabState);		//把界面的值传到
					//5、检查传进去的对象属性是否合法
                    try
                    {
		...
		/// </summary>
		/// <param name="pobjTabState">数据传输的目的类对象</param>
