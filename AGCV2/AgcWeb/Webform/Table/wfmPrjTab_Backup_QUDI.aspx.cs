...
namespace AGC.Webform
{
	...
	/// </summary>
	public partial class wfmPrjTab_Backup_QUDI : System.Web.UI.Page
	{
...
//			this.RegisterStartupScript("js",js);
   ...
   //         ShowCode(autoGC5Cs_M.A_GenGeneralLogicCode(myEncoding);	//已完成
		...
		protected void lbBackupTabName_Click(object sender, System.EventArgs e)
		{
			//操作步骤：
			//1、获取被选的记录列表
			//2、调用逻辑层的函数备份被选字段
			///
			ArrayList arrRec = clsCommForWebForm.Get_DgCheckedArr(dgPrjTab);
			if (clsPrjTabBLEx.BackupPrjTabInfo(arrRec) == true)
			{
				lblMsg2.Text = "被选工程表名备份成功!";
			}
			else
			{
				lblMsg2.Text = "被选工程表名备份失败，请查原因!";
			}
			BindDg_PrjTab();
		...
		protected void dgPrjTab_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			if (e.Item.ItemType == ListItemType.Header)
			{
				...
				if (lbSelAll.Text == "全选")
				{
					Set_DataGridAllChecked(dgPrjTab, true);
					lbSelAll.Text = "不选";
				}
				else if (lbSelAll.Text == "不选")
				{
					Set_DataGridAllChecked(dgPrjTab, false);
					lbSelAll.Text = "全选";
				}
			}
