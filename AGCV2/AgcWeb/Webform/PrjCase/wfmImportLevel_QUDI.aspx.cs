...
{
	/// <summary>
	/// wfmImportLevel_QUDI 的摘要说明。其中Q代表查询，U代表修改
	/// </summary>
	public partial class wfmImportLevel_QUDI : System.Web.UI.Page
	{
		...
		///　　　     在这里是把值传到表控件中
		/// </summary>
		/// <param name="strImportLevelId">表记录的关键字，显示该表关键字的内容</param>
		private void ShowData(string strImportLevelId)
		{
			...
			//1、检查关键字是否为空；
			if (strImportLevelId == "") return ;		//如果关键字为空就返回退出
			//2、检查该关键字的记录是否存在，如果不存在就返回不显示；
			if (clsImportLevelBL.IsExist(strImportLevelId) == false)		//检查该关键字的记录是否存在
			{
				string ss = "关键字为：" + strImportLevelId + "的不存在!";
		...
		/// </summary>
		/// <param name="pobjImportLevel">表实体类对象</param>
		...
		protected void btnOKUpd_Click(object sender, System.EventArgs e)
		{
			...
			switch(strCommandText)
			{
				...
				case "确认添加":
					...
					//2、定义对象并初始化对象
					//clsImportLevelEN objImportLevel;	//定义对象
					objImportLevel = new clsImportLevelEN(wucImportLevel1.ImportLevelId);	//初始化新对象
					//3、判断是否有相同的关键字,如果主键是标识递增型就不需要判断是否存在
					if (clsImportLevelBL.IsExist(wucImportLevel1.ImportLevelId))	//判断是否有相同的关键字
					{
						strMsg = "关键字字段已有相同的值";
					...
					//4、把值从界面层传到逻辑层或数据实体层
					PutDataToImportLevelClass(objImportLevel);		//把界面的值传到
					//5、检查传进去的对象属性是否合法
				...
				case "确认修改":
					...
					//5、把修改后的内容显示在DATAGRID中
					BindDg_ImportLevel();
					wucImportLevel1.Clear();//添空控件中的内容
					//恢复<确认修改>变成<添加>
					btnOKUpd.Text = "添加";
					break;
			}
		...
		/// </summary>
		/// <param name="pobjImportLevel">数据传输的目的类对象</param>
