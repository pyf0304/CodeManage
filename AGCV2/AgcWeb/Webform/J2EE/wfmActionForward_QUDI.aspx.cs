...
{
	/// <summary>
	/// wfmActionForward_QUDI 的摘要说明。其中Q代表查询，U代表修改
	/// </summary>
	public partial class wfmActionForward_QUDI : CommWebPageBase
    {
		...
		private void BindDg_ActionForward()
		{
			...
			objDV.Sort = strSortActionForwardBy;     ///注意这一行代码有可能需要注释掉，目前这行仅仅在单独的查询界面中起作用
		...
		///　　　     在这里是把值传到表控件中
		/// </summary>
		/// <param name="lngActionForwardId">表记录的关键字，显示该表关键字的内容</param>
		private void ShowData(long lngActionForwardId)
		{
			...
			//1、检查关键字是否为空；
			if (lngActionForwardId == 0) return ;		//如果关键字为空就返回退出
			//2、检查该关键字的记录是否存在，如果不存在就返回不显示；
			if (clsActionForwardBL.IsExist(lngActionForwardId) == false)		//检查该关键字的记录是否存在
			{
				string ss = "关键字为：" + lngActionForwardId + "的不存在!";
		...
		/// </summary>
		/// <param name="pobjActionForward">表实体类对象</param>
...
//			wucActionForward1.ActionId = pobjActionForward.ActionId;
		...
		protected void btnOKUpd_Click(object sender, System.EventArgs e)
		{
			...
			switch(strCommandText)
			{
				...
				case "确认添加":
					...
					//2、定义对象并初始化对象
					//clsActionForwardEN objActionForward;	//定义对象
					objActionForward = new clsActionForwardEN();	//初始化新对象
					//3、判断是否有相同的关键字,如果主键是标识递增型就不需要判断是否存在
					//4、把值从界面层传到逻辑层或数据实体层
					PutDataToActionForwardClass(objActionForward);		//把界面的值传到
					//5、检查传进去的对象属性是否合法
					...
					//7、把新添的记录内容显示在DATAGRID中
					BindDg_ActionForward();
					wucActionForward1.Clear();		//清空控件中内容
					///恢复<确认添加>变成<添加>
					btnOKUpd.Text = "添加";
					break;
				case "确认修改":
					...
					//5、把修改后的内容显示在DATAGRID中
					BindDg_ActionForward();
					wucActionForward1.Clear();//添空控件中的内容
					//恢复<确认修改>变成<添加>
					btnOKUpd.Text = "添加";
					break;
			}
		...
		/// </summary>
		/// <param name="pobjActionForward">数据传输的目的类对象</param>
        ...
        protected void btnExportExcel4Dg_Click(object sender, System.EventArgs e)
		{
			...
			string strFileName = "ActionForward信息导出.xls";
			string strFolderName;
			string strDownLoadFileName;
			arrColName.Add("ActionForwardId"); arrCnName.Add("ActionForwardId");
			arrColName.Add("ActionId"); arrCnName.Add("ActionId");
			arrColName.Add("ForwardName"); arrCnName.Add("Forward名称");
			arrColName.Add("ForwardPath"); arrCnName.Add("Forward路径");
