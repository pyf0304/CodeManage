...
{
	/// <summary>
	/// wfmActionForm_QUDI 的摘要说明。其中Q代表查询，U代表修改
	/// </summary>
...
///                    clsCommForWebForm.CheckObjUser(pobjUser, this);
		...
		private void BindDg_ActionForm()
		{
			...
			objDV.Sort = strSortActionFormBy;     ///注意这一行代码有可能需要注释掉，目前这行仅仅在单独的查询界面中起作用
		...
		///　　　     在这里是把值传到表控件中
		/// </summary>
		/// <param name="strActionFormId">表记录的关键字，显示该表关键字的内容</param>
		private void ShowData(string strActionFormId)
		{
			...
			//1、检查关键字是否为空；
			if (strActionFormId == "") return ;		//如果关键字为空就返回退出
			//2、检查该关键字的记录是否存在，如果不存在就返回不显示；
			if (clsActionFormBL.IsExist(strActionFormId) == false)		//检查该关键字的记录是否存在
			{
				string ss = "关键字为：" + strActionFormId + "的不存在!";
		...
		/// </summary>
		/// <param name="pobjActionForm">表实体类对象</param>
		...
		protected void btnOKUpd_Click(object sender, System.EventArgs e)
		{
			...
			switch(strCommandText)
			{
				...
				case "确认添加":
					...
					//2、定义对象并初始化对象
					//clsActionFormEN objActionForm;	//定义对象
					objActionForm = new clsActionFormEN(wucActionForm1.ActionFormId);	//初始化新对象
					//3、判断是否有相同的关键字,如果主键是标识递增型就不需要判断是否存在
					if (clsActionFormBL.IsExist(wucActionForm1.ActionFormId))	//判断是否有相同的关键字
					{
						strMsg = "关键字字段已有相同的值";
					...
					//4、把值从界面层传到逻辑层或数据实体层
					PutDataToActionFormClass(objActionForm);		//把界面的值传到
					//5、检查传进去的对象属性是否合法
                    try
                    {
					...
					{
						strMsg = "在同一个界面组中已有相同的ActionForm，不能重复添加!";
						clsCommonJsFunc.Alert(this, strMsg);
						return;
					}
					//6、把数据实体层的数据存贮到数据库中
		...
		/// </summary>
		/// <param name="pobjActionForm">数据传输的目的类对象</param>
		...
		protected void btnExportExcel4Dg_Click(object sender, System.EventArgs e)
		{
			...
			string strFileName = "ActionForm信息导出.xls";
			string strFolderName;
			string strDownLoadFileName;
			arrColName.Add("ActionFormId"); arrCnName.Add("ActionFormId");
			arrColName.Add("ActionFormName"); arrCnName.Add("ActionForm名称");
			arrColName.Add("ViewGroupId"); arrCnName.Add("界面组ID");
			arrColName.Add("FormBeanType"); arrCnName.Add("FormBean类型类");
