...
{
	/// <summary>
	/// wfmPrjUserRole_QUDI 的摘要说明。其中Q代表查询，U代表修改
	/// </summary>
	public partial class wfmPrjUserRole_QUDI : System.Web.UI.Page
	{
		...
		private void ShowData(string strRoleId)
		{
			...
			//2、检查该关键字的记录是否存在，如果不存在就返回不显示；
			if (clsPrjUserRoleBL.IsExist(strRoleId) == false)		//检查该关键字的记录是否存在
			{
		...
		/// </summary>
		/// <param name="pobjPrjUserRole">表实体类对象</param>
		...
		protected void btnOKUpd_Click(object sender, System.EventArgs e)
		{
			...
			switch(strCommandText)
			{
				...
				case "确认添加":
					...
					//2、定义对象并初始化对象
					//clsPrjUserRoleEN objPrjUserRole;	//定义对象
					objPrjUserRole = new clsPrjUserRoleEN(wucPrjUserRole1.RoleId);	//初始化新对象
					//3、判断是否有相同的关键字,如果主键是标识递增型就不需要判断是否存在
					if (clsPrjUserRoleBL.IsExist(wucPrjUserRole1.RoleId))	//判断是否有相同的关键字
					{
						strMsg = "关键字字段已有相同的值";
					...
					//4、把值从界面层传到逻辑层或数据实体层
					PutDataToUserRoleClass(objPrjUserRole);		//把界面的值传到
					//5、检查传进去的对象属性是否合法
                    try
                    {
				...
				case "确认修改":
					...
					//5、把修改后的内容显示在DATAGRID中
					BindDg_UserRole();
					wucPrjUserRole1.Clear();//添空控件中的内容
					//恢复<确认修改>变成<添加>
					btnOKUpd.Text = "添加";
					break;
			}
		...
		/// </summary>
		/// <param name="pobjPrjUserRole">数据传输的目的类对象</param>
