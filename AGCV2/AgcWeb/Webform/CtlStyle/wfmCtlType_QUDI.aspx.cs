...
{
	/// <summary>
	/// wfmCtlTypeAbbr_QUDI 的摘要说明。其中Q代表查询，U代表修改
	/// </summary>
	public partial class wfmCtlTypeAbbr_QUDI : System.Web.UI.Page
	{
		...
		///　　　     在这里是把值传到表控件中
		/// </summary>
		/// <param name="strCtlTypeId">表记录的关键字，显示该表关键字的内容</param>
		private void ShowData(string strCtlTypeId)
		{
			...
			//1、检查关键字是否为空；
			if (strCtlTypeId == "") return ;		//如果关键字为空就返回退出
			//2、检查该关键字的记录是否存在，如果不存在就返回不显示；
			if (clsCtlTypeBL.IsExist(strCtlTypeId) == false)		//检查该关键字的记录是否存在
			{
				string ss = "关键字为：" + strCtlTypeId + "的不存在!";
		...
		/// </summary>
		/// <param name="pobjCtlTypeAbbr">表实体类对象</param>
		...
		protected void btnOKUpd_Click(object sender, System.EventArgs e)
		{
			...
			switch(strCommandText)
			{
				...
				case "确认添加":
					...
					//2、定义对象并初始化对象
					//clsCtlTypeEN objCtlTypeAbbr;	//定义对象
					objCtlTypeAbbr = new clsCtlTypeEN(wucCtlType1.CtlTypeId);	//初始化新对象
					//3、判断是否有相同的关键字,如果主键是标识递增型就不需要判断是否存在
					if (clsCtlTypeBL.IsExist(wucCtlType1.CtlTypeId))	//判断是否有相同的关键字
					{
						strMsg = "关键字字段已有相同的值";
					...
					//4、把值从界面层传到逻辑层或数据实体层
					PutDataToCtlTypeAbbrClass(objCtlTypeAbbr);		//把界面的值传到
					//5、检查传进去的对象属性是否合法
                    ///
		...
		/// </summary>
		/// <param name="pobjCtlTypeAbbr">数据传输的目的类对象</param>
