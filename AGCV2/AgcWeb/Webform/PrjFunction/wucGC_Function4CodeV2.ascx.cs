...
namespace AGC.Webform
{
    ...
    /// </summary>
    public partial class wucGC_Function4CodeV2 : System.Web.UI.UserControl
    {
        /// <summary>
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(vsFuncId4Code) == true) return;
            ShowFunctionWorkFlow(vsFuncId4Code);
        }
        ...
        public void ShowFunctionWorkFlow(string strFuncId4Code)
        {
            ...
            int intMaxlevelNum = arrwf_StepPointRela.Max(x => x.LevelNo ?? 0);
            int intI = 0;
            Dictionary<int, int> dicColNum = new Dictionary<int, int>();
            for (intI = 0; intI <= intMaxlevelNum; intI++)
            {
                IEnumerable<clswf_StepPointRelaEN> arrwf_StepPointRela_Level_i = arrwf_StepPointRela.Where(x => x.LevelNo == intI);
                int intColNum = arrwf_StepPointRela_Level_i.Count();
                if (intColNum > 0)
                {
                    dicColNum.Add(intI, intColNum);
                }
            }
            int intMaxColNum = dicColNum.Max(x => x.Value);
            ...
            for (intI = 0; intI <= intMaxlevelNum; intI++)
            {
                ...
                foreach (clswf_StepPointRelaEN objInFor in arrwf_StepPointRela_Level_i)
                {
                    ...
                    if (dicColNum[intI] == 1)
                    {
                        objCell.ColumnSpan = intMaxColNum;
                        objCell.CssClass = "text-center";
                    }
                    ...
                    if (objwf_Point.TabName == "NoTab")
                    {
                        ...
                        objButton.Text = string.Format("{0}", objwf_Point.PointName);
                        objButton.ToolTip = string.Format("{0}", objwf_Point.PointName);
                        objButton.CssClass = "text-warning";
                        objButton.OnClientClick = "";
                        objLabel.Text = "";// string.Format("不需要生成代码",                            objwf_Point.PointName);
                        objLabel.Visible = true;
                        objLabel.CssClass = "text-muted font-weight-bold";
                    }
                    else
                    {
                        ...
                        objButton.Visible = true;
                        objButton.Text = string.Format("{0}({1})", objCodeType.CodeTypeName, objProgLangType.ProgLangTypeSimName);
                        objButton.ToolTip = string.Format("{0}-{1}", objCodeType.CodeTypeId, objFunction4Code.FuncId4Code);
                        objButton.CssClass = "btn btn-outline-info btn-sm";
                        objButton.Click += btnGeneCode_Click;
                        objLabel.Text = string.Format("{0}",
    objFunction4Code.FunctionSignatureSim, objFunction4Code.FuncCHName4Code,
