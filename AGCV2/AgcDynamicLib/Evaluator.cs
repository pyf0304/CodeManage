...
namespace AgcDynamicLib
{
    ...
    {
        private string filepath = Path.Combine(clsSysParaEN_Local.strDynamicLibPath, "DynamicLib.config");
        ...
        private void ConstructEvaluator(EvaluatorItem[] items)
        {
            ...
            code.Append("namespace EvalGuy { ");                  //生成代码的命名空间为EvalGuy，和本代码一样   

            code.Append(" public class _Evaluator { ");          //产生 _Evaluator 类，所有可执行代码均在此类中运行   
            code.AppendFormat("   public int mySum(int a, int b)");
            code.Append("  {");
            code.AppendFormat("   return a+b;");
            code.Append("   }  ");
        ...
        public int EvaluateInt(string name, object[] myParams)
        {
            return (int)Evaluate(name, myParams);
        }
        /// <summary>   
        /// 执行字符串并返回字符串型值   
        /// </summary>   
        /// <param name="name">执行字符串名称</param>   
        /// <returns>执行结果</returns>   
        public string EvaluateString(string name, object[] myParams)
        {
            return (string)Evaluate(name, myParams);
        }
        /// <summary>   
        /// 执行字符串并返回布尔型值   
        /// </summary>   
        /// <param name="name">执行字符串名称</param>   
        /// <returns>执行结果</returns>   
        public bool EvaluateBool(string name, object[] myParams)
        {
            return (bool)Evaluate(name, myParams);
        }
        /// <summary>   
        /// 执行字符串并返 object 型值   
        /// </summary>   
        /// <param name="name">执行字符串名称</param>   
        /// <returns>执行结果</returns>   
        public object Evaluate(string name, object[] myParams)
        {
            ...
            return mi.Invoke(_Compiled, myParams);                  //执行 mi 所引用的方法   
        }
        ...
        static public int EvaluateToInteger(string code, object[] myParams)
        {
            Evaluator eval = new Evaluator(typeof(int), code, staticMethodName);//生成 Evaluator 类的对像   
            return (int)eval.Evaluate(staticMethodName, myParams);                        //执行并返回整型数据   
        }
        /// <summary>   
        /// 执行表达式并返回字符串型值   
        /// </summary>   
        /// <param name="code">要执行的表达式</param>   
        /// <returns>运算结果</returns>   
        static public string EvaluateToString(string code, object[] myParams)
        {
            Evaluator eval = new Evaluator(typeof(string), code, staticMethodName);//生成 Evaluator 类的对像   
            return (string)eval.Evaluate(staticMethodName, myParams);                     //执行并返回字符串型数据   
        }
        /// <summary>   
        /// 执行表达式并返回布尔型值   
        /// </summary>   
        /// <param name="code">要执行的表达式</param>   
        /// <returns>运算结果</returns>   
        static public bool EvaluateToBool(string code, object[] myParams)
        {
            Evaluator eval = new Evaluator(typeof(bool), code, staticMethodName);//生成 Evaluator 类的对像   
            return (bool)eval.Evaluate(staticMethodName, myParams);                       //执行并返回布尔型数据   
        }
        /// <summary>   
        /// 执行表达式并返回 object 型值   
        /// </summary>   
        /// <param name="code">要执行的表达式</param>   
        /// <returns>运算结果</returns>   
        static public object EvaluateToObject(string code, object[] myParams)
        {
            Evaluator eval = new Evaluator(typeof(object), code, staticMethodName);//生成 Evaluator 类的对像   
            return eval.Evaluate(staticMethodName, myParams);                             //执行并返回 object 型数据   
        }
        ...
        /// 静态方法的执行字符串名称   
        /// </summary>   
        private const string staticMethodName = "mySum";
        /// <summary>   
