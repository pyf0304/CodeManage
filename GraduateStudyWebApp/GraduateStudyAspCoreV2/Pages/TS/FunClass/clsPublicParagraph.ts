...
export class clsPublicParagraph extends clsGeneralTab2 {
   
    public AttributeName = new Array("TextId","TextValue", "NewTextValue", "IsTag", "IsChange", "LocationOne", "LocationTwo");
    //以下是属性变量
    ...
    private mstrTextId: string = "";    //顺序Id
    private mstrTextValue: string = "";    //文本
    private mstrNewTextValue: string = "";    //新文本
    private mbolIsTag: boolean = false;    //是否标签
    private mbolIsChange: boolean = false;    //是否改变
    private mstrLocationOne: string = "";    //位置1
    private mstrLocationTwo: string = "";    //位置2
 ...
 //是否改变
    ...
    public static GetNewStringList(strText: string, strNewText: string): string {
        ...
        let arrOldLst: Array<clsPublicParagraph> = this.SaveObjListByString(strText);

        //2.过滤标签后，赋值改变的新的对象列表
        let arrNewList: Array<clsPublicParagraph> = this.GetDataList(arrOldLst, strNewText);
        ...
        for (var i = 0; i < arrNewList.length; i++) {
            //通过获取的新的标签内容;
            var strTabNewContent: string = arrNewList[i].NewTextValue;
    ...
    public static GetDataList(arr: Array<clsPublicParagraph>, strNewTextValue: string): Array<clsPublicParagraph> {
