...
import { gs_PaperGroup_GetObjLst_Cache } from "../../L3_ForWApi/GraduateEduPaper/clsgs_PaperGroupWApi.js";
...
import { gs_PaperGroup_GetObjLstAsync } from "../../L3_ForWApi/GraduateEduPaper/clsgs_PaperGroupWApi.js";
import { gs_PaperGroup_SortFunByKey } from "../../L3_ForWApi/GraduateEduPaper/clsgs_PaperGroupWApi.js";
import { Format } from "../../PubFun/clsString.js";
import { gs_PaperGroup_FilterFunByKey } from "../../L3_ForWApi/GraduateEduPaper/clsgs_PaperGroupWApi.js";
import { clsSysPara4WebApi } from "../../PubConfig/clsSysPara4WebApi.js";
export const gs_PaperGroupEx_Controller = "gs_PaperGroupExApi";
export const gs_PaperGroupEx_ConstructorName = "gs_PaperGroupEx";
...
**/
export function gs_PaperGroupEx_GetWebApiUrl(strController: string, strAction: string): string {
...
**/
export function gs_PaperGroupEx_CopyToEx(objgs_PaperGroupENS: clsgs_PaperGroupEN): clsgs_PaperGroupENEx {
    const strThisFuncName = gs_PaperGroupEx_CopyToEx.name;
    const objgs_PaperGroupENT = new clsgs_PaperGroupENEx();
    try {
        ObjectAssign(objgs_PaperGroupENT, objgs_PaperGroupENS);
        return objgs_PaperGroupENT;
    }
    catch (e) {
        const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, gs_PaperGroupEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export async function gs_PaperGroupEx_GetObjExLstByPager_Cache(objPagerPara: stuPagerPara, strUpdUser: string): Promise<Array<clsgs_PaperGroupENEx>> {
    const strThisFuncName = "GetObjLstByPager_Cache";
    const arrgs_PaperGroupObjLst = await gs_PaperGroup_GetObjLst_Cache(strUpdUser);
    ...
    if (arrgs_PaperGroupExObjLst.length == 0) return arrgs_PaperGroupExObjLst;
    let arrgs_PaperGroup_Sel: Array<clsgs_PaperGroupENEx> = arrgs_PaperGroupExObjLst;
    ...
    try {
        ...
        const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, gs_PaperGroupEx_ConstructorName, strThisFuncName);
        console.error(strMsg);
        throw new Error(strMsg);
    }
    return new Array<clsgs_PaperGroupENEx>();
}
 ...
 * @returns 对象列表
*/
export async function gs_PaperGroupEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara): Promise<Array<clsgs_PaperGroupENEx>> {
    const strThisFuncName = "GetObjExLstByPagerAsync";
    const arrgs_PaperGroupObjLst = await gs_PaperGroup_GetObjLstAsync(objPagerPara.whereCond);
    const arrgs_PaperGroupExObjLst = arrgs_PaperGroupObjLst.map(gs_PaperGroupEx_CopyToEx);
    const objSortInfo = GetSortExpressInfo(objPagerPara);
    if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
        for (const objInFor of arrgs_PaperGroupExObjLst) {
 ...
 * @returns 返回两个对象比较的结果
*/
export function gs_PaperGroupEx_SortFunByKey(strKey: string, AscOrDesc: string) {
    const strThisFuncName = "SortFunByKey";
    if (AscOrDesc == "Asc" || AscOrDesc == "") {
        switch (strKey) {
            default:
                return gs_PaperGroup_SortFunByKey(strKey, AscOrDesc);
        }
    }
    else {
        switch (strKey) {
...
*/
export function gs_PaperGroupEx_FuncMapByFldName(strFldName: string, objgs_PaperGroupEx: clsgs_PaperGroupENEx) {
...
*/
export async function gs_PaperGroupEx_FilterFunByKey(strKey: string, value: any) {
    const strThisFuncName = "FilterFunByKey";
    let strMsg = "";
    switch (strKey) {
        default:
            return gs_PaperGroup_FilterFunByKey(strKey, value);
    }
}
