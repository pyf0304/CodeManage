...
import { JournalSubjectCategory_FilterFunByKey, JournalSubjectCategory_GetObjLstAsync, JournalSubjectCategory_GetObjLst_Cache, JournalSubjectCategory_SortFunByKey } from "../../L3_ForWApi/GraduateEduPaper/clsJournalSubjectCategoryWApi.js";
...
**/
export function JournalSubjectCategoryEx_GetWebApiUrl(strController: string, strAction: string): string {
...
**/
export function JournalSubjectCategoryEx_CopyToEx(objJournalSubjectCategoryENS: clsJournalSubjectCategoryEN): clsJournalSubjectCategoryENEx {
    ...
    try {
        ...
        const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, journalSubjectCategoryEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export async function JournalSubjectCategoryEx_GetObjExLstByPager_Cache(objPagerPara: stuPagerPara): Promise<Array<clsJournalSubjectCategoryENEx>> {
    const strThisFuncName = "GetObjLstByPager_Cache";
    const arrJournalSubjectCategoryObjLst = await JournalSubjectCategory_GetObjLst_Cache();
    ...
    let arrJournalSubjectCategory_Sel: Array<clsJournalSubjectCategoryENEx> = arrJournalSubjectCategoryExObjLst;
    ...
    try {
        ...
        const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, journalSubjectCategoryEx_ConstructorName, strThisFuncName);
        console.error(strMsg);
 ...
 * @returns 对象列表
*/
export async function JournalSubjectCategoryEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara): Promise<Array<clsJournalSubjectCategoryENEx>> {
    const strThisFuncName = "GetObjExLstByPagerAsync";
    const arrJournalSubjectCategoryObjLst = await JournalSubjectCategory_GetObjLstAsync(objPagerPara.whereCond);
    const arrJournalSubjectCategoryExObjLst = arrJournalSubjectCategoryObjLst.map(JournalSubjectCategoryEx_CopyToEx);
    const objSortInfo = GetSortExpressInfo(objPagerPara);
    if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
        for (const objInFor of arrJournalSubjectCategoryExObjLst) {
...
*/
export function JournalSubjectCategoryEx_FuncMapByFldName(strFldName: string, objJournalSubjectCategoryEx: clsJournalSubjectCategoryENEx) {
...
export async function JournalSubjectCategoryEx_ImportDataFromCsv(strCsv: string, strUserId: string): Promise<number> {
    const strThisFuncName = "";
    var strAction = "ImportDataFromCsv";
    let strUrl = JournalSubjectCategoryEx_GetWebApiUrl(journalSubjectCategoryEx_Controller, strAction);
    ...
    try {
        ...
        if (error.statusText == "error") {
            const strInfo = Format("网络错误！访问地址:{0}不成功！(in {1}.{2})", strUrl, journalSubjectCategoryEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else if (error.statusText == "Not Found") {
            const strInfo = Format("网络错误！访问地址:{0}可能不存在！(in {1}.{2})", strUrl, journalSubjectCategoryEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else {
