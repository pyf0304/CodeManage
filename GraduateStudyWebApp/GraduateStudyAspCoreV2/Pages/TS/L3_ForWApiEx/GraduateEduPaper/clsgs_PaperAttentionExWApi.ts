...
import { gs_PaperAttention_GetObjLstAsync } from "../../L3_ForWApi/GraduateEduPaper/clsgs_PaperAttentionWApi.js";
import { gs_PaperAttention_SortFunByKey } from "../../L3_ForWApi/GraduateEduPaper/clsgs_PaperAttentionWApi.js";
import { Format } from "../../PubFun/clsString.js";
import { gs_PaperAttention_FilterFunByKey } from "../../L3_ForWApi/GraduateEduPaper/clsgs_PaperAttentionWApi.js";
import { clsSysPara4WebApi } from "../../PubConfig/clsSysPara4WebApi.js";
export const gs_PaperAttentionEx_Controller = "gs_PaperAttentionExApi";
export const gs_PaperAttentionEx_ConstructorName = "gs_PaperAttentionEx";
...
**/
export function gs_PaperAttentionEx_GetWebApiUrl(strController: string, strAction: string): string {
...
**/
export function gs_PaperAttentionEx_CopyToEx(objgs_PaperAttentionENS: clsgs_PaperAttentionEN): clsgs_PaperAttentionENEx {
    const strThisFuncName = gs_PaperAttentionEx_CopyToEx.name;
    const objgs_PaperAttentionENT = new clsgs_PaperAttentionENEx();
    try {
        ObjectAssign(objgs_PaperAttentionENT, objgs_PaperAttentionENS);
        return objgs_PaperAttentionENT;
    }
    catch (e) {
        const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, gs_PaperAttentionEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export async function gs_PaperAttentionEx_GetObjExLstByPager_Cache(objPagerPara: stuPagerPara, strUserId: string): Promise<Array<clsgs_PaperAttentionENEx>> {
    const strThisFuncName = "GetObjLstByPager_Cache";
    const arrgs_PaperAttentionObjLst = await gs_PaperAttention_GetObjLst_Cache(strUserId);
    ...
    if (arrgs_PaperAttentionExObjLst.length == 0) return arrgs_PaperAttentionExObjLst;
    let arrgs_PaperAttention_Sel: Array<clsgs_PaperAttentionENEx> = arrgs_PaperAttentionExObjLst;
    ...
    try {
        ...
        const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, gs_PaperAttentionEx_ConstructorName, strThisFuncName);
        console.error(strMsg);
        throw new Error(strMsg);
    }
    return new Array<clsgs_PaperAttentionENEx>();
}
 ...
 * @returns 对象列表
*/
export async function gs_PaperAttentionEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara): Promise<Array<clsgs_PaperAttentionENEx>> {
    const strThisFuncName = "GetObjExLstByPagerAsync";
    const arrgs_PaperAttentionObjLst = await gs_PaperAttention_GetObjLstAsync(objPagerPara.whereCond);
    const arrgs_PaperAttentionExObjLst = arrgs_PaperAttentionObjLst.map(gs_PaperAttentionEx_CopyToEx);
    const objSortInfo = GetSortExpressInfo(objPagerPara);
    if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
        for (const objInFor of arrgs_PaperAttentionExObjLst) {
 ...
 * @returns 返回两个对象比较的结果
*/
export function gs_PaperAttentionEx_SortFunByKey(strKey: string, AscOrDesc: string) {
    const strThisFuncName = "SortFunByKey";
    if (AscOrDesc == "Asc" || AscOrDesc == "") {
        switch (strKey) {
            default:
                return gs_PaperAttention_SortFunByKey(strKey, AscOrDesc);
        }
    }
    else {
        switch (strKey) {
...
*/
export function gs_PaperAttentionEx_FuncMapByFldName(strFldName: string, objgs_PaperAttentionEx: clsgs_PaperAttentionENEx) {
...
*/
export async function gs_PaperAttentionEx_FilterFunByKey(strKey: string, value: any) {
    const strThisFuncName = "FilterFunByKey";
    let strMsg = "";
    switch (strKey) {
        default:
            return gs_PaperAttention_FilterFunByKey(strKey, value);
    }
}
