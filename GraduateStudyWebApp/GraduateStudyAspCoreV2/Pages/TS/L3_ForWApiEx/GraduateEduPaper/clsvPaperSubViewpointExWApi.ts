...
import { vPaperSubViewpoint_FilterFunByKey, vPaperSubViewpoint_GetObjLstAsync, vPaperSubViewpoint_GetObjLstByJSONObjLst, vPaperSubViewpoint_GetObjLst_Cache, vPaperSubViewpoint_SortFunByKey } from "../../L3_ForWApi/GraduateEduPaper/clsvPaperSubViewpointWApi.js";
    ...
    try {
        ...
        if (data.errorId == 0) {
            ...
            if (returnObjLst == null) {
                const strNullInfo = Format("获取数据为null, 请注意!(in {0}.{1})", vPaperSubViewpointEx_Controller, strThisFuncName);
        ...
        if (error.statusText == "error") {
            const strInfo = Format("网络错误！访问地址:{0}不成功！(in {1}.{2})", strUrl, vPaperSubViewpointEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else if (error.statusText == "Not Found") {
            const strInfo = Format("网络错误！访问地址:{0}可能不存在！(in {1}.{2})", strUrl, vPaperSubViewpointEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else {
