...
import { gs_PaperParagraphVer_GetObjLstByJSONObjLst } from "../../L3_ForWApi/GraduateEduPaper/clsgs_PaperParagraphVerWApi.js";
import { gs_PaperParagraph_FilterFunByKey, gs_PaperParagraph_GetObjLstAsync, gs_PaperParagraph_GetObjLstByJSONObjLst, gs_PaperParagraph_GetObjLst_Cache, gs_PaperParagraph_SortFunByKey } from "../../L3_ForWApi/GraduateEduPaper/clsgs_PaperParagraphWApi.js";
...
export async function gs_PaperParagraphEx_GetObjLstAsync(strWhereCond: string): Promise<Array<clsgs_PaperParagraphVerEN>> {
    ...
    try {
        ...
        if (data.errorId == 0) {
            ...
            if (returnObjLst == null) {
                const strNullInfo = Format("获取数据为null, 请注意!(in {0}.{1})", gs_PaperParagraphEx_Controller, strThisFuncName);
        ...
        if (error.statusText == "error") {
            const strInfo = Format("网络错误！访问地址:{0}不成功！(in {1}.{2})", strUrl, gs_PaperParagraphEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else if (error.statusText == "Not Found") {
            const strInfo = Format("网络错误！访问地址:{0}可能不存在！(in {1}.{2})", strUrl, gs_PaperParagraphEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else {
