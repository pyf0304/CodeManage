...
import { clsUsersENEx } from "../../L0_Entity/UserManage_Share/clsUsersENEx.js";
import { Users_FilterFunByKey, Users_GetObjLstAsync, Users_GetObjLst_Cache, Users_SortFunByKey } from "../../L3_ForWApi/UserManage_Share/clsUsersWApi.js";
...
export async function UsersEx_DelRecordAsync(strUserId: string): Promise<number> {
    ...
    try {
        ...
        if (error.statusText == "error") {
            const strInfo = Format("网络错误！访问地址:{0}不成功！(in {1}.{2})", strUrl, usersEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else if (error.statusText == "Not Found") {
            const strInfo = Format("网络错误！访问地址:{0}可能不存在！(in {1}.{2})", strUrl, usersEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else {
