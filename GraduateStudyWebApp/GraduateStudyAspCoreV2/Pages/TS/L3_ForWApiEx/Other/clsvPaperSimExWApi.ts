...
import { clsvPaperSimENEx } from "../../L0_Entity/GraduateEduPaper/clsvPaperSimENEx.js";
import { vPaperSim_FilterFunByKey, vPaperSim_GetObjLstAsync, vPaperSim_GetObjLstByJSONObjLst, vPaperSim_GetObjLst_Cache, vPaperSim_SortFunByKey } from "../../L3_ForWApi/GraduateEduPaper/clsvPaperSimWApi.js";
//import * as QQ from "q";
    ...
    try {
        ...
        if (data.errorId == 0) {
            ...
            if (returnObjLst == null) {
                const strNullInfo = Format("获取数据为null, 请注意!(in {0}.{1})", vPaperSimEx_Controller, strThisFuncName);
        ...
        if (error.statusText == "error") {
            const strInfo = Format("网络错误！访问地址:{0}不成功！(in {1}.{2})", strUrl, vPaperSimEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else if (error.statusText == "Not Found") {
            const strInfo = Format("网络错误！访问地址:{0}可能不存在！(in {1}.{2})", strUrl, vPaperSimEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else {
...
export async function vPaperSimEx_GetSubObjLstById_CurrEduCls_Cache(objvPaperSim_Cond: clsvPaperSimEN, strid_CurrEduCls: string) {
    ...
    //console.log(dicFldComparisonOp);
    try {
        var sstrKeys = GetObjKeys(objvPaperSim_Cond);
        //console.log(sstrKeys);
