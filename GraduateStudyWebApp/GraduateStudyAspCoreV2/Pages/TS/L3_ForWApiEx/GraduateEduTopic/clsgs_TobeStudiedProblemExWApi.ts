...
import { clsgs_TobeStudiedProblemENEx } from "../../L0_Entity/GraduateEduTopic/clsgs_TobeStudiedProblemENEx.js";
import { gs_TobeStudiedProblem_FilterFunByKey, gs_TobeStudiedProblem_GetObjLstAsync, gs_TobeStudiedProblem_GetObjLstByJSONObjLst, gs_TobeStudiedProblem_SortFunByKey } from "../../L3_ForWApi/GraduateEduTopic/clsgs_TobeStudiedProblemWApi.js";
...
**/
export function gs_TobeStudiedProblemEx_GetWebApiUrl(strController: string, strAction: string): string {
...
**/
export function gs_TobeStudiedProblemEx_CopyToEx(objgs_TobeStudiedProblemENS: clsgs_TobeStudiedProblemEN): clsgs_TobeStudiedProblemENEx {
    ...
    try {
        ...
        const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, gs_TobeStudiedProblemEx_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export async function gs_TobeStudiedProblemEx_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara): Promise<Array<clsgs_TobeStudiedProblemENEx>> {
    const strThisFuncName = "GetObjExLstByPagerAsync";
    const arrgs_TobeStudiedProblemExObjLst = await gs_TobeStudiedProblem_GetObjLstAsync(objPagerPara.whereCond);
    ...
    let arrgs_TobeStudiedProblem_Sel: Array<clsgs_TobeStudiedProblemENEx> = arrgs_TobeStudiedProblemExObjLst;
    try {
        ...
        const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, gs_TobeStudiedProblemEx_ConstructorName, strThisFuncName);
        console.error(strMsg);
...
*/
export function gs_TobeStudiedProblemEx_FuncMapByFldName(strFldName: string, objgs_TobeStudiedProblemEx: clsgs_TobeStudiedProblemENEx) {
...
export async function gs_TobeStudiedProblemEx_GetProblemYearNumObjLstAsync(strWhereCond: string): Promise<Array<clsgs_TobeStudiedProblemEN>> {
    const strThisFuncName = "GetProblemYearNumObjLstAsync";
    var strAction = "GetProblemYearNumObjLst";
    let strUrl = gs_TobeStudiedProblemEx_GetWebApiUrl(gs_TobeStudiedProblemEx_Controller, strAction);
    ...
    try {
        ...
        if (data.errorId == 0) {
            ...
            if (returnObjLst == null) {
                const strNullInfo = Format("获取数据为null, 请注意!(in {0}.{1})", gs_TobeStudiedProblemEx_Controller, strThisFuncName);
        ...
        if (error.statusText == "error") {
            const strInfo = Format("网络错误！访问地址:{0}不成功！(in {1}.{2})", strUrl, gs_TobeStudiedProblemEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else if (error.statusText == "Not Found") {
            const strInfo = Format("网络错误！访问地址:{0}可能不存在！(in {1}.{2})", strUrl, gs_TobeStudiedProblemEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else {
...
/// <param name = "strWhereCond">条件</param>
/// <returns>获取的相应对象列表</returns>
export async function gs_TobeStudiedProblemEx_GetProblemMonthNumObjLstAsync(strWhereCond: string): Promise<Array<clsgs_TobeStudiedProblemEN>> {
...
/// <param name = "strWhereCond">条件</param>
/// <returns>获取的相应对象列表</returns>
export async function gs_TobeStudiedProblemEx_GetProblemDateNumObjLstAsync(strWhereCond: string): Promise<Array<clsgs_TobeStudiedProblemEN>> {
