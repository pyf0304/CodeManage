...
import { gs_UserRelaPosition_FilterFunByKey, gs_UserRelaPosition_GetObjLstAsync, gs_UserRelaPosition_GetObjLst_Cache, gs_UserRelaPosition_SortFunByKey } from "../../L3_ForWApi/GraduateEduTools/clsgs_UserRelaPositionWApi.js";
...
export async function gs_UserRelaPositionEx_AddNewRecordAsync(strJSON: string): Promise<boolean> {
    ...
    try {
        ...
        if (error.statusText == "error") {
            const strInfo = Format("网络错误！访问地址:{0}不成功！(in {1}.{2})", strUrl, gs_UserRelaPositionEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else if (error.statusText == "Not Found") {
            const strInfo = Format("网络错误！访问地址:{0}可能不存在！(in {1}.{2})", strUrl, gs_UserRelaPositionEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else {
