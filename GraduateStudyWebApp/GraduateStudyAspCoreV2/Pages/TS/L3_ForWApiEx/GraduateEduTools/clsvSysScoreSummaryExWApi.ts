...
import { vSysScoreSummary_FilterFunByKey, vSysScoreSummary_GetObjLstAsync, vSysScoreSummary_GetObjLstByJSONObjLst, vSysScoreSummary_GetObjLst_Cache, vSysScoreSummary_SortFunByKey } from "../../L3_ForWApi/GraduateEduTools/clsvSysScoreSummaryWApi.js";
    ...
    try {
        ...
        if (data.errorId == 0) {
            ...
            if (returnObjLst == null) {
                const strNullInfo = Format("获取数据为null, 请注意!(in {0}.{1})", vSysScoreSummaryEx_Controller, strThisFuncName);
        ...
        if (error.statusText == "error") {
            const strInfo = Format("网络错误！访问地址:{0}不成功！(in {1}.{2})", strUrl, vSysScoreSummaryEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else if (error.statusText == "Not Found") {
            const strInfo = Format("网络错误！访问地址:{0}可能不存在！(in {1}.{2})", strUrl, vSysScoreSummaryEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else {
