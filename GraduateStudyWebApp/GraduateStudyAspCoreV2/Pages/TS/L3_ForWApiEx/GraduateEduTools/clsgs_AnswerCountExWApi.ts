...
import { gs_AnswerCount_FilterFunByKey, gs_AnswerCount_GetObjLstByJSONObjLst, gs_AnswerCount_GetObjLst_ClientCache, gs_AnswerCount_GetObjLst_localStorage, gs_AnswerCount_GetObjLst_sessionStorage } from "../../L3_ForWApi/GraduateEduTools/clsgs_AnswerCountWApi.js";
...
export async function gs_AnswerCountEx_AddNewRecordWithMaxIdAsync(objgs_AnswerCountEN: clsgs_AnswerCountEN): Promise<string> {
    ...
    try {
        ...
        if (error.statusText == "error") {
            const strInfo = Format("网络错误！访问地址:{0}不成功！(in {1}.{2})", strUrl, gs_AnswerCountEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else if (error.statusText == "Not Found") {
            const strInfo = Format("网络错误！访问地址:{0}可能不存在！(in {1}.{2})", strUrl, gs_AnswerCountEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else {
...
export async function gs_AnswerCountEx_GetSubObjLst_Cache(objgs_AnswerCount_Cond: clsgs_AnswerCountEN, strid_CurrEduCls: string) {
    ...
    //console.log(dicFldComparisonOp);
    try {
        var sstrKeys = GetObjKeys(objgs_AnswerCount_Cond);
        //console.log(sstrKeys);
    ...
    try {
        ...
        if (data.errorId == 0) {
            ...
            if (returnObjLst == null) {
                const strNullInfo = Format("获取数据为null, 请注意!(in {0}.{1})", gs_AnswerCountEx_Controller, strThisFuncName);
