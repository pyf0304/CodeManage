...
export async function qa_AnswerEx_DelRecordAsyncEx(strAnswerId: string): Promise<number> {
    ...
    try {
        ...
        if (error.statusText == "error") {
            const strInfo = Format("网络错误！访问地址:{0}不成功！(in {1}.{2})", strUrl, qa_AnswerEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else if (error.statusText == "Not Found") {
            const strInfo = Format("网络错误！访问地址:{0}可能不存在！(in {1}.{2})", strUrl, qa_AnswerEx_Controller, strThisFuncName);
            console.error(strInfo);
            throw (strInfo);
        }
        else {
