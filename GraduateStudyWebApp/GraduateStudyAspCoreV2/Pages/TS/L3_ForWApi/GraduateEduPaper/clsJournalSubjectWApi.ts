 ...
 * @returns 对象
 **/
export async function JournalSubject_GetObjByJournalSubjectIdAsync(strJournalSubjectId: string): Promise<clsJournalSubjectEN|null>  
{
const strThisFuncName = "GetObjByJournalSubjectIdAsync";

if (IsNullOrEmpty(strJournalSubjectId) == true)
{
  const strMsg = Format("参数:[strJournalSubjectId]不能为空！(In clsJournalSubjectWApi.GetObjByJournalSubjectIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strJournalSubjectId.length != 4)
{
const strMsg = Format("缓存分类变量:[strJournalSubjectId]的长度:[{0}]不正确！(clsJournalSubjectWApi.GetObjByJournalSubjectIdAsync)", strJournalSubjectId.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(strJournalSubjectId) == true)
{
  const strMsg = Format("参数:[strJournalSubjectId]不能为空！(In clsJournalSubjectWApi.GetObjByJournalSubjectId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strJournalSubjectId.length != 4)
{
const strMsg = Format("缓存分类变量:[strJournalSubjectId]的长度:[{0}]不正确！(clsJournalSubjectWApi.GetObjByJournalSubjectId_Cache)", strJournalSubjectId.length);
console.error(strMsg);
throw (strMsg);
}
...
try
{
const arrJournalSubject_Sel: Array <clsJournalSubjectEN> = arrJournalSubjectObjLst_Cache.filter(x => 
 x.journalSubjectId == strJournalSubjectId );
...
if (IsNullOrEmpty(strJournalSubjectId) == true)
{
  const strMsg = Format("参数:[strJournalSubjectId]不能为空！(In clsJournalSubjectWApi.GetObjByJournalSubjectId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
if (strJournalSubjectId.length != 4)
{
const strMsg = Format("缓存分类变量:[strJournalSubjectId]的长度:[{0}]不正确！(clsJournalSubjectWApi.GetObjByJournalSubjectId_localStorage)", strJournalSubjectId.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(strJournalSubjectId) == true)
{
  const strMsg = Format("参数:[strJournalSubjectId]不能为空！(In clsJournalSubjectWApi.GetNameByJournalSubjectId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strJournalSubjectId.length != 4)
{
const strMsg = Format("缓存分类变量:[strJournalSubjectId]的长度:[{0}]不正确！(clsJournalSubjectWApi.GetNameByJournalSubjectId_Cache)", strJournalSubjectId.length);
console.error(strMsg);
throw (strMsg);
}
const arrJournalSubjectObjLst_Cache = await JournalSubject_GetObjLst_Cache();
...
strOutFldName, clsJournalSubjectEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 返回一个关键字值列表
*/
export async function JournalSubject_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string): Promise<Array<string>> 
{
...
if (arrJournalSubject_Sel.length == 0) return [];
 ...
 * @returns 第一条记录对象
 **/
export async function JournalSubject_GetFirstObjAsync(strWhereCond: string): Promise<clsJournalSubjectEN|null>  
{
...
const strAction = "UpdateRecord";
 if (objJournalSubjectEN.sfUpdFldSetStr === undefined || objJournalSubjectEN.sfUpdFldSetStr === null || objJournalSubjectEN.sfUpdFldSetStr === "")
{
