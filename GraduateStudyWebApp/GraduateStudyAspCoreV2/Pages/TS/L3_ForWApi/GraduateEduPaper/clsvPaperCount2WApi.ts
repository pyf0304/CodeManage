 ...
 * @returns 对象
 **/
export async function vPaperCount2_GetObjByPaperIdAsync(strPaperId: string): Promise<clsvPaperCount2EN|null>  
{
const strThisFuncName = "GetObjByPaperIdAsync";

if (IsNullOrEmpty(strPaperId) == true)
{
  const strMsg = Format("参数:[strPaperId]不能为空！(In clsvPaperCount2WApi.GetObjByPaperIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strPaperId.length != 8)
{
const strMsg = Format("缓存分类变量:[strPaperId]的长度:[{0}]不正确！(clsvPaperCount2WApi.GetObjByPaperIdAsync)", strPaperId.length);
console.error(strMsg);
throw (strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function vPaperCount2_GetFirstObjAsync(strWhereCond: string): Promise<clsvPaperCount2EN|null>  
{
...
if (objPagerPara.pageIndex == 0) return new Array<clsvPaperCount2EN>();
