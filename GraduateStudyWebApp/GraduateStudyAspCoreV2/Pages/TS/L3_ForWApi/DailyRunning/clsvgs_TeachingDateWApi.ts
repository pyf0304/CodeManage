...
import { clsvgs_TeachingDateEN } from "../../L0_Entity/DailyRunning/clsvgs_TeachingDateEN.js";
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsvgs_TeachingDateWApi.GetObjBymId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
try
{
const arrvgs_TeachingDate_Sel = arrvgs_TeachingDateObjLst_Cache.filter(x => 
 x.mId == lngmId );
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsvgs_TeachingDateWApi.GetObjBymId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
...
{
  const strMsg = Format("参数:[strid_CurrEduCls_C]不能为空！(In clsvgs_TeachingDateWApi.func)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls_C.length != 8)
{
const strMsg = Format("缓存分类变量:[strid_CurrEduCls_C]的长度:[{0}]不正确！(clsvgs_TeachingDateWApi.func)", strid_CurrEduCls_C.length);
console.error(strMsg);
throw (strMsg);
}
 ...
 * @returns 返回一个关键字值列表
*/
export async function vgs_TeachingDate_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string, strid_CurrEduCls_C: string): Promise<Array<number>>
{
//const strThisFuncName = "funcKey";

if (IsNullOrEmpty(strid_CurrEduCls_C) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls_C]不能为空！(In clsvgs_TeachingDateWApi.funcKey)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls_C.length != 8)
{
const strMsg = Format("缓存分类变量:[strid_CurrEduCls_C]的长度:[{0}]不正确！(clsvgs_TeachingDateWApi.funcKey)", strid_CurrEduCls_C.length);
console.error(strMsg);
throw (strMsg);
}
...
if (arrvgs_TeachingDate_Sel.length == 0) return [];
...
{
strWhereCond = Format(clsvgs_TeachingDateEN.WhereFormat, strid_CurrEduCls);
}
else
{
...
{
const strMsg = Format("缓存分类变量:[id_CurrEduCls]不能为空！(in clsvgs_TeachingDateWApi.vgs_TeachingDate_GetObjLst_Cache() )");
console.error(strMsg);
throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
{
const strMsg = Format("缓存分类变量:[strid_CurrEduCls]的长度:[{0}]不正确！(in clsvgs_TeachingDateWApi.vgs_TeachingDate_GetObjLst_Cache() )", strid_CurrEduCls.length);
console.error(strMsg);
throw (strMsg);
}
