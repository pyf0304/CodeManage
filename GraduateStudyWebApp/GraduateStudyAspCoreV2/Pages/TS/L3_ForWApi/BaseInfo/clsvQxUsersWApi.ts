...
if (IsNullOrEmpty(strUserId) == true)
{
  const strMsg = Format("参数:[strUserId]不能为空！(In clsvQxUsersWApi.GetObjByUserId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
try
{
const arrvQxUsers_Sel: Array <clsvQxUsersEN> = arrvQxUsersObjLst_Cache.filter(x => 
 x.userId == strUserId );
...
if (IsNullOrEmpty(strUserId) == true)
{
  const strMsg = Format("参数:[strUserId]不能为空！(In clsvQxUsersWApi.GetObjByUserId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
...
strOutFldName, clsvQxUsersEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
...
if (objvQxUsers.GetFldValue(strOutFldName) == null) return "";
 ...
 * @returns 返回一个关键字值列表
*/
export async function vQxUsers_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string): Promise<Array<string>> 
{
...
if (arrvQxUsers_Sel.length == 0) return [];
