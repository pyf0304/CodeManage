 ...
 * @returns 对象
 **/
export async function vRTResearchResult_GetObjBymIdAsync(lngmId: number): Promise<clsvRTResearchResultEN|null>  
{
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsvRTResearchResultWApi.GetObjBymId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
try
{
const arrvRTResearchResult_Sel: Array <clsvRTResearchResultEN> = arrvRTResearchResultObjLst_Cache.filter(x => 
 x.mId == lngmId );
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsvRTResearchResultWApi.GetObjBymId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
...
strOutFldName, clsvRTResearchResultEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 返回一个关键字值列表
*/
export async function vRTResearchResult_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string): Promise<Array<number>> 
{
...
if (arrvRTResearchResult_Sel.length == 0) return [];
 ...
 * @returns 第一条记录对象
 **/
export async function vRTResearchResult_GetFirstObjAsync(strWhereCond: string): Promise<clsvRTResearchResultEN|null>  
{
