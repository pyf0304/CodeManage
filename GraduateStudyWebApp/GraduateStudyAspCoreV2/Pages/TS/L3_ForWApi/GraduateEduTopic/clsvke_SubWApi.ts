 ...
 * @returns 对象
 **/
export async function vke_Sub_GetObjByke_SubIdAsync(strke_SubId: string): Promise<clsvke_SubEN|null>  
{
const strThisFuncName = "GetObjByke_SubIdAsync";

if (IsNullOrEmpty(strke_SubId) == true)
{
  const strMsg = Format("参数:[strke_SubId]不能为空！(In clsvke_SubWApi.GetObjByke_SubIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strke_SubId.length != 4)
{
const strMsg = Format("缓存分类变量:[strke_SubId]的长度:[{0}]不正确！(clsvke_SubWApi.GetObjByke_SubIdAsync)", strke_SubId.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(strke_SubId) == true)
{
  const strMsg = Format("参数:[strke_SubId]不能为空！(In clsvke_SubWApi.GetObjByke_SubId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strke_SubId.length != 4)
{
const strMsg = Format("缓存分类变量:[strke_SubId]的长度:[{0}]不正确！(clsvke_SubWApi.GetObjByke_SubId_Cache)", strke_SubId.length);
console.error(strMsg);
throw (strMsg);
}
...
try
{
const arrvke_Sub_Sel: Array <clsvke_SubEN> = arrvke_SubObjLst_Cache.filter(x => 
...
if (IsNullOrEmpty(strke_SubId) == true)
{
  const strMsg = Format("参数:[strke_SubId]不能为空！(In clsvke_SubWApi.GetObjByke_SubId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
if (strke_SubId.length != 4)
{
const strMsg = Format("缓存分类变量:[strke_SubId]的长度:[{0}]不正确！(clsvke_SubWApi.GetObjByke_SubId_localStorage)", strke_SubId.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(strke_SubId) == true)
{
  const strMsg = Format("参数:[strke_SubId]不能为空！(In clsvke_SubWApi.GetNameByke_SubId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strke_SubId.length != 4)
{
const strMsg = Format("缓存分类变量:[strke_SubId]的长度:[{0}]不正确！(clsvke_SubWApi.GetNameByke_SubId_Cache)", strke_SubId.length);
console.error(strMsg);
throw (strMsg);
}
const arrvke_SubObjLst_Cache = await vke_Sub_GetObjLst_Cache();
...
strOutFldName, clsvke_SubEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
...
if (objvke_Sub.GetFldValue(strOutFldName) == null) return "";
...
return (a: clsvke_SubEN, b: clsvke_SubEN) => {
if (a.ke_SuperName_cn == null) return -1;
if (b.ke_SuperName_cn == null) return 1;
 ...
 * @returns 返回一个关键字值列表
*/
export async function vke_Sub_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string): Promise<Array<string>> 
{
...
if (arrvke_Sub_Sel.length == 0) return [];
 ...
 * @returns 第一条记录对象
 **/
export async function vke_Sub_GetFirstObjAsync(strWhereCond: string): Promise<clsvke_SubEN|null>  
{
...
if (objPagerPara.pageIndex == 0) return new Array<clsvke_SubEN>();
const arrvke_SubObjLst_Cache = await vke_Sub_GetObjLst_Cache();
