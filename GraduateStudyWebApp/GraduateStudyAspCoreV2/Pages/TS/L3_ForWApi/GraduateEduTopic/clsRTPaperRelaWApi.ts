...
import { vRTPaperRela_ReFreshThisCache } from "../../L3_ForWApi/GraduateEduTopic/clsvRTPaperRelaWApi.js";
 ...
 * @returns 对象
 **/
export async function RTPaperRela_GetObjBymIdAsync(lngmId: number): Promise<clsRTPaperRelaEN|null>  
{
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsRTPaperRelaWApi.GetObjBymId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
try
{
const arrRTPaperRela_Sel: Array <clsRTPaperRelaEN> = arrRTPaperRelaObjLst_Cache.filter(x => 
 x.mId == lngmId );
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsRTPaperRelaWApi.GetObjBymId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
...
strOutFldName, clsRTPaperRelaEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 返回一个关键字值列表
*/
export async function RTPaperRela_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string): Promise<Array<number>> 
{
...
if (arrRTPaperRela_Sel.length == 0) return [];
 ...
 * @returns 第一条记录对象
 **/
export async function RTPaperRela_GetFirstObjAsync(strWhereCond: string): Promise<clsRTPaperRelaEN|null>  
{
...
const strAction = "UpdateRecord";
 if (objRTPaperRelaEN.sfUpdFldSetStr === undefined || objRTPaperRelaEN.sfUpdFldSetStr === null || objRTPaperRelaEN.sfUpdFldSetStr === "")
{
