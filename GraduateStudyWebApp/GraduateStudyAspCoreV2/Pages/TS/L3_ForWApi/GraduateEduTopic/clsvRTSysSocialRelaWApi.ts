 ...
 * @returns 对象
 **/
export async function vRTSysSocialRela_GetObjBymIdAsync(lngmId: number): Promise<clsvRTSysSocialRelaEN|null>  
{
const strThisFuncName = "GetObjBymIdAsync";

if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsvRTSysSocialRelaWApi.GetObjBymIdAsync)");
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsvRTSysSocialRelaWApi.GetObjBymId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
try
{
const arrvRTSysSocialRela_Sel: Array <clsvRTSysSocialRelaEN> = arrvRTSysSocialRelaObjLst_Cache.filter(x => 
 x.mId == lngmId );
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsvRTSysSocialRelaWApi.GetObjBymId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsvRTSysSocialRelaWApi.GetNameBymId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
const arrvRTSysSocialRelaObjLst_Cache = await vRTSysSocialRela_GetObjLst_Cache();
...
if (arrvRTSysSocialRela_Sel.length == 0) return [];
