 ...
 * @returns 对象
 **/
export async function gs_TobeStudiedProblem_GetObjByProblemIdAsync(strProblemId: string): Promise<clsgs_TobeStudiedProblemEN|null>  
{
const strThisFuncName = "GetObjByProblemIdAsync";

if (IsNullOrEmpty(strProblemId) == true)
{
  const strMsg = Format("参数:[strProblemId]不能为空！(In clsgs_TobeStudiedProblemWApi.GetObjByProblemIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strProblemId.length != 10)
{
const strMsg = Format("缓存分类变量:[strProblemId]的长度:[{0}]不正确！(clsgs_TobeStudiedProblemWApi.GetObjByProblemIdAsync)", strProblemId.length);
console.error(strMsg);
throw (strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function gs_TobeStudiedProblem_GetFirstObjAsync(strWhereCond: string): Promise<clsgs_TobeStudiedProblemEN|null>  
{
...
const strAction = "UpdateRecord";
 if (objgs_TobeStudiedProblemEN.sfUpdFldSetStr === undefined || objgs_TobeStudiedProblemEN.sfUpdFldSetStr === null || objgs_TobeStudiedProblemEN.sfUpdFldSetStr === "")
{
