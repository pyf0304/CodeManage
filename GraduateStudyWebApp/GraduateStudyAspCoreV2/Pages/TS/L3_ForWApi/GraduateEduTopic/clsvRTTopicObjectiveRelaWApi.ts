 ...
 * @returns 对象
 **/
export async function vRTTopicObjectiveRela_GetObjBymIdAsync(lngmId: number): Promise<clsvRTTopicObjectiveRelaEN|null>  
{
const strThisFuncName = "GetObjBymIdAsync";

if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsvRTTopicObjectiveRelaWApi.GetObjBymIdAsync)");
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsvRTTopicObjectiveRelaWApi.GetObjBymId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
try
{
const arrvRTTopicObjectiveRela_Sel: Array <clsvRTTopicObjectiveRelaEN> = arrvRTTopicObjectiveRelaObjLst_Cache.filter(x => 
 x.mId == lngmId );
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsvRTTopicObjectiveRelaWApi.GetObjBymId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
...
strOutFldName, clsvRTTopicObjectiveRelaEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 返回一个关键字值列表
*/
export async function vRTTopicObjectiveRela_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string): Promise<Array<number>> 
{
...
if (arrvRTTopicObjectiveRela_Sel.length == 0) return [];
 ...
 * @returns 第一条记录对象
 **/
export async function vRTTopicObjectiveRela_GetFirstObjAsync(strWhereCond: string): Promise<clsvRTTopicObjectiveRelaEN|null>  
{
