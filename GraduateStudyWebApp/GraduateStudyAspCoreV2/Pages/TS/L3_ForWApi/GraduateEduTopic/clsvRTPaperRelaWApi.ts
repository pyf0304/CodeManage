...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsvRTPaperRelaWApi.GetObjBymId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
try
{
const arrvRTPaperRela_Sel: Array <clsvRTPaperRelaEN> = arrvRTPaperRelaObjLst_Cache.filter(x => 
 x.mId == lngmId );
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsvRTPaperRelaWApi.GetObjBymId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
...
if (arrvRTPaperRela_Sel.length == 0) return [];
