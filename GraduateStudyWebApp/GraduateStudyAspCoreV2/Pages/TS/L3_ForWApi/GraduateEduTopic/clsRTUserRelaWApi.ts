...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsRTUserRelaWApi.GetObjBymId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
try
{
const arrRTUserRela_Sel = arrRTUserRelaObjLst_Cache.filter(x => 
 x.mId == lngmId );
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsRTUserRelaWApi.GetObjBymId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
 ...
 * @returns 返回一个输出字段值
*/
export async function RTUserRela_func(strInFldName:string , strOutFldName:string , strInValue:string 
, strTopicId_C: string)
{
...
{
  const strMsg = Format("参数:[strTopicId_C]不能为空！(In clsRTUserRelaWApi.func)");
console.error(strMsg);
 throw (strMsg);
}
if (strTopicId_C.length != 8)
{
const strMsg = Format("缓存分类变量:[strTopicId_C]的长度:[{0}]不正确！(clsRTUserRelaWApi.func)", strTopicId_C.length);
console.error(strMsg);
throw (strMsg);
}
...
strOutFldName, clsRTUserRelaEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 返回一个关键字值列表
*/
export async function RTUserRela_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string, strTopicId_C: string): Promise<Array<number>>
{
//const strThisFuncName = "funcKey";

if (IsNullOrEmpty(strTopicId_C) == true)
{
  const strMsg = Format("参数:[strTopicId_C]不能为空！(In clsRTUserRelaWApi.funcKey)");
console.error(strMsg);
 throw (strMsg);
}
if (strTopicId_C.length != 8)
{
const strMsg = Format("缓存分类变量:[strTopicId_C]的长度:[{0}]不正确！(clsRTUserRelaWApi.funcKey)", strTopicId_C.length);
console.error(strMsg);
throw (strMsg);
}
...
if (arrRTUserRela_Sel.length == 0) return [];
 ...
 * @returns 第一条记录对象
 **/
export async function RTUserRela_GetFirstObjAsync(strWhereCond: string): Promise<clsRTUserRelaEN|null>  
{
...
{
strWhereCond = Format(clsRTUserRelaEN.WhereFormat, strTopicId);
}
else
{
...
{
const strMsg = Format("缓存分类变量:[TopicId]不能为空！(in clsRTUserRelaWApi.RTUserRela_GetObjLst_Cache() )");
console.error(strMsg);
throw (strMsg);
}
if (strTopicId.length != 8)
{
const strMsg = Format("缓存分类变量:[strTopicId]的长度:[{0}]不正确！(in clsRTUserRelaWApi.RTUserRela_GetObjLst_Cache() )", strTopicId.length);
console.error(strMsg);
throw (strMsg);
}
...
const strAction = "UpdateRecord";
 if (objRTUserRelaEN.sfUpdFldSetStr === undefined || objRTUserRelaEN.sfUpdFldSetStr === null || objRTUserRelaEN.sfUpdFldSetStr === "")
{
...
if (IsNullOrEmpty(strTopicId) == true)
{
  const strMsg = Format("参数:[strTopicId]不能为空！(In clsRTUserRelaWApi.clsRTUserRelaWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strTopicId.length != 8)
{
const strMsg = Format("缓存分类变量:[strTopicId]的长度:[{0}]不正确！(clsRTUserRelaWApi.clsRTUserRelaWApi.ReFreshCache)", strTopicId.length);
console.error(strMsg);
throw (strMsg);
}
