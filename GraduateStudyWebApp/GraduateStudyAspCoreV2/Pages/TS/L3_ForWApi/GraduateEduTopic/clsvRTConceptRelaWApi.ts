 ...
 * @returns 对象
 **/
export async function vRTConceptRela_GetObjBymIdAsync(lngmId: number): Promise<clsvRTConceptRelaEN|null>  
{
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsvRTConceptRelaWApi.GetObjBymId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
try
{
const arrvRTConceptRela_Sel: Array <clsvRTConceptRelaEN> = arrvRTConceptRelaObjLst_Cache.filter(x => 
 x.mId == lngmId );
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsvRTConceptRelaWApi.GetObjBymId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
...
strOutFldName, clsvRTConceptRelaEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 返回一个关键字值列表
*/
export async function vRTConceptRela_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string): Promise<Array<number>> 
{
...
if (arrvRTConceptRela_Sel.length == 0) return [];
 ...
 * @returns 第一条记录对象
 **/
export async function vRTConceptRela_GetFirstObjAsync(strWhereCond: string): Promise<clsvRTConceptRelaEN|null>  
{
