 ...
 * @returns 对象
 **/
export async function gs_TeacherTask_GetObjBymIdAsync(lngmId: number): Promise<clsgs_TeacherTaskEN|null>  
{
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsgs_TeacherTaskWApi.GetObjBymId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
try
{
const arrgs_TeacherTask_Sel: Array <clsgs_TeacherTaskEN> = arrgs_TeacherTaskObjLst_Cache.filter(x => 
 x.mId == lngmId );
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsgs_TeacherTaskWApi.GetObjBymId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
...
strOutFldName, clsgs_TeacherTaskEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 返回一个关键字值列表
*/
export async function gs_TeacherTask_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string): Promise<Array<number>> 
{
...
if (arrgs_TeacherTask_Sel.length == 0) return [];
 ...
 * @returns 第一条记录对象
 **/
export async function gs_TeacherTask_GetFirstObjAsync(strWhereCond: string): Promise<clsgs_TeacherTaskEN|null>  
{
...
const strAction = "UpdateRecord";
 if (objgs_TeacherTaskEN.sfUpdFldSetStr === undefined || objgs_TeacherTaskEN.sfUpdFldSetStr === null || objgs_TeacherTaskEN.sfUpdFldSetStr === "")
{
