 ...
 * @returns 对象
 **/
export async function vgs_ResearchResult_GetObjByResultIdAsync(strResultId: string): Promise<clsvgs_ResearchResultEN|null>  
{
const strThisFuncName = "GetObjByResultIdAsync";

if (IsNullOrEmpty(strResultId) == true)
{
  const strMsg = Format("参数:[strResultId]不能为空！(In clsvgs_ResearchResultWApi.GetObjByResultIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strResultId.length != 10)
{
const strMsg = Format("缓存分类变量:[strResultId]的长度:[{0}]不正确！(clsvgs_ResearchResultWApi.GetObjByResultIdAsync)", strResultId.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(strResultId) == true)
{
  const strMsg = Format("参数:[strResultId]不能为空！(In clsvgs_ResearchResultWApi.GetObjByResultId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strResultId.length != 10)
{
const strMsg = Format("缓存分类变量:[strResultId]的长度:[{0}]不正确！(clsvgs_ResearchResultWApi.GetObjByResultId_Cache)", strResultId.length);
console.error(strMsg);
throw (strMsg);
}
...
try
{
const arrvgs_ResearchResult_Sel: Array <clsvgs_ResearchResultEN> = arrvgs_ResearchResultObjLst_Cache.filter(x => 
 x.resultId == strResultId );
...
if (IsNullOrEmpty(strResultId) == true)
{
  const strMsg = Format("参数:[strResultId]不能为空！(In clsvgs_ResearchResultWApi.GetObjByResultId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
if (strResultId.length != 10)
{
const strMsg = Format("缓存分类变量:[strResultId]的长度:[{0}]不正确！(clsvgs_ResearchResultWApi.GetObjByResultId_localStorage)", strResultId.length);
console.error(strMsg);
throw (strMsg);
}
...
strOutFldName, clsvgs_ResearchResultEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 返回一个关键字值列表
*/
export async function vgs_ResearchResult_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string): Promise<Array<string>> 
{
...
if (arrvgs_ResearchResult_Sel.length == 0) return [];
 ...
 * @returns 第一条记录对象
 **/
export async function vgs_ResearchResult_GetFirstObjAsync(strWhereCond: string): Promise<clsvgs_ResearchResultEN|null>  
{
