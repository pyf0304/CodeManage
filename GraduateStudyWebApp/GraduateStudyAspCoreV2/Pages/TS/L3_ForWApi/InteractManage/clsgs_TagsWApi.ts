...
if (IsNullOrEmpty(strTagsId) == true)
{
  const strMsg = Format("参数:[strTagsId]不能为空！(In clsgs_TagsWApi.GetObjByTagsId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strTagsId.length != 10)
{
const strMsg = Format("缓存分类变量:[strTagsId]的长度:[{0}]不正确！(clsgs_TagsWApi.GetObjByTagsId_Cache)", strTagsId.length);
console.error(strMsg);
throw (strMsg);
}
...
try
{
const arrgs_Tags_Sel: Array <clsgs_TagsEN> = arrgs_TagsObjLst_Cache.filter(x => 
 x.tagsId == strTagsId );
...
if (IsNullOrEmpty(strTagsId) == true)
{
  const strMsg = Format("参数:[strTagsId]不能为空！(In clsgs_TagsWApi.GetObjByTagsId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
if (strTagsId.length != 10)
{
const strMsg = Format("缓存分类变量:[strTagsId]的长度:[{0}]不正确！(clsgs_TagsWApi.GetObjByTagsId_localStorage)", strTagsId.length);
console.error(strMsg);
throw (strMsg);
}
...
strOutFldName, clsgs_TagsEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
...
if (objgs_Tags.GetFldValue(strOutFldName) == null) return "";
 ...
 * @returns 返回一个关键字值列表
*/
export async function gs_Tags_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string): Promise<Array<string>> 
{
...
if (arrgs_Tags_Sel.length == 0) return [];
...
if (Object.prototype.hasOwnProperty.call(objgs_Tags_Cond.dicFldComparisonOp, clsgs_TagsEN.con_UserId) == true)
{
const strComparisonOp_UserId:string = objgs_Tags_Cond.dicFldComparisonOp[clsgs_TagsEN.con_UserId];
strWhereCond += Format(" And {0} {2} '{1}'", clsgs_TagsEN.con_UserId, objgs_Tags_Cond.userId, strComparisonOp_UserId);
}
