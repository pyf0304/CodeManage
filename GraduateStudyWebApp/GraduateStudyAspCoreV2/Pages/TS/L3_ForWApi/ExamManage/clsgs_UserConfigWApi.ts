 ...
 * @returns 对象
 **/
export async function gs_UserConfig_GetObjByConfigIdAsync(lngConfigId: number): Promise<clsgs_UserConfigEN|null>  
{
const strThisFuncName = "GetObjByConfigIdAsync";

if (lngConfigId == 0)
{
  const strMsg = Format("参数:[lngConfigId]不能为空！(In clsgs_UserConfigWApi.GetObjByConfigIdAsync)");
...
if (lngConfigId == 0)
{
  const strMsg = Format("参数:[lngConfigId]不能为空！(In clsgs_UserConfigWApi.GetObjByConfigId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
try
{
const arrgs_UserConfig_Sel: Array <clsgs_UserConfigEN> = arrgs_UserConfigObjLst_Cache.filter(x => 
 x.configId == lngConfigId );
...
if (lngConfigId == 0)
{
  const strMsg = Format("参数:[lngConfigId]不能为空！(In clsgs_UserConfigWApi.GetObjByConfigId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
...
strOutFldName, clsgs_UserConfigEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 返回一个关键字值列表
*/
export async function gs_UserConfig_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string): Promise<Array<number>> 
{
...
if (arrgs_UserConfig_Sel.length == 0) return [];
 ...
 * @returns 第一条记录对象
 **/
export async function gs_UserConfig_GetFirstObjAsync(strWhereCond: string): Promise<clsgs_UserConfigEN|null>  
{
...
const strAction = "UpdateRecord";
 if (objgs_UserConfigEN.sfUpdFldSetStr === undefined || objgs_UserConfigEN.sfUpdFldSetStr === null || objgs_UserConfigEN.sfUpdFldSetStr === "")
{
