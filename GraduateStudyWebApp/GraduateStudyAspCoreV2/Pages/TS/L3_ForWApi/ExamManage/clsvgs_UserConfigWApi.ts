 ...
 * @returns 对象
 **/
export async function vgs_UserConfig_GetObjByConfigIdAsync(lngConfigId: number): Promise<clsvgs_UserConfigEN|null>  
{
const strThisFuncName = "GetObjByConfigIdAsync";

if (lngConfigId == 0)
{
  const strMsg = Format("参数:[lngConfigId]不能为空！(In clsvgs_UserConfigWApi.GetObjByConfigIdAsync)");
...
if (lngConfigId == 0)
{
  const strMsg = Format("参数:[lngConfigId]不能为空！(In clsvgs_UserConfigWApi.GetObjByConfigId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
try
{
const arrvgs_UserConfig_Sel: Array <clsvgs_UserConfigEN> = arrvgs_UserConfigObjLst_Cache.filter(x => 
...
if (lngConfigId == 0)
{
  const strMsg = Format("参数:[lngConfigId]不能为空！(In clsvgs_UserConfigWApi.GetObjByConfigId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
...
strOutFldName, clsvgs_UserConfigEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 返回一个关键字值列表
*/
export async function vgs_UserConfig_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string): Promise<Array<number>> 
{
...
if (arrvgs_UserConfig_Sel.length == 0) return [];
 ...
 * @returns 第一条记录对象
 **/
export async function vgs_UserConfig_GetFirstObjAsync(strWhereCond: string): Promise<clsvgs_UserConfigEN|null>  
{
