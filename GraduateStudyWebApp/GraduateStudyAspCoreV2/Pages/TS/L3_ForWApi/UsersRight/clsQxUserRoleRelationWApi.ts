...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsQxUserRoleRelationWApi.GetObjBymId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
try
{
const arrQxUserRoleRelation_Sel: Array <clsQxUserRoleRelationEN> = arrQxUserRoleRelationObjLst_Cache.filter(x => 
 x.mId == lngmId );
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsQxUserRoleRelationWApi.GetObjBymId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
...
strOutFldName, clsQxUserRoleRelationEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 返回一个关键字值列表
*/
export async function QxUserRoleRelation_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string): Promise<Array<number>> 
{
...
if (arrQxUserRoleRelation_Sel.length == 0) return [];
