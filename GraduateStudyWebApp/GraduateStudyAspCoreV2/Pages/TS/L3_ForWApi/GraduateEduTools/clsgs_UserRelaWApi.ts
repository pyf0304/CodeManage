 ...
 * @returns 对象
 **/
export async function gs_UserRela_GetObjBymIdAsync(lngmId: number): Promise<clsgs_UserRelaEN|null>  
{
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsgs_UserRelaWApi.GetObjBymId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
try
{
const arrgs_UserRela_Sel: Array <clsgs_UserRelaEN> = arrgs_UserRelaObjLst_Cache.filter(x => 
 x.mId == lngmId );
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsgs_UserRelaWApi.GetObjBymId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
 ...
 * @returns 返回一个输出字段值
*/
export async function gs_UserRela_func(strInFldName:string , strOutFldName:string , strInValue:string 
...
{
  const strMsg = Format("参数:[strid_CurrEduCls_C]不能为空！(In clsgs_UserRelaWApi.func)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls_C.length != 8)
{
const strMsg = Format("缓存分类变量:[strid_CurrEduCls_C]的长度:[{0}]不正确！(clsgs_UserRelaWApi.func)", strid_CurrEduCls_C.length);
console.error(strMsg);
throw (strMsg);
}
...
strOutFldName, clsgs_UserRelaEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 返回一个关键字值列表
*/
export async function gs_UserRela_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string, strid_CurrEduCls_C: string): Promise<Array<number>>
{
//const strThisFuncName = "funcKey";

if (IsNullOrEmpty(strid_CurrEduCls_C) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls_C]不能为空！(In clsgs_UserRelaWApi.funcKey)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls_C.length != 8)
{
const strMsg = Format("缓存分类变量:[strid_CurrEduCls_C]的长度:[{0}]不正确！(clsgs_UserRelaWApi.funcKey)", strid_CurrEduCls_C.length);
console.error(strMsg);
throw (strMsg);
}
...
if (arrgs_UserRela_Sel.length == 0) return [];
 ...
 * @returns 第一条记录对象
 **/
export async function gs_UserRela_GetFirstObjAsync(strWhereCond: string): Promise<clsgs_UserRelaEN|null>  
{
...
{
strWhereCond = Format(clsgs_UserRelaEN.WhereFormat, strid_CurrEduCls);
}
else
{
...
{
const strMsg = Format("缓存分类变量:[id_CurrEduCls]不能为空！(in clsgs_UserRelaWApi.gs_UserRela_GetObjLst_Cache() )");
console.error(strMsg);
throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
{
const strMsg = Format("缓存分类变量:[strid_CurrEduCls]的长度:[{0}]不正确！(in clsgs_UserRelaWApi.gs_UserRela_GetObjLst_Cache() )", strid_CurrEduCls.length);
console.error(strMsg);
throw (strMsg);
}
...
const strAction = "UpdateRecord";
 if (objgs_UserRelaEN.sfUpdFldSetStr === undefined || objgs_UserRelaEN.sfUpdFldSetStr === null || objgs_UserRelaEN.sfUpdFldSetStr === "")
{
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In clsgs_UserRelaWApi.clsgs_UserRelaWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
{
const strMsg = Format("缓存分类变量:[strid_CurrEduCls]的长度:[{0}]不正确！(clsgs_UserRelaWApi.clsgs_UserRelaWApi.ReFreshCache)", strid_CurrEduCls.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(pobjgs_UserRelaEN.userId) == false && GetStrLen(pobjgs_UserRelaEN.userId) > 18)
{
