 ...
 * @returns 对象
 **/
export async function vSysScoreWeight_GetObjByScoreWeightIdAsync(strScoreWeightId: string): Promise<clsvSysScoreWeightEN|null>  
{
const strThisFuncName = "GetObjByScoreWeightIdAsync";

if (IsNullOrEmpty(strScoreWeightId) == true)
{
  const strMsg = Format("参数:[strScoreWeightId]不能为空！(In clsvSysScoreWeightWApi.GetObjByScoreWeightIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strScoreWeightId.length != 1)
{
const strMsg = Format("缓存分类变量:[strScoreWeightId]的长度:[{0}]不正确！(clsvSysScoreWeightWApi.GetObjByScoreWeightIdAsync)", strScoreWeightId.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(strScoreWeightId) == true)
{
  const strMsg = Format("参数:[strScoreWeightId]不能为空！(In clsvSysScoreWeightWApi.GetObjByScoreWeightId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strScoreWeightId.length != 1)
{
const strMsg = Format("缓存分类变量:[strScoreWeightId]的长度:[{0}]不正确！(clsvSysScoreWeightWApi.GetObjByScoreWeightId_Cache)", strScoreWeightId.length);
console.error(strMsg);
throw (strMsg);
}
...
try
{
const arrvSysScoreWeight_Sel: Array <clsvSysScoreWeightEN> = arrvSysScoreWeightObjLst_Cache.filter(x => 
 x.scoreWeightId == strScoreWeightId );
...
if (IsNullOrEmpty(strScoreWeightId) == true)
{
  const strMsg = Format("参数:[strScoreWeightId]不能为空！(In clsvSysScoreWeightWApi.GetObjByScoreWeightId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
if (strScoreWeightId.length != 1)
{
const strMsg = Format("缓存分类变量:[strScoreWeightId]的长度:[{0}]不正确！(clsvSysScoreWeightWApi.GetObjByScoreWeightId_localStorage)", strScoreWeightId.length);
console.error(strMsg);
throw (strMsg);
}
...
strOutFldName, clsvSysScoreWeightEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 返回一个关键字值列表
*/
export async function vSysScoreWeight_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string): Promise<Array<string>> 
{
...
if (arrvSysScoreWeight_Sel.length == 0) return [];
 ...
 * @returns 第一条记录对象
 **/
export async function vSysScoreWeight_GetFirstObjAsync(strWhereCond: string): Promise<clsvSysScoreWeightEN|null>  
{
