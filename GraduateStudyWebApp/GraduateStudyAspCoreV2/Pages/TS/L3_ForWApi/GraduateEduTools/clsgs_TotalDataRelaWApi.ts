 ...
 * @returns 对象
 **/
export async function gs_TotalDataRela_GetObjBymIdAsync(lngmId: number): Promise<clsgs_TotalDataRelaEN|null>  
{
const strThisFuncName = "GetObjBymIdAsync";

if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsgs_TotalDataRelaWApi.GetObjBymIdAsync)");
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsgs_TotalDataRelaWApi.GetObjBymId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
try
{
const arrgs_TotalDataRela_Sel: Array <clsgs_TotalDataRelaEN> = arrgs_TotalDataRelaObjLst_Cache.filter(x => 
 x.mId == lngmId );
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsgs_TotalDataRelaWApi.GetObjBymId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
 ...
 * @returns 返回一个输出字段值
*/
export async function gs_TotalDataRela_func(strInFldName:string , strOutFldName:string , strInValue:string 
...
{
  const strMsg = Format("参数:[strid_CurrEduCls_C]不能为空！(In clsgs_TotalDataRelaWApi.func)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls_C.length != 8)
{
const strMsg = Format("缓存分类变量:[strid_CurrEduCls_C]的长度:[{0}]不正确！(clsgs_TotalDataRelaWApi.func)", strid_CurrEduCls_C.length);
console.error(strMsg);
throw (strMsg);
}
...
strOutFldName, clsgs_TotalDataRelaEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
...
if (objgs_TotalDataRela.GetFldValue(strOutFldName) == null) return "";
...
return (a: clsgs_TotalDataRelaEN, b: clsgs_TotalDataRelaEN) => {
if (a.tdRelaTypeId == null) return -1;
if (b.tdRelaTypeId == null) return 1;
 ...
 * @returns 返回一个关键字值列表
*/
export async function gs_TotalDataRela_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string, strid_CurrEduCls_C: string): Promise<Array<number>>
{
//const strThisFuncName = "funcKey";

if (IsNullOrEmpty(strid_CurrEduCls_C) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls_C]不能为空！(In clsgs_TotalDataRelaWApi.funcKey)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls_C.length != 8)
{
const strMsg = Format("缓存分类变量:[strid_CurrEduCls_C]的长度:[{0}]不正确！(clsgs_TotalDataRelaWApi.funcKey)", strid_CurrEduCls_C.length);
console.error(strMsg);
throw (strMsg);
}
...
if (arrgs_TotalDataRela == null) return [];
...
if (arrgs_TotalDataRela_Sel.length == 0) return [];
 ...
 * @returns 第一条记录对象
 **/
export async function gs_TotalDataRela_GetFirstObjAsync(strWhereCond: string): Promise<clsgs_TotalDataRelaEN|null>  
{
...
if (IsNullOrEmpty(clsgs_TotalDataRelaEN.WhereFormat) == false)
{
strWhereCond = Format(clsgs_TotalDataRelaEN.WhereFormat, strid_CurrEduCls);
}
else
{
...
{
const strMsg = Format("缓存分类变量:[id_CurrEduCls]不能为空！(in clsgs_TotalDataRelaWApi.gs_TotalDataRela_GetObjLst_Cache() )");
console.error(strMsg);
throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
{
const strMsg = Format("缓存分类变量:[strid_CurrEduCls]的长度:[{0}]不正确！(in clsgs_TotalDataRelaWApi.gs_TotalDataRela_GetObjLst_Cache() )", strid_CurrEduCls.length);
console.error(strMsg);
throw (strMsg);
}
...
if (objPagerPara.pageIndex == 0) return new Array<clsgs_TotalDataRelaEN>();
const arrgs_TotalDataRelaObjLst_Cache = await gs_TotalDataRela_GetObjLst_Cache(strid_CurrEduCls);
...
const strAction = "UpdateRecord";
 if (objgs_TotalDataRelaEN.sfUpdFldSetStr === undefined || objgs_TotalDataRelaEN.sfUpdFldSetStr === null || objgs_TotalDataRelaEN.sfUpdFldSetStr === "")
{
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In clsgs_TotalDataRelaWApi.clsgs_TotalDataRelaWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
{
const strMsg = Format("缓存分类变量:[strid_CurrEduCls]的长度:[{0}]不正确！(clsgs_TotalDataRelaWApi.clsgs_TotalDataRelaWApi.ReFreshCache)", strid_CurrEduCls.length);
console.error(strMsg);
throw (strMsg);
}
...
pobjgs_TotalDataRelaEN.sfUpdFldSetStr = pobjgs_TotalDataRelaEN.updFldString;
let strJson = "";
try
{
...
objgs_TotalDataRelaENT.sfUpdFldSetStr = objgs_TotalDataRelaENS.updFldString; //sfUpdFldSetStr
}
