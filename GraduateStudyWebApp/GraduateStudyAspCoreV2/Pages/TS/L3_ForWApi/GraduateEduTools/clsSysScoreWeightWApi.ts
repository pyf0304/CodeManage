 ...
 * @returns 对象
 **/
export async function SysScoreWeight_GetObjByScoreWeightIdAsync(strScoreWeightId: string): Promise<clsSysScoreWeightEN|null>  
{
const strThisFuncName = "GetObjByScoreWeightIdAsync";

if (IsNullOrEmpty(strScoreWeightId) == true)
{
  const strMsg = Format("参数:[strScoreWeightId]不能为空！(In clsSysScoreWeightWApi.GetObjByScoreWeightIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strScoreWeightId.length != 1)
{
const strMsg = Format("缓存分类变量:[strScoreWeightId]的长度:[{0}]不正确！(clsSysScoreWeightWApi.GetObjByScoreWeightIdAsync)", strScoreWeightId.length);
console.error(strMsg);
throw (strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function SysScoreWeight_GetFirstObjAsync(strWhereCond: string): Promise<clsSysScoreWeightEN|null>  
{
...
const strAction = "UpdateRecord";
 if (objSysScoreWeightEN.sfUpdFldSetStr === undefined || objSysScoreWeightEN.sfUpdFldSetStr === null || objSysScoreWeightEN.sfUpdFldSetStr === "")
{
