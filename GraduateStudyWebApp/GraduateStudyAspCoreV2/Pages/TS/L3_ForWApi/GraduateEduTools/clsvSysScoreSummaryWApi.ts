 ...
 * @returns 对象
 **/
export async function vSysScoreSummary_GetObjBymIdAsync(lngmId: number): Promise<clsvSysScoreSummaryEN|null>  
{
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsvSysScoreSummaryWApi.GetObjBymId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
try
{
const arrvSysScoreSummary_Sel: Array <clsvSysScoreSummaryEN> = arrvSysScoreSummaryObjLst_Cache.filter(x => 
 x.mId == lngmId );
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空！(In clsvSysScoreSummaryWApi.GetObjBymId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
 ...
 * @returns 返回一个输出字段值
*/
export async function vSysScoreSummary_func(strInFldName:string , strOutFldName:string , strInValue:string 
...
{
  const strMsg = Format("参数:[strid_CurrEduCls_C]不能为空！(In clsvSysScoreSummaryWApi.func)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls_C.length != 8)
{
const strMsg = Format("缓存分类变量:[strid_CurrEduCls_C]的长度:[{0}]不正确！(clsvSysScoreSummaryWApi.func)", strid_CurrEduCls_C.length);
console.error(strMsg);
throw (strMsg);
}
...
strOutFldName, clsvSysScoreSummaryEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 返回一个关键字值列表
*/
export async function vSysScoreSummary_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string, strid_CurrEduCls_C: string): Promise<Array<number>>
{
//const strThisFuncName = "funcKey";

if (IsNullOrEmpty(strid_CurrEduCls_C) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls_C]不能为空！(In clsvSysScoreSummaryWApi.funcKey)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls_C.length != 8)
{
const strMsg = Format("缓存分类变量:[strid_CurrEduCls_C]的长度:[{0}]不正确！(clsvSysScoreSummaryWApi.funcKey)", strid_CurrEduCls_C.length);
console.error(strMsg);
throw (strMsg);
}
...
if (arrvSysScoreSummary_Sel.length == 0) return [];
 ...
 * @returns 第一条记录对象
 **/
export async function vSysScoreSummary_GetFirstObjAsync(strWhereCond: string): Promise<clsvSysScoreSummaryEN|null>  
{
...
{
strWhereCond = Format(clsvSysScoreSummaryEN.WhereFormat, strid_CurrEduCls);
}
else
{
...
{
const strMsg = Format("缓存分类变量:[id_CurrEduCls]不能为空！(in clsvSysScoreSummaryWApi.vSysScoreSummary_GetObjLst_Cache() )");
console.error(strMsg);
throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
{
const strMsg = Format("缓存分类变量:[strid_CurrEduCls]的长度:[{0}]不正确！(in clsvSysScoreSummaryWApi.vSysScoreSummary_GetObjLst_Cache() )", strid_CurrEduCls.length);
console.error(strMsg);
throw (strMsg);
}
