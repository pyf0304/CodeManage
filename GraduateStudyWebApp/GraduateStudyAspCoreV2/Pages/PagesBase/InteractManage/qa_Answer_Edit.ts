...
import { qa_Answer_CheckPropertyNew } from "../../TS/L3_ForWApi/InteractManage/clsqa_AnswerWApi.js";
import { qa_Answer_AddNewRecordWithMaxIdAsync } from "../../TS/L3_ForWApi/InteractManage/clsqa_AnswerWApi.js";
import { qa_Answer_ReFreshCache } from "../../TS/L3_ForWApi/InteractManage/clsqa_AnswerWApi.js";
import { qa_Answer_GetUniCondStr_AnswerId } from "../../TS/L3_ForWApi/InteractManage/clsqa_AnswerWApi.js";
import { qa_Answer_IsExistRecordAsync } from "../../TS/L3_ForWApi/InteractManage/clsqa_AnswerWApi.js";
import { qa_Answer_GetUniCondStr4Update_AnswerId } from "../../TS/L3_ForWApi/InteractManage/clsqa_AnswerWApi.js";
import { clsqa_AnswerEN } from "../../TS/L0_Entity/InteractManage/clsqa_AnswerEN.js";
import { qa_Answer_IsExistAsync } from "../../TS/L3_ForWApi/InteractManage/clsqa_AnswerWApi.js";
import { qa_Answer_GetObjByAnswerIdAsync } from "../../TS/L3_ForWApi/InteractManage/clsqa_AnswerWApi.js";
import { qa_Answer_CheckProperty4Update } from "../../TS/L3_ForWApi/InteractManage/clsqa_AnswerWApi.js";
import { qa_Answer_UpdateRecordAsync } from "../../TS/L3_ForWApi/InteractManage/clsqa_AnswerWApi.js";
 ...
 **/
export abstract class qa_Answer_Edit 
{
public opType = "";
public keyId = "";
...
public static objPage_Edit: qa_Answer_Edit;
public objqa_Answer: clsqa_AnswerEN;
...
this.objqa_Answer = new clsqa_AnswerEN();
}
...
//检查相关控件是否存在
CheckControlExist(this.divName4Edit, "div", "divEditDialog_qa_Answer");
CheckControlExist(this.divName4Edit, "h4", "lblDialogTitle_qa_Answer");
...
if (qa_Answer_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01)
{
...
const strCommandText: string = this.btnSubmit_qa_Answer;
try {
...
this.iShowList.BindGv(clsqa_AnswerEN._CurrTabName, returnKeyId);
}
}
else
{
...
this.iShowList.BindGv(clsqa_AnswerEN._CurrTabName, returnBool.toString());
}
}
break;
case "确认修改":
...
try
{
const returnString = await qa_Answer_GetMaxStrIdAsync();
if (returnString == "")
{
const strInfo = Format("获取表qa_Answer的最大关键字为空，不成功，请检查!");
//显示信息框
alert(strInfo);
}
else
...
const objqa_AnswerEN = new clsqa_AnswerEN();
try
{
...
try
{
qa_Answer_CheckPropertyNew(objqa_AnswerEN);
}
catch(e)
{
...
let returnBool = false;
const returnKeyId = await qa_Answer_AddNewRecordWithMaxIdAsync(objqa_AnswerEN);
if (IsNullOrEmpty(returnKeyId) == false)
{
returnBool = true;
}
if (returnBool == true)
{
qa_Answer_ReFreshCache(qa_Answer_Edit.TopicId_Cache);
 ...
 **/
public async CheckUniCond4Add_AnswerId(objqa_AnswerEN: clsqa_AnswerEN): Promise<boolean>{
const strThisFuncName = this.CheckUniCond4Add_AnswerId.name;
const strUniquenessCondition = qa_Answer_GetUniCondStr_AnswerId(objqa_AnswerEN);
const bolIsExistCondition = await qa_Answer_IsExistRecordAsync(strUniquenessCondition);
 ...
 **/
public async CheckUniCond4Update_AnswerId(objqa_AnswerEN: clsqa_AnswerEN): Promise<boolean>{
const strThisFuncName = this.CheckUniCond4Update_AnswerId.name;
const strUniquenessCondition = qa_Answer_GetUniCondStr4Update_AnswerId(objqa_AnswerEN);
const bolIsExistCondition = await qa_Answer_IsExistRecordAsync(strUniquenessCondition);
if (bolIsExistCondition == true)
{
const strMsg = Format("不能满足唯一性条件。满足条件：{0}的记录已经存在！", strUniquenessCondition);
...
const responseKeyId = await qa_Answer_AddNewRecordWithMaxIdAsync(objqa_AnswerEN);
...
const strThisFuncName = this.ShowData.name;
        ...
        //2、检查该关键字的记录是否存在,如果不存在就返回不显示；
        let objqa_AnswerEN = new clsqa_AnswerEN();
        try {
            const returnBool = await qa_Answer_IsExistAsync(strAnswerId);
            if (returnBool == false) {
                const strInfo = Format("关键字:[{0}] 的记录不存在!", strAnswerId);
...
console.error(strMsg);
        ...
        try {
const objqa_AnswerEN_Const = await qa_Answer_GetObjByAnswerIdAsync(strAnswerId);
 ...
 **/
public async UpdateRecord(strAnswerId: string):Promise<boolean> 
{
...
try
{
qa_Answer_CheckProperty4Update(objqa_AnswerEN);
}
catch(e)
{
const strMsg = Format("检查数据不成功,{0}.(in {1}.{2})", e, this.constructor.name, strThisFuncName);
...
public  set answerContent(value: string) {
 SetInputValueByIdInDiv(this.divName4Edit, "txtAnswerContent", value);
}
 /**
 * 答案内容 (Used In PutDataToClass())
 **/
public  get answerContent():string {
const strValue = GetInputValueInDiv(this.divName4Edit, "txtAnswerContent");
 ...
 **/
public  set btnCancel_qa_Answer(value: string) {
 SetButtonHtmlByIdInDiv(this.divName4Edit, "btnCancel_qa_Answer", value);
}
 /**
 * 获取按钮的标题
 **/
public  get btnSubmit_qa_Answer():string {
const strValue = GetButtonHtmlInDiv(this.divName4Edit, "btnSubmit_qa_Answer");
return strValue;
}
 /**
 * 设置确定按钮的标题(Used In AddNewRecord())
 **/
public  set btnSubmit_qa_Answer(value: string) {
 SetButtonHtmlByIdInDiv(this.divName4Edit, "btnSubmit_qa_Answer", value);
}
}