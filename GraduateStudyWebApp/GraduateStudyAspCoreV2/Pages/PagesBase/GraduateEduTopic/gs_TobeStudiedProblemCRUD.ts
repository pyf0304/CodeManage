 ...
 **/
//import $ from "jquery";
import { vgs_TobeStudiedProblem_GetRecCountByCondAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsvgs_TobeStudiedProblemWApi.js";
import { vgs_TobeStudiedProblem_GetObjLstAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsvgs_TobeStudiedProblemWApi.js";
import { gs_TobeStudiedProblem_DelRecordAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsgs_TobeStudiedProblemWApi.js";
...
import { clsvgs_TobeStudiedProblemENEx } from "../../TS/L0_Entity/GraduateEduTopic/clsvgs_TobeStudiedProblemENEx.js";
import { vgs_TobeStudiedProblemEx_FuncMapByFldName } from "../../TS/L3_ForWApiEx/GraduateEduTopic/clsvgs_TobeStudiedProblemExWApi.js";
import { vgs_TobeStudiedProblem_GetObjLstByPagerAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsvgs_TobeStudiedProblemWApi.js";
import { gs_TobeStudiedProblem_GetObjLstByProblemIdLstAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsgs_TobeStudiedProblemWApi.js";
import { gs_TobeStudiedProblem_GetMaxStrIdAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsgs_TobeStudiedProblemWApi.js";
import { gs_TobeStudiedProblem_AddNewRecordAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsgs_TobeStudiedProblemWApi.js";
import { gs_TobeStudiedProblem_Delgs_TobeStudiedProblemsAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsgs_TobeStudiedProblemWApi.js";
...
{
public static objPage_CRUD: gs_TobeStudiedProblemCRUD;
 ...
 **/
public CombineData(arrvgs_TobeStudiedProblemObjLst: Array<clsvgs_TobeStudiedProblemEN>, arrDataColumn: Array<clsDataColumn>) 
{
...
const strFileName = Format("待研究问题({0})导出.xlsx",
 ...
 **/
public async ExportExcel_vgs_TobeStudiedProblem() 
{
...
{
const strMsg = Format("在显示列表时，排序字段(sortvgs_TobeStudiedProblemBy)为空，请检查！(In BindGv_vgs_TobeStudiedProblem_Cache)");
console.error(strMsg);
alert(strMsg);
return;
}
...
try
{
this.recCount = await vgs_TobeStudiedProblem_GetRecCountByCondAsync(strWhereCond);
if (this.recCount == 0)
{
...
arrvgs_TobeStudiedProblemObjLst = await vgs_TobeStudiedProblem_GetObjLstAsync(strWhereCond);
}
catch(e)
{
...
try
{
const objgs_TobeStudiedProblemEN = await gs_TobeStudiedProblem_GetObjByProblemIdAsync(strProblemId);
...
{
strWhereCond += Format(" And {0} like '% {1}%'", clsvgs_TobeStudiedProblemEN.con_ProblemTitle, this.problemTitle_q);
}
if ( this.problemDate_q != "")
{
strWhereCond += Format(" And {0} like '% {1}%'", clsvgs_TobeStudiedProblemEN.con_ProblemDate, this.problemDate_q);
}
}
catch(objException)
{
const strMsg:string = Format("(errid:WiTsCs0009)在组合查询条件(Combinegs_TobeStudiedProblemCondition)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
return strWhereCond;
}
...
{
const strMsg:string = Format("(errid:WiTsCs0010)在组合查询条件对象(Combinegs_TobeStudiedProblemConditionObj)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
...
//例如 1 = 1 && UserName = '张三'
let strWhereCond = " 1 = 1 ";
const objvgs_TobeStudiedProblem_Cond = new clsvgs_TobeStudiedProblemENEx();
//如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
try
{
if ( this.problemTitle_q != "")
...
{
const strMsg:string = Format("(errid:WiTsCs0013)在组合导出Excel条件对象(Combinegs_TobeStudiedProblemConditionObj4ExportExcel)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
objvgs_TobeStudiedProblem_Cond.whereCond = strWhereCond;
return objvgs_TobeStudiedProblem_Cond;
 ...
 **/
public async BindTab_vgs_TobeStudiedProblem(divContainer: string, arrvgs_TobeStudiedProblemObjLst: Array<clsvgs_TobeStudiedProblemEN>) 
{
...
await BindTab_V(o, arrvgs_TobeStudiedProblemObjLst, arrDataColumn,  clsvgs_TobeStudiedProblemEN.con_ProblemId, this); 
 ...
 **/
public async ExtendFldFuncMap(arrvgs_TobeStudiedProblemExObjLst: Array<clsvgs_TobeStudiedProblemENEx>, arrDataColumn: Array<clsDataColumn>) {
...
if (gs_TobeStudiedProblemCRUD.sortvgs_TobeStudiedProblemBy == null)
{
const strMsg = Format("在显示列表时，排序字段(sortvgs_TobeStudiedProblemBy)为空，请检查！(In BindGv_vgs_TobeStudiedProblem)");
console.error(strMsg);
alert(strMsg);
return;
}
...
orderBy: gs_TobeStudiedProblemCRUD.sortvgs_TobeStudiedProblemBy,//如果该字段为空，就使用下面的排序函数
 ...
 * @returns 返回两个对象比较的结果
 **/
public SortFun_ExportExcel(a:clsvgs_TobeStudiedProblemEN , b:clsvgs_TobeStudiedProblemEN): number 
{
...
const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(objAnchorElement, gs_TobeStudiedProblemCRUD.ascOrDesc4SortFun, gs_TobeStudiedProblemCRUD.sortvgs_TobeStudiedProblemBy, strSortExpress);
...
try
{
const arrgs_TobeStudiedProblemObjLst = await gs_TobeStudiedProblem_GetObjLstByProblemIdLstAsync(arrProblemId);
...
const strValue = GetInputValueInDiv(this.divName4Query, "txtProblemDate_q");
...
public  set problemDate_q(value: string) {
 SetInputValueByIdInDiv(this.divName4Query, "txtProblemDate_q", value);
}
 /**
 * 问题标题 (Used In CombineCondition())
 **/
public  get problemTitle_q():string {
const strValue = GetInputValueInDiv(this.divName4Query, "txtProblemTitle_q");
if (strValue == undefined) return "";
else return strValue.toString();
}
 /**
...
public  set problemTitle_q(value: string) {
 SetInputValueByIdInDiv(this.divName4Query, "txtProblemTitle_q", value);
}
 /**
