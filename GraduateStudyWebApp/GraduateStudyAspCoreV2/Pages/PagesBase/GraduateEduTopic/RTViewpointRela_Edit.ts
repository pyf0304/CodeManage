...
import { Viewpoint_BindDdl_ViewpointIdByCourseIdInDiv_Cache } from "../../TS/L3_ForWApi/GraduateEduTopic/clsViewpointWApi.js";
import { RTViewpointRela_CheckPropertyNew } from "../../TS/L3_ForWApi/GraduateEduTopic/clsRTViewpointRelaWApi.js";
import { RTViewpointRela_AddNewRecordAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsRTViewpointRelaWApi.js";
import { RTViewpointRela_GetUniCondStr_TopicId_ViewpointId } from "../../TS/L3_ForWApi/GraduateEduTopic/clsRTViewpointRelaWApi.js";
import { RTViewpointRela_IsExistRecordAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsRTViewpointRelaWApi.js";
import { RTViewpointRela_GetUniCondStr4Update_TopicId_ViewpointId } from "../../TS/L3_ForWApi/GraduateEduTopic/clsRTViewpointRelaWApi.js";
import { RTViewpointRela_IsExistAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsRTViewpointRelaWApi.js";
import { RTViewpointRela_GetObjBymIdAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsRTViewpointRelaWApi.js";
import { RTViewpointRela_CheckProperty4Update } from "../../TS/L3_ForWApi/GraduateEduTopic/clsRTViewpointRelaWApi.js";
 ...
 **/
export abstract class RTViewpointRela_Edit 
{
public opType = "";
public keyId = "";
...
public static objPage_Edit: RTViewpointRela_Edit;
public objRTViewpointRela: clsRTViewpointRelaEN;
...
this.objRTViewpointRela = new clsRTViewpointRelaEN();
}
 ...
 **/
public ShowDialog_RTViewpointRela(strOp:string) {
const strThisFuncName = this.ShowDialog_RTViewpointRela.name;
//检查相关控件是否存在
CheckControlExist(this.divName4Edit, "div", "divEditDialog_RTViewpointRela");
CheckControlExist(this.divName4Edit, "h4", "lblDialogTitle_RTViewpointRela");
 ...
 **/
public HideDialog_RTViewpointRela() {
...
if (RTViewpointRela_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01)
{
 ...
 **/
    ...
    public async SetDdl_ViewpointIdInDiv(courseId: string, strid_CurrEduCls:string)
{
await Viewpoint_BindDdl_ViewpointIdByCourseIdInDiv_Cache(this.divName4Edit, "ddlViewpointId" , courseId, strid_CurrEduCls);//
}
...
const strCommandText: string = this.btnSubmit_RTViewpointRela;
try {
...
this.iShowList.BindGv(clsRTViewpointRelaEN._CurrTabName, returnBool.toString());
}
}
break;
case "确认修改":
...
pobjRTViewpointRelaEN.SetUpdDate(clsDateTime.getTodayDateTimeStr(1));// 修改日期
pobjRTViewpointRelaEN.SetUpdUser(clsPublocalStorage.userId);// 修改人
...
const objRTViewpointRelaEN = new clsRTViewpointRelaEN();
try
{
...
try
{
RTViewpointRela_CheckPropertyNew(objRTViewpointRelaEN);
}
catch(e)
{
...
let returnBool = false;
returnBool = await RTViewpointRela_AddNewRecordAsync(objRTViewpointRelaEN);
if (returnBool == true)
{
 ...
 **/
public async CheckUniCond4Add_TopicId_ViewpointId(objRTViewpointRelaEN: clsRTViewpointRelaEN): Promise<boolean>{
const strThisFuncName = this.CheckUniCond4Add_TopicId_ViewpointId.name;
const strUniquenessCondition = RTViewpointRela_GetUniCondStr_TopicId_ViewpointId(objRTViewpointRelaEN);
const bolIsExistCondition = await RTViewpointRela_IsExistRecordAsync(strUniquenessCondition);
 ...
 **/
public async CheckUniCond4Update_TopicId_ViewpointId(objRTViewpointRelaEN: clsRTViewpointRelaEN): Promise<boolean>{
const strThisFuncName = this.CheckUniCond4Update_TopicId_ViewpointId.name;
const strUniquenessCondition = RTViewpointRela_GetUniCondStr4Update_TopicId_ViewpointId(objRTViewpointRelaEN);
const bolIsExistCondition = await RTViewpointRela_IsExistRecordAsync(strUniquenessCondition);
if (bolIsExistCondition == true)
{
const strMsg = Format("不能满足唯一性条件。满足条件：{0}的记录已经存在！", strUniquenessCondition);
...
const strThisFuncName = this.ShowData.name;
        ...
        //2、检查该关键字的记录是否存在,如果不存在就返回不显示；
        let objRTViewpointRelaEN = new clsRTViewpointRelaEN();
        try {
            const returnBool = await RTViewpointRela_IsExistAsync(lngmId);
            if (returnBool == false) {
...
console.error(strMsg);
        ...
        try {
const objRTViewpointRelaEN_Const = await RTViewpointRela_GetObjBymIdAsync(lngmId);
 ...
 **/
public GetDataFromRTViewpointRelaClass(pobjRTViewpointRelaEN: clsRTViewpointRelaEN )
{
...
try
{
RTViewpointRela_CheckProperty4Update(objRTViewpointRelaEN);
}
catch(e)
{
const strMsg = Format("检查数据不成功,{0}.(in {1}.{2})", e, this.constructor.name, strThisFuncName);
...
const returnBool = await RTViewpointRela_UpdateRecordAsync(objRTViewpointRelaEN);
if (returnBool == true)
{
//RTViewpointRela_ReFreshCache();
}
 ...
 **/
public  set btnCancel_RTViewpointRela(value: string) {
 SetButtonHtmlByIdInDiv(this.divName4Edit, "btnCancel_RTViewpointRela", value);
}
 /**
 * 获取按钮的标题
 **/
public  get btnSubmit_RTViewpointRela():string {
const strValue = GetButtonHtmlInDiv(this.divName4Edit, "btnSubmit_RTViewpointRela");
return strValue;
}
 /**
 * 设置确定按钮的标题(Used In AddNewRecord())
 **/
public  set btnSubmit_RTViewpointRela(value: string) {
 SetButtonHtmlByIdInDiv(this.divName4Edit, "btnSubmit_RTViewpointRela", value);
}
 /**
 * 备注 (Used In Clear())
 **/
...
public  set proposePeople(value: string) {
 SetInputValueByIdInDiv(this.divName4Edit, "txtProposePeople", value);
}
 /**
 * 提出人 (Used In PutDataToClass())
 **/
public  get proposePeople():string {
const strValue = GetInputValueInDiv(this.divName4Edit, "txtProposePeople");
if (strValue == undefined) return "";
else return strValue.toString();
}
 /**
...
SetSelectValueByIdInDiv(this.divName4Edit, "ddlViewpointId", value);
}
 /**
 * 观点Id (Used In PutDataToClass())
 **/
public  get viewpointId():string {
const strValue = GetSelectValueInDiv(this.divName4Edit, "ddlViewpointId");
if (strValue == undefined) return "";
else if (strValue == "0") return "";
else return strValue;
}
