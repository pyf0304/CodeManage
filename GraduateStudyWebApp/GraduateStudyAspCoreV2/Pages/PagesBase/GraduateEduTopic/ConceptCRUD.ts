...
import { Concept_GetObjLstAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsConceptWApi.js";
import { Concept_GetRecCountByCond_Cache } from "../../TS/L3_ForWApi/GraduateEduTopic/clsConceptWApi.js";
import { Concept_GetSubObjLst_Cache } from "../../TS/L3_ForWApi/GraduateEduTopic/clsConceptWApi.js";
import { Concept_DelRecordAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsConceptWApi.js";
...
import { clsConceptENEx } from "../../TS/L0_Entity/GraduateEduTopic/clsConceptENEx.js";
import { ConceptEx_FuncMapByFldName } from "../../TS/L3_ForWApiEx/GraduateEduTopic/clsConceptExWApi.js";
import { Concept_SortFun_Defa } from "../../TS/L3_ForWApi/GraduateEduTopic/clsConceptWApi.js";
import { Concept_GetObjLstByPager_Cache } from "../../TS/L3_ForWApi/GraduateEduTopic/clsConceptWApi.js";
import { Concept_GetObjLstByConceptIdLstAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsConceptWApi.js";
import { Concept_GetMaxStrIdAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsConceptWApi.js";
import { Concept_AddNewRecordAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsConceptWApi.js";
import { Concept_DelConceptsAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsConceptWApi.js";
 ...
 **/
export abstract class ConceptCRUD implements clsOperateList
{
public static objPage_CRUD: ConceptCRUD;
public static sortFun_Static: (ascOrDesc: string) => (x: any, y: any) => number;
public static ascOrDesc4SortFun = "Asc";
public static sortConceptBy = "";
 ...
 **/
public CombineData(arrConceptObjLst: Array<clsConceptEN>, arrDataColumn: Array<clsDataColumn>) 
{
...
const objEN: clsConceptEN = arrConceptObjLst[i];
for (let j = 0; j < intColNum; j++)
{
...
const strFileName = Format("观点概念({0})导出.xlsx",
 clsConceptEN._CurrTabName);
 ...
 **/
public async ExportExcel_Concept() 
{
const strThisFuncName = this.ExportExcel_Concept.name;
if (ConceptCRUD.sortConceptBy == null)
{
const strMsg = Format("在显示列表时，排序字段(sortConceptBy)为空，请检查！(In BindGv_Concept_Cache)");
console.error(strMsg);
alert(strMsg);
return;
}
 ...
 **/
public async ExportExcel_Concept_Cache() 
{
...
objConcept_Cond.SetCondFldValue(clsConceptEN.con_id_CurrEduCls, ConceptCRUD.id_CurrEduCls_Cache, "=");
const strWhereCond = JSON.stringify(objConcept_Cond);
const intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex);//获取当前页
 let arrConceptObjLst: Array <clsConceptEN> = [];
try
{
this.recCount = await Concept_GetRecCountByCond_Cache(objConcept_Cond, ConceptCRUD.id_CurrEduCls_Cache);
if (this.recCount == 0)
{
const lblMsg: HTMLSpanElement = < HTMLSpanElement > document.createElement("span");
lblMsg.innerHTML = Format("根据条件:[{0}]获取的对象列表数为0！", objConcept_Cond.whereCond);
const divDataLst: HTMLDivElement = <HTMLDivElement> document.getElementById("divDataLst");
if (divDataLst != null)
{
divDataLst.innerText = "";
...
const strMsg = Format("在绑定Gv_Cache过程中，根据条件:[{0}]获取的对象列表数为0！", objConcept_Cond.whereCond);
console.error("Error: ", strMsg);
//console.trace();
alert(strMsg);
return;
}
arrConceptObjLst = await Concept_GetSubObjLst_Cache(objConcept_Cond, ConceptCRUD.id_CurrEduCls_Cache);
}
catch(e)
{
const strMsg = Format("绑定GridView不成功,{0}.(in {1}.{2})", e, this.constructor.name, strThisFuncName);
...
if (arrConceptObjLst.length == 0)
{
const strKey = Format("{0}_{1}", clsConceptEN._CurrTabName, ConceptCRUD.id_CurrEduCls_Cache);
 ...
 **/
public async btnSelectRecordInTab_Click(strConceptId: string) {
 ...
 **/
public async DelRecord(strConceptId: string) {
const strThisFuncName = this.DelRecord.name;
try
{
const returnInt = await Concept_DelRecordAsync(strConceptId);
if (returnInt > 0)
{
Concept_ReFreshCache(ConceptCRUD.id_CurrEduCls_Cache);
 ...
 * @param sender:参数列表
 **/
public async SelectRecord(strConceptId: string) 
{
...
{
strWhereCond += Format(" And {0} like '% {1}%'", clsConceptEN.con_ConceptName, this.conceptName_q);
}
...
{
const strMsg:string = Format("(errid:WiTsCs0009)在组合查询条件(CombineConceptCondition)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
return strWhereCond;
}
...
{
const strMsg:string = Format("(errid:WiTsCs0010)在组合查询条件对象(CombineConceptConditionObj)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
 ...
 * @returns 条件串(strWhereCond)
 **/
 public async CombineConceptConditionObj4ExportExcel():Promise<clsConceptEN> 
{
//使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
//例如 1 = 1 && UserName = '张三'
let strWhereCond = " 1 = 1 ";
...
{
const strMsg:string = Format("(errid:WiTsCs0013)在组合导出Excel条件对象(CombineConceptConditionObj4ExportExcel)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
objConcept_Cond.whereCond = strWhereCond;
return objConcept_Cond;
...
{
fldName: clsConceptEN.con_ConceptContent,
sortBy: clsConceptEN.con_ConceptContent,
sortFun: SortFun,
getDataSource: "",
colHeader: "概念内容",
text: "",
...
{
fldName: clsConceptEN.con_ConceptName,
sortBy: clsConceptEN.con_ConceptName,
sortFun: SortFun,
getDataSource: "",
colHeader: "概念名称",
text: "",
...
{
fldName: clsConceptEN.con_IsSubmit,
sortBy: clsConceptEN.con_IsSubmit,
sortFun: SortFun,
getDataSource: "",
colHeader: "是否提交",
text: "",
...
{
fldName: clsConceptEN.con_UpdDate,
sortBy: clsConceptEN.con_UpdDate,
sortFun: SortFun,
getDataSource: "",
colHeader: "修改日期",
text: "",
...
await BindTab(o, arrConceptObjLst, arrDataColumn,  clsConceptEN.con_ConceptId, this); 
 ...
 **/
public async ExtendFldFuncMap(arrConceptExObjLst: Array<clsConceptENEx>, arrDataColumn: Array<clsDataColumn>) {
const arrFldName = clsConceptEN.AttributeName;
for (const objDataColumn of arrDataColumn) {
if (IsNullOrEmpty(objDataColumn.fldName) == true) continue;
if (arrFldName.indexOf(objDataColumn.fldName) > -1) continue;
for (const objInFor of arrConceptExObjLst) {
await ConceptEx_FuncMapByFldName(objDataColumn.fldName, objInFor);
}
}
}
...
orderBy: ConceptCRUD.sortConceptBy,//如果该字段为空，就使用下面的排序函数
sortFun: Concept_SortFun_Defa
}
arrConceptObjLst = await Concept_GetObjLstByPager_Cache(objPagerPara, ConceptCRUD.id_CurrEduCls_Cache);
}
catch(e)
{
const strMsg = Format("绑定GridView不成功,{0}.(in {1}.{2})", e, this.constructor.name, strThisFuncName);
 ...
 * @returns 返回两个对象比较的结果
 **/
public SortFun_ExportExcel(a:clsConceptEN , b:clsConceptEN): number 
{
...
const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(objAnchorElement, ConceptCRUD.ascOrDesc4SortFun, ConceptCRUD.sortConceptBy, strSortExpress);
 ...
 **/
public async CopyRecord(arrConceptId: Array<string>) {
const strThisFuncName = this.CopyRecord.name;
try
{
const arrConceptObjLst = await Concept_GetObjLstByConceptIdLstAsync(arrConceptId);
//console.log('responseText=');
//console.log(responseText);
let intCount = 0;
for (const objInFor of arrConceptObjLst)
{
const strMaxStrId = await Concept_GetMaxStrIdAsync();
//console.log('strMaxStrId=' + strMaxStrId);
objInFor.conceptId = strMaxStrId;
const returnBool = await Concept_AddNewRecordAsync(objInFor);
 ...
 **/
public async DelMultiRecord(arrConceptId: Array<string>) {
const strThisFuncName = this.DelMultiRecord.name;
try
{
const returnInt = await Concept_DelConceptsAsync(arrConceptId);
if (returnInt > 0)
{
Concept_ReFreshCache(ConceptCRUD.id_CurrEduCls_Cache);
const strInfo = Format("删除记录成功,共删除{0}条记录!", returnInt);
 ...
 **/
public ShowConceptObj(divContainer: string, objConcept: clsConceptEN) 
{
...
const sstrKeys = GetObjKeys(objConcept);
...
const strValue = GetInputValueInDiv(this.divName4Query, "txtConceptName_q");
 ...
 * 概念名称 (Used In CombineCondition())
 **/
public  set conceptName_q(value: string) {
 SetInputValueByIdInDiv(this.divName4Query, "txtConceptName_q", value);
}
 /**
