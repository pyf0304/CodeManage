 ...
 **/
//import $ from "jquery";
import { vRTUserRela_GetRecCountByCondAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsvRTUserRelaWApi.js";
import { vRTUserRela_GetObjLstAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsvRTUserRelaWApi.js";
import { vRTUserRela_GetRecCountByCond_Cache } from "../../TS/L3_ForWApi/GraduateEduTopic/clsvRTUserRelaWApi.js";
import { vRTUserRela_GetSubObjLst_Cache } from "../../TS/L3_ForWApi/GraduateEduTopic/clsvRTUserRelaWApi.js";
...
import { RTUserRela_DelRecordAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsRTUserRelaWApi.js";
...
import { vRTUserRelaEx_FuncMapByFldName } from "../../TS/L3_ForWApiEx/GraduateEduTopic/clsvRTUserRelaExWApi.js";
import { vRTUserRela_GetObjLstByPagerAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsvRTUserRelaWApi.js";
import { vRTUserRela_SortFun_Defa } from "../../TS/L3_ForWApi/GraduateEduTopic/clsvRTUserRelaWApi.js";
import { vRTUserRela_GetObjLstByPager_Cache } from "../../TS/L3_ForWApi/GraduateEduTopic/clsvRTUserRelaWApi.js";
import { RTUserRela_GetObjLstBymIdLstAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsRTUserRelaWApi.js";
import { RTUserRela_AddNewRecordAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsRTUserRelaWApi.js";
import { RTUserRela_DelRTUserRelasAsync } from "../../TS/L3_ForWApi/GraduateEduTopic/clsRTUserRelaWApi.js";
...
{
public static objPage_CRUD: RTUserRelaCRUD;
 ...
 **/
public CombineData(arrvRTUserRelaObjLst: Array<clsvRTUserRelaEN>, arrDataColumn: Array<clsDataColumn>) 
{
...
const strFileName = Format("主题用户关系({0})导出.xlsx",
 clsvRTUserRelaEN._CurrTabName);
 ...
 **/
public async ExportExcel_vRTUserRela() 
{
const strThisFuncName = this.ExportExcel_vRTUserRela.name;
if (RTUserRelaCRUD.sortvRTUserRelaBy == null)
{
const strMsg = Format("在显示列表时，排序字段(sortvRTUserRelaBy)为空，请检查！(In BindGv_vRTUserRela_Cache)");
console.error(strMsg);
alert(strMsg);
return;
}
 ...
 **/
public async ExportExcel_vRTUserRela_Cache() 
{
...
objvRTUserRela_Cond.SetCondFldValue(clsvRTUserRelaEN.con_TopicId, RTUserRelaCRUD.TopicId_Cache, "=");
const strWhereCond = JSON.stringify(objvRTUserRela_Cond);
const intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex);//获取当前页
 let arrvRTUserRelaObjLst: Array <clsvRTUserRelaEN> = [];
try
{
this.recCount = await vRTUserRela_GetRecCountByCond_Cache(objvRTUserRela_Cond, RTUserRelaCRUD.TopicId_Cache);
if (this.recCount == 0)
{
const lblMsg: HTMLSpanElement = < HTMLSpanElement > document.createElement("span");
lblMsg.innerHTML = Format("根据条件:[{0}]获取的对象列表数为0！", objvRTUserRela_Cond.whereCond);
const divDataLst: HTMLDivElement = <HTMLDivElement> document.getElementById("divDataLst");
if (divDataLst != null)
{
divDataLst.innerText = "";
...
const strMsg = Format("在绑定Gv_Cache过程中，根据条件:[{0}]获取的对象列表数为0！", objvRTUserRela_Cond.whereCond);
console.error("Error: ", strMsg);
//console.trace();
alert(strMsg);
return;
}
arrvRTUserRelaObjLst = await vRTUserRela_GetSubObjLst_Cache(objvRTUserRela_Cond, RTUserRelaCRUD.TopicId_Cache);
}
catch(e)
{
const strMsg = Format("绑定GridView不成功,{0}.(in {1}.{2})", e, this.constructor.name, strThisFuncName);
...
if (arrvRTUserRelaObjLst.length == 0)
{
const strKey = Format("{0}_{1}", clsRTUserRelaEN._CurrTabName, RTUserRelaCRUD.TopicId_Cache);
...
{
await XzClg_BindDdl_id_XzCollegeInDiv_Cache(this.divName4Query, "ddlid_XzCollege_q" );//
}
...
try
{
const objRTUserRelaEN = await RTUserRela_GetObjBymIdAsync(lngmId);
...
if ( this.userId_q != "")
{
strWhereCond += Format(" And {0} like '% {1}%'", clsvRTUserRelaEN.con_UserId, this.userId_q);
}
if ( this.topicName_q != "")
{
strWhereCond += Format(" And {0} like '% {1}%'", clsvRTUserRelaEN.con_TopicName, this.topicName_q);
}
if ( this.id_XzCollege_q != "" && this.id_XzCollege_q != "0")
{
strWhereCond += Format(" And {0} = '{1}'", clsvRTUserRelaEN.con_id_XzCollege, this.id_XzCollege_q);
}
if ( this.id_XzMajor_q != "" && this.id_XzMajor_q != "0")
{
strWhereCond += Format(" And {0} = '{1}'", clsvRTUserRelaEN.con_id_XzMajor, this.id_XzMajor_q);
}
}
catch(objException)
{
const strMsg:string = Format("(errid:WiTsCs0009)在组合查询条件(CombineRTUserRelaCondition)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
return strWhereCond;
}
...
{
strWhereCond += Format(" And {0} like '% {1}%'", clsvRTUserRelaEN.con_UserId, this.userId_q);
objvRTUserRela_Cond.SetCondFldValue(clsvRTUserRelaEN.con_UserId, this.userId_q, "like");
}
if ( this.topicName_q != "")
{
strWhereCond += Format(" And {0} like '% {1}%'", clsvRTUserRelaEN.con_TopicName, this.topicName_q);
objvRTUserRela_Cond.SetCondFldValue(clsvRTUserRelaEN.con_TopicName, this.topicName_q, "like");
}
if ( this.id_XzCollege_q != "" && this.id_XzCollege_q != "0")
{
strWhereCond += Format(" And {0} = '{1}'", clsvRTUserRelaEN.con_id_XzCollege, this.id_XzCollege_q);
objvRTUserRela_Cond.SetCondFldValue(clsvRTUserRelaEN.con_id_XzCollege, this.id_XzCollege_q, "=");
}
if ( this.id_XzMajor_q != "" && this.id_XzMajor_q != "0")
{
strWhereCond += Format(" And {0} = '{1}'", clsvRTUserRelaEN.con_id_XzMajor, this.id_XzMajor_q);
objvRTUserRela_Cond.SetCondFldValue(clsvRTUserRelaEN.con_id_XzMajor, this.id_XzMajor_q, "=");
}

}
catch(objException)
{
const strMsg:string = Format("(errid:WiTsCs0010)在组合查询条件对象(CombineRTUserRelaConditionObj)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
...
{
const strMsg:string = Format("(errid:WiTsCs0013)在组合导出Excel条件对象(CombineRTUserRelaConditionObj4ExportExcel)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
objvRTUserRela_Cond.whereCond = strWhereCond;
return objvRTUserRela_Cond;
...
await BindTab_V(o, arrvRTUserRelaObjLst, arrDataColumn,  clsvRTUserRelaEN.con_mId, this); 
 ...
 **/
public async ExtendFldFuncMap(arrvRTUserRelaExObjLst: Array<clsvRTUserRelaENEx>, arrDataColumn: Array<clsDataColumn>) {
...
if (RTUserRelaCRUD.sortvRTUserRelaBy == null)
{
const strMsg = Format("在显示列表时，排序字段(sortvRTUserRelaBy)为空，请检查！(In BindGv_vRTUserRela)");
console.error(strMsg);
alert(strMsg);
return;
}
...
orderBy: RTUserRelaCRUD.sortvRTUserRelaBy,//如果该字段为空，就使用下面的排序函数
 ...
 **/
public async BindGv_vRTUserRela_Cache() 
{
...
whereCond: strWhereCond,
orderBy: RTUserRelaCRUD.sortvRTUserRelaBy,//如果该字段为空，就使用下面的排序函数
sortFun: vRTUserRela_SortFun_Defa
}
arrvRTUserRelaObjLst = await vRTUserRela_GetObjLstByPager_Cache(objPagerPara, RTUserRelaCRUD.TopicId_Cache);
}
catch(e)
{
const strMsg = Format("绑定GridView不成功,{0}.(in {1}.{2})", e, this.constructor.name, strThisFuncName);
 ...
 * @returns 返回两个对象比较的结果
 **/
public SortFun_ExportExcel(a:clsvRTUserRelaEN , b:clsvRTUserRelaEN): number 
{
...
const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(objAnchorElement, RTUserRelaCRUD.ascOrDesc4SortFun, RTUserRelaCRUD.sortvRTUserRelaBy, strSortExpress);
 ...
 **/
public ShowRTUserRelaObj(divContainer: string, objRTUserRela: clsRTUserRelaEN) 
{
...
const strValue = GetSelectValueInDiv(this.divName4Query, "ddlid_XzCollege_q");
 ...
 * 学院流水号 (Used In CombineCondition())
 **/
public  set id_XzCollege_q(value: string) {
SetSelectValueByIdInDiv(this.divName4Query, "ddlid_XzCollege_q", value);
}
 /**
