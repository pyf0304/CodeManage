...
import { SubViewpointType_CheckPropertyNew } from "../../TS/L3_ForWApi/ParameterTable/clsSubViewpointTypeWApi.js";
import { SubViewpointType_AddNewRecordWithMaxIdAsync } from "../../TS/L3_ForWApi/ParameterTable/clsSubViewpointTypeWApi.js";
import { SubViewpointType_ReFreshCache } from "../../TS/L3_ForWApi/ParameterTable/clsSubViewpointTypeWApi.js";
import { SubViewpointType_GetUniCondStr_SubViewpointTypeName } from "../../TS/L3_ForWApi/ParameterTable/clsSubViewpointTypeWApi.js";
import { SubViewpointType_IsExistRecordAsync } from "../../TS/L3_ForWApi/ParameterTable/clsSubViewpointTypeWApi.js";
import { SubViewpointType_GetUniCondStr4Update_SubViewpointTypeName } from "../../TS/L3_ForWApi/ParameterTable/clsSubViewpointTypeWApi.js";
import { clsSubViewpointTypeEN } from "../../TS/L0_Entity/ParameterTable/clsSubViewpointTypeEN.js";
import { SubViewpointType_IsExistAsync } from "../../TS/L3_ForWApi/ParameterTable/clsSubViewpointTypeWApi.js";
import { SubViewpointType_GetObjBySubViewpointTypeIdAsync } from "../../TS/L3_ForWApi/ParameterTable/clsSubViewpointTypeWApi.js";
import { SubViewpointType_CheckProperty4Update } from "../../TS/L3_ForWApi/ParameterTable/clsSubViewpointTypeWApi.js";
import { SubViewpointType_UpdateRecordAsync } from "../../TS/L3_ForWApi/ParameterTable/clsSubViewpointTypeWApi.js";
 ...
 **/
export abstract class SubViewpointType_Edit 
{
public opType = "";
public keyId = "";

public static strPageDispModeId = "01";//PopupBox(弹出框)
public static objPage_Edit: SubViewpointType_Edit;
public objSubViewpointType: clsSubViewpointTypeEN;
...
this.objSubViewpointType = new clsSubViewpointTypeEN();
}
 ...
 **/
public ShowDialog_SubViewpointType(strOp:string) {
const strThisFuncName = this.ShowDialog_SubViewpointType.name;
//检查相关控件是否存在
CheckControlExist(this.divName4Edit, "div", "divEditDialog_SubViewpointType");
CheckControlExist(this.divName4Edit, "h4", "lblDialogTitle_SubViewpointType");
 ...
 **/
public HideDialog_SubViewpointType() {
...
if (SubViewpointType_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01)
{
...
const strCommandText: string = this.btnSubmit_SubViewpointType;
try {
...
this.iShowList.BindGv(clsSubViewpointTypeEN._CurrTabName, returnKeyId);
}
}
else
{
...
try
{
const returnString = await SubViewpointType_GetMaxStrIdAsync();
if (returnString == "")
{
const strInfo = Format("获取表SubViewpointType的最大关键字为空，不成功，请检查!");
//显示信息框
alert(strInfo);
}
else
...
pobjSubViewpointTypeEN.SetUpdDate(clsDateTime.getTodayDateTimeStr(1));// 修改日期
...
const objSubViewpointTypeEN = new clsSubViewpointTypeEN();
try
{
...
try
{
SubViewpointType_CheckPropertyNew(objSubViewpointTypeEN);
}
catch(e)
{
...
let returnBool = false;
const returnKeyId = await SubViewpointType_AddNewRecordWithMaxIdAsync(objSubViewpointTypeEN);
if (IsNullOrEmpty(returnKeyId) == false)
{
returnBool = true;
}
 ...
 **/
public async CheckUniCond4Add_SubViewpointTypeName(objSubViewpointTypeEN: clsSubViewpointTypeEN): Promise<boolean>{
const strThisFuncName = this.CheckUniCond4Add_SubViewpointTypeName.name;
const strUniquenessCondition = SubViewpointType_GetUniCondStr_SubViewpointTypeName(objSubViewpointTypeEN);
const bolIsExistCondition = await SubViewpointType_IsExistRecordAsync(strUniquenessCondition);
 ...
 **/
public async CheckUniCond4Update_SubViewpointTypeName(objSubViewpointTypeEN: clsSubViewpointTypeEN): Promise<boolean>{
const strThisFuncName = this.CheckUniCond4Update_SubViewpointTypeName.name;
const strUniquenessCondition = SubViewpointType_GetUniCondStr4Update_SubViewpointTypeName(objSubViewpointTypeEN);
const bolIsExistCondition = await SubViewpointType_IsExistRecordAsync(strUniquenessCondition);
if (bolIsExistCondition == true)
{
const strMsg = Format("不能满足唯一性条件。满足条件：{0}的记录已经存在！", strUniquenessCondition);
...
const responseKeyId = await SubViewpointType_AddNewRecordWithMaxIdAsync(objSubViewpointTypeEN);
...
const strThisFuncName = this.ShowData.name;
        ...
        //2、检查该关键字的记录是否存在,如果不存在就返回不显示；
        let objSubViewpointTypeEN = new clsSubViewpointTypeEN();
        try {
            const returnBool = await SubViewpointType_IsExistAsync(strSubViewpointTypeId);
            if (returnBool == false) {
                const strInfo = Format("关键字:[{0}] 的记录不存在!", strSubViewpointTypeId);
...
console.error(strMsg);
        ...
        try {
const objSubViewpointTypeEN_Const = await SubViewpointType_GetObjBySubViewpointTypeIdAsync(strSubViewpointTypeId);
 ...
 **/
public async UpdateRecord(strSubViewpointTypeId: string):Promise<boolean> 
{
...
await this.PutDataToSubViewpointTypeClass(objSubViewpointTypeEN);
objSubViewpointTypeEN.sfUpdFldSetStr = objSubViewpointTypeEN.updFldString;//设置哪些字段被修改(脏字段)
...
try
{
SubViewpointType_CheckProperty4Update(objSubViewpointTypeEN);
}
catch(e)
{
const strMsg = Format("检查数据不成功,{0}.(in {1}.{2})", e, this.constructor.name, strThisFuncName);
...
const returnBool = await SubViewpointType_UpdateRecordAsync(objSubViewpointTypeEN);
if (returnBool == true)
{
SubViewpointType_ReFreshCache();
}
 ...
 **/
public  set btnCancel_SubViewpointType(value: string) {
 SetButtonHtmlByIdInDiv(this.divName4Edit, "btnCancel_SubViewpointType", value);
}
 /**
 * 获取按钮的标题
 **/
public  get btnSubmit_SubViewpointType():string {
const strValue = GetButtonHtmlInDiv(this.divName4Edit, "btnSubmit_SubViewpointType");
return strValue;
}
 /**
 * 设置确定按钮的标题(Used In AddNewRecord())
 **/
public  set btnSubmit_SubViewpointType(value: string) {
 SetButtonHtmlByIdInDiv(this.divName4Edit, "btnSubmit_SubViewpointType", value);
}
 /**
 * 备注 (Used In Clear())
 **/
...
public  set subViewpointTypeId(value: string) {
 SetInputValueByIdInDiv(this.divName4Edit, "txtSubViewpointTypeId", value);
}
 /**
 * 类型Id (Used In PutDataToClass())
 **/
public  get subViewpointTypeId():string {
const strValue = GetInputValueInDiv(this.divName4Edit, "txtSubViewpointTypeId");
if (strValue == undefined) return "";
else return strValue.toString();
}
 /**
...
public  set subViewpointTypeName(value: string) {
 SetInputValueByIdInDiv(this.divName4Edit, "txtSubViewpointTypeName", value);
}
 /**
 * 类型名称 (Used In PutDataToClass())
 **/
public  get subViewpointTypeName():string {
const strValue = GetInputValueInDiv(this.divName4Edit, "txtSubViewpointTypeName");
if (strValue == undefined) return "";
else return strValue.toString();
}
 /**
