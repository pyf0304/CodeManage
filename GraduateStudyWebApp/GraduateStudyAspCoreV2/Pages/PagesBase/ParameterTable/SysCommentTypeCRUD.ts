 ...
 **/
//import $ from "jquery";
import { SysCommentType_GetRecCountByCondAsync } from "../../TS/L3_ForWApi/ParameterTable/clsSysCommentTypeWApi.js";
import { SysCommentType_GetObjLstAsync } from "../../TS/L3_ForWApi/ParameterTable/clsSysCommentTypeWApi.js";
import { SysCommentType_GetRecCountByCond_Cache } from "../../TS/L3_ForWApi/ParameterTable/clsSysCommentTypeWApi.js";
import { SysCommentType_GetSubObjLst_Cache } from "../../TS/L3_ForWApi/ParameterTable/clsSysCommentTypeWApi.js";
import { SysCommentType_DelRecordAsync } from "../../TS/L3_ForWApi/ParameterTable/clsSysCommentTypeWApi.js";
import { SysCommentType_GetObjByCommentTypeIdAsync } from "../../TS/L3_ForWApi/ParameterTable/clsSysCommentTypeWApi.js";
import { clsSysCommentTypeEN } from "../../TS/L0_Entity/ParameterTable/clsSysCommentTypeEN.js";
import { clsSysCommentTypeENEx } from "../../TS/L0_Entity/ParameterTable/clsSysCommentTypeENEx.js";
import { SysCommentTypeEx_FuncMapByFldName } from "../../TS/L3_ForWApiEx/ParameterTable/clsSysCommentTypeExWApi.js";
import { SysCommentType_GetObjLstByPagerAsync } from "../../TS/L3_ForWApi/ParameterTable/clsSysCommentTypeWApi.js";
import { SysCommentType_SortFun_Defa } from "../../TS/L3_ForWApi/ParameterTable/clsSysCommentTypeWApi.js";
import { SysCommentType_GetObjLstByPager_Cache } from "../../TS/L3_ForWApi/ParameterTable/clsSysCommentTypeWApi.js";
import { SysCommentType_GetObjLstByCommentTypeIdLstAsync } from "../../TS/L3_ForWApi/ParameterTable/clsSysCommentTypeWApi.js";
import { SysCommentType_AddNewRecordAsync } from "../../TS/L3_ForWApi/ParameterTable/clsSysCommentTypeWApi.js";
import { SysCommentType_DelSysCommentTypesAsync } from "../../TS/L3_ForWApi/ParameterTable/clsSysCommentTypeWApi.js";
 ...
 **/
export abstract class SysCommentTypeCRUD implements clsOperateList
{
public static objPage_CRUD: SysCommentTypeCRUD;
public static sortFun_Static: (ascOrDesc: string) => (x: any, y: any) => number;
public static ascOrDesc4SortFun = "Asc";
public static sortSysCommentTypeBy = "";
 ...
 **/
public CombineData(arrSysCommentTypeObjLst: Array<clsSysCommentTypeEN>, arrDataColumn: Array<clsDataColumn>) 
{
...
const objEN: clsSysCommentTypeEN = arrSysCommentTypeObjLst[i];
for (let j = 0; j < intColNum; j++)
{
...
const strFileName = Format("评论类型表({0})导出.xlsx",
 clsSysCommentTypeEN._CurrTabName);
 ...
 **/
public async ExportExcel_SysCommentType() 
{
const strThisFuncName = this.ExportExcel_SysCommentType.name;
if (SysCommentTypeCRUD.sortSysCommentTypeBy == null)
{
const strMsg = Format("在显示列表时，排序字段(sortSysCommentTypeBy)为空，请检查！(In BindGv_SysCommentType_Cache)");
console.error(strMsg);
alert(strMsg);
return;
}
...
const intCurrPageIndex = GetCurrPageIndex(this.objPager.currPageIndex);//获取当前页
 let arrSysCommentTypeObjLst: Array <clsSysCommentTypeEN> = [];
try
{
...
arrSysCommentTypeObjLst = await SysCommentType_GetObjLstAsync(strWhereCond);
}
catch(e)
{
 ...
 **/
public async ExportExcel_SysCommentType_Cache() 
{
...
try
{
this.recCount = await SysCommentType_GetRecCountByCond_Cache(objSysCommentType_Cond);
if (this.recCount == 0)
{
const lblMsg: HTMLSpanElement = < HTMLSpanElement > document.createElement("span");
lblMsg.innerHTML = Format("根据条件:[{0}]获取的对象列表数为0！", objSysCommentType_Cond.whereCond);
const divDataLst: HTMLDivElement = <HTMLDivElement> document.getElementById("divDataLst");
if (divDataLst != null)
{
divDataLst.innerText = "";
...
const strMsg = Format("在绑定Gv_Cache过程中，根据条件:[{0}]获取的对象列表数为0！", objSysCommentType_Cond.whereCond);
console.error("Error: ", strMsg);
//console.trace();
alert(strMsg);
return;
}
arrSysCommentTypeObjLst = await SysCommentType_GetSubObjLst_Cache(objSysCommentType_Cond);
}
catch(e)
{
const strMsg = Format("绑定GridView不成功,{0}.(in {1}.{2})", e, this.constructor.name, strThisFuncName);
...
if (arrSysCommentTypeObjLst.length == 0)
{
const strKey = Format("{0}", clsSysCommentTypeEN._CurrTabName);
 ...
 **/
public async btnSelectRecordInTab_Click(strCommentTypeId: string) {
 ...
 **/
public async DelRecord(strCommentTypeId: string) {
const strThisFuncName = this.DelRecord.name;
try
{
const returnInt = await SysCommentType_DelRecordAsync(strCommentTypeId);
if (returnInt > 0)
{
 ...
 * @param sender:参数列表
 **/
public async SelectRecord(strCommentTypeId: string) 
{
...
Redirect("/Index/Main_SysCommentType");
}
catch(e)
{
 ...
 **/
 public async CombineSysCommentTypeCondition():Promise<string> 
{
...
{
strWhereCond += Format(" And {0} like '% {1}%'", clsSysCommentTypeEN.con_CommentTypeName, this.commentTypeName_q);
}
}
catch(objException)
{
const strMsg:string = Format("(errid:WiTsCs0009)在组合查询条件(CombineSysCommentTypeCondition)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
return strWhereCond;
}
 ...
 * @returns 条件串(strWhereCond)
 **/
 public async CombineSysCommentTypeConditionObj(): Promise<clsSysCommentTypeEN> 
{
//使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
//例如 1 = 1 && UserName = '张三'
let strWhereCond = " 1 = 1 ";
...
{
strWhereCond += Format(" And {0} like '% {1}%'", clsSysCommentTypeEN.con_CommentTypeName, this.commentTypeName_q);
objSysCommentType_Cond.SetCondFldValue(clsSysCommentTypeEN.con_CommentTypeName, this.commentTypeName_q, "like");
}

}
catch(objException)
{
const strMsg:string = Format("(errid:WiTsCs0010)在组合查询条件对象(CombineSysCommentTypeConditionObj)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
 ...
 * @returns 条件串(strWhereCond)
 **/
 public async CombineSysCommentTypeConditionObj4ExportExcel():Promise<clsSysCommentTypeEN> 
{
//使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
//例如 1 = 1 && UserName = '张三'
let strWhereCond = " 1 = 1 ";
...
{
const strMsg:string = Format("(errid:WiTsCs0013)在组合导出Excel条件对象(CombineSysCommentTypeConditionObj4ExportExcel)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
objSysCommentType_Cond.whereCond = strWhereCond;
return objSysCommentType_Cond;
...
{
fldName: clsSysCommentTypeEN.con_CommentTypeId,
sortBy: clsSysCommentTypeEN.con_CommentTypeId,
sortFun: SortFun,
getDataSource: "",
colHeader: "评论类型Id",
text: "",
...
{
fldName: clsSysCommentTypeEN.con_CommentTypeName,
sortBy: clsSysCommentTypeEN.con_CommentTypeName,
sortFun: SortFun,
getDataSource: "",
colHeader: "评论类型名",
text: "",
...
{
fldName: clsSysCommentTypeEN.con_CommentTable,
sortBy: clsSysCommentTypeEN.con_CommentTable,
sortFun: SortFun,
getDataSource: "",
colHeader: "评论表",
text: "",
...
await BindTab(o, arrSysCommentTypeObjLst, arrDataColumn,  clsSysCommentTypeEN.con_CommentTypeId, this); 
 ...
 **/
public async ExtendFldFuncMap(arrSysCommentTypeExObjLst: Array<clsSysCommentTypeENEx>, arrDataColumn: Array<clsDataColumn>) {
...
await SysCommentTypeEx_FuncMapByFldName(objDataColumn.fldName, objInFor);
}
}
}
...
if (SysCommentTypeCRUD.sortSysCommentTypeBy == null)
{
const strMsg = Format("在显示列表时，排序字段(sortSysCommentTypeBy)为空，请检查！(In BindGv_SysCommentType)");
console.error(strMsg);
alert(strMsg);
return;
}
...
orderBy: SysCommentTypeCRUD.sortSysCommentTypeBy,//如果该字段为空，就使用下面的排序函数
 ...
 **/
public async BindGv_SysCommentType_Cache() 
{
...
whereCond: strWhereCond,
orderBy: SysCommentTypeCRUD.sortSysCommentTypeBy,//如果该字段为空，就使用下面的排序函数
sortFun: SysCommentType_SortFun_Defa
}
arrSysCommentTypeObjLst = await SysCommentType_GetObjLstByPager_Cache(objPagerPara);
}
catch(e)
{
const strMsg = Format("绑定GridView不成功,{0}.(in {1}.{2})", e, this.constructor.name, strThisFuncName);
 ...
 * @returns 返回两个对象比较的结果
 **/
public SortFun_ExportExcel(a:clsSysCommentTypeEN , b:clsSysCommentTypeEN): number 
{
const strThisFuncName = this.SortFun_ExportExcel.name;
if (a.commentTypeId == b.commentTypeId) return a.commentTypeId.localeCompare(b.commentTypeId) ;
...
const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(objAnchorElement, SysCommentTypeCRUD.ascOrDesc4SortFun, SysCommentTypeCRUD.sortSysCommentTypeBy, strSortExpress);
 ...
 **/
public async CopyRecord(arrCommentTypeId: Array<string>) {
const strThisFuncName = this.CopyRecord.name;
try
{
const arrSysCommentTypeObjLst = await SysCommentType_GetObjLstByCommentTypeIdLstAsync(arrCommentTypeId);
//console.log('responseText=');
//console.log(responseText);
let intCount = 0;
for (const objInFor of arrSysCommentTypeObjLst)
{
const returnBool = await SysCommentType_AddNewRecordAsync(objInFor);
 ...
 **/
public async DelMultiRecord(arrCommentTypeId: Array<string>) {
const strThisFuncName = this.DelMultiRecord.name;
try
{
const returnInt = await SysCommentType_DelSysCommentTypesAsync(arrCommentTypeId);
if (returnInt > 0)
{
//SysCommentType_ReFreshCache();
const strInfo = Format("删除记录成功,共删除{0}条记录!", returnInt);
 ...
 **/
public ShowSysCommentTypeObj(divContainer: string, objSysCommentType: clsSysCommentTypeEN) 
{
...
const sstrKeys = GetObjKeys(objSysCommentType);
