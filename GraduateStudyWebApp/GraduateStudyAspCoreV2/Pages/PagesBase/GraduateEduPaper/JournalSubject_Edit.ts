...
import { JournalSubject_CheckPropertyNew } from "../../TS/L3_ForWApi/GraduateEduPaper/clsJournalSubjectWApi.js";
import { JournalSubject_AddNewRecordWithMaxIdAsync } from "../../TS/L3_ForWApi/GraduateEduPaper/clsJournalSubjectWApi.js";
import { JournalSubject_ReFreshCache } from "../../TS/L3_ForWApi/GraduateEduPaper/clsJournalSubjectWApi.js";
import { JournalSubject_GetUniCondStr_JournalSubjectCategoryId_JournalSubjectName } from "../../TS/L3_ForWApi/GraduateEduPaper/clsJournalSubjectWApi.js";
import { JournalSubject_IsExistRecordAsync } from "../../TS/L3_ForWApi/GraduateEduPaper/clsJournalSubjectWApi.js";
import { JournalSubject_GetUniCondStr4Update_JournalSubjectCategoryId_JournalSubjectName } from "../../TS/L3_ForWApi/GraduateEduPaper/clsJournalSubjectWApi.js";
import { clsJournalSubjectEN } from "../../TS/L0_Entity/GraduateEduPaper/clsJournalSubjectEN.js";
import { JournalSubject_IsExistAsync } from "../../TS/L3_ForWApi/GraduateEduPaper/clsJournalSubjectWApi.js";
import { JournalSubject_GetObjByJournalSubjectIdAsync } from "../../TS/L3_ForWApi/GraduateEduPaper/clsJournalSubjectWApi.js";
import { JournalSubject_CheckProperty4Update } from "../../TS/L3_ForWApi/GraduateEduPaper/clsJournalSubjectWApi.js";
import { JournalSubject_UpdateRecordAsync } from "../../TS/L3_ForWApi/GraduateEduPaper/clsJournalSubjectWApi.js";
 ...
 **/
public ShowDialog_JournalSubject(strOp:string) {
const strThisFuncName = this.ShowDialog_JournalSubject.name;
//检查相关控件是否存在
CheckControlExist(this.divName4Edit, "div", "divEditDialog_JournalSubject");
CheckControlExist(this.divName4Edit, "h4", "lblDialogTitle_JournalSubject");
...
const strCommandText: string = this.btnSubmit_JournalSubject;
try {
...
this.iShowList.BindGv_Cache(clsJournalSubjectEN._CurrTabName, returnKeyId);
}
}
else
{
...
this.iShowList.BindGv_Cache(clsJournalSubjectEN._CurrTabName, returnBool.toString());
}
}
break;
case "确认修改":
...
try
{
const returnString = await JournalSubject_GetMaxStrIdAsync();
if (returnString == "")
{
const strInfo = Format("获取表JournalSubject的最大关键字为空，不成功，请检查!");
//显示信息框
alert(strInfo);
}
else
 ...
 **/
public async PutDataToJournalSubjectClass(pobjJournalSubjectEN: clsJournalSubjectEN)
{
...
const objJournalSubjectEN = new clsJournalSubjectEN();
try
{
...
let returnBool = false;
const returnKeyId = await JournalSubject_AddNewRecordWithMaxIdAsync(objJournalSubjectEN);
if (IsNullOrEmpty(returnKeyId) == false)
{
returnBool = true;
}
 ...
 **/
public async CheckUniCond4Add_JournalSubjectCategoryId_JournalSubjectName(objJournalSubjectEN: clsJournalSubjectEN): Promise<boolean>{
const strThisFuncName = this.CheckUniCond4Add_JournalSubjectCategoryId_JournalSubjectName.name;
const strUniquenessCondition = JournalSubject_GetUniCondStr_JournalSubjectCategoryId_JournalSubjectName(objJournalSubjectEN);
const bolIsExistCondition = await JournalSubject_IsExistRecordAsync(strUniquenessCondition);
 ...
 **/
public async CheckUniCond4Update_JournalSubjectCategoryId_JournalSubjectName(objJournalSubjectEN: clsJournalSubjectEN): Promise<boolean>{
const strThisFuncName = this.CheckUniCond4Update_JournalSubjectCategoryId_JournalSubjectName.name;
const strUniquenessCondition = JournalSubject_GetUniCondStr4Update_JournalSubjectCategoryId_JournalSubjectName(objJournalSubjectEN);
const bolIsExistCondition = await JournalSubject_IsExistRecordAsync(strUniquenessCondition);
if (bolIsExistCondition == true)
{
const strMsg = Format("不能满足唯一性条件。满足条件：{0}的记录已经存在！", strUniquenessCondition);
...
const strThisFuncName = this.ShowData.name;
        ...
        //2、检查该关键字的记录是否存在,如果不存在就返回不显示；
        let objJournalSubjectEN = new clsJournalSubjectEN();
        try {
            const returnBool = await JournalSubject_IsExistAsync(strJournalSubjectId);
            if (returnBool == false) {
                const strInfo = Format("关键字:[{0}] 的记录不存在!", strJournalSubjectId);
...
console.error(strMsg);
        ...
        try {
const objJournalSubjectEN_Const = await JournalSubject_GetObjByJournalSubjectIdAsync(strJournalSubjectId);
...
await this.PutDataToJournalSubjectClass(objJournalSubjectEN);
objJournalSubjectEN.sfUpdFldSetStr = objJournalSubjectEN.updFldString;//设置哪些字段被修改(脏字段)
...
const returnBool = await JournalSubject_UpdateRecordAsync(objJournalSubjectEN);
if (returnBool == true)
{
JournalSubject_ReFreshCache();
}
...
public  set btnCancel_JournalSubject(value: string) {
 SetButtonHtmlByIdInDiv(this.divName4Edit, "btnCancel_JournalSubject", value);
}
 /**
 * 获取按钮的标题
 **/
public  get btnSubmit_JournalSubject():string {
const strValue = GetButtonHtmlInDiv(this.divName4Edit, "btnSubmit_JournalSubject");
return strValue;
}
 /**
...
public  set btnSubmit_JournalSubject(value: string) {
 SetButtonHtmlByIdInDiv(this.divName4Edit, "btnSubmit_JournalSubject", value);
}
 /**
...
public  set journalSubjectCode(value: string) {
 SetInputValueByIdInDiv(this.divName4Edit, "txtJournalSubjectCode", value);
}
 /**
 * 期刊学科代码 (Used In PutDataToClass())
 **/
public  get journalSubjectCode():string {
const strValue = GetInputValueInDiv(this.divName4Edit, "txtJournalSubjectCode");
if (strValue == undefined) return "";
...
public  set journalSubjectId(value: string) {
 SetInputValueByIdInDiv(this.divName4Edit, "txtJournalSubjectId", value);
}
 /**
 * 期刊学科Id (Used In PutDataToClass())
 **/
public  get journalSubjectId():string {
const strValue = GetInputValueInDiv(this.divName4Edit, "txtJournalSubjectId");
if (strValue == undefined) return "";
else return strValue.toString();
}
 /**
...
public  set journalSubjectName(value: string) {
 SetInputValueByIdInDiv(this.divName4Edit, "txtJournalSubjectName", value);
}
 /**
 * 期刊学科名称 (Used In PutDataToClass())
 **/
public  get journalSubjectName():string {
const strValue = GetInputValueInDiv(this.divName4Edit, "txtJournalSubjectName");
if (strValue == undefined) return "";
else return strValue.toString();
}
 /**
