   ...
   */
    ...
    public async BindDdl_PaperGroupId(ddlPaperGroupId: string) {
        ...
        try {
            ...
            BindDdl_ObjLst(ddlPaperGroupId, arrObjLst_Sel, clsgs_PaperGroupEN.con_PaperGroupId, clsgs_PaperGroupEN.con_PaperGroupName, "论文组");
    ...
    public async PaperAttentionTree() {
        ...
        if ($("#hidMyResearch").val() == 1) {
            ...
            if (arrvgs_PaperIdNumObjLst.length > 0) {
                ...
                var strWhereCond3 = " paperId in(" + arrPaperId + ") and userId='" + clsPublocalStorage.userId + "'";
        ...
        if (arrgs_PaperGroupObjLst.length > 0) {
            for (var i = 0; i < arrgs_PaperGroupObjLst.length; i++) {
                ...
                for (var j = 0; j < arrvgs_PaperAttentionObjLst.length; j++) {
                    ...
                    strhtml += '<li id="' + strPaperId + '" onclick=btnSelectPaper("' + strPaperId + '",' + arrvgs_PaperAttentionObjLst[j].paperAttentionId + ',"' + clsPublocalStorage.id_CurrEduCls + '")>';
                    //默认存放第一条数据显示；
                    //判断存放的id控件是否为空；
