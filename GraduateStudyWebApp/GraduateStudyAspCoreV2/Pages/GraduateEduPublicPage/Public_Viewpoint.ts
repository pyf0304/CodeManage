...
import { clsvViewpointExEN } from "../TS/L0_Entity/GraduateEduTopic/clsvViewpointExEN.js";
...
export class Public_Viewpoint {
    ...
    private static async BindList_vViewpointEx(strOperationType: string, strVType: string, arrvViewpointExObjLst: Array<clsvViewpointExEN>): Promise<string> {
        ...
        arrViewpointAttachmentObjLst2 = await ViewpointAttachment_GetObjLstAsync(strWhereCondAttachment);
        //获取用户缓存数据
        var arrUsers: Array<clsvUsersSimEN> = await vUsersSim_GetObjLst_Cache(clsSysPara4WebApi.cmPrjId);
        //点赞
        ...
        for (var i = 0; i < arrvViewpointExObjLst.length; i++) {
            ...
            if (strOperationType == "06" || strOperationType == "07") {
                ...
                if (strOperationType == "06") {
                    ...
                    strhtml += "&nbsp;&nbsp;<button class=\"layui-btn layui-btn layui-btn-xs\" onclick=\"xadmin.open('" + strTitle + "评论', '../GraduateEduTools/SysComment?Key=" + strViewpointId + "&Type=" + strType + "&User=" + arrvViewpointExObjLst[i].updUser + "&pubParentId=" + clsPrivateSessionStorage.topicId + "&FontSize=" + $("#hidFontSize").val() + "')\"> <i class=\"layui-icon\" >&#xe642;</i>添加评论<span class=\"layui-badge\">" + arrvViewpointExObjLst[i].appraiseCount + "</span></button >";
