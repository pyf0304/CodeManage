...
(function (factory) {
    ...
    **/
    async function vXzMajorDirection_GetObjByMajorDirectionIdAsync(strMajorDirectionId) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strMajorDirectionId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strMajorDirectionId]不能为空！(In clsvXzMajorDirectionWApi.GetObjByMajorDirectionIdAsync)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strMajorDirectionId.length != 8) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strMajorDirectionId]的长度:[{0}]不正确！(clsvXzMajorDirectionWApi.GetObjByMajorDirectionIdAsync)", strMajorDirectionId.length);
            console.error(strMsg);
            throw (strMsg);
        }
    ...
    */
    async function vXzMajorDirection_GetObjByMajorDirectionId_Cache(strMajorDirectionId, bolTryAsyncOnce = true) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strMajorDirectionId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strMajorDirectionId]不能为空！(In clsvXzMajorDirectionWApi.GetObjByMajorDirectionId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strMajorDirectionId.length != 8) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strMajorDirectionId]的长度:[{0}]不正确！(clsvXzMajorDirectionWApi.GetObjByMajorDirectionId_Cache)", strMajorDirectionId.length);
            console.error(strMsg);
            throw (strMsg);
        }
        ...
        try {
            ...
            if (arrvXzMajorDirection_Sel.length > 0) {
                ...
                if (bolTryAsyncOnce == true) {
                    const objvXzMajorDirection_Const = await vXzMajorDirection_GetObjByMajorDirectionIdAsync(strMajorDirectionId);
                    if (objvXzMajorDirection_Const != null) {
                        vXzMajorDirection_ReFreshThisCache();
                        return objvXzMajorDirection_Const;
                    }
                }
                return null;
            }
    ...
    */
    async function vXzMajorDirection_GetObjByMajorDirectionId_localStorage(strMajorDirectionId) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strMajorDirectionId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strMajorDirectionId]不能为空！(In clsvXzMajorDirectionWApi.GetObjByMajorDirectionId_localStorage)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strMajorDirectionId.length != 8) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strMajorDirectionId]的长度:[{0}]不正确！(clsvXzMajorDirectionWApi.GetObjByMajorDirectionId_localStorage)", strMajorDirectionId.length);
            console.error(strMsg);
            throw (strMsg);
        }
    ...
    */
    async function vXzMajorDirection_func(strInFldName, strOutFldName, strInValue) {
        ...
        if (clsvXzMajorDirectionEN_js_1.clsvXzMajorDirectionEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_1.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strOutFldName, clsvXzMajorDirectionEN_js_1.clsvXzMajorDirectionEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
        ...
        if (objvXzMajorDirection == null)
            return "";
        if (objvXzMajorDirection.GetFldValue(strOutFldName) == null)
            return "";
    ...
    */
     ...
     * @returns 返回一个关键字值列表
    */
    async function vXzMajorDirection_funcKey(strInFldName, strInValue, strComparisonOp) {
        ...
        const arrvXzMajorDirection = await vXzMajorDirection_GetObjLst_Cache();
        if (arrvXzMajorDirection == null)
            return [];
        let arrvXzMajorDirection_Sel = arrvXzMajorDirection;
        const strType = typeof (strInValue);
        switch (strType) {
            case "string":
                switch (strComparisonOp) {
                    case enumComparisonOp_js_1.enumComparisonOp.Equal_01: // " = "
                        arrvXzMajorDirection_Sel = arrvXzMajorDirection_Sel.filter(x => x.GetFldValue(strInFldName).toString() == strInValue.toString());
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.Like_03:
                        arrvXzMajorDirection_Sel = arrvXzMajorDirection_Sel.filter(x => x.GetFldValue(strInFldName).toString().indexOf(strInValue.toString()) != -1);
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.In_04:
                        const arrValues = strInValue.split(',');
                        arrvXzMajorDirection_Sel = arrvXzMajorDirection_Sel.filter(x => arrValues.indexOf(x.GetFldValue(strInFldName).toString()) != -1);
                        break;
                }
                break;
            case "boolean":
                ...
                if (strComparisonOp == enumComparisonOp_js_1.enumComparisonOp.Equal_01) {
                    arrvXzMajorDirection_Sel = arrvXzMajorDirection_Sel.filter(x => x.GetFldValue(strInFldName) == strInValue);
                }
                break;
            case "number":
        ...
        if (arrvXzMajorDirection_Sel.length == 0)
            return [];
        return arrvXzMajorDirection_Sel.map(x => x.majorDirectionId);
    }
    exports.vXzMajorDirection_funcKey = vXzMajorDirection_funcKey;
    /**
    * 根据条件获取满足条件的第一条记录
    ...
    **/
    async function vXzMajorDirection_GetObjLst_ClientCache() {
        ...
        if (CacheHelper_js_1.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrvXzMajorDirectionExObjLst_Cache = CacheHelper_js_1.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_1.CacheHelper.Add(strKey, arrvXzMajorDirectionExObjLst);
                ...
                switch (strType) {
                    case "string":
                        ...
                        if (strComparisonOp == "=") {
                            ...
                            arrvXzMajorDirection_Sel = arrvXzMajorDirection_Sel.filter(x => x.GetFldValue(strKey).toString().length > Number(strValue.toString()));
                        }
