...
(function (factory) {
    ...
    **/
    async function vSysScoreSummary_GetObjBymIdAsync(lngmId) {
        ...
        if (lngmId == 0) {
            const strMsg = (0, clsString_js_1.Format)("参数:[lngmId]不能为空！(In clsvSysScoreSummaryWApi.GetObjBymIdAsync)");
    ...
    */
    async function vSysScoreSummary_GetObjBymId_Cache(lngmId, strid_CurrEduCls, bolTryAsyncOnce = true) {
        ...
        if (lngmId == 0) {
            const strMsg = (0, clsString_js_1.Format)("参数:[lngmId]不能为空！(In clsvSysScoreSummaryWApi.GetObjBymId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        ...
        try {
            ...
            if (arrvSysScoreSummary_Sel.length > 0) {
                ...
                if (bolTryAsyncOnce == true) {
                    const objvSysScoreSummary_Const = await vSysScoreSummary_GetObjBymIdAsync(lngmId);
                    if (objvSysScoreSummary_Const != null) {
                        vSysScoreSummary_ReFreshThisCache(strid_CurrEduCls);
                        return objvSysScoreSummary_Const;
                    }
                }
                return null;
            }
    ...
    */
    async function vSysScoreSummary_GetObjBymId_localStorage(lngmId) {
        ...
        if (lngmId == 0) {
            const strMsg = (0, clsString_js_1.Format)("参数:[lngmId]不能为空！(In clsvSysScoreSummaryWApi.GetObjBymId_localStorage)");
            console.error(strMsg);
            throw (strMsg);
        }
    ...
    */
    async function vSysScoreSummary_func(strInFldName, strOutFldName, strInValue, strid_CurrEduCls_C) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strid_CurrEduCls_C) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strid_CurrEduCls_C]不能为空！(In clsvSysScoreSummaryWApi.func)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strid_CurrEduCls_C.length != 8) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strid_CurrEduCls_C]的长度:[{0}]不正确！(clsvSysScoreSummaryWApi.func)", strid_CurrEduCls_C.length);
            console.error(strMsg);
            throw (strMsg);
        }
        ...
        if (clsvSysScoreSummaryEN_js_1.clsvSysScoreSummaryEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_1.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strOutFldName, clsvSysScoreSummaryEN_js_1.clsvSysScoreSummaryEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
        ...
        if (objvSysScoreSummary == null)
            return "";
        if (objvSysScoreSummary.GetFldValue(strOutFldName) == null)
            return "";
    ...
    */
     ...
     * @returns 返回一个关键字值列表
    */
    async function vSysScoreSummary_funcKey(strInFldName, strInValue, strComparisonOp, strid_CurrEduCls_C) {
        //const strThisFuncName = "funcKey";
        if ((0, clsString_js_1.IsNullOrEmpty)(strid_CurrEduCls_C) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strid_CurrEduCls_C]不能为空！(In clsvSysScoreSummaryWApi.funcKey)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strid_CurrEduCls_C.length != 8) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strid_CurrEduCls_C]的长度:[{0}]不正确！(clsvSysScoreSummaryWApi.funcKey)", strid_CurrEduCls_C.length);
            console.error(strMsg);
            throw (strMsg);
        }
        if (strInFldName == clsvSysScoreSummaryEN_js_1.clsvSysScoreSummaryEN.con_mId) {
        ...
        const arrvSysScoreSummary = await vSysScoreSummary_GetObjLst_Cache(strid_CurrEduCls_C);
        if (arrvSysScoreSummary == null)
            return [];
        let arrvSysScoreSummary_Sel = arrvSysScoreSummary;
        const strType = typeof (strInValue);
        switch (strType) {
            case "string":
                switch (strComparisonOp) {
                    case enumComparisonOp_js_1.enumComparisonOp.Equal_01: // " = "
                        arrvSysScoreSummary_Sel = arrvSysScoreSummary_Sel.filter(x => x.GetFldValue(strInFldName).toString() == strInValue.toString());
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.Like_03:
                        arrvSysScoreSummary_Sel = arrvSysScoreSummary_Sel.filter(x => x.GetFldValue(strInFldName).toString().indexOf(strInValue.toString()) != -1);
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.In_04:
                        const arrValues = strInValue.split(',');
                        arrvSysScoreSummary_Sel = arrvSysScoreSummary_Sel.filter(x => arrValues.indexOf(x.GetFldValue(strInFldName).toString()) != -1);
                        break;
                }
                break;
            case "boolean":
                ...
                if (strComparisonOp == enumComparisonOp_js_1.enumComparisonOp.Equal_01) {
                    arrvSysScoreSummary_Sel = arrvSysScoreSummary_Sel.filter(x => x.GetFldValue(strInFldName) == strInValue);
                }
                break;
            case "number":
                if (Number(strInValue) == 0)
        ...
        if (arrvSysScoreSummary_Sel.length == 0)
            return [];
        return arrvSysScoreSummary_Sel.map(x => x.mId);
    }
    exports.vSysScoreSummary_funcKey = vSysScoreSummary_funcKey;
    /**
    * 根据条件获取满足条件的第一条记录
    ...
    **/
    async function vSysScoreSummary_GetObjLst_ClientCache(strid_CurrEduCls) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(clsvSysScoreSummaryEN_js_1.clsvSysScoreSummaryEN.WhereFormat) == false) {
            strWhereCond = (0, clsString_js_1.Format)(clsvSysScoreSummaryEN_js_1.clsvSysScoreSummaryEN.WhereFormat, strid_CurrEduCls);
        }
        else {
        ...
        if (CacheHelper_js_1.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrvSysScoreSummaryExObjLst_Cache = CacheHelper_js_1.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_1.CacheHelper.Add(strKey, arrvSysScoreSummaryExObjLst);
    ...
    **/
    async function vSysScoreSummary_GetObjLst_Cache(strid_CurrEduCls) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strid_CurrEduCls) == true) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[id_CurrEduCls]不能为空！(in clsvSysScoreSummaryWApi.vSysScoreSummary_GetObjLst_Cache() )");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strid_CurrEduCls.length != 8) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strid_CurrEduCls]的长度:[{0}]不正确！(in clsvSysScoreSummaryWApi.vSysScoreSummary_GetObjLst_Cache() )", strid_CurrEduCls.length);
            console.error(strMsg);
            throw (strMsg);
        }
                ...
                switch (strType) {
                    case "string":
                        ...
                        if (strComparisonOp == "=") {
                            ...
                            arrvSysScoreSummary_Sel = arrvSysScoreSummary_Sel.filter(x => x.GetFldValue(strKey).toString().length > Number(strValue.toString()));
                        }
