...
(function (factory) {
    ...
    **/
    async function vqa_Questions_GetObjByQuestionsIdAsync(strQuestionsId) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strQuestionsId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strQuestionsId]不能为空！(In clsvqa_QuestionsWApi.GetObjByQuestionsIdAsync)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strQuestionsId.length != 8) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strQuestionsId]的长度:[{0}]不正确！(clsvqa_QuestionsWApi.GetObjByQuestionsIdAsync)", strQuestionsId.length);
            console.error(strMsg);
            throw (strMsg);
        }
    ...
    */
    async function vqa_Questions_GetObjByQuestionsId_Cache(strQuestionsId, bolTryAsyncOnce = true) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strQuestionsId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strQuestionsId]不能为空！(In clsvqa_QuestionsWApi.GetObjByQuestionsId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strQuestionsId.length != 8) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strQuestionsId]的长度:[{0}]不正确！(clsvqa_QuestionsWApi.GetObjByQuestionsId_Cache)", strQuestionsId.length);
            console.error(strMsg);
            throw (strMsg);
        }
        ...
        try {
            ...
            if (arrvqa_Questions_Sel.length > 0) {
                ...
                if (bolTryAsyncOnce == true) {
                    const objvqa_Questions_Const = await vqa_Questions_GetObjByQuestionsIdAsync(strQuestionsId);
                    if (objvqa_Questions_Const != null) {
                        vqa_Questions_ReFreshThisCache();
                        return objvqa_Questions_Const;
                    }
                }
                return null;
            }
    ...
    */
    async function vqa_Questions_GetObjByQuestionsId_localStorage(strQuestionsId) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strQuestionsId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strQuestionsId]不能为空！(In clsvqa_QuestionsWApi.GetObjByQuestionsId_localStorage)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strQuestionsId.length != 8) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strQuestionsId]的长度:[{0}]不正确！(clsvqa_QuestionsWApi.GetObjByQuestionsId_localStorage)", strQuestionsId.length);
            console.error(strMsg);
            throw (strMsg);
        }
    ...
    */
    async function vqa_Questions_func(strInFldName, strOutFldName, strInValue) {
        ...
        if (clsvqa_QuestionsEN_js_1.clsvqa_QuestionsEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_1.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strOutFldName, clsvqa_QuestionsEN_js_1.clsvqa_QuestionsEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
        ...
        if (objvqa_Questions == null)
            return "";
        if (objvqa_Questions.GetFldValue(strOutFldName) == null)
            return "";
    ...
    */
     ...
     * @returns 返回一个关键字值列表
    */
    async function vqa_Questions_funcKey(strInFldName, strInValue, strComparisonOp) {
        ...
        const arrvqa_Questions = await vqa_Questions_GetObjLst_Cache();
        if (arrvqa_Questions == null)
            return [];
        let arrvqa_Questions_Sel = arrvqa_Questions;
        const strType = typeof (strInValue);
        switch (strType) {
            case "string":
                switch (strComparisonOp) {
                    case enumComparisonOp_js_1.enumComparisonOp.Equal_01: // " = "
                        arrvqa_Questions_Sel = arrvqa_Questions_Sel.filter(x => x.GetFldValue(strInFldName).toString() == strInValue.toString());
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.Like_03:
                        arrvqa_Questions_Sel = arrvqa_Questions_Sel.filter(x => x.GetFldValue(strInFldName).toString().indexOf(strInValue.toString()) != -1);
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.In_04:
                        const arrValues = strInValue.split(',');
                        arrvqa_Questions_Sel = arrvqa_Questions_Sel.filter(x => arrValues.indexOf(x.GetFldValue(strInFldName).toString()) != -1);
                        break;
                }
                break;
            case "boolean":
                ...
                if (strComparisonOp == enumComparisonOp_js_1.enumComparisonOp.Equal_01) {
                    arrvqa_Questions_Sel = arrvqa_Questions_Sel.filter(x => x.GetFldValue(strInFldName) == strInValue);
                }
                break;
            case "number":
        ...
        if (arrvqa_Questions_Sel.length == 0)
            return [];
        return arrvqa_Questions_Sel.map(x => x.questionsId);
    }
    exports.vqa_Questions_funcKey = vqa_Questions_funcKey;
    /**
    * 根据条件获取满足条件的第一条记录
    ...
    **/
    async function vqa_Questions_GetObjLst_ClientCache() {
        ...
        if (CacheHelper_js_1.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrvqa_QuestionsExObjLst_Cache = CacheHelper_js_1.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_1.CacheHelper.Add(strKey, arrvqa_QuestionsExObjLst);
                ...
                switch (strType) {
                    case "string":
                        ...
                        if (strComparisonOp == "=") {
                            ...
                            arrvqa_Questions_Sel = arrvqa_Questions_Sel.filter(x => x.GetFldValue(strKey).toString().length > Number(strValue.toString()));
                        }
