...
(function (factory) {
    ...
    **/
    async function vObjectiveAttachment_GetObjByObjectiveAttachmentIdAsync(lngObjectiveAttachmentId) {
        ...
        if (lngObjectiveAttachmentId == 0) {
            const strMsg = (0, clsString_js_1.Format)("参数:[lngObjectiveAttachmentId]不能为空！(In clsvObjectiveAttachmentWApi.GetObjByObjectiveAttachmentIdAsync)");
    ...
    */
    async function vObjectiveAttachment_GetObjByObjectiveAttachmentId_Cache(lngObjectiveAttachmentId, strid_CurrEduCls, bolTryAsyncOnce = true) {
        ...
        if (lngObjectiveAttachmentId == 0) {
            const strMsg = (0, clsString_js_1.Format)("参数:[lngObjectiveAttachmentId]不能为空！(In clsvObjectiveAttachmentWApi.GetObjByObjectiveAttachmentId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
    ...
    */
    async function vObjectiveAttachment_GetObjByObjectiveAttachmentId_localStorage(lngObjectiveAttachmentId) {
        ...
        if (lngObjectiveAttachmentId == 0) {
            const strMsg = (0, clsString_js_1.Format)("参数:[lngObjectiveAttachmentId]不能为空！(In clsvObjectiveAttachmentWApi.GetObjByObjectiveAttachmentId_localStorage)");
            console.error(strMsg);
            throw (strMsg);
        }
    ...
    */
    async function vObjectiveAttachment_func(strInFldName, strOutFldName, strInValue, strid_CurrEduCls_C) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strid_CurrEduCls_C) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strid_CurrEduCls_C]不能为空！(In clsvObjectiveAttachmentWApi.func)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strid_CurrEduCls_C.length != 8) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strid_CurrEduCls_C]的长度:[{0}]不正确！(clsvObjectiveAttachmentWApi.func)", strid_CurrEduCls_C.length);
            console.error(strMsg);
            throw (strMsg);
        }
        ...
        if (clsvObjectiveAttachmentEN_js_1.clsvObjectiveAttachmentEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_1.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strOutFldName, clsvObjectiveAttachmentEN_js_1.clsvObjectiveAttachmentEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
        ...
        if (objvObjectiveAttachment == null)
            return "";
        if (objvObjectiveAttachment.GetFldValue(strOutFldName) == null)
            return "";
    ...
    */
     ...
     * @returns 返回一个关键字值列表
    */
    async function vObjectiveAttachment_funcKey(strInFldName, strInValue, strComparisonOp, strid_CurrEduCls_C) {
        //const strThisFuncName = "funcKey";
        if ((0, clsString_js_1.IsNullOrEmpty)(strid_CurrEduCls_C) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strid_CurrEduCls_C]不能为空！(In clsvObjectiveAttachmentWApi.funcKey)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strid_CurrEduCls_C.length != 8) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strid_CurrEduCls_C]的长度:[{0}]不正确！(clsvObjectiveAttachmentWApi.funcKey)", strid_CurrEduCls_C.length);
            console.error(strMsg);
            throw (strMsg);
        }
        if (strInFldName == clsvObjectiveAttachmentEN_js_1.clsvObjectiveAttachmentEN.con_ObjectiveAttachmentId) {
        ...
        if (arrvObjectiveAttachment == null)
            return [];
    ...
    exports.vObjectiveAttachment_funcKey = vObjectiveAttachment_funcKey;
    /**
    * 根据条件获取满足条件的第一条记录
    ...
    **/
    async function vObjectiveAttachment_GetObjLst_ClientCache(strid_CurrEduCls) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(clsvObjectiveAttachmentEN_js_1.clsvObjectiveAttachmentEN.WhereFormat) == false) {
            strWhereCond = (0, clsString_js_1.Format)(clsvObjectiveAttachmentEN_js_1.clsvObjectiveAttachmentEN.WhereFormat, strid_CurrEduCls);
        }
        else {
        ...
        if (CacheHelper_js_1.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrvObjectiveAttachmentExObjLst_Cache = CacheHelper_js_1.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_1.CacheHelper.Add(strKey, arrvObjectiveAttachmentExObjLst);
    ...
    **/
    async function vObjectiveAttachment_GetObjLst_Cache(strid_CurrEduCls) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strid_CurrEduCls) == true) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[id_CurrEduCls]不能为空！(in clsvObjectiveAttachmentWApi.vObjectiveAttachment_GetObjLst_Cache() )");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strid_CurrEduCls.length != 8) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strid_CurrEduCls]的长度:[{0}]不正确！(in clsvObjectiveAttachmentWApi.vObjectiveAttachment_GetObjLst_Cache() )", strid_CurrEduCls.length);
            console.error(strMsg);
            throw (strMsg);
        }
