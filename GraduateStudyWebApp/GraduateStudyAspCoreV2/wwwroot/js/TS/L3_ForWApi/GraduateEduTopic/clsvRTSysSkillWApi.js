...
(function (factory) {
    ...
    **/
    async function vRTSysSkill_GetObjBymIdAsync(lngmId) {
        ...
        if (lngmId == 0) {
            const strMsg = (0, clsString_js_1.Format)("参数:[lngmId]不能为空！(In clsvRTSysSkillWApi.GetObjBymIdAsync)");
    ...
    */
    async function vRTSysSkill_GetObjBymId_Cache(lngmId, bolTryAsyncOnce = true) {
        ...
        if (lngmId == 0) {
            const strMsg = (0, clsString_js_1.Format)("参数:[lngmId]不能为空！(In clsvRTSysSkillWApi.GetObjBymId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
    ...
    */
    async function vRTSysSkill_GetObjBymId_localStorage(lngmId) {
        ...
        if (lngmId == 0) {
            const strMsg = (0, clsString_js_1.Format)("参数:[lngmId]不能为空！(In clsvRTSysSkillWApi.GetObjBymId_localStorage)");
            console.error(strMsg);
            throw (strMsg);
        }
    ...
    */
    async function vRTSysSkill_GetNameBymId_Cache(lngmId) {
        ...
        if (lngmId == 0) {
            const strMsg = (0, clsString_js_1.Format)("参数:[lngmId]不能为空！(In clsvRTSysSkillWApi.GetNameBymId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        const arrvRTSysSkillObjLst_Cache = await vRTSysSkill_GetObjLst_Cache();
    ...
    */
    async function vRTSysSkill_func(strInFldName, strOutFldName, strInValue) {
        ...
        if (clsvRTSysSkillEN_js_1.clsvRTSysSkillEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_1.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strOutFldName, clsvRTSysSkillEN_js_1.clsvRTSysSkillEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
        ...
        if (objvRTSysSkill == null)
            return "";
        if (objvRTSysSkill.GetFldValue(strOutFldName) == null)
            return "";
    ...
    */
     ...
     * @returns 返回一个关键字值列表
    */
    async function vRTSysSkill_funcKey(strInFldName, strInValue, strComparisonOp) {
    ...
    exports.vRTSysSkill_funcKey = vRTSysSkill_funcKey;
    /**
    * 根据条件获取满足条件的第一条记录
    ...
    **/
    async function vRTSysSkill_GetObjLst_ClientCache() {
        ...
        if (CacheHelper_js_1.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrvRTSysSkillExObjLst_Cache = CacheHelper_js_1.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_1.CacheHelper.Add(strKey, arrvRTSysSkillExObjLst);
