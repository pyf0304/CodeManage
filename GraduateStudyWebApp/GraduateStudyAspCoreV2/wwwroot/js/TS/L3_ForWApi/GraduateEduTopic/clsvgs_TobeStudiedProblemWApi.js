...
(function (factory) {
    ...
    **/
    async function vgs_TobeStudiedProblem_GetObjByProblemIdAsync(strProblemId) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strProblemId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strProblemId]不能为空！(In clsvgs_TobeStudiedProblemWApi.GetObjByProblemIdAsync)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strProblemId.length != 10) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strProblemId]的长度:[{0}]不正确！(clsvgs_TobeStudiedProblemWApi.GetObjByProblemIdAsync)", strProblemId.length);
            console.error(strMsg);
            throw (strMsg);
        }
    ...
    */
    async function vgs_TobeStudiedProblem_GetObjByProblemId_Cache(strProblemId, bolTryAsyncOnce = true) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strProblemId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strProblemId]不能为空！(In clsvgs_TobeStudiedProblemWApi.GetObjByProblemId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strProblemId.length != 10) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strProblemId]的长度:[{0}]不正确！(clsvgs_TobeStudiedProblemWApi.GetObjByProblemId_Cache)", strProblemId.length);
            console.error(strMsg);
            throw (strMsg);
        }
        ...
        try {
            ...
            if (arrvgs_TobeStudiedProblem_Sel.length > 0) {
                ...
                if (bolTryAsyncOnce == true) {
                    const objvgs_TobeStudiedProblem_Const = await vgs_TobeStudiedProblem_GetObjByProblemIdAsync(strProblemId);
                    if (objvgs_TobeStudiedProblem_Const != null) {
                        vgs_TobeStudiedProblem_ReFreshThisCache();
                        return objvgs_TobeStudiedProblem_Const;
                    }
                }
                return null;
            }
    ...
    */
    async function vgs_TobeStudiedProblem_GetObjByProblemId_localStorage(strProblemId) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strProblemId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strProblemId]不能为空！(In clsvgs_TobeStudiedProblemWApi.GetObjByProblemId_localStorage)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strProblemId.length != 10) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strProblemId]的长度:[{0}]不正确！(clsvgs_TobeStudiedProblemWApi.GetObjByProblemId_localStorage)", strProblemId.length);
            console.error(strMsg);
            throw (strMsg);
        }
    ...
    */
    async function vgs_TobeStudiedProblem_func(strInFldName, strOutFldName, strInValue) {
        ...
        if (clsvgs_TobeStudiedProblemEN_js_1.clsvgs_TobeStudiedProblemEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_1.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strOutFldName, clsvgs_TobeStudiedProblemEN_js_1.clsvgs_TobeStudiedProblemEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
        ...
        if (objvgs_TobeStudiedProblem == null)
            return "";
        if (objvgs_TobeStudiedProblem.GetFldValue(strOutFldName) == null)
            return "";
    ...
    */
     ...
     * @returns 返回一个关键字值列表
    */
    async function vgs_TobeStudiedProblem_funcKey(strInFldName, strInValue, strComparisonOp) {
        ...
        const arrvgs_TobeStudiedProblem = await vgs_TobeStudiedProblem_GetObjLst_Cache();
        if (arrvgs_TobeStudiedProblem == null)
            return [];
        let arrvgs_TobeStudiedProblem_Sel = arrvgs_TobeStudiedProblem;
        const strType = typeof (strInValue);
        switch (strType) {
            case "string":
                switch (strComparisonOp) {
                    case enumComparisonOp_js_1.enumComparisonOp.Equal_01: // " = "
                        arrvgs_TobeStudiedProblem_Sel = arrvgs_TobeStudiedProblem_Sel.filter(x => x.GetFldValue(strInFldName).toString() == strInValue.toString());
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.Like_03:
                        arrvgs_TobeStudiedProblem_Sel = arrvgs_TobeStudiedProblem_Sel.filter(x => x.GetFldValue(strInFldName).toString().indexOf(strInValue.toString()) != -1);
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.In_04:
                        const arrValues = strInValue.split(',');
                        arrvgs_TobeStudiedProblem_Sel = arrvgs_TobeStudiedProblem_Sel.filter(x => arrValues.indexOf(x.GetFldValue(strInFldName).toString()) != -1);
                        break;
                }
                break;
            case "boolean":
                ...
                if (strComparisonOp == enumComparisonOp_js_1.enumComparisonOp.Equal_01) {
                    arrvgs_TobeStudiedProblem_Sel = arrvgs_TobeStudiedProblem_Sel.filter(x => x.GetFldValue(strInFldName) == strInValue);
                }
                break;
            case "number":
        ...
        if (arrvgs_TobeStudiedProblem_Sel.length == 0)
            return [];
        return arrvgs_TobeStudiedProblem_Sel.map(x => x.problemId);
    }
    exports.vgs_TobeStudiedProblem_funcKey = vgs_TobeStudiedProblem_funcKey;
    /**
    * 根据条件获取满足条件的第一条记录
    ...
    **/
    async function vgs_TobeStudiedProblem_GetObjLst_ClientCache() {
        ...
        if (CacheHelper_js_1.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrvgs_TobeStudiedProblemExObjLst_Cache = CacheHelper_js_1.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_1.CacheHelper.Add(strKey, arrvgs_TobeStudiedProblemExObjLst);
                ...
                switch (strType) {
                    case "string":
                        ...
                        if (strComparisonOp == "=") {
                            ...
                            arrvgs_TobeStudiedProblem_Sel = arrvgs_TobeStudiedProblem_Sel.filter(x => x.GetFldValue(strKey).toString().length > Number(strValue.toString()));
                        }
