...
(function (factory) {
    ...
    **/
    async function vRTTopicObjectiveRela_GetObjBymIdAsync(lngmId) {
        ...
        if (lngmId == 0) {
            const strMsg = (0, clsString_js_1.Format)("参数:[lngmId]不能为空！(In clsvRTTopicObjectiveRelaWApi.GetObjBymIdAsync)");
    ...
    */
    async function vRTTopicObjectiveRela_GetObjBymId_Cache(lngmId, bolTryAsyncOnce = true) {
        ...
        if (lngmId == 0) {
            const strMsg = (0, clsString_js_1.Format)("参数:[lngmId]不能为空！(In clsvRTTopicObjectiveRelaWApi.GetObjBymId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        ...
        try {
            ...
            if (arrvRTTopicObjectiveRela_Sel.length > 0) {
                ...
                if (bolTryAsyncOnce == true) {
                    const objvRTTopicObjectiveRela_Const = await vRTTopicObjectiveRela_GetObjBymIdAsync(lngmId);
                    if (objvRTTopicObjectiveRela_Const != null) {
                        vRTTopicObjectiveRela_ReFreshThisCache();
                        return objvRTTopicObjectiveRela_Const;
                    }
                }
                return null;
            }
    ...
    */
    async function vRTTopicObjectiveRela_GetObjBymId_localStorage(lngmId) {
        ...
        if (lngmId == 0) {
            const strMsg = (0, clsString_js_1.Format)("参数:[lngmId]不能为空！(In clsvRTTopicObjectiveRelaWApi.GetObjBymId_localStorage)");
            console.error(strMsg);
            throw (strMsg);
        }
    ...
    */
    async function vRTTopicObjectiveRela_func(strInFldName, strOutFldName, strInValue) {
        ...
        if (clsvRTTopicObjectiveRelaEN_js_1.clsvRTTopicObjectiveRelaEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_1.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strOutFldName, clsvRTTopicObjectiveRelaEN_js_1.clsvRTTopicObjectiveRelaEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
        ...
        if (objvRTTopicObjectiveRela == null)
            return "";
        if (objvRTTopicObjectiveRela.GetFldValue(strOutFldName) == null)
            return "";
    ...
    */
     ...
     * @returns 返回一个关键字值列表
    */
    async function vRTTopicObjectiveRela_funcKey(strInFldName, strInValue, strComparisonOp) {
        ...
        const arrvRTTopicObjectiveRela = await vRTTopicObjectiveRela_GetObjLst_Cache();
        if (arrvRTTopicObjectiveRela == null)
            return [];
        let arrvRTTopicObjectiveRela_Sel = arrvRTTopicObjectiveRela;
        const strType = typeof (strInValue);
        switch (strType) {
            case "string":
                switch (strComparisonOp) {
                    case enumComparisonOp_js_1.enumComparisonOp.Equal_01: // " = "
                        arrvRTTopicObjectiveRela_Sel = arrvRTTopicObjectiveRela_Sel.filter(x => x.GetFldValue(strInFldName).toString() == strInValue.toString());
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.Like_03:
                        arrvRTTopicObjectiveRela_Sel = arrvRTTopicObjectiveRela_Sel.filter(x => x.GetFldValue(strInFldName).toString().indexOf(strInValue.toString()) != -1);
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.In_04:
                        const arrValues = strInValue.split(',');
                        arrvRTTopicObjectiveRela_Sel = arrvRTTopicObjectiveRela_Sel.filter(x => arrValues.indexOf(x.GetFldValue(strInFldName).toString()) != -1);
                        break;
                }
                break;
            case "boolean":
                ...
                if (strComparisonOp == enumComparisonOp_js_1.enumComparisonOp.Equal_01) {
                    arrvRTTopicObjectiveRela_Sel = arrvRTTopicObjectiveRela_Sel.filter(x => x.GetFldValue(strInFldName) == strInValue);
                }
                break;
            case "number":
                if (Number(strInValue) == 0)
        ...
        if (arrvRTTopicObjectiveRela_Sel.length == 0)
            return [];
        return arrvRTTopicObjectiveRela_Sel.map(x => x.mId);
    }
    exports.vRTTopicObjectiveRela_funcKey = vRTTopicObjectiveRela_funcKey;
    /**
    * 根据条件获取满足条件的第一条记录
    ...
    **/
    async function vRTTopicObjectiveRela_GetObjLst_ClientCache() {
        ...
        if (CacheHelper_js_1.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrvRTTopicObjectiveRelaExObjLst_Cache = CacheHelper_js_1.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_1.CacheHelper.Add(strKey, arrvRTTopicObjectiveRelaExObjLst);
                ...
                switch (strType) {
                    case "string":
                        ...
                        if (strComparisonOp == "=") {
                            ...
                            arrvRTTopicObjectiveRela_Sel = arrvRTTopicObjectiveRela_Sel.filter(x => x.GetFldValue(strKey).toString().length > Number(strValue.toString()));
                        }
