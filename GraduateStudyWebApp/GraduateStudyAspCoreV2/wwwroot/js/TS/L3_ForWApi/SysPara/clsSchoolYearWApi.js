...
(function (factory) {
    ...
    **/
    async function SchoolYear_GetObjBySchoolYearAsync(strSchoolYear) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(strSchoolYear) == true) {
            const strMsg = (0, clsString_js_2.Format)("参数:[strSchoolYear]不能为空！(In clsSchoolYearWApi.GetObjBySchoolYearAsync)");
        ...
        try {
            ...
            if (error.statusText == "error") {
                const strInfo = (0, clsString_js_2.Format)("网络错误！访问地址:{0}不成功！(in {1}.{2})", strUrl, exports.schoolYear_ConstructorName, strThisFuncName);
                console.error(strInfo);
                throw (strInfo);
            }
            else if (error.statusText == "Not Found") {
                const strInfo = (0, clsString_js_2.Format)("网络错误！访问地址:{0}可能不存在！(in {1}.{2})", strUrl, exports.schoolYear_ConstructorName, strThisFuncName);
                console.error(strInfo);
                throw (strInfo);
            }
            else {
    ...
    */
    async function SchoolYear_GetObjBySchoolYear_Cache(strSchoolYear, bolTryAsyncOnce = true) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(strSchoolYear) == true) {
            const strMsg = (0, clsString_js_2.Format)("参数:[strSchoolYear]不能为空！(In clsSchoolYearWApi.GetObjBySchoolYear_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        ...
        try {
            ...
            if (arrSchoolYear_Sel.length > 0) {
                ...
                if (bolTryAsyncOnce == true) {
                    const objSchoolYear_Const = await SchoolYear_GetObjBySchoolYearAsync(strSchoolYear);
                    if (objSchoolYear_Const != null) {
                        SchoolYear_ReFreshThisCache();
                        return objSchoolYear_Const;
                    }
                }
                return null;
            }
    ...
    */
    async function SchoolYear_GetObjBySchoolYear_localStorage(strSchoolYear) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(strSchoolYear) == true) {
            const strMsg = (0, clsString_js_2.Format)("参数:[strSchoolYear]不能为空！(In clsSchoolYearWApi.GetObjBySchoolYear_localStorage)");
            console.error(strMsg);
            throw (strMsg);
        }
        const strKey = (0, clsString_js_2.Format)("{0}_{1}", clsSchoolYearEN_js_1.clsSchoolYearEN._CurrTabName, strSchoolYear);
        if (strKey == "") {
    ...
    */
    async function SchoolYear_GetNameBySchoolYear_Cache(strSchoolYear) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(strSchoolYear) == true) {
            const strMsg = (0, clsString_js_2.Format)("参数:[strSchoolYear]不能为空！(In clsSchoolYearWApi.GetNameBySchoolYear_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        const arrSchoolYearObjLst_Cache = await SchoolYear_GetObjLst_Cache();
    ...
    */
    async function SchoolYear_func(strInFldName, strOutFldName, strInValue) {
        ...
        if (clsSchoolYearEN_js_1.clsSchoolYearEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_2.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strOutFldName, clsSchoolYearEN_js_1.clsSchoolYearEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
        ...
        if (objSchoolYear == null)
            return "";
        if (objSchoolYear.GetFldValue(strOutFldName) == null)
            return "";
        ...
        if (AscOrDesc == "Asc" || AscOrDesc == "") {
            switch (strKey) {
                ...
                case clsSchoolYearEN_js_1.clsSchoolYearEN.con_SchoolYearName:
                    return (a, b) => {
                        if (a.schoolYearName == null)
                            return -1;
                        if (b.schoolYearName == null)
                            return 1;
    ...
    */
     ...
     * @returns 返回一个关键字值列表
    */
    async function SchoolYear_funcKey(strInFldName, strInValue, strComparisonOp) {
        ...
        const arrSchoolYear = await SchoolYear_GetObjLst_Cache();
        if (arrSchoolYear == null)
            return [];
        let arrSchoolYear_Sel = arrSchoolYear;
        const strType = typeof (strInValue);
        switch (strType) {
            case "string":
                switch (strComparisonOp) {
                    case enumComparisonOp_js_1.enumComparisonOp.Equal_01: // " = "
                        arrSchoolYear_Sel = arrSchoolYear_Sel.filter(x => x.GetFldValue(strInFldName).toString() == strInValue.toString());
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.Like_03:
                        arrSchoolYear_Sel = arrSchoolYear_Sel.filter(x => x.GetFldValue(strInFldName).toString().indexOf(strInValue.toString()) != -1);
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.In_04:
                        const arrValues = strInValue.split(',');
                        arrSchoolYear_Sel = arrSchoolYear_Sel.filter(x => arrValues.indexOf(x.GetFldValue(strInFldName).toString()) != -1);
                        break;
                }
                break;
            case "boolean":
                ...
                if (strComparisonOp == enumComparisonOp_js_1.enumComparisonOp.Equal_01) {
                    arrSchoolYear_Sel = arrSchoolYear_Sel.filter(x => x.GetFldValue(strInFldName) == strInValue);
                }
                break;
            case "number":
        ...
        if (arrSchoolYear_Sel.length == 0)
            return [];
        return arrSchoolYear_Sel.map(x => x.schoolYear);
    }
    exports.SchoolYear_funcKey = SchoolYear_funcKey;
    /**
    * 根据条件获取满足条件的第一条记录
    ...
    **/
    async function SchoolYear_GetObjLst_ClientCache() {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(clsSchoolYearEN_js_1.clsSchoolYearEN.CacheAddiCondition) == false) {
            strWhereCond += (0, clsString_js_2.Format)(" and {0}", clsSchoolYearEN_js_1.clsSchoolYearEN.CacheAddiCondition);
        }
        if (strKey == "") {
            console.error("关键字为空！不正确");
            throw new Error("关键字为空！不正确");
        ...
        if (CacheHelper_js_1.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrSchoolYearExObjLst_Cache = CacheHelper_js_1.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_1.CacheHelper.Add(strKey, arrSchoolYearExObjLst);
            const strInfo = (0, clsString_js_2.Format)("[ClientCache]Key:[{0}]的缓存已经建立，对象列表数：{1}!", strKey, arrSchoolYearExObjLst.length);
            console.log(strInfo);
    ...
    **/
    async function SchoolYear_GetObjLst_localStorage() {
        ...
        try {
            ...
            const strInfo = (0, clsString_js_2.Format)("[localStorage]Key:[{0}]的缓存已经建立，对象列表数：{1}!", strKey, arrSchoolYearExObjLst.length);
            console.log(strInfo);
            return arrSchoolYearExObjLst;
        }
        catch (e) {
        ...
        try {
            ...
            if (data.errorId == 0) {
                ...
                if (returnObjLst == null) {
                    const strNullInfo = (0, clsString_js_2.Format)("获取数据为null, 请注意!(in {0}.{1})", exports.schoolYear_ConstructorName, strThisFuncName);
    ...
    **/
    async function SchoolYear_GetObjLst_sessionStorage() {
        ...
        try {
            ...
            const strInfo = (0, clsString_js_2.Format)("[sessionStorage]Key:[{0}]的缓存已经建立，对象列表数：{1}!", strKey, arrSchoolYearExObjLst.length);
            console.log(strInfo);
            return arrSchoolYearExObjLst;
        }
        catch (e) {
                ...
                switch (strType) {
                    case "string":
                        ...
                        if (strComparisonOp == "=") {
                            ...
                            arrSchoolYear_Sel = arrSchoolYear_Sel.filter(x => x.GetFldValue(strKey).toString().length > Number(strValue.toString()));
                        }
    ...
    **/
    async function SchoolYear_DelRecordAsync(strSchoolYear) {
        ...
        strUrl = (0, clsString_js_2.Format)("{0}/?Id={1}", strUrl, strSchoolYear);
        try {
    ...
    **/
    async function SchoolYear_UpdateRecordAsync(objSchoolYearEN) {
        ...
        if (objSchoolYearEN.sf_UpdFldSetStr === undefined || objSchoolYearEN.sf_UpdFldSetStr === null || objSchoolYearEN.sf_UpdFldSetStr === "") {
            const strMsg = (0, clsString_js_2.Format)("对象(关键字: {0})的【修改字段集】为空，不能修改!", objSchoolYearEN.schoolYear);
            throw strMsg;
        }
        const strUrl = (0, clsSysPara4WebApi_js_1.GetWebApiUrl)(exports.schoolYear_Controller, strAction);
        try {
    ...
    */
    async function SchoolYear_IsExistRecord_Cache(objSchoolYear_Cond) {
        ...
        try {
            ...
            const strMsg = (0, clsString_js_2.Format)("根据条件:[{0}]判断是否存在不成功!(in {1}.{2})", JSON.stringify(objSchoolYear_Cond), exports.schoolYear_ConstructorName, strThisFuncName);
            console.error(strMsg);
            throw new Error(strMsg);
        }
        return false;
    ...
    */
    async function SchoolYear_IsExist_Cache(strSchoolYear) {
        ...
        try {
            ...
            const strMsg = (0, clsString_js_2.Format)("根据关键字:[{0}]判断是否存在不成功!(in {1}.{2})", strSchoolYear, exports.schoolYear_ConstructorName, strThisFuncName);
            console.error(strMsg);
            alert(strMsg);
        }
        return false;
    ...
    */
    async function SchoolYear_BindDdl_SchoolYearInDiv_Cache(strDivName, strDdlName) {
        ...
        if (objDdl == null) {
            const strMsg = (0, clsString_js_2.Format)("下拉框：{0} 不存在！(In BindDdl_SchoolYearInDiv)", strDdlName);
            alert(strMsg);
            console.error(strMsg);
            throw (strMsg);
        }
    ...
    */
    function SchoolYear_CheckPropertyNew(pobjSchoolYearEN) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjSchoolYearEN.schoolYear) == false && (0, clsString_js_2.GetStrLen)(pobjSchoolYearEN.schoolYear) > 10) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjSchoolYearEN.schoolYearName) == false && (0, clsString_js_2.GetStrLen)(pobjSchoolYearEN.schoolYearName) > 10) {
        ...
        //检查字段的数据类型是否正确
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjSchoolYearEN.schoolYear) == false && undefined !== pobjSchoolYearEN.schoolYear && clsString_js_1.tzDataType.isString(pobjSchoolYearEN.schoolYear) === false) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjSchoolYearEN.schoolYearName) == false && undefined !== pobjSchoolYearEN.schoolYearName && clsString_js_1.tzDataType.isString(pobjSchoolYearEN.schoolYearName) === false) {
        ...
        if (null != pobjSchoolYearEN.isCurrentSchoolYear && undefined !== pobjSchoolYearEN.isCurrentSchoolYear && clsString_js_1.tzDataType.isBoolean(pobjSchoolYearEN.isCurrentSchoolYear) === false) {
        ...
        if (null != pobjSchoolYearEN.isVisible && undefined !== pobjSchoolYearEN.isVisible && clsString_js_1.tzDataType.isBoolean(pobjSchoolYearEN.isVisible) === false) {
    ...
    */
    function SchoolYear_CheckProperty4Update(pobjSchoolYearEN) {
        ...
        //检查主键是否为Null或者空！
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjSchoolYearEN.schoolYear) === true
    ...
    */
    function SchoolYear_GetCombineCondition(objSchoolYear_Cond) {
        ...
        if (Object.prototype.hasOwnProperty.call(objSchoolYear_Cond.dicFldComparisonOp, clsSchoolYearEN_js_1.clsSchoolYearEN.con_SchoolYear) == true) {
            ...
            strWhereCond += (0, clsString_js_2.Format)(" And {0} {2} '{1}'", clsSchoolYearEN_js_1.clsSchoolYearEN.con_SchoolYear, objSchoolYear_Cond.schoolYear, strComparisonOp_SchoolYear);
        }
        if (Object.prototype.hasOwnProperty.call(objSchoolYear_Cond.dicFldComparisonOp, clsSchoolYearEN_js_1.clsSchoolYearEN.con_SchoolYearName) == true) {
            ...
            strWhereCond += (0, clsString_js_2.Format)(" And {0} {2} '{1}'", clsSchoolYearEN_js_1.clsSchoolYearEN.con_SchoolYearName, objSchoolYear_Cond.schoolYearName, strComparisonOp_SchoolYearName);
        }
        if (Object.prototype.hasOwnProperty.call(objSchoolYear_Cond.dicFldComparisonOp, clsSchoolYearEN_js_1.clsSchoolYearEN.con_IsCurrentSchoolYear) == true) {
            if (objSchoolYear_Cond.isCurrentSchoolYear == true) {
                strWhereCond += (0, clsString_js_2.Format)(" And {0} = '1'", clsSchoolYearEN_js_1.clsSchoolYearEN.con_IsCurrentSchoolYear);
            }
            else {
                strWhereCond += (0, clsString_js_2.Format)(" And {0} = '0'", clsSchoolYearEN_js_1.clsSchoolYearEN.con_IsCurrentSchoolYear);
            }
        }
        if (Object.prototype.hasOwnProperty.call(objSchoolYear_Cond.dicFldComparisonOp, clsSchoolYearEN_js_1.clsSchoolYearEN.con_IsVisible) == true) {
            if (objSchoolYear_Cond.isVisible == true) {
                strWhereCond += (0, clsString_js_2.Format)(" And {0} = '1'", clsSchoolYearEN_js_1.clsSchoolYearEN.con_IsVisible);
            }
            else {
                strWhereCond += (0, clsString_js_2.Format)(" And {0} = '0'", clsSchoolYearEN_js_1.clsSchoolYearEN.con_IsVisible);
            }
        }
        return strWhereCond;
    }
