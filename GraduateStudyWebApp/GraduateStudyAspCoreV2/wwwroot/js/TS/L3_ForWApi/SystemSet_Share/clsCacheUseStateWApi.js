...
(function (factory) {
    ...
    **/
    async function CacheUseState_GetObjBymIdAsync(lngmId) {
        ...
        if (lngmId == 0) {
            const strMsg = (0, clsString_js_2.Format)("参数:[lngmId]不能为空！(In clsCacheUseStateWApi.GetObjBymIdAsync)");
        ...
        try {
            ...
            if (error.statusText == "error") {
                const strInfo = (0, clsString_js_2.Format)("网络错误！访问地址:{0}不成功！(in {1}.{2})", strUrl, exports.cacheUseState_ConstructorName, strThisFuncName);
                console.error(strInfo);
                throw (strInfo);
            }
            else if (error.statusText == "Not Found") {
                const strInfo = (0, clsString_js_2.Format)("网络错误！访问地址:{0}可能不存在！(in {1}.{2})", strUrl, exports.cacheUseState_ConstructorName, strThisFuncName);
                console.error(strInfo);
                throw (strInfo);
            }
            else {
    ...
    */
    async function CacheUseState_GetObjBymId_Cache(lngmId, strUserId, bolTryAsyncOnce = true) {
        ...
        if (lngmId == 0) {
            const strMsg = (0, clsString_js_2.Format)("参数:[lngmId]不能为空！(In clsCacheUseStateWApi.GetObjBymId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        ...
        try {
            ...
            if (arrCacheUseState_Sel.length > 0) {
                ...
                if (bolTryAsyncOnce == true) {
                    const objCacheUseState_Const = await CacheUseState_GetObjBymIdAsync(lngmId);
                    if (objCacheUseState_Const != null) {
                        CacheUseState_ReFreshThisCache(strUserId);
                        return objCacheUseState_Const;
                    }
                }
                return null;
            }
    ...
    */
    async function CacheUseState_GetObjBymId_localStorage(lngmId) {
        ...
        if (lngmId == 0) {
            const strMsg = (0, clsString_js_2.Format)("参数:[lngmId]不能为空！(In clsCacheUseStateWApi.GetObjBymId_localStorage)");
            console.error(strMsg);
            throw (strMsg);
        }
        const strKey = (0, clsString_js_2.Format)("{0}_{1}", clsCacheUseStateEN_js_1.clsCacheUseStateEN._CurrTabName, lngmId);
        if (strKey == "") {
    ...
    */
    async function CacheUseState_UpdateObjInLst_Cache(objCacheUseState, strUserId) {
        ...
        try {
            const arrCacheUseStateObjLst_Cache = await CacheUseState_GetObjLst_Cache(strUserId);
            const obj = arrCacheUseStateObjLst_Cache.find(x => x.userId == objCacheUseState.userId && x.cacheModeId == objCacheUseState.cacheModeId && x.cacheKey == objCacheUseState.cacheKey);
            if (obj != null) {
    ...
    */
    async function CacheUseState_GetNameBymId_Cache(lngmId, strUserId) {
        ...
        if (lngmId == 0) {
            const strMsg = (0, clsString_js_2.Format)("参数:[lngmId]不能为空！(In clsCacheUseStateWApi.GetNameBymId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        const arrCacheUseStateObjLst_Cache = await CacheUseState_GetObjLst_Cache(strUserId);
    ...
    */
    async function CacheUseState_func(strInFldName, strOutFldName, strInValue, strUserId_C) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(strUserId_C) == true) {
            const strMsg = (0, clsString_js_2.Format)("参数:[strUserId_C]不能为空！(In clsCacheUseStateWApi.func)");
            console.error(strMsg);
            throw (strMsg);
        }
        ...
        if (clsCacheUseStateEN_js_1.clsCacheUseStateEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_2.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strOutFldName, clsCacheUseStateEN_js_1.clsCacheUseStateEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
        ...
        if (objCacheUseState == null)
            return "";
        if (objCacheUseState.GetFldValue(strOutFldName) == null)
            return "";
    ...
    function CacheUseState_SortFunByKey(strKey, AscOrDesc) {
        ...
        if (AscOrDesc == "Asc" || AscOrDesc == "") {
            switch (strKey) {
                ...
                case clsCacheUseStateEN_js_1.clsCacheUseStateEN.con_UseDate:
                    return (a, b) => {
                        if (a.useDate == null)
                            return -1;
                        if (b.useDate == null)
    ...
    */
     ...
     * @returns 返回一个关键字值列表
    */
    async function CacheUseState_funcKey(strInFldName, strInValue, strComparisonOp, strUserId_C) {
        //const strThisFuncName = "funcKey";
        if ((0, clsString_js_2.IsNullOrEmpty)(strUserId_C) == true) {
            const strMsg = (0, clsString_js_2.Format)("参数:[strUserId_C]不能为空！(In clsCacheUseStateWApi.funcKey)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strInFldName == clsCacheUseStateEN_js_1.clsCacheUseStateEN.con_mId) {
        ...
        const arrCacheUseState = await CacheUseState_GetObjLst_Cache(strUserId_C);
        if (arrCacheUseState == null)
            return [];
        let arrCacheUseState_Sel = arrCacheUseState;
        const strType = typeof (strInValue);
        switch (strType) {
            case "string":
                switch (strComparisonOp) {
                    case enumComparisonOp_js_1.enumComparisonOp.Equal_01: // " = "
                        arrCacheUseState_Sel = arrCacheUseState_Sel.filter(x => x.GetFldValue(strInFldName).toString() == strInValue.toString());
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.Like_03:
                        arrCacheUseState_Sel = arrCacheUseState_Sel.filter(x => x.GetFldValue(strInFldName).toString().indexOf(strInValue.toString()) != -1);
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.In_04:
                        const arrValues = strInValue.split(',');
                        arrCacheUseState_Sel = arrCacheUseState_Sel.filter(x => arrValues.indexOf(x.GetFldValue(strInFldName).toString()) != -1);
                        break;
                }
                break;
            case "boolean":
                ...
                if (strComparisonOp == enumComparisonOp_js_1.enumComparisonOp.Equal_01) {
                    arrCacheUseState_Sel = arrCacheUseState_Sel.filter(x => x.GetFldValue(strInFldName) == strInValue);
                }
                break;
            case "number":
                if (Number(strInValue) == 0)
        ...
        if (arrCacheUseState_Sel.length == 0)
            return [];
        return arrCacheUseState_Sel.map(x => x.mId);
    }
    exports.CacheUseState_funcKey = CacheUseState_funcKey;
    /**
    * 根据条件获取满足条件的第一条记录
    ...
    **/
    async function CacheUseState_GetObjLst_ClientCache(strUserId) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(clsCacheUseStateEN_js_1.clsCacheUseStateEN.WhereFormat) == false) {
            strWhereCond = (0, clsString_js_2.Format)(clsCacheUseStateEN_js_1.clsCacheUseStateEN.WhereFormat, strUserId);
        }
        else {
            strWhereCond = (0, clsString_js_2.Format)("UserId='{0}'", strUserId);
        }
        const strKey = (0, clsString_js_2.Format)("{0}_{1}", clsCacheUseStateEN_js_1.clsCacheUseStateEN._CurrTabName, strUserId);
        if ((0, clsString_js_2.IsNullOrEmpty)(clsCacheUseStateEN_js_1.clsCacheUseStateEN.CacheAddiCondition) == false) {
            strWhereCond += (0, clsString_js_2.Format)(" and {0}", clsCacheUseStateEN_js_1.clsCacheUseStateEN.CacheAddiCondition);
        }
        if (strKey == "") {
            console.error("关键字为空！不正确");
            throw new Error("关键字为空！不正确");
        ...
        if (CacheHelper_js_1.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrCacheUseStateExObjLst_Cache = CacheHelper_js_1.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_1.CacheHelper.Add(strKey, arrCacheUseStateExObjLst);
            const strInfo = (0, clsString_js_2.Format)("[ClientCache]Key:[{0}]的缓存已经建立，对象列表数：{1}!", strKey, arrCacheUseStateExObjLst.length);
            console.log(strInfo);
    ...
    **/
    async function CacheUseState_GetObjLst_localStorage(strUserId) {
        ...
        try {
            ...
            const strInfo = (0, clsString_js_2.Format)("[localStorage]Key:[{0}]的缓存已经建立，对象列表数：{1}!", strKey, arrCacheUseStateExObjLst.length);
            console.log(strInfo);
            return arrCacheUseStateExObjLst;
        }
        catch (e) {
        ...
        try {
            ...
            if (data.errorId == 0) {
                ...
                if (returnObjLst == null) {
                    const strNullInfo = (0, clsString_js_2.Format)("获取数据为null, 请注意!(in {0}.{1})", exports.cacheUseState_ConstructorName, strThisFuncName);
    ...
    **/
    async function CacheUseState_GetObjLst_sessionStorage(strUserId) {
        ...
        try {
            ...
            const strInfo = (0, clsString_js_2.Format)("[sessionStorage]Key:[{0}]的缓存已经建立，对象列表数：{1}!", strKey, arrCacheUseStateExObjLst.length);
            console.log(strInfo);
            return arrCacheUseStateExObjLst;
        }
        catch (e) {
    ...
    **/
    async function CacheUseState_GetObjLst_Cache(strUserId) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(strUserId) == true) {
            const strMsg = (0, clsString_js_2.Format)("缓存分类变量:[UserId]不能为空！(in clsCacheUseStateWApi.CacheUseState_GetObjLst_Cache() )");
            console.error(strMsg);
            throw (strMsg);
        }
                ...
                switch (strType) {
                    case "string":
                        ...
                        if (strComparisonOp == "=") {
                            ...
                            arrCacheUseState_Sel = arrCacheUseState_Sel.filter(x => x.GetFldValue(strKey).toString().length > Number(strValue.toString()));
                        }
    ...
    **/
    async function CacheUseState_UpdateRecordAsync(objCacheUseStateEN) {
        ...
        if (objCacheUseStateEN.sf_UpdFldSetStr === undefined || objCacheUseStateEN.sf_UpdFldSetStr === null || objCacheUseStateEN.sf_UpdFldSetStr === "") {
            const strMsg = (0, clsString_js_2.Format)("对象(关键字: {0})的【修改字段集】为空，不能修改!", objCacheUseStateEN.mId);
            throw strMsg;
        }
        const strUrl = (0, clsSysPara4WebApi_js_1.GetWebApiUrl)(exports.cacheUseState_Controller, strAction);
        try {
    ...
    */
    async function CacheUseState_IsExistRecord_Cache(objCacheUseState_Cond, strUserId) {
        ...
        try {
            ...
            const strMsg = (0, clsString_js_2.Format)("根据条件:[{0}]判断是否存在不成功!(in {1}.{2})", JSON.stringify(objCacheUseState_Cond), exports.cacheUseState_ConstructorName, strThisFuncName);
            console.error(strMsg);
            throw new Error(strMsg);
        }
        return false;
    ...
    */
    async function CacheUseState_IsExist_Cache(lngmId, strUserId) {
        ...
        try {
            ...
            const strMsg = (0, clsString_js_2.Format)("根据关键字:[{0}]判断是否存在不成功!(in {1}.{2})", lngmId, exports.cacheUseState_ConstructorName, strThisFuncName);
            console.error(strMsg);
            alert(strMsg);
        }
        return false;
    ...
    **/
    function CacheUseState_ReFreshCache(strUserId) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(strUserId) == true) {
            const strMsg = (0, clsString_js_2.Format)("参数:[strUserId]不能为空！(In clsCacheUseStateWApi.clsCacheUseStateWApi.ReFreshCache)");
            console.error(strMsg);
            throw (strMsg);
        }
    ...
    */
    async function CacheUseState__Cache(strDivName, strDdlName, strUserId) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(strUserId) == true) {
            const strMsg = (0, clsString_js_2.Format)("参数:[strUserId]不能为空！(In clsCacheUseStateWApi.)");
            console.error(strMsg);
            throw (strMsg);
        }
    ...
    */
    function CacheUseState_CheckPropertyNew(pobjCacheUseStateEN) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.userId) === true) {
            throw new Error("(errid:Watl000058)字段[用户ID]不能为空(In 缓存使用状态)!(clsCacheUseStateBL:CheckPropertyNew)");
        }
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.cacheModeId) === true) {
            throw new Error("(errid:Watl000058)字段[缓存方式Id]不能为空(In 缓存使用状态)!(clsCacheUseStateBL:CheckPropertyNew)");
        }
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.cacheKey) === true) {
            throw new Error("(errid:Watl000058)字段[缓存关键字]不能为空(In 缓存使用状态)!(clsCacheUseStateBL:CheckPropertyNew)");
        }
        //检查字段长度， 若字符型字段长度超出规定的长度，即非法！
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.userId) == false && (0, clsString_js_2.GetStrLen)(pobjCacheUseStateEN.userId) > 18) {
            throw new Error("(errid:Watl000059)字段[用户ID(userId)]的长度不能超过18(In 缓存使用状态(CacheUseState))!值:$(pobjCacheUseStateEN.userId)(clsCacheUseStateBL:CheckPropertyNew)");
        }
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.cacheModeId) == false && (0, clsString_js_2.GetStrLen)(pobjCacheUseStateEN.cacheModeId) > 2) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.cacheKey) == false && (0, clsString_js_2.GetStrLen)(pobjCacheUseStateEN.cacheKey) > 50) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.useDate) == false && (0, clsString_js_2.GetStrLen)(pobjCacheUseStateEN.useDate) > 50) {
            throw new Error("(errid:Watl000059)字段[使用日期(useDate)]的长度不能超过50(In 缓存使用状态(CacheUseState))!值:$(pobjCacheUseStateEN.useDate)(clsCacheUseStateBL:CheckPropertyNew)");
        }
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.memo) == false && (0, clsString_js_2.GetStrLen)(pobjCacheUseStateEN.memo) > 1000) {
        ...
        //检查字段的数据类型是否正确
        if (null != pobjCacheUseStateEN.mId && undefined !== pobjCacheUseStateEN.mId && clsString_js_1.tzDataType.isNumber(pobjCacheUseStateEN.mId) === false) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.userId) == false && undefined !== pobjCacheUseStateEN.userId && clsString_js_1.tzDataType.isString(pobjCacheUseStateEN.userId) === false) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.cacheModeId) == false && undefined !== pobjCacheUseStateEN.cacheModeId && clsString_js_1.tzDataType.isString(pobjCacheUseStateEN.cacheModeId) === false) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.cacheKey) == false && undefined !== pobjCacheUseStateEN.cacheKey && clsString_js_1.tzDataType.isString(pobjCacheUseStateEN.cacheKey) === false) {
        ...
        if (null != pobjCacheUseStateEN.cacheSize && undefined !== pobjCacheUseStateEN.cacheSize && clsString_js_1.tzDataType.isNumber(pobjCacheUseStateEN.cacheSize) === false) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.useDate) == false && undefined !== pobjCacheUseStateEN.useDate && clsString_js_1.tzDataType.isString(pobjCacheUseStateEN.useDate) === false) {
            throw new Error("(errid:Watl000060)字段[使用日期(useDate)]的值:[$(pobjCacheUseStateEN.useDate)], 非法，应该为字符型(In 缓存使用状态(CacheUseState))!(clsCacheUseStateBL:CheckPropertyNew)");
        }
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.memo) == false && undefined !== pobjCacheUseStateEN.memo && clsString_js_1.tzDataType.isString(pobjCacheUseStateEN.memo) === false) {
    ...
    */
    function CacheUseState_CheckProperty4Update(pobjCacheUseStateEN) {
        //检查字段长度， 若字符型字段长度超出规定的长度，即非法！
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.userId) == false && (0, clsString_js_2.GetStrLen)(pobjCacheUseStateEN.userId) > 18) {
            throw new Error("(errid:Watl000062)字段[用户ID(userId)]的长度不能超过18(In 缓存使用状态(CacheUseState))!值:$(pobjCacheUseStateEN.userId)(clsCacheUseStateBL:CheckProperty4Update)");
        }
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.cacheModeId) == false && (0, clsString_js_2.GetStrLen)(pobjCacheUseStateEN.cacheModeId) > 2) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.useDate) == false && (0, clsString_js_2.GetStrLen)(pobjCacheUseStateEN.useDate) > 50) {
            throw new Error("(errid:Watl000062)字段[使用日期(useDate)]的长度不能超过50(In 缓存使用状态(CacheUseState))!值:$(pobjCacheUseStateEN.useDate)(clsCacheUseStateBL:CheckProperty4Update)");
        }
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.memo) == false && (0, clsString_js_2.GetStrLen)(pobjCacheUseStateEN.memo) > 1000) {
        ...
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.useDate) == false && undefined !== pobjCacheUseStateEN.useDate && clsString_js_1.tzDataType.isString(pobjCacheUseStateEN.useDate) === false) {
            throw new Error("(errid:Watl000063)字段[使用日期(useDate)]的值:[$(pobjCacheUseStateEN.useDate)], 非法，应该为字符型(In 缓存使用状态(CacheUseState))!(clsCacheUseStateBL:CheckProperty4Update)");
        }
        if ((0, clsString_js_2.IsNullOrEmpty)(pobjCacheUseStateEN.memo) == false && undefined !== pobjCacheUseStateEN.memo && clsString_js_1.tzDataType.isString(pobjCacheUseStateEN.memo) === false) {
    ...
    */
    function CacheUseState_GetCombineCondition(objCacheUseState_Cond) {
        ...
        if (Object.prototype.hasOwnProperty.call(objCacheUseState_Cond.dicFldComparisonOp, clsCacheUseStateEN_js_1.clsCacheUseStateEN.con_mId) == true) {
            ...
            strWhereCond += (0, clsString_js_2.Format)(" And {0} {2} {1}", clsCacheUseStateEN_js_1.clsCacheUseStateEN.con_mId, objCacheUseState_Cond.mId, strComparisonOp_mId);
        }
        if (Object.prototype.hasOwnProperty.call(objCacheUseState_Cond.dicFldComparisonOp, clsCacheUseStateEN_js_1.clsCacheUseStateEN.con_UserId) == true) {
            ...
            strWhereCond += (0, clsString_js_2.Format)(" And {0} {2} '{1}'", clsCacheUseStateEN_js_1.clsCacheUseStateEN.con_UserId, objCacheUseState_Cond.userId, strComparisonOp_UserId);
        }
        if (Object.prototype.hasOwnProperty.call(objCacheUseState_Cond.dicFldComparisonOp, clsCacheUseStateEN_js_1.clsCacheUseStateEN.con_CacheModeId) == true) {
            ...
            strWhereCond += (0, clsString_js_2.Format)(" And {0} {2} '{1}'", clsCacheUseStateEN_js_1.clsCacheUseStateEN.con_CacheModeId, objCacheUseState_Cond.cacheModeId, strComparisonOp_CacheModeId);
        }
        if (Object.prototype.hasOwnProperty.call(objCacheUseState_Cond.dicFldComparisonOp, clsCacheUseStateEN_js_1.clsCacheUseStateEN.con_CacheKey) == true) {
            ...
            strWhereCond += (0, clsString_js_2.Format)(" And {0} {2} '{1}'", clsCacheUseStateEN_js_1.clsCacheUseStateEN.con_CacheKey, objCacheUseState_Cond.cacheKey, strComparisonOp_CacheKey);
        }
        if (Object.prototype.hasOwnProperty.call(objCacheUseState_Cond.dicFldComparisonOp, clsCacheUseStateEN_js_1.clsCacheUseStateEN.con_CacheSize) == true) {
            ...
            strWhereCond += (0, clsString_js_2.Format)(" And {0} {2} {1}", clsCacheUseStateEN_js_1.clsCacheUseStateEN.con_CacheSize, objCacheUseState_Cond.cacheSize, strComparisonOp_CacheSize);
        }
        if (Object.prototype.hasOwnProperty.call(objCacheUseState_Cond.dicFldComparisonOp, clsCacheUseStateEN_js_1.clsCacheUseStateEN.con_UseDate) == true) {
            const strComparisonOp_UseDate = objCacheUseState_Cond.dicFldComparisonOp[clsCacheUseStateEN_js_1.clsCacheUseStateEN.con_UseDate];
            strWhereCond += (0, clsString_js_2.Format)(" And {0} {2} '{1}'", clsCacheUseStateEN_js_1.clsCacheUseStateEN.con_UseDate, objCacheUseState_Cond.useDate, strComparisonOp_UseDate);
        }
        if (Object.prototype.hasOwnProperty.call(objCacheUseState_Cond.dicFldComparisonOp, clsCacheUseStateEN_js_1.clsCacheUseStateEN.con_Memo) == true) {
            ...
            strWhereCond += (0, clsString_js_2.Format)(" And {0} {2} '{1}'", clsCacheUseStateEN_js_1.clsCacheUseStateEN.con_Memo, objCacheUseState_Cond.memo, strComparisonOp_Memo);
        }
        return strWhereCond;
    }
    ...
    * @returns 条件串(strWhereCond)
    **/
    function CacheUseState_GetUniCondStr_UserId_CacheModeId_CacheKey(objCacheUseStateEN) {
        let strWhereCond = " 1 = 1 ";
        strWhereCond += (0, clsString_js_2.Format)(" and UserId = '{0}'", objCacheUseStateEN.userId);
        strWhereCond += (0, clsString_js_2.Format)(" and CacheModeId = '{0}'", objCacheUseStateEN.cacheModeId);
        strWhereCond += (0, clsString_js_2.Format)(" and CacheKey = '{0}'", objCacheUseStateEN.cacheKey);
        return strWhereCond;
    }
    exports.CacheUseState_GetUniCondStr_UserId_CacheModeId_CacheKey = CacheUseState_GetUniCondStr_UserId_CacheModeId_CacheKey;
    /**
    *获取唯一性条件串(Uniqueness)--CacheUseState(缓存使用状态),根据唯一约束条件来生成
    ...
    * @returns 条件串(strWhereCond)
    **/
    function CacheUseState_GetUniCondStr4Update_UserId_CacheModeId_CacheKey(objCacheUseStateEN) {
        let strWhereCond = " 1 = 1 ";
        strWhereCond += (0, clsString_js_2.Format)(" and mId <> '{0}'", objCacheUseStateEN.mId);
        strWhereCond += (0, clsString_js_2.Format)(" and UserId = '{0}'", objCacheUseStateEN.userId);
        strWhereCond += (0, clsString_js_2.Format)(" and CacheModeId = '{0}'", objCacheUseStateEN.cacheModeId);
        strWhereCond += (0, clsString_js_2.Format)(" and CacheKey = '{0}'", objCacheUseStateEN.cacheKey);
        return strWhereCond;
    }
    exports.CacheUseState_GetUniCondStr4Update_UserId_CacheModeId_CacheKey = CacheUseState_GetUniCondStr4Update_UserId_CacheModeId_CacheKey;
    /**
     * 把同一个类的对象,复制到另一个对象
