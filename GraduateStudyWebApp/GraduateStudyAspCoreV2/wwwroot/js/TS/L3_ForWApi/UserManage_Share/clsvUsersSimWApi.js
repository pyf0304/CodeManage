...
(function (factory) {
    ...
    **/
    async function vUsersSim_GetObjByUserIdAsync(strUserId) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strUserId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strUserId]不能为空！(In clsvUsersSimWApi.GetObjByUserIdAsync)");
     ...
     * @returns 对象
    */
    async function vUsersSim_GetObjByUserId_Cache(strUserId, strCmPrjId, bolTryAsyncOnce = true) {
        const strThisFuncName = "GetObjByUserId_Cache";
        if ((0, clsString_js_1.IsNullOrEmpty)(strUserId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strUserId]不能为空！(In clsvUsersSimWApi.GetObjByUserId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        const arrvUsersSimObjLst_Cache = await vUsersSim_GetObjLst_Cache(strCmPrjId);
        try {
    ...
    */
    async function vUsersSim_GetObjByUserId_localStorage(strUserId) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strUserId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strUserId]不能为空！(In clsvUsersSimWApi.GetObjByUserId_localStorage)");
            console.error(strMsg);
            throw (strMsg);
        }
     ...
     * @returns 对象
    */
    async function vUsersSim_GetNameByUserId_Cache(strUserId, strCmPrjId) {
        const strThisFuncName = "GetNameByUserId_Cache";
        if ((0, clsString_js_1.IsNullOrEmpty)(strUserId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strUserId]不能为空！(In clsvUsersSimWApi.GetNameByUserId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        const arrvUsersSimObjLst_Cache = await vUsersSim_GetObjLst_Cache(strCmPrjId);
     ...
     * @returns 返回一个输出字段值
    */
    async function vUsersSim_func(strInFldName, strOutFldName, strInValue, strCmPrjId_C) {
        //const strThisFuncName = "func";
        if ((0, clsString_js_1.IsNullOrEmpty)(strCmPrjId_C) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strCmPrjId_C]不能为空！(In clsvUsersSimWApi.func)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strCmPrjId_C.length != 6) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strCmPrjId_C]的长度:[{0}]不正确！(clsvUsersSimWApi.func)", strCmPrjId_C.length);
            console.error(strMsg);
            throw (strMsg);
        }
        ...
        if (clsvUsersSimEN_js_1.clsvUsersSimEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_1.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strOutFldName, clsvUsersSimEN_js_1.clsvUsersSimEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
        ...
        const objvUsersSim = await vUsersSim_GetObjByUserId_Cache(strUserId, strCmPrjId_C);
        if (objvUsersSim == null)
            return "";
        if (objvUsersSim.GetFldValue(strOutFldName) == null)
            return "";
    ...
    */
     ...
     * @returns 返回一个关键字值列表
    */
    async function vUsersSim_funcKey(strInFldName, strInValue, strComparisonOp, strCmPrjId_C) {
        //const strThisFuncName = "funcKey";
        if ((0, clsString_js_1.IsNullOrEmpty)(strCmPrjId_C) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strCmPrjId_C]不能为空！(In clsvUsersSimWApi.funcKey)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strCmPrjId_C.length != 6) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strCmPrjId_C]的长度:[{0}]不正确！(clsvUsersSimWApi.funcKey)", strCmPrjId_C.length);
            console.error(strMsg);
            throw (strMsg);
        }
        if (strInFldName == clsvUsersSimEN_js_1.clsvUsersSimEN.con_UserId) {
        ...
        const arrvUsersSim = await vUsersSim_GetObjLst_Cache(strCmPrjId_C);
        if (arrvUsersSim == null)
            return [];
        let arrvUsersSim_Sel = arrvUsersSim;
        const strType = typeof (strInValue);
        switch (strType) {
            case "string":
                switch (strComparisonOp) {
                    case enumComparisonOp_js_1.enumComparisonOp.Equal_01: // " = "
                        arrvUsersSim_Sel = arrvUsersSim_Sel.filter(x => x.GetFldValue(strInFldName).toString() == strInValue.toString());
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.Like_03:
                        arrvUsersSim_Sel = arrvUsersSim_Sel.filter(x => x.GetFldValue(strInFldName).toString().indexOf(strInValue.toString()) != -1);
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.In_04:
                        const arrValues = strInValue.split(',');
                        arrvUsersSim_Sel = arrvUsersSim_Sel.filter(x => arrValues.indexOf(x.GetFldValue(strInFldName).toString()) != -1);
                        break;
                }
                break;
            case "boolean":
                ...
                if (strComparisonOp == enumComparisonOp_js_1.enumComparisonOp.Equal_01) {
                    arrvUsersSim_Sel = arrvUsersSim_Sel.filter(x => x.GetFldValue(strInFldName) == strInValue);
                }
                break;
            case "number":
        ...
        if (arrvUsersSim_Sel.length == 0)
            return [];
        return arrvUsersSim_Sel.map(x => x.userId);
    }
    exports.vUsersSim_funcKey = vUsersSim_funcKey;
    /**
    * 根据条件获取满足条件的第一条记录
    ...
    **/
    async function vUsersSim_GetObjLst_ClientCache(strCmPrjId) {
        const strThisFuncName = "GetObjLst_ClientCache";
        //初始化列表缓存
        let strWhereCond = "1=1";
        if ((0, clsString_js_1.IsNullOrEmpty)(clsvUsersSimEN_js_1.clsvUsersSimEN.WhereFormat) == false) {
            strWhereCond = (0, clsString_js_1.Format)(clsvUsersSimEN_js_1.clsvUsersSimEN.WhereFormat, strCmPrjId);
        }
        else {
            strWhereCond = (0, clsString_js_1.Format)("CmPrjId='{0}'", strCmPrjId);
        }
        const strKey = (0, clsString_js_1.Format)("{0}_{1}", clsvUsersSimEN_js_1.clsvUsersSimEN._CurrTabName, strCmPrjId);
        ...
        if (CacheHelper_js_1.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrvUsersSimExObjLst_Cache = CacheHelper_js_1.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_1.CacheHelper.Add(strKey, arrvUsersSimExObjLst);
    ...
    * @returns 从本地缓存中获取的对象列表
    **/
    async function vUsersSim_GetObjLst_localStorage(strCmPrjId) {
    ...
    * @returns 从本地缓存中获取的对象列表
    **/
    async function vUsersSim_GetObjLst_localStorage_PureCache(strCmPrjId) {
    ...
    * @returns 从本地缓存中获取的对象列表
    **/
    async function vUsersSim_GetObjLst_sessionStorage(strCmPrjId) {
    ...
    * @returns 从本地缓存中获取的对象列表
    **/
    async function vUsersSim_GetObjLst_sessionStorage_PureCache(strCmPrjId) {
    ...
    * @returns 从本地缓存中获取的对象列表
    **/
    async function vUsersSim_GetObjLst_Cache(strCmPrjId) {
        //const strThisFuncName = "GetObjLst_Cache";
        if ((0, clsString_js_1.IsNullOrEmpty)(strCmPrjId) == true) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[CmPrjId]不能为空！(in clsvUsersSimWApi.vUsersSim_GetObjLst_Cache() )");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strCmPrjId.length != 6) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strCmPrjId]的长度:[{0}]不正确！(in clsvUsersSimWApi.vUsersSim_GetObjLst_Cache() )", strCmPrjId.length);
            console.error(strMsg);
            throw (strMsg);
        }
        ...
        switch (clsvUsersSimEN_js_1.clsvUsersSimEN.CacheModeId) {
            case "04": //sessionStorage
                arrvUsersSimObjLst_Cache = await vUsersSim_GetObjLst_sessionStorage(strCmPrjId);
                break;
            case "03": //localStorage
                arrvUsersSimObjLst_Cache = await vUsersSim_GetObjLst_localStorage(strCmPrjId);
                break;
            case "02": //ClientCache
                arrvUsersSimObjLst_Cache = await vUsersSim_GetObjLst_ClientCache(strCmPrjId);
                break;
            default:
                arrvUsersSimObjLst_Cache = await vUsersSim_GetObjLst_ClientCache(strCmPrjId);
                break;
    ...
    * @returns 从本地缓存中获取的对象列表
    **/
    async function vUsersSim_GetObjLst_PureCache(strCmPrjId) {
        //const strThisFuncName = "GetObjLst_PureCache";
        let arrvUsersSimObjLst_Cache;
        switch (clsvUsersSimEN_js_1.clsvUsersSimEN.CacheModeId) {
            case "04": //sessionStorage
                arrvUsersSimObjLst_Cache = await vUsersSim_GetObjLst_sessionStorage_PureCache(strCmPrjId);
                break;
            case "03": //localStorage
                arrvUsersSimObjLst_Cache = await vUsersSim_GetObjLst_localStorage_PureCache(strCmPrjId);
                break;
            case "02": //ClientCache
     ...
     * @returns 对象列表子集
    */
    async function vUsersSim_GetSubObjLst_Cache(objvUsersSim_Cond, strCmPrjId) {
    ...
    **/
     ...
     * @returns 对象列表
    */
    async function vUsersSim_GetObjLstByUserIdLst_Cache(arrUserIdLst, strCmPrjId) {
    ...
    */
     ...
     * @returns 对象列表
    */
    async function vUsersSim_GetObjLstByPager_Cache(objPagerPara, strCmPrjId) {
    ...
    **/
     ...
     * @returns 对象列表子集
    */
    async function vUsersSim_IsExistRecord_Cache(objvUsersSim_Cond, strCmPrjId) {
    ...
    */
     ...
     * @returns 对象
    */
    async function vUsersSim_IsExist_Cache(strUserId, strCmPrjId) {
    ...
    **/
     ...
     * @returns 对象列表记录数
    */
    async function vUsersSim_GetRecCountByCond_Cache(objvUsersSim_Cond, strCmPrjId) {
    ...
    **/
    function vUsersSim_ReFreshThisCache(strCmPrjId) {
    ...
    */
    async function vUsersSim_BindDdl_UserIdInDiv_Cache(strDivName, strDdlName, strCmPrjId) {
        const strThisFuncName = "BindDdl_UserIdInDiv_Cache";
        if ((0, clsString_js_1.IsNullOrEmpty)(strCmPrjId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strCmPrjId]不能为空！(In clsvUsersSimWApi.BindDdl_UserIdInDiv)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strCmPrjId.length != 6) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strCmPrjId]的长度:[{0}]不正确！(clsvUsersSimWApi.BindDdl_UserIdInDiv)", strCmPrjId.length);
            console.error(strMsg);
            throw (strMsg);
        }
        ...
        const arrObjLst_Sel = await vUsersSim_GetObjLst_Cache(strCmPrjId);
    ...
    exports.vUsersSim_BindDdl_UserIdInDiv_Cache = vUsersSim_BindDdl_UserIdInDiv_Cache;
    /**
     * 把一个对象转化为一个JSON串
     * 作者:pyf
     * 日期:2023-03-13
