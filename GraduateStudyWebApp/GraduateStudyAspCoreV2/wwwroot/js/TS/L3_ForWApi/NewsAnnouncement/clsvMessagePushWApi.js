...
(function (factory) {
    ...
    **/
    async function vMessagePush_GetObjByMessagePushIdAsync(strMessagePushId) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strMessagePushId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strMessagePushId]不能为空！(In clsvMessagePushWApi.GetObjByMessagePushIdAsync)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strMessagePushId.length != 8) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strMessagePushId]的长度:[{0}]不正确！(clsvMessagePushWApi.GetObjByMessagePushIdAsync)", strMessagePushId.length);
            console.error(strMsg);
            throw (strMsg);
        }
    ...
    */
    async function vMessagePush_GetObjByMessagePushId_Cache(strMessagePushId, bolTryAsyncOnce = true) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strMessagePushId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strMessagePushId]不能为空！(In clsvMessagePushWApi.GetObjByMessagePushId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strMessagePushId.length != 8) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strMessagePushId]的长度:[{0}]不正确！(clsvMessagePushWApi.GetObjByMessagePushId_Cache)", strMessagePushId.length);
            console.error(strMsg);
            throw (strMsg);
        }
        ...
        try {
            ...
            if (arrvMessagePush_Sel.length > 0) {
                ...
                if (bolTryAsyncOnce == true) {
                    const objvMessagePush_Const = await vMessagePush_GetObjByMessagePushIdAsync(strMessagePushId);
                    if (objvMessagePush_Const != null) {
                        vMessagePush_ReFreshThisCache();
                        return objvMessagePush_Const;
                    }
                }
                return null;
            }
    ...
    */
    async function vMessagePush_GetObjByMessagePushId_localStorage(strMessagePushId) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strMessagePushId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strMessagePushId]不能为空！(In clsvMessagePushWApi.GetObjByMessagePushId_localStorage)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strMessagePushId.length != 8) {
            const strMsg = (0, clsString_js_1.Format)("缓存分类变量:[strMessagePushId]的长度:[{0}]不正确！(clsvMessagePushWApi.GetObjByMessagePushId_localStorage)", strMessagePushId.length);
            console.error(strMsg);
            throw (strMsg);
        }
    ...
    */
    async function vMessagePush_func(strInFldName, strOutFldName, strInValue) {
        ...
        if (clsvMessagePushEN_js_1.clsvMessagePushEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_1.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strOutFldName, clsvMessagePushEN_js_1.clsvMessagePushEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
        ...
        if (objvMessagePush == null)
            return "";
        if (objvMessagePush.GetFldValue(strOutFldName) == null)
            return "";
        ...
        if (AscOrDesc == "Asc" || AscOrDesc == "") {
            switch (strKey) {
                ...
                case clsvMessagePushEN_js_1.clsvMessagePushEN.con_MessageTypeName:
                    return (a, b) => {
                        if (a.messageTypeName == null)
                            return -1;
                        if (b.messageTypeName == null)
                            return 1;
    ...
    */
     ...
     * @returns 返回一个关键字值列表
    */
    async function vMessagePush_funcKey(strInFldName, strInValue, strComparisonOp) {
        ...
        const arrvMessagePush = await vMessagePush_GetObjLst_Cache();
        if (arrvMessagePush == null)
            return [];
        let arrvMessagePush_Sel = arrvMessagePush;
        const strType = typeof (strInValue);
        switch (strType) {
            case "string":
                switch (strComparisonOp) {
                    case enumComparisonOp_js_1.enumComparisonOp.Equal_01: // " = "
                        arrvMessagePush_Sel = arrvMessagePush_Sel.filter(x => x.GetFldValue(strInFldName).toString() == strInValue.toString());
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.Like_03:
                        arrvMessagePush_Sel = arrvMessagePush_Sel.filter(x => x.GetFldValue(strInFldName).toString().indexOf(strInValue.toString()) != -1);
                        break;
                    case enumComparisonOp_js_1.enumComparisonOp.In_04:
                        const arrValues = strInValue.split(',');
                        arrvMessagePush_Sel = arrvMessagePush_Sel.filter(x => arrValues.indexOf(x.GetFldValue(strInFldName).toString()) != -1);
                        break;
                }
                break;
            case "boolean":
                ...
                if (strComparisonOp == enumComparisonOp_js_1.enumComparisonOp.Equal_01) {
                    arrvMessagePush_Sel = arrvMessagePush_Sel.filter(x => x.GetFldValue(strInFldName) == strInValue);
                }
                break;
            case "number":
        ...
        if (arrvMessagePush_Sel.length == 0)
            return [];
        return arrvMessagePush_Sel.map(x => x.messagePushId);
    }
    exports.vMessagePush_funcKey = vMessagePush_funcKey;
    /**
    * 根据条件获取满足条件的第一条记录
    ...
    **/
    async function vMessagePush_GetObjLst_ClientCache() {
        ...
        if (CacheHelper_js_1.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrvMessagePushExObjLst_Cache = CacheHelper_js_1.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_1.CacheHelper.Add(strKey, arrvMessagePushExObjLst);
                ...
                switch (strType) {
                    case "string":
                        ...
                        if (strComparisonOp == "=") {
                            ...
                            arrvMessagePush_Sel = arrvMessagePush_Sel.filter(x => x.GetFldValue(strKey).toString().length > Number(strValue.toString()));
                        }
