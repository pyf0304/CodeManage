(function (factory) {
    ...
    exports.clsRTViewpointRelaEN = void 0;
    /**
    ...
    **/
    ...
    class clsRTViewpointRelaEN extends clsGeneralTab_js_1.clsGeneralTab {
        //以下是属性变量
        /**
         * 构造函数
        ...
        */
        GetFldValue(strFldName) {
            ...
            switch (strFldName) {
                ...
                default:
                    strMsg = `字段名:[${strFldName}]在表对象:[RTViewpointRela]中不存在！`;
        ...
        SetFldValue(strFldName, strValue) {
            ...
            switch (strFldName) {
                ...
                case "sf_FldComparisonOp":
                    ...
                    break;
                default:
                    strMsg = `字段名:[${strFldName}]在表对象:[RTViewpointRela]中不存在！(in ${this.constructor.name}.${strThisFuncName})`;
                    console.error(strMsg);
                    break;
            }
        }
    ...
    exports.clsRTViewpointRelaEN = clsRTViewpointRelaEN;
    clsRTViewpointRelaEN.CacheAddiCondition = ""; //缓存附加条件，作为向后台调取数据的附加条件
    clsRTViewpointRelaEN.CacheModeId = "05"; //未知
    clsRTViewpointRelaEN.IsUseDelSign = false; //使用删除标志，记录不能删除，仅设置删除标志
    clsRTViewpointRelaEN.WhereFormat = ""; //条件格式串
    clsRTViewpointRelaEN._CurrTabName = "RTViewpointRela"; //当前表名,与该类相关的表名
    clsRTViewpointRelaEN._KeyFldName = "mId"; //当前表中的关键字名称,与该类相关的表中关键字名
    clsRTViewpointRelaEN.mintAttributeCount = 9;
    clsRTViewpointRelaEN.AttributeName = ["mId", "topicId", "viewpointId", "proposePeople", "updDate", "updUser", "memo", "classificationId", "id_CurrEduCls"];
});
