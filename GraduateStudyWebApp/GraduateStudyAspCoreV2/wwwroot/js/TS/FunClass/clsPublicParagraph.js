...
(function (factory) {
    ...
    exports.clsPublicParagraph = void 0;
    const clsGeneralTab2_js_1 = require("../PubFun/clsGeneralTab2.js");
    class clsPublicParagraph extends clsGeneralTab2_js_1.clsGeneralTab2 {
        //以下是属性变量
        /// <summary>
        /// 构造函数
        ...
        constructor() {
            super();
            this.AttributeName = new Array("TextId", "TextValue", "NewTextValue", "IsTag", "IsChange", "LocationOne", "LocationTwo");
            this.mstrTextId = ""; //顺序Id
            this.mstrTextValue = ""; //文本
            this.mstrNewTextValue = ""; //新文本
            this.mbolIsTag = false; //是否标签
            this.mbolIsChange = false; //是否改变
            this.mstrLocationOne = ""; //位置1
            this.mstrLocationTwo = ""; //位置2
        }
        //顺序Id
        get TextId() {
        ...
        static GetNewStringList(strText, strNewText) {
            //1.通过标签数据获取，分段截取的对象列表数据；
            let arrOldLst = this.SaveObjListByString(strText);
            //2.过滤标签后，赋值改变的新的对象列表
            let arrNewList = this.GetDataList(arrOldLst, strNewText);
            //定义一个接受拼接字符串变量
            var strNewTextContent = "";
            //先循环不带标签的数据；
            for (var i = 0; i < arrNewList.length; i++) {
                //通过获取的新的标签内容;
                var strTabNewContent = arrNewList[i].NewTextValue;
                var strTextId = arrNewList[i].TextId;
        ...
        static SaveObjListByString(strText) {
            //用来记录是标签开始，还是结束；
            var Ismark = false;
            ...
            for (var i = 0; i < strText.length; i++) {
                ...
                if (strText[i] == "<") {
                    ...
                    if (strTextValue != "") {
                        ...
                        var objPublicParagraph = new clsPublicParagraph();
        ...
        static GetDataList(arr, strNewTextValue) {
    ...
    exports.clsPublicParagraph = clsPublicParagraph;
    ;
});
