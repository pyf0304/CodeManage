...
(function (factory) {
    ...
    **/
    ...
    const clsCmPrjIdUserIdRelaExWApi_js_1 = require("./clsCmPrjIdUserIdRelaExWApi.js");
    ...
    * @returns 返回一个输出字段值
    */
    async function vUsersSimEx_func(strInFldName, strOutFldName, strInValue) {
        ...
        if (clsvUsersSimEN_js_1.clsvUsersSimEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_2.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strOutFldName, clsvUsersSimEN_js_1.clsvUsersSimEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
        ...
        if (objvUsersSim == null) {
            await (0, clsCmPrjIdUserIdRelaExWApi_js_1.CmPrjIdUserIdRelaEx_AddUserId)(strUserId);
            return "";
        }
    ...
    exports.vUsersSimEx_func = vUsersSimEx_func;
    /**
     * 根据关键字获取相关对象, 从缓存中获取.
     ...
     * @returns 对象
    */
    async function vUsersSimEx_GetObjByUserId_Cache(strUserId, bolTryAsyncOnce = true) {
        const strThisFuncName = "GetObjByUserId_Cache";
        if ((0, clsString_js_1.IsNullOrEmpty)(strUserId) == true) {
            const strMsg = (0, clsString_js_2.Format)("参数:[strUserId]不能为空！(In GetObjByUserId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        const arrvUsersSimObjLst_Cache = await (0, clsvUsersSimWApi_js_1.vUsersSim_GetObjLst_Cache)(clsSysPara4WebApi_js_1.clsSysPara4WebApi.cmPrjId);
        try {
            ...
            if (arrvUsersSim_Sel.length > 0) {
                ...
                if (bolTryAsyncOnce == true) {
                    const objvUsersSim_Const = await (0, clsvUsersSimWApi_js_1.vUsersSim_GetObjByUserIdAsync)(strUserId);
            ...
            const strMsg = (0, clsString_js_2.Format)("错误:[{0}]. \n根据关键字:[{1}]获取相应的对象不成功!(in {2}.{3})", e, strUserId, exports.vUsersSimEx_ConstructorName, strThisFuncName);
            console.error(strMsg);
        }
        return null;
    }
    exports.vUsersSimEx_GetObjByUserId_Cache = vUsersSimEx_GetObjByUserId_Cache;
});
