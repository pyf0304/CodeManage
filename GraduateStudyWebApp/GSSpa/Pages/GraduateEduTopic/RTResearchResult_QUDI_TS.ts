...
import { vRTResearchResult_GetObjLstAsync, vRTResearchResult_GetObjLstByPagerAsync, vRTResearchResult_GetObjLstByPager_Cache, vRTResearchResult_GetRecCountByCondAsync, vRTResearchResult_GetRecCountByCond_Cache } from "../TS/L3_ForWApi/GraduateEduTopic/clsvRTResearchResultWApi.js";
import { RTResearchResult_AddNewRecordAsync, RTResearchResult_DelRecordAsync, RTResearchResult_DelRTResearchResultsAsync, RTResearchResult_GetObjBymIdAsync, RTResearchResult_IsExistAsync, RTResearchResult_UpdateRecordAsync } from "../TS/L3_ForWApi/GraduateEduTopic/clsRTResearchResultWApi.js";
...
*/
export class RTResearchResult_QUDI_TS implements clsOperateList{
   ...
   */
    public async Page_Load() {
        ...
        try {
            ...
            const arrPaper_Cache = await Paper_GetObjLstAsync("1=1");//编辑区域
            const arrvRTResearchResult_Cache = await vRTResearchResult_GetObjLstAsync("1=1");
   ...
   */
    public async Page_Load_Cache() {
        ...
        try {
            ...
            this.RecCount = await vRTResearchResult_GetRecCountByCond_Cache(objvRTResearchResult_Cond);
            //2、显示无条件的表内容在GridView中
    ...
    public async btnQuery_Click(strListDiv: string) {
        ...
        try {
            ...
            const responseObjLst = await vRTResearchResult_GetObjLstByPagerAsync(objPagerPara).then((jsonData) => {
                var arrvRTResearchResultObjLst: Array<clsvRTResearchResultEN> = <Array<clsvRTResearchResultEN>>jsonData;
   ...
   */
    public async DetailRecord(lngmId: number) {
        ...
        this.btnCancel = "关闭";
        try {
            const responseText = await RTResearchResult_GetObjBymIdAsync(lngmId);
            var objRTResearchResultEN: clsRTResearchResultEN = <clsRTResearchResultEN>responseText;
   ...
   */
    public async SelectRecord(lngmId: number) {
        try {
            ...
            clsCommonFunc4Web.Redirect("/Index/Main_vRTResearchResult");
        }
        catch (e) {
            console.error(e);
            var strMsg: string = `根据关键字获取相应的记录的对象不成功,${e}.`;
   ...
   */
    public async btnExportExcel_Click() {
        ...
        try {
            const responseText = await vRTResearchResult_GetObjLstAsync(strWhereCond).then((jsonData) => {
                var arrvRTResearchResultObjLst: Array<clsvRTResearchResultEN> = <Array<clsvRTResearchResultEN>>jsonData;
                this.BindTab_vRTResearchResult(this.mstrListDiv, arrvRTResearchResultObjLst);
            });
        }
   ...
   */
    ...
    (AutoGCLib.WA_ViewScriptCS_TS4TypeScript:Gen_WApi_Ts_CombineConditionObj)
     <returns>条件串(strWhereCond)</returns>
   */
    public CombinevRTResearchResultConditionObj(): clsvRTResearchResultEN {
        //使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
        //例如 1 = 1 && userName = '张三'
        var strWhereCond: string = " 1 = 1 ";
        var objvRTResearchResult_Cond: clsvRTResearchResultEN = new clsvRTResearchResultEN();
        //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
        try {
            if (this.UserName_q != "") {
                strWhereCond += ` And ${clsvRTResearchResultEN.con_UserName} like '% ${this.UserName_q}%'`;
            ...
            var strMsg: string = `(errid:WiTsCs0005)在组合查询条件对象(CombineRTResearchResultConditionObj)时出错!请联系管理员!${objException}`;
            throw strMsg;
        }
   ...
   */
    public async BindGv_vRTResearchResult_Cache() {
        var strListDiv: string = this.mstrListDiv;
        var objvRTResearchResult_Cond = this.CombinevRTResearchResultConditionObj();
        ...
        try {
            ...
            var arrvRTResearchResultObjLst = await vRTResearchResult_GetObjLstByPager_Cache(objPagerPara);
        }
        catch (e) {
            console.error('catch(e)=');
            console.error(e);
        ...
        if (arrvRTResearchResultObjLst.length == 0) {
            var strMsg: string = `在BindGv_Cache过程中，根据条件对象获取的对象列表数为0！(Key=RTResearchResult)`;
            alert(strMsg);
            return;
        }
        try {
   ...
   */
    public PutDataToRTResearchResultClass(pobjRTResearchResultEN: clsRTResearchResultEN) {
   ...
   */
    public async AddNewRecordSave() {
        ...
        var objRTResearchResultEN: clsRTResearchResultEN = new clsRTResearchResultEN();
        this.PutDataToRTResearchResultClass(objRTResearchResultEN);
        try {
            const responseText2 = await RTResearchResult_AddNewRecordAsync(objRTResearchResultEN);
   ...
   */
    public async ShowData(lngmId: number) {
        ...
        var objRTResearchResultEN: clsRTResearchResultEN = new clsRTResearchResultEN();
        try {
            const responseText = await RTResearchResult_IsExistAsync(lngmId);
        ...
        try {
            const responseText = await RTResearchResult_GetObjBymIdAsync(lngmId);
            objRTResearchResultEN = <clsRTResearchResultEN>responseText;
        }
        catch (e) {
            var strMsg: string = `根据关键字获取相应的记录的对象不成功,${e}.`;
            alert(strMsg);
   ...
   */
    public async UpdateRecordSave() {
        ...
        try {
            const responseText = await RTResearchResult_UpdateRecordAsync(objRTResearchResultEN);
            var returnBool: boolean = !!responseText;
            if (returnBool == true) {
            }
            return returnBool;
   ...
   */
    public async DelMultiRecord(arrmId: Array<string>) {
        try {
            const responseText = await RTResearchResult_DelRTResearchResultsAsync(arrmId);
            var returnInt: number = responseText;
            if (returnInt > 0) {
                var strInfo: string = `删除记录成功,共删除${returnInt}条记录!`;
                //显示信息框
   ...
   */
    public ShowRTResearchResultObj(divContainer: string, objRTResearchResult: clsRTResearchResultEN) {
        ...
        var sstrKeys = clsCommonFunc4Web.GetObjKeys(objRTResearchResult);
