...
import { RTUserRela_AddNewRecordAsync, RTUserRela_DelRecordAsync, RTUserRela_DelRTUserRelasAsync, RTUserRela_GetObjBymIdAsync, RTUserRela_IsExistAsync, RTUserRela_UpdateRecordAsync } from "../TS/L3_ForWApi/GraduateEduTopic/clsRTUserRelaWApi.js";
...
*/
export class RTUserRela_QUDI implements clsOperateList
{
...
try
{
const responseText = await vRTUserRela_GetObjLstAsync(strWhereCond).then((jsonData)=>{
var arrvRTUserRelaObjLst : Array<clsvRTUserRelaEN> = < Array<clsvRTUserRelaEN>>jsonData;
...
{
strWhereCond += ` And ${clsvRTUserRelaEN.con_id_XzCollege } = '${ this.id_XzCollege_q }'`;
}
if ( this.id_XzMajor_q != "" && this.id_XzMajor_q != "0")
{
strWhereCond += ` And ${clsvRTUserRelaEN.con_id_XzMajor } = '${ this.id_XzMajor_q }'`;
}
}
catch(objException)
{
...
clsCommonFunc4Web.BindDdl_ObjLst(ddlid_XzCollege, arrXzClgObjLst, clsXzClgEN.con_id_XzCollege, clsXzClgEN.con_CollegeName, "学院");
...
try
{
const responseText = RTUserRela_DelRTUserRelasAsync(arrmId).then((jsonData)=>{
...
try
{
const responseText = RTUserRela_GetObjBymIdAsync(lngmId).then((jsonData)=>{
...
*/
public QueryvRTUserRelaLst(strListDiv:string) 
{
var strWhereCond = this.CombinevRTUserRelaCondition();
var intCurrPageIndex = this.getCurrPageIndex(this.divName4Pager);//获取当前页
return new Promise((resolve, reject) => {
...
const responseText = vRTUserRela_GetObjLstByPagerAsync(objPagerPara).then((jsonData)=>{
var arrvRTUserRelaObjLst : Array<clsvRTUserRelaEN> = < Array<clsvRTUserRelaEN>>jsonData;
this.BindTab_vRTUserRela(strListDiv, arrvRTUserRelaObjLst);
...
*/
  public async ShowData(lngmId: number) {
        ...
        var objRTUserRelaEN: clsRTUserRelaEN = new clsRTUserRelaEN();
        try {
            const responseText = await RTUserRela_IsExistAsync(lngmId);
        ...
        try {
            const responseText = await RTUserRela_GetObjBymIdAsync(lngmId);
            objRTUserRelaEN = <clsRTUserRelaEN>responseText;
        }
        catch (e) {
            var strMsg: string = `根据关键字获取相应的记录的对象不成功,${e}.`;
            alert(strMsg);
...
try
{
const responseText = RTUserRela_UpdateRecordAsync(objRTUserRelaEN).then((jsonData)=>{
var returnBool: boolean = jsonData;
if (returnBool == true)
{
