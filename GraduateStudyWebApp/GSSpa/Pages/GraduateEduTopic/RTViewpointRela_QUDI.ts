...
import { Viewpoint_GetObjLstAsync } from "../TS/L3_ForWApi/GraduateEduTopic/clsViewpointWApi.js";
import { ViewpointType_GetObjLstAsync } from "../TS/L3_ForWApi/ParameterTable/clsViewpointTypeWApi.js";
import { RTViewpointRela_AddNewRecordAsync, RTViewpointRela_DelRecordAsync, RTViewpointRela_DelRTViewpointRelasAsync, RTViewpointRela_GetObjBymIdAsync, RTViewpointRela_IsExistAsync, RTViewpointRela_UpdateRecordAsync } from "../TS/L3_ForWApi/GraduateEduTopic/clsRTViewpointRelaWApi.js";
...
*/
export class RTViewpointRela_QUDI implements clsOperateList{
   ...
   */
    public async btnExportExcel_Click() {
        ...
        try {
            const responseText = await vRTViewpointRela_GetObjLstAsync(strWhereCond).then((jsonData) => {
   ...
   */
    ...
    /// </summary>
    /// <returns>获取主键字段、名称字段两列的所有记录记录的dataTable</returns>
    public BindDdl_ViewpointId(ddlViewpointId: string, strWhereCond: string = "1 =1") {
        //var strWhereCond = " 1 =1 ";
        ...
        return new Promise((resolve, reject) => {
            try {
                const responseText = Viewpoint_GetObjLstAsync(strWhereCond).then((jsonData) => {
                    var arrViewpointObjLst: Array<clsViewpointEN> = <Array<clsViewpointEN>>jsonData;
                    clsCommonFunc4Web.BindDdl_ObjLst(ddlViewpointId, arrViewpointObjLst, clsViewpointEN.con_ViewpointId, clsViewpointEN.con_ViewpointName, "观点表");
   ...
   */
    public DetailRecord(lngmId: number) {
        ...
        return new Promise((resolve, reject) => {
            try {
                const responseText = RTViewpointRela_GetObjBymIdAsync(lngmId).then((jsonData) => {
                    var objRTViewpointRelaEN: clsRTViewpointRelaEN = <clsRTViewpointRelaEN>jsonData;
   ...
   */
    public QueryvRTViewpointRelaLst(strListDiv: string) {
        var strWhereCond = this.CombinevRTViewpointRelaCondition();
        var intCurrPageIndex = this.getCurrPageIndex(this.divName4Pager);//获取当前页
        return new Promise((resolve, reject) => {
            try {
                ...
                const responseText = vRTViewpointRela_GetObjLstByPagerAsync(objPagerPara).then((jsonData) => {
                    var arrvRTViewpointRelaObjLst: Array<clsvRTViewpointRelaEN> = <Array<clsvRTViewpointRelaEN>>jsonData;
                    this.BindTab_vRTViewpointRela(strListDiv, arrvRTViewpointRelaObjLst);
   ...
   */
    public async SelectRecord(lngmId: number) {
        try {
            const responseText = await RTViewpointRela_GetObjBymIdAsync(lngmId);
            var objRTViewpointRelaEN: clsRTViewpointRelaEN = <clsRTViewpointRelaEN>responseText;
            console.log("完成SelectRecord!");
            clsCommonFunc4Web.Redirect("/Index/Main_vRTViewpointRela");
        }
        catch (e) {
            console.error(e);
            var strMsg: string = `根据关键字获取相应的记录的对象不成功,${e}.`;
   ...
   */
    public async ShowData(lngmId: number) {
        ...
        var objRTViewpointRelaEN: clsRTViewpointRelaEN = new clsRTViewpointRelaEN();
        try {
            const responseText = await RTViewpointRela_IsExistAsync(lngmId);
        ...
        try {
            const responseText = await RTViewpointRela_GetObjBymIdAsync(lngmId);
            objRTViewpointRelaEN = <clsRTViewpointRelaEN>responseText;
        }
        catch (e) {
            var strMsg: string = `根据关键字获取相应的记录的对象不成功,${e}.`;
            alert(strMsg);
   ...
   */
    public ShowRTViewpointRelaObj(divContainer: string, objRTViewpointRela: clsRTViewpointRelaEN) {
        ...
        var sstrKeys = clsCommonFunc4Web.GetObjKeys(objRTViewpointRela);
