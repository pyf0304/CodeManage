...
import { SysComment_AddNewRecordAsync, SysComment_AddNewRecordWithMaxIdAsync, SysComment_DelRecordAsync, SysComment_DelSysCommentsAsync, SysComment_GetMaxStrIdAsync, SysComment_GetObjByCommentIdAsync, SysComment_IsExistAsync, SysComment_UpdateRecordAsync } from "../TS/L3_ForWApi/GraduateEduTools/clsSysCommentWApi.js";
import { vSysComment_GetObjLstAsync, vSysComment_GetObjLstByPagerAsync, vSysComment_GetObjLstByPager_Cache, vSysComment_GetRecCountByCondAsync, vSysComment_GetRecCountByCond_Cache } from "../TS/L3_ForWApi/GraduateEduTools/clsvSysCommentWApi.js";
...
*/
export class SysComment_QUDI_TS implements clsOperateList{
   ...
   */
    public async Page_Load_Cache() {
        ...
        try {
            ...
            this.RecCount = await vSysComment_GetRecCountByCond_Cache(objvSysComment_Cond, strid_CurrEduCls);
            //2、显示无条件的表内容在GridView中
    ...
    public async btnQuery_Click(strListDiv: string) {
        ...
        try {
            ...
            const responseObjLst = await vSysComment_GetObjLstByPagerAsync(objPagerPara).then((jsonData) => {
                var arrvSysCommentObjLst: Array<clsvSysCommentEN> = <Array<clsvSysCommentEN>>jsonData;
   ...
   */
    public async DetailRecord(strCommentId: string) {
        this.btnOKUpd = "";
        this.btnCancel = "关闭";
        try {
            const responseText = await SysComment_GetObjByCommentIdAsync(strCommentId);
            var objSysCommentEN: clsSysCommentEN = <clsSysCommentEN>responseText;
   ...
   */
    public async SelectRecord(strCommentId: string) {
        try {
            ...
            clsCommonFunc4Web.Redirect("/Index/Main_vSysComment");
        }
        catch (e) {
            console.error(e);
            var strMsg: string = `根据关键字获取相应的记录的对象不成功,${e}.`;
   ...
   */
    public async btnExportExcel_Click() {
        ...
        try {
            const responseText = await vSysComment_GetObjLstAsync(strWhereCond).then((jsonData) => {
                var arrvSysCommentObjLst: Array<clsvSysCommentEN> = <Array<clsvSysCommentEN>>jsonData;
                this.BindTab_vSysComment(this.mstrListDiv, arrvSysCommentObjLst);
            });
        }
   ...
   */
    public async CombinevSysCommentConditionObj(): Promise<clsvSysCommentEN> {
        ...
        //例如 1 = 1 && userName = '张三'
        var strWhereCond: string = " 1 = 1 ";
        var objvSysComment_Cond: clsvSysCommentEN = new clsvSysCommentEN();
        //获取用户缓存数据
        var arrUsers: Array<clsvUsersSimEN> = await vUsersSim_GetObjLst_Cache();
        //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
        var strUserId: string = "";
        ...
        try {
            if (this.UserName_q != "") {
                strWhereCond += ` And ${clsvSysCommentEN.con_UpdUser} = '${strUserId}'`;
                objvSysComment_Cond.SetCondFldValue(clsvSysCommentEN.con_UpdUser, strUserId, "=");
            }
            //if (this.CommentTypeName_q != "") {
            //    strWhereCond += ` And ${clsvSysCommentEN.con_CommentTypeName} like '% ${this.CommentTypeName_q}%'`;
            ...
            var strMsg: string = `(errid:WiTsCs0005)在组合查询条件对象(CombineSysCommentConditionObj)时出错!请联系管理员!${objException}`;
            throw strMsg;
        }
   ...
   */
    public BindTab_vSysComment(divContainer: string, arrvSysCommentObjLst: Array<clsvSysCommentEN>) {
        ...
        clsCommonFunc4Web.BindTabV2(o, arrvSysCommentObjLst, arrDataColumn, "commentId");
   ...
   */
    public async BindGv_vSysComment_Cache() {
        ...
        try {
            ...
            var arrvSysCommentObjLst = await vSysComment_GetObjLstByPager_Cache(objPagerPara, strid_CurrEduCls);
        }
        catch (e) {
            console.error('catch(e)=');
            console.error(e);
        ...
        if (arrvSysCommentObjLst.length == 0) {
            var strMsg: string = `在BindGv_Cache过程中，根据条件对象获取的对象列表数为0！(Key=SysComment)`;
            alert(strMsg);
            return;
        }
        try {
   ...
   */
    public async GetMaxStrId(strKeyCtrlName) {
        ...
        try {
            const responseText = await SysComment_GetMaxStrIdAsync();
            var returnString: string = responseText.toString();
            if (returnString == "") {
                var strInfo: string = `获取表SysComment的最大关键字为空，不成功，请检查!`;
                ...
                var strInfo: string = `获取表SysComment的最大关键字为：${returnString}!`;
                $('#lblResult40').val(strInfo);
                //显示信息框
                alert(strInfo);
                $(strKeyCtrlName).val(returnString);
   ...
   */
    public PutDataToSysCommentClass(pobjSysCommentEN: clsSysCommentEN) {
   ...
   */
    public async AddNewRecordSave() {
        ...
        try {
            const responseText = await SysComment_IsExistAsync(objSysCommentEN.commentId);
            var bolIsExist: boolean = responseText;
            if (bolIsExist == true) {
                var strMsg: string = `添加记录时，关键字：${objSysCommentEN.commentId}已经存在！`;
                alert(strMsg);
                return responseText;//一定要有一个返回值，否则会出错！
            }
            const responseText2 = await SysComment_AddNewRecordAsync(objSysCommentEN);
   ...
   */
    public async ShowData(strCommentId: string) {
        ...
        var objSysCommentEN: clsSysCommentEN = new clsSysCommentEN();
        try {
            const responseText = await SysComment_IsExistAsync(strCommentId);
            var returnBool: boolean = responseText;
            if (returnBool == false) {
                var strInfo: string = `关键字:[${strCommentId}] 的记录不存在!`;
        ...
        try {
            const responseText = await SysComment_GetObjByCommentIdAsync(strCommentId);
            objSysCommentEN = <clsSysCommentEN>responseText;
        }
        catch (e) {
            var strMsg: string = `根据关键字获取相应的记录的对象不成功,${e}.`;
            alert(strMsg);
   ...
   */
    ...
    /* 根据关键字获取相应的记录的对象
     ...
     <param name = "sender">参数列表</param>
   */
    public async UpdateRecord(strCommentId: string) {
        this.btnOKUpd = "确认修改";
        this.btnCancel = "取消修改";
   ...
   */
    public async UpdateRecordSave() {
        ...
        try {
            const responseText = await SysComment_UpdateRecordAsync(objSysCommentEN);
            var returnBool: boolean = !!responseText;
            if (returnBool == true) {
            }
            return returnBool;
   ...
   */
    public async DelMultiRecord(arrCommentId: Array<string>) {
        try {
            const responseText = await SysComment_DelSysCommentsAsync(arrCommentId);
            var returnInt: number = responseText;
            if (returnInt > 0) {
                var strInfo: string = `删除记录成功,共删除${returnInt}条记录!`;
                //显示信息框
   ...
   */
    public ShowSysCommentObj(divContainer: string, objSysComment: clsSysCommentEN) {
        ...
        var sstrKeys = clsCommonFunc4Web.GetObjKeys(objSysComment);
   ...
   */
    public set hidSortvSysCommentBy(value: string) {
        $("#hidSortvSysCommentBy").val(value);
    }
    /*
    * 设置排序字段
   */
    public get hidSortvSysCommentBy(): string {
