...
import { Section_AddNewRecordAsync, Section_DelRecordAsync, Section_DelSectionsAsync, Section_GetMaxStrIdAsync, Section_GetObjBySectionIdAsync, Section_IsExistAsync, Section_UpdateRecordAsync } from "../TS/L3_ForWApi/GraduateEduPaper/clsSectionWApi.js";
import { vSection_GetObjLstAsync, vSection_GetObjLstByPagerAsync, vSection_GetRecCountByCondAsync } from "../TS/L3_ForWApi/GraduateEduPaper/clsvSectionWApi.js";
...
declare var $;
declare var window;
/* Section_QUDI_TS 的摘要说明。其中Q代表查询,U代表修改
 (AutoGCLib.WA_ViewScriptCS_TS4TypeScript:GeneCode)
*/
export class Section_QUDI_TS implements clsOperateList{
   ...
   */
    public async btnDelRecord_Click() {
        try {
            ...
            const responseText2 = await this.BindGv_vSection();
        }
        catch (e) {
            console.error('catch(e)=');
            console.error(e);
   ...
   */
    public DelMultiRecord(arrSectionId: Array<string>) {
        return new Promise((resolve, reject) => {
            try {
                const responseText = Section_DelSectionsAsync(arrSectionId).then((jsonData) => {
   ...
   */
    public DelRecord(strSectionId: string) {
        return new Promise((resolve, reject) => {
            try {
                const responseText = Section_DelRecordAsync(strSectionId).then((jsonData) => {
                    var returnInt: number = jsonData;
                    if (returnInt > 0) {
                        var strInfo: string = `删除记录成功,共删除${returnInt}条记录!`;
                        //显示信息框
   ...
   */
    public DetailRecord(strSectionId: string) {
        this.btnOKUpd = "";
        this.btnCancel = "关闭";
        return new Promise((resolve, reject) => {
            try {
                const responseText = Section_GetObjBySectionIdAsync(strSectionId).then((jsonData) => {
                    var objSectionEN: clsSectionEN = <clsSectionEN>jsonData;
   ...
   */
    public GetDataFromSectionClass(pobjSectionEN: clsSectionEN) {
        this.sectionId = pobjSectionEN.sectionId;// 节Id
        this.sectionName = pobjSectionEN.sectionName;// 节名
        this.paperId = pobjSectionEN.paperId;// 论文Id
        this.orderNum = String(pobjSectionEN.orderNum);// 排序
        this.updDate = pobjSectionEN.updDate;// 修改日期
        this.updUser = pobjSectionEN.updUser;// 修改人
        this.memo = pobjSectionEN.memo;// 备注
    }
    ...
    public async GetMaxStrId(strKeyCtrlName) {
        ...
        try {
            ...
            if (returnString == "") {
                ...
                var strInfo: string = `获取表Section的最大关键字为：${returnString}!`;
                $('#lblResult35').val(strInfo);
                //显示信息框
                alert(strInfo);
                $(strKeyCtrlName).val(returnString);
   ...
   */
    public PutDataToSectionClass(pobjSectionEN: clsSectionEN) {
        ...
        pobjSectionEN.paperId = this.paperId;// 论文Id
        pobjSectionEN.orderNum = Number(this.orderNum);// 排序
        pobjSectionEN.updDate = this.updDate;// 修改日期
        pobjSectionEN.updUser = this.updUser;// 修改人
   ...
   */
    public QueryvSectionLst(strListDiv: string) {
        var strWhereCond = this.CombinevSectionCondition();
        var intCurrPageIndex = this.getCurrPageIndex(this.divName4Pager);//获取当前页
        return new Promise((resolve, reject) => {
            try {
                ...
                const responseText = vSection_GetObjLstByPagerAsync(objPagerPara).then((jsonData) => {
                    var arrvSectionObjLst: Array<clsvSectionEN> = <Array<clsvSectionEN>>jsonData;
                    this.BindTab_vSection(strListDiv, arrvSectionObjLst);
                    console.log("完成QueryvSectionLst!");
                    resolve(jsonData);
                });
            }
            catch (e) {
   ...
   */
    ...
    public SetKeyReadOnly(bolReadonly: boolean) {
        $('#txtSectionId').prop['ReadOnly'] = bolReadonly;
    }
   ...
   */
    ...
    /* 根据关键字获取相应的记录的对象
     ...
     <param name = "sender">参数列表</param>
   */
    public UpdateRecord(strSectionId: string) {
        this.btnOKUpd = "确认修改";
        this.btnCancel = "取消修改";
   ...
   */
    public UpdateRecordSave(): Promise<boolean> {
        ...
        return new Promise((resolve, reject) => {
            try {
                const responseText = Section_UpdateRecordAsync(objSectionEN).then((jsonData) => {
                    var returnBool: boolean = jsonData;
                    if (returnBool == true) {
   ...
   */
    ...
    * 节名称
   */
    public set orderNum(value: string) {
        $("#txtOrderNum").val(value);
    }
    /*
    * 节名称
   */
    public get orderNum(): string {
   ...
   */
    public set sectionId(value: string) {
        $("#txtSectionId").val(value);
    }
    /*
    * 节Id
   */
    public get sectionId(): string {
        return $("#txtSectionId").val();
    }
    /*
    * 节名
   */
