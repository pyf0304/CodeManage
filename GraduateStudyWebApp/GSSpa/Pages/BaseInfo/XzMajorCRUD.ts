...
import { clsvXzMajorEN } from "../TS/L0_Entity/BaseInfo/clsvXzMajorEN.js";
import { clsXzClgEN } from "../TS/L0_Entity/BaseInfo/clsXzClgEN.js";
import { clsXzMajorShoolTypeEN } from "../TS/L0_Entity/BaseInfo/clsXzMajorShoolTypeEN.js";
import { arrSelectedKeys, clsCommonFunc4Web } from "../TS/PubFun/clsCommonFunc4Web2.js";
import { clsDataColumn } from "../TS/PubFun/clsDataColumn.js";
import { stuPagerPara } from "../TS/PubFun/stuPagerPara.js";
 
import { XzMajor_AddNewRecordAsync, XzMajor_AddNewRecordWithMaxIdAsync, XzMajor_DelRecordAsync, XzMajor_DelXzMajorsAsync, XzMajor_GetMaxStrIdAsync, XzMajor_GetObjByid_XzMajorAsync, XzMajor_IsExistAsync, XzMajor_UpdateRecordAsync } from "../TS/L3_ForWApi/BaseInfo/clsXzMajorWApi.js";
...
import { XzMajorShoolType_BindDdl_id_XzMajorShoolTypeInDiv_Cache } from "../TS/L3_ForWApi/BaseInfo/clsXzMajorShoolTypeWApi.js";
import { vXzMajor_GetObjLstAsync, vXzMajor_GetObjLstByPagerAsync, vXzMajor_GetObjLstByPager_Cache, vXzMajor_GetRecCountByCondAsync, vXzMajor_GetRecCountByCond_Cache } from "../TS/L3_ForWApi/BaseInfo/clsvXzMajorWApi.js";
 /*
...
*/
export class XzMajorCRUD implements clsOperateList
{
...
var strUrl:string = "./XzMajor_Edit/";
...
try
{
const responseRecCount = await vXzMajor_GetRecCountByCondAsync(strWhereCond).then((jsonData) => {
...
const responseObjLst = await vXzMajor_GetObjLstByPagerAsync(objPagerPara).then((jsonData) => {
var arrvXzMajorObjLst : Array<clsvXzMajorEN> = < Array<clsvXzMajorEN>>jsonData;
...
{
// 在此处放置用户代码以初始化页面
 var objXzMajorShoolType_Cond: clsXzMajorShoolTypeEN = new clsXzMajorShoolTypeEN();//编辑区域
// var objXzClg_Cond: clsXzClgEN = new clsXzClgEN();//编辑区域
const ddlid_XzMajorShoolType = await XzMajorShoolType_BindDdl_id_XzMajorShoolTypeInDiv_Cache(this.divName4List, "ddlid_XzMajorShoolType");//编辑区域
...
{
// 在此处放置用户代码以初始化页面
 var objXzClg_Cond:clsXzClgEN = new clsXzClgEN();//查询区域
const ddlid_XzCollege_q = await XzClg_BindDdl_id_XzCollegeInDiv_Cache(this.divName4List, "ddlid_XzCollege_q");//查询区域
}
...
*/
public async DelRecord(strid_XzMajor: string) {
try
{
const responseText = await XzMajor_DelRecordAsync(strid_XzMajor);
...
*/
public async DetailRecord(strid_XzMajor: string) 
{
this.btnOKUpd = "";
this.btnCancel = "关闭";
try
{
const responseText = await XzMajor_GetObjByid_XzMajorAsync(strid_XzMajor);
var objXzMajorEN: clsXzMajorEN = <clsXzMajorEN>responseText;
...
alert(strMsg);
 ...
 根据关键字选择相应的记录
  ...
  <param name = "sender">参数列表</param>
*/
public async SelectRecord(strid_XzMajor: string) 
{
try
{
const responseText = await XzMajor_GetObjByid_XzMajorAsync(strid_XzMajor);
...
clsCommonFunc4Web.Redirect("/Index/Main_vXzMajor");
}
catch(e)
{
console.error(e);
...
try
{
const responseText = await vXzMajor_GetObjLstAsync(strWhereCond).then((jsonData)=>{
var arrvXzMajorObjLst : Array<clsvXzMajorEN> = < Array<clsvXzMajorEN>>jsonData;
this.BindTab_vXzMajor(this.mstrListDiv, arrvXzMajorObjLst);
});
}
...
*/
 public CombinevXzMajorCondition():string 
{
...
{
strWhereCond += ` And ${clsvXzMajorEN.con_id_XzCollege } = '${ this.id_XzCollege_q }'`;
}
}
catch(objException)
{
var strMsg:string = `(errid:WiTsCs0002)在组合查询条件(CombineXzMajorCondition)时出错!请联系管理员!${objException}`;
throw strMsg;
}
return strWhereCond;
}
 ...
 (AutoGCLib.WA_ViewScriptCS_TS4TypeScript:Gen_WApi_Ts_CombineConditionObj)
  <returns>条件串(strWhereCond)</returns>
*/
 public CombinevXzMajorConditionObj():clsvXzMajorEN 
{
//使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
//例如 1 = 1 && userName = '张三'
var strWhereCond: string = " 1 = 1 ";
var objvXzMajor_Cond: clsvXzMajorEN = new clsvXzMajorEN();
//如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
try
{
if ( this.MajorID_q != "")
{
strWhereCond += ` And ${ clsvXzMajorEN.con_MajorID} like '% ${ this.MajorID_q }%'`;
objvXzMajor_Cond.SetCondFldValue(clsvXzMajorEN.con_MajorID, this.MajorID_q, "like");
}
if ( this.majorName_q != "")
{
strWhereCond += ` And ${ clsvXzMajorEN.con_MajorName} like '% ${ this.majorName_q }%'`;
objvXzMajor_Cond.SetCondFldValue(clsvXzMajorEN.con_MajorName, this.majorName_q, "like");
}
if ( this.id_XzCollege_q != "" && this.id_XzCollege_q != "0")
{
strWhereCond += ` And ${clsvXzMajorEN.con_id_XzCollege } = '${ this.id_XzCollege_q }'`;
objvXzMajor_Cond.SetCondFldValue(clsvXzMajorEN.con_id_XzCollege, this.id_XzCollege_q, "=");
}
}
catch(objException)
{
var strMsg:string = `(errid:WiTsCs0005)在组合查询条件对象(CombineXzMajorConditionObj)时出错!请联系管理员!${objException}`;
throw strMsg;
}
...
*/
public BindTab_vXzMajor(divContainer: string, arrvXzMajorObjLst: Array<clsvXzMajorEN>) 
{
...
clsCommonFunc4Web.BindTabV2(o, arrvXzMajorObjLst, arrDataColumn, "id_XzMajor"); 
...
*/
public async BindGv_vXzMajor() 
{
var strListDiv: string = this.mstrListDiv;
var strWhereCond = this.CombinevXzMajorCondition();
var intCurrPageIndex = this.getCurrPageIndex(this.divName4Pager);//获取当前页
 var arrvXzMajorObjLst: Array <clsvXzMajorEN> = [];
try
{
const responseRecCount = await vXzMajor_GetRecCountByCondAsync(strWhereCond).then((jsonData) => {
this.RecCount = jsonData;
...
const responseObjLst = await vXzMajor_GetObjLstByPagerAsync(objPagerPara).then((jsonData)=>{
arrvXzMajorObjLst = < Array<clsvXzMajorEN>>jsonData;
});
}
catch(e)
{
...
*/
public async BindGv_vXzMajor_Cache() 
{
var strListDiv: string = this.mstrListDiv;
...
try
{
this.RecCount = await vXzMajor_GetRecCountByCond_Cache(objvXzMajor_Cond);
var objPagerPara: stuPagerPara = {
pageIndex: intCurrPageIndex,
pageSize: this.pageSize,
whereCond: strWhereCond,
...
var arrvXzMajorObjLst = await vXzMajor_GetObjLstByPager_Cache(objPagerPara);
}
catch(e)
{
console.error('catch(e)=');
...
if (arrvXzMajorObjLst.length == 0)
{
var strMsg: string = `在BindGv_Cache过程中，根据条件对象获取的对象列表数为0！(Key=XzMajor)`;
alert(strMsg);
return;
}
try
...
{
var strInfo: string = `获取表XzMajor的最大关键字为：${returnString}!`;
$('#lblResult40').val(strInfo);
//显示信息框
alert(strInfo);
 $(strKeyCtrlName).val(returnString);
...
*/
public PutDataToXzMajorClass(pobjXzMajorEN: clsXzMajorEN)
{
...
pobjXzMajorEN.majorDirection = this.MajorDirection;// 专业方向
    ...
    pobjXzMajorEN.modifyUserID = clsPubSessionStorage.GetSession_UserId().toString();// userId;// 修改人
}
...
var objXzMajorEN: clsXzMajorEN = new clsXzMajorEN();
   this.PutDataToXzMajorClass(objXzMajorEN);
try
{
const responseText = await XzMajor_IsExistAsync(objXzMajorEN.id_XzMajor);
var bolIsExist: boolean = responseText;
if (bolIsExist == true)
{
var strMsg: string = `添加记录时，关键字：${objXzMajorEN.id_XzMajor }已经存在！`;
alert(strMsg);
return responseText;//一定要有一个返回值，否则会出错！
}
const responseText2 = await XzMajor_AddNewRecordAsync(objXzMajorEN);
...
try
{
const responseText2 = await XzMajor_AddNewRecordWithMaxIdAsync(objXzMajorEN);
...
*/
  public async ShowData(strid_XzMajor: string) {
        //操作步骤:
        //1、检查关键字是否为空；
        ...
        var objXzMajorEN: clsXzMajorEN = new clsXzMajorEN();
        try {
            const responseText = await XzMajor_IsExistAsync(strid_XzMajor);
            var returnBool: boolean = responseText;
            if (returnBool == false) {
                var strInfo: string = `关键字:[${ strid_XzMajor }] 的记录不存在!`;
        ...
        try {
            const responseText = await XzMajor_GetObjByid_XzMajorAsync(strid_XzMajor);
            objXzMajorEN = <clsXzMajorEN>responseText;
        }
        catch (e) {
            var strMsg: string = `根据关键字获取相应的记录的对象不成功,${e}.`;
            alert(strMsg);
...
this.IsVisible = pobjXzMajorEN.isVisible;// 是否显示
 ...
 /* 根据关键字获取相应的记录的对象
  ...
  <param name = "sender">参数列表</param>
*/
public async UpdateRecord(strid_XzMajor: string) 
{
this.btnOKUpd = "确认修改";
this.btnCancel = "取消修改";
...
try
{
const responseText = await XzMajor_UpdateRecordAsync(objXzMajorEN);
var returnBool: boolean = !!responseText;
if (returnBool == true)
{
}
...
*/
public async DelMultiRecord(arrid_XzMajor: Array<string>) {
try
{
const responseText = await XzMajor_DelXzMajorsAsync(arrid_XzMajor);
var returnInt: number = responseText;
if (returnInt > 0)
{
var strInfo: string = `删除记录成功,共删除${returnInt}条记录!`;
...
*/
public ShowXzMajorObj(divContainer: string, objXzMajor: clsXzMajorEN) 
{
...
var sstrKeys = clsCommonFunc4Web.GetObjKeys(objXzMajor);
...
*/
public set hidSortvXzMajorBy(value: string) {
$("#hidSortvXzMajorBy").val(value);
}
 /*
 * 设置排序字段
*/
public get hidSortvXzMajorBy():string {
 ...
 * 专业学校类型流水号
*/
public set id_XzMajorShoolType(value: string) {
$("#ddlid_XzMajorShoolType").val(value);
}
 /*
 * 专业学校类型流水号
*/
public get id_XzMajorShoolType():string {
