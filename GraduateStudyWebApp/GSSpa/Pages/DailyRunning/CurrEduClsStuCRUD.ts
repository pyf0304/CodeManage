...
import { CurrEduClsStu_AddNewRecordAsync, CurrEduClsStu_DelCurrEduClsStusAsync, CurrEduClsStu_DelRecordAsync, CurrEduClsStu_GetObjByid_EduClsStuAsync, CurrEduClsStu_IsExistAsync, CurrEduClsStu_UpdateRecordAsync } from "../TS/L3_ForWApi/DailyRunning/clsCurrEduClsStuWApi.js";
import { vCurrEduClsStu_GetObjLstAsync, vCurrEduClsStu_GetObjLstByPagerAsync, vCurrEduClsStu_GetObjLstByPager_Cache, vCurrEduClsStu_GetRecCountByCondAsync, vCurrEduClsStu_GetRecCountByCond_Cache } from "../TS/L3_ForWApi/DailyRunning/clsvCurrEduClsStuWApi.js";
/*
...
*/
export class CurrEduClsStuCRUD implements clsOperateList{
   ...
   */
    ...
    public async AddDPV_Edit(divName4Edit) {
        var strUrl: string = "./WA_CurrEduClsStu_Edit/";
   ...
   */
    public async btnQuery_Click() {
        ...
        try {
            const responseRecCount = await vCurrEduClsStu_GetRecCountByCondAsync(strWhereCond).then((jsonData) => {
            ...
            const responseObjLst = await vCurrEduClsStu_GetObjLstByPagerAsync(objPagerPara).then((jsonData) => {
                var arrvCurrEduClsStuObjLst: Array<clsvCurrEduClsStuEN> = <Array<clsvCurrEduClsStuEN>>jsonData;
   ...
   */
    public async DelRecord(lngid_EduClsStu: number) {
        try {
            const responseText = await CurrEduClsStu_DelRecordAsync(lngid_EduClsStu);
   ...
   */
    public async DetailRecord(lngid_EduClsStu: number) {
        this.btnOKUpd = "";
        this.btnCancel = "关闭";
        try {
            const responseText = await CurrEduClsStu_GetObjByid_EduClsStuAsync(lngid_EduClsStu);
            var objCurrEduClsStuEN: clsCurrEduClsStuEN = <clsCurrEduClsStuEN>responseText;
    ...
    根据关键字选择相应的记录
     ...
     <param name = "sender">参数列表</param>
   */
    public async SelectRecord(lngid_EduClsStu: number) {
        try {
            const responseText = await CurrEduClsStu_GetObjByid_EduClsStuAsync(lngid_EduClsStu);
            var objCurrEduClsStuEN: clsCurrEduClsStuEN = <clsCurrEduClsStuEN>responseText;
            console.log("完成SelectRecord!");
            clsCommonFunc4Web.Redirect("/Index/Main_vCurrEduClsStu");
        }
        catch (e) {
            console.error(e);
            var strMsg: string = `根据关键字获取相应的记录的对象不成功,${e}.`;
   ...
   */
    public async btnExportExcel_Click() {
        ...
        try {
            const responseText = await vCurrEduClsStu_GetObjLstAsync(strWhereCond).then((jsonData) => {
                var arrvCurrEduClsStuObjLst: Array<clsvCurrEduClsStuEN> = <Array<clsvCurrEduClsStuEN>>jsonData;
                this.BindTab_vCurrEduClsStu(this.mstrListDiv, arrvCurrEduClsStuObjLst);
            });
        }
   ...
   */
    public CombinevCurrEduClsStuCondition(): string {
    ...
    (AutoGCLib.WA_ViewScriptCS_TS4TypeScript:Gen_WApi_Ts_CombineConditionObj)
     <returns>条件串(strWhereCond)</returns>
   */
    public CombinevCurrEduClsStuConditionObj(): clsvCurrEduClsStuEN {
        //使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
        //例如 1 = 1 && userName = '张三'
        var strWhereCond: string = " 1 = 1 ";
        ...
        try {
            if (this.CurrEduClsId_q != "") {
                strWhereCond += ` And ${clsvCurrEduClsStuEN.con_CurrEduClsId} like '% ${this.CurrEduClsId_q}%'`;
                objvCurrEduClsStu_Cond.SetCondFldValue(clsvCurrEduClsStuEN.con_CurrEduClsId, this.CurrEduClsId_q, "like");
            }
            if (this.eduClsName_q != "") {
                strWhereCond += ` And ${clsvCurrEduClsStuEN.con_EduClsName} like '% ${this.eduClsName_q}%'`;
                objvCurrEduClsStu_Cond.SetCondFldValue(clsvCurrEduClsStuEN.con_EduClsName, this.eduClsName_q, "like");
            }
            //if ( this.CourseName_q != "")
            //{
            //strWhereCond += ` And ${ clsvCurrEduClsStuEN.con_CourseName} like '% ${ this.CourseName_q }%'`;
            ...
            var strMsg: string = `(errid:WiTsCs0005)在组合查询条件对象(CombineCurrEduClsStuConditionObj)时出错!请联系管理员!${objException}`;
            throw strMsg;
        }
   ...
   */
    public BindTab_vCurrEduClsStu(divContainer: string, arrvCurrEduClsStuObjLst: Array<clsvCurrEduClsStuEN>) {
        ...
        clsCommonFunc4Web.BindTabV2(o, arrvCurrEduClsStuObjLst, arrDataColumn, "id_EduClsStu");
   ...
   */
    public async BindGv_vCurrEduClsStu_Cache() {
        var strListDiv: string = this.mstrListDiv;
        ...
        try {
            ...
            this.RecCount = await vCurrEduClsStu_GetRecCountByCond_Cache(objvCurrEduClsStu_Cond, strid_CurrEduCls);
            var objPagerPara: stuPagerPara = {
                pageIndex: intCurrPageIndex,
                pageSize: this.pageSize,
                whereCond: strWhereCond,
            ...
            var arrvCurrEduClsStuObjLst = await vCurrEduClsStu_GetObjLstByPager_Cache(objPagerPara, strid_CurrEduCls);
        }
        catch (e) {
            console.error('catch(e)=');
            console.error(e);
        ...
        if (arrvCurrEduClsStuObjLst.length == 0) {
            var strMsg: string = `在BindGv_Cache过程中，根据条件对象获取的对象列表数为0！(Key=CurrEduClsStu)`;
            alert(strMsg);
            return;
        }
        try {
   ...
   */
    public PutDataToCurrEduClsStuClass(pobjCurrEduClsStuEN: clsCurrEduClsStuEN) {
   ...
   */
    public async ShowData(lngid_EduClsStu: number) {
        //操作步骤:
        //1、检查关键字是否为空；
        ...
        var objCurrEduClsStuEN: clsCurrEduClsStuEN = new clsCurrEduClsStuEN();
        try {
            const responseText = await CurrEduClsStu_IsExistAsync(lngid_EduClsStu);
            var returnBool: boolean = responseText;
            if (returnBool == false) {
                var strInfo: string = `关键字:[${lngid_EduClsStu}] 的记录不存在!`;
        ...
        try {
            const responseText = await CurrEduClsStu_GetObjByid_EduClsStuAsync(lngid_EduClsStu);
            objCurrEduClsStuEN = <clsCurrEduClsStuEN>responseText;
        }
        catch (e) {
            var strMsg: string = `根据关键字获取相应的记录的对象不成功,${e}.`;
            alert(strMsg);
   ...
   */
    public GetDataFromCurrEduClsStuClass(pobjCurrEduClsStuEN: clsCurrEduClsStuEN) {
    ...
    /* 根据关键字获取相应的记录的对象
     ...
     <param name = "sender">参数列表</param>
   */
    public async UpdateRecord(lngid_EduClsStu: number) {
        this.btnOKUpd = "确认修改";
        this.btnCancel = "取消修改";
   ...
   */
    public async UpdateRecordSave() {
        ...
        try {
            const responseText = await CurrEduClsStu_UpdateRecordAsync(objCurrEduClsStuEN);
            var returnBool: boolean = !!responseText;
            if (returnBool == true) {
            }
            return returnBool;
   ...
   */
    public async DelMultiRecord(arrid_EduClsStu: Array<string>) {
        try {
            const responseText = await CurrEduClsStu_DelCurrEduClsStusAsync(arrid_EduClsStu);
            var returnInt: number = responseText;
            if (returnInt > 0) {
                var strInfo: string = `删除记录成功,共删除${returnInt}条记录!`;
                //显示信息框
   ...
   */
    public ShowCurrEduClsStuObj(divContainer: string, objCurrEduClsStu: clsCurrEduClsStuEN) {
        var o = document.getElementById(divContainer);
        if (o == null) {
            alert(`${divContainer}不存在！`);
            return;
        }
        var sstrKeys = clsCommonFunc4Web.GetObjKeys(objCurrEduClsStu);
   ...
   */
    public set hidSortvCurrEduClsStuBy(value: string) {
        $("#hidSortvCurrEduClsStuBy").val(value);
    }
    /*
    * 设置排序字段
   */
    public get hidSortvCurrEduClsStuBy(): string {
   ...
   */
    ...
    * 学生流水号
   */
    public set id_Stu(value: string) {
        $("#txtid_Stu").val(value);
    }
    /*
    * 学生流水号
   */
    public get id_Stu(): string {
