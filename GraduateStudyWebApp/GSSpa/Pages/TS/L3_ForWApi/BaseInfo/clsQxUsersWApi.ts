 ...
 * @returns 对象
 **/
export async function QxUsers_GetObjByUserIdAsync(strUserId: string): Promise<clsQxUsersEN>  
{
...
strInFldName, clsQxUsersEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function QxUsers_GetFirstObjAsync(strWhereCond: string): Promise<clsQxUsersEN>  
{
...
clsQxUsersEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsQxUsersEN._CurrTabName);
...
if (IsNullOrEmpty(pobjQxUsersEN.userId) == false && GetStrLen(pobjQxUsersEN.userId) > 20)
{
...
if (IsNullOrEmpty(pobjQxUsersEN.userName) == false && GetStrLen(pobjQxUsersEN.userName) > 50)
{
