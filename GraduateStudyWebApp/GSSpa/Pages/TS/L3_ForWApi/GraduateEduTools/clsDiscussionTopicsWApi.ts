 ...
 * @returns 对象
 **/
export async function DiscussionTopics_GetObjByTopicsIdAsync(strTopicsId: string): Promise<clsDiscussionTopicsEN>  
{
const strThisFuncName = "GetObjByTopicsIdAsync";

if (IsNullOrEmpty(strTopicsId) == true)
{
  const strMsg = Format("参数:[strTopicsId]不能为空！(In GetObjByTopicsIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strTopicsId.length != 8)
{
const strMsg = Format("缓存分类变量:[strTopicsId]的长度:[{0}]不正确！", strTopicsId.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(strTopicsId) == true)
{
  const strMsg = Format("参数:[strTopicsId]不能为空！(In GetObjByTopicsId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strTopicsId.length != 8)
...
if (IsNullOrEmpty(strTopicsId) == true)
{
  const strMsg = Format("参数:[strTopicsId]不能为空！(In GetObjByTopicsId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
if (strTopicsId.length != 8)
 ...
 * @returns 返回一个输出字段值
*/
export async function DiscussionTopics_func(strInFldName:string , strOutFldName:string , strInValue:string , strid_CurrEduCls_C: string)
{
...
strInFldName, clsDiscussionTopicsEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function DiscussionTopics_GetFirstObjAsync(strWhereCond: string): Promise<clsDiscussionTopicsEN>  
{
...
clsDiscussionTopicsEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsDiscussionTopicsEN._CurrTabName);
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In clsDiscussionTopicsWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
