 ...
 * @returns 对象
 **/
export async function SysComment_GetObjByCommentIdAsync(strCommentId: string): Promise<clsSysCommentEN>  
{
const strThisFuncName = "GetObjByCommentIdAsync";

if (IsNullOrEmpty(strCommentId) == true)
{
  const strMsg = Format("参数:[strCommentId]不能为空！(In GetObjByCommentIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strCommentId.length != 10)
{
const strMsg = Format("缓存分类变量:[strCommentId]的长度:[{0}]不正确！", strCommentId.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(strCommentId) == true)
{
  const strMsg = Format("参数:[strCommentId]不能为空！(In GetObjByCommentId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strCommentId.length != 10)
...
if (IsNullOrEmpty(strCommentId) == true)
{
  const strMsg = Format("参数:[strCommentId]不能为空！(In GetObjByCommentId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
if (strCommentId.length != 10)
 ...
 * @returns 返回一个输出字段值
*/
export async function SysComment_func(strInFldName:string , strOutFldName:string , strInValue:string , strid_CurrEduCls_C: string)
{
...
strInFldName, clsSysCommentEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function SysComment_GetFirstObjAsync(strWhereCond: string): Promise<clsSysCommentEN>  
{
...
clsSysCommentEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsSysCommentEN._CurrTabName);
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In clsSysCommentWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
...
if (IsNullOrEmpty(pobjSysCommentEN.userId) == false && GetStrLen(pobjSysCommentEN.userId) > 20)
{
