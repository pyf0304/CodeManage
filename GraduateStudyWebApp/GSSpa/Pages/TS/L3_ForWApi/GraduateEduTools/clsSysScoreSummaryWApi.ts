 ...
 * @returns 对象
 **/
export async function SysScoreSummary_GetObjBymIdAsync(lngmId: number): Promise<clsSysScoreSummaryEN>  
{
 ...
 * @returns 返回一个输出字段值
*/
export async function SysScoreSummary_func(strInFldName:string , strOutFldName:string , strInValue:number , strid_CurrEduCls_C: string)
{
...
strInFldName, clsSysScoreSummaryEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function SysScoreSummary_GetFirstObjAsync(strWhereCond: string): Promise<clsSysScoreSummaryEN>  
{
...
clsSysScoreSummaryEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsSysScoreSummaryEN._CurrTabName);
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In clsSysScoreSummaryWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
...
if (IsNullOrEmpty(pobjSysScoreSummaryEN.userId) == false && GetStrLen(pobjSysScoreSummaryEN.userId) > 20)
{
