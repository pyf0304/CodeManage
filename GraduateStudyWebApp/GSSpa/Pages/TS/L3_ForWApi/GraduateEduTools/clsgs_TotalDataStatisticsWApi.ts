 ...
 * @returns 对象
 **/
export async function gs_TotalDataStatistics_GetObjByTotalDataIdAsync(strTotalDataId: string): Promise<clsgs_TotalDataStatisticsEN>  
{
 ...
 * @returns 返回一个输出字段值
*/
export async function gs_TotalDataStatistics_func(strInFldName:string , strOutFldName:string , strInValue:string , strid_CurrEduCls_C: string)
{
...
strInFldName, clsgs_TotalDataStatisticsEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function gs_TotalDataStatistics_GetFirstObjAsync(strWhereCond: string): Promise<clsgs_TotalDataStatisticsEN>  
{
...
clsgs_TotalDataStatisticsEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsgs_TotalDataStatisticsEN._CurrTabName);
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In clsgs_TotalDataStatisticsWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
