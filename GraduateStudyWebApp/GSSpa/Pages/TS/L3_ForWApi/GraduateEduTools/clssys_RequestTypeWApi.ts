 ...
 * @returns 对象
 **/
export async function sys_RequestType_GetObjByRequestTypeIdAsync(strRequestTypeId: string): Promise<clssys_RequestTypeEN>  
{
const strThisFuncName = "GetObjByRequestTypeIdAsync";

if (IsNullOrEmpty(strRequestTypeId) == true)
{
  const strMsg = Format("参数:[strRequestTypeId]不能为空！(In GetObjByRequestTypeIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strRequestTypeId.length != 2)
{
const strMsg = Format("缓存分类变量:[strRequestTypeId]的长度:[{0}]不正确！", strRequestTypeId.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(strRequestTypeId) == true)
{
  const strMsg = Format("参数:[strRequestTypeId]不能为空！(In GetObjByRequestTypeId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strRequestTypeId.length != 2)
...
if (IsNullOrEmpty(strRequestTypeId) == true)
{
  const strMsg = Format("参数:[strRequestTypeId]不能为空！(In GetObjByRequestTypeId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
if (strRequestTypeId.length != 2)
...
strInFldName, clssys_RequestTypeEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function sys_RequestType_GetFirstObjAsync(strWhereCond: string): Promise<clssys_RequestTypeEN>  
{
...
clssys_RequestTypeEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clssys_RequestTypeEN._CurrTabName);
