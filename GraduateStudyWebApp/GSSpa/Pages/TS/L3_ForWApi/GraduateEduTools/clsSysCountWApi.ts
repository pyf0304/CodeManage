 ...
 * @returns 对象
 **/
export async function SysCount_GetObjByCountIdAsync(strCountId: string): Promise<clsSysCountEN>  
{
const strThisFuncName = "GetObjByCountIdAsync";

if (IsNullOrEmpty(strCountId) == true)
{
  const strMsg = Format("参数:[strCountId]不能为空！(In GetObjByCountIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strCountId.length != 10)
{
const strMsg = Format("缓存分类变量:[strCountId]的长度:[{0}]不正确！", strCountId.length);
console.error(strMsg);
throw (strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function SysCount_GetFirstObjAsync(strWhereCond: string): Promise<clsSysCountEN>  
{
