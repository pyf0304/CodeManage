 ...
 * @returns 对象
 **/
export async function gs_UserRelaPosition_GetObjBymIdAsync(lngmId: number): Promise<clsgs_UserRelaPositionEN>  
{
 ...
 * @returns 返回一个输出字段值
*/
export async function gs_UserRelaPosition_func(strInFldName:string , strOutFldName:string , strInValue:number , strid_CurrEduCls_C: string)
{
...
strInFldName, clsgs_UserRelaPositionEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function gs_UserRelaPosition_GetFirstObjAsync(strWhereCond: string): Promise<clsgs_UserRelaPositionEN>  
{
...
clsgs_UserRelaPositionEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsgs_UserRelaPositionEN._CurrTabName);
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In clsgs_UserRelaPositionWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
...
if (IsNullOrEmpty(pobjgs_UserRelaPositionEN.userId) == false && GetStrLen(pobjgs_UserRelaPositionEN.userId) > 20)
{
