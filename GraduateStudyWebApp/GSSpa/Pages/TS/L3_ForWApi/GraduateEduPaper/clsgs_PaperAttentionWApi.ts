 ...
 * @returns 对象
 **/
export async function gs_PaperAttention_GetObjByPaperAttentionIdAsync(lngPaperAttentionId: number): Promise<clsgs_PaperAttentionEN>  
{
const strThisFuncName = "GetObjByPaperAttentionIdAsync";

if (lngPaperAttentionId == 0)
{
  const strMsg = Format("参数:[lngPaperAttentionId]不能为空！(In GetObjByPaperAttentionIdAsync)");
...
if (lngPaperAttentionId == 0)
{
  const strMsg = Format("参数:[lngPaperAttentionId]不能为空！(In GetObjByPaperAttentionId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
if (lngPaperAttentionId == 0)
{
  const strMsg = Format("参数:[lngPaperAttentionId]不能为空！(In GetObjByPaperAttentionId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
 ...
 * @returns 返回一个输出字段值
*/
export async function gs_PaperAttention_func(strInFldName:string , strOutFldName:string , strInValue:number , strUserId_C: string)
{
 ...
 * @returns 第一条记录对象
 **/
export async function gs_PaperAttention_GetFirstObjAsync(strWhereCond: string): Promise<clsgs_PaperAttentionEN>  
{
...
clsgs_PaperAttentionEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsgs_PaperAttentionEN._CurrTabName);
...
if (IsNullOrEmpty(strUserId) == true)
{
  const strMsg = Format("参数:[strUserId]不能为空！(In clsgs_PaperAttentionWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
...
if (IsNullOrEmpty(pobjgs_PaperAttentionEN.userId) == false && GetStrLen(pobjgs_PaperAttentionEN.userId) > 20)
{
