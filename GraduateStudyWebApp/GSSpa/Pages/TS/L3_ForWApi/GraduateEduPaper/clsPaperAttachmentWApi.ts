 ...
 * @returns 对象
 **/
export async function PaperAttachment_GetObjByPaperAttachmentIdAsync(lngPaperAttachmentId: number): Promise<clsPaperAttachmentEN>  
{
const strThisFuncName = "GetObjByPaperAttachmentIdAsync";

if (lngPaperAttachmentId == 0)
{
  const strMsg = Format("参数:[lngPaperAttachmentId]不能为空！(In GetObjByPaperAttachmentIdAsync)");
...
if (lngPaperAttachmentId == 0)
{
  const strMsg = Format("参数:[lngPaperAttachmentId]不能为空！(In GetObjByPaperAttachmentId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
if (lngPaperAttachmentId == 0)
{
  const strMsg = Format("参数:[lngPaperAttachmentId]不能为空！(In GetObjByPaperAttachmentId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
 ...
 * @returns 返回一个输出字段值
*/
export async function PaperAttachment_func(strInFldName:string , strOutFldName:string , strInValue:number , strid_CurrEduCls_C: string)
{
...
strInFldName, clsPaperAttachmentEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function PaperAttachment_GetFirstObjAsync(strWhereCond: string): Promise<clsPaperAttachmentEN>  
{
...
clsPaperAttachmentEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsPaperAttachmentEN._CurrTabName);
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In clsPaperAttachmentWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
