 ...
 * @returns 对象
 **/
export async function PaperEduClsRela_GetObjBymIdAsync(lngmId: number): Promise<clsPaperEduClsRelaEN>  
{
 ...
 * @returns 返回一个输出字段值
*/
export async function PaperEduClsRela_func(strInFldName:string , strOutFldName:string , strInValue:number , strid_CurrEduCls_C: string)
{
...
strInFldName, clsPaperEduClsRelaEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
...
if (a.paperId == b.paperId) return a.id_CurrEduCls.localeCompare(b.id_CurrEduCls);
 ...
 * @returns 第一条记录对象
 **/
export async function PaperEduClsRela_GetFirstObjAsync(strWhereCond: string): Promise<clsPaperEduClsRelaEN>  
{
...
clsPaperEduClsRelaEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsPaperEduClsRelaEN._CurrTabName);
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In clsPaperEduClsRelaWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
