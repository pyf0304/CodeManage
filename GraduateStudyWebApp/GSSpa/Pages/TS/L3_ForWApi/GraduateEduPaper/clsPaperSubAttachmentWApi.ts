 ...
 * @returns 对象
 **/
export async function PaperSubAttachment_GetObjByPaperSubAttachmentIdAsync(lngPaperSubAttachmentId: number): Promise<clsPaperSubAttachmentEN>  
{
...
if (lngPaperSubAttachmentId == 0)
{
  const strMsg = Format("参数:[lngPaperSubAttachmentId]不能为空！(In GetObjByPaperSubAttachmentId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
if (lngPaperSubAttachmentId == 0)
{
  const strMsg = Format("参数:[lngPaperSubAttachmentId]不能为空！(In GetObjByPaperSubAttachmentId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
...
strInFldName, clsPaperSubAttachmentEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function PaperSubAttachment_GetFirstObjAsync(strWhereCond: string): Promise<clsPaperSubAttachmentEN>  
{
...
clsPaperSubAttachmentEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsPaperSubAttachmentEN._CurrTabName);
...
{
let strWhereCond = " 1 = 1 ";
 strWhereCond +=  Format(" and SubViewpointId = '{0}'", objPaperSubAttachmentEN.subViewpointId);
