 ...
 * @returns 对象
 **/
export async function vPaperSim_GetObjByPaperIdAsync(strPaperId: string): Promise<clsvPaperSimEN>  
{
...
try
{
const arrvPaperSim_Sel: Array <clsvPaperSimEN> = arrvPaperSimObjLst_Cache.filter(x => x.paperId == strPaperId);
...
strInFldName, clsvPaperSimEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function vPaperSim_GetFirstObjAsync(strWhereCond: string): Promise<clsvPaperSimEN>  
{
...
clsvPaperSimEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsvPaperSimEN._CurrTabName);
...
{
const arrvPaperSimObjLst_Cache = await vPaperSim_GetObjLst_Cache();
const arrvPaperSim_Sel: Array <clsvPaperSimEN> = arrvPaperSimObjLst_Cache.filter(x => arrPaperIdLst.indexOf(x.paperId)>-1);
return arrvPaperSim_Sel;
}
catch (e)
{
...
if (Object.prototype.hasOwnProperty.call(objvPaperSim_Cond.dicFldComparisonOp, clsvPaperSimEN.con_IsGSuser) == true)
{
if (objvPaperSim_Cond.isGSuser == true)
{
strWhereCond += Format(" And {0} = '1'", clsvPaperSimEN.con_IsGSuser);
}
else
{
strWhereCond += Format(" And {0} = '0'", clsvPaperSimEN.con_IsGSuser);
}
}
 return strWhereCond;
}
