 ...
 * @returns 对象
 **/
export async function gs_RTqa_PaperRela_GetObjBymIdAsync(lngmId: number): Promise<clsgs_RTqa_PaperRelaEN>  
{
 ...
 * @returns 返回一个输出字段值
*/
export async function gs_RTqa_PaperRela_func(strInFldName:string , strOutFldName:string , strInValue:number , strTopicId_C: string)
{
...
strInFldName, clsgs_RTqa_PaperRelaEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function gs_RTqa_PaperRela_GetFirstObjAsync(strWhereCond: string): Promise<clsgs_RTqa_PaperRelaEN>  
{
...
clsgs_RTqa_PaperRelaEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsgs_RTqa_PaperRelaEN._CurrTabName);
...
if (IsNullOrEmpty(strTopicId) == true)
{
  const strMsg = Format("参数:[strTopicId]不能为空！(In clsgs_RTqa_PaperRelaWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strTopicId.length != 8)
