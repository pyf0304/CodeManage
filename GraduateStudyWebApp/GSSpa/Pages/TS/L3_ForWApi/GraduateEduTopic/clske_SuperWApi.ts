 ...
 * @returns 对象
 **/
export async function ke_Super_GetObjByke_SuperIdAsync(strke_SuperId: string): Promise<clske_SuperEN>  
{
const strThisFuncName = "GetObjByke_SuperIdAsync";

if (IsNullOrEmpty(strke_SuperId) == true)
{
  const strMsg = Format("参数:[strke_SuperId]不能为空！(In GetObjByke_SuperIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strke_SuperId.length != 4)
{
const strMsg = Format("缓存分类变量:[strke_SuperId]的长度:[{0}]不正确！", strke_SuperId.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(strke_SuperId) == true)
{
  const strMsg = Format("参数:[strke_SuperId]不能为空！(In GetObjByke_SuperId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strke_SuperId.length != 4)
...
{
objke_Super = await ke_Super_GetObjByke_SuperIdAsync(strke_SuperId);
if (objke_Super != null)
{
...
if (IsNullOrEmpty(strke_SuperId) == true)
{
  const strMsg = Format("参数:[strke_SuperId]不能为空！(In GetObjByke_SuperId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
if (strke_SuperId.length != 4)
...
if (IsNullOrEmpty(strke_SuperId) == true)
{
  const strMsg = Format("参数:[strke_SuperId]不能为空！(In GetNameByke_SuperId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strke_SuperId.length != 4)
...
strInFldName, clske_SuperEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function ke_Super_GetFirstObjAsync(strWhereCond: string): Promise<clske_SuperEN>  
{
...
clske_SuperEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clske_SuperEN._CurrTabName);
