 ...
 * @returns 对象
 **/
export async function Concept_GetObjByConceptIdAsync(strConceptId: string): Promise<clsConceptEN>  
{
 ...
 * @returns 返回一个输出字段值
*/
export async function Concept_func(strInFldName:string , strOutFldName:string , strInValue:string , strid_CurrEduCls_C: string)
{
...
strInFldName, clsConceptEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function Concept_GetFirstObjAsync(strWhereCond: string): Promise<clsConceptEN>  
{
...
clsConceptEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsConceptEN._CurrTabName);
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In clsConceptWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
