 ...
 * @returns 对象
 **/
export async function vViewpoint_GetObjByViewpointIdAsync(strViewpointId: string): Promise<clsvViewpointEN>  
{
const strThisFuncName = "GetObjByViewpointIdAsync";

if (IsNullOrEmpty(strViewpointId) == true)
{
  const strMsg = Format("参数:[strViewpointId]不能为空！(In GetObjByViewpointIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strViewpointId.length != 8)
{
const strMsg = Format("缓存分类变量:[strViewpointId]的长度:[{0}]不正确！", strViewpointId.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(strViewpointId) == true)
{
  const strMsg = Format("参数:[strViewpointId]不能为空！(In GetObjByViewpointId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strViewpointId.length != 8)
...
if (IsNullOrEmpty(strViewpointId) == true)
{
  const strMsg = Format("参数:[strViewpointId]不能为空！(In GetObjByViewpointId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
if (strViewpointId.length != 8)
 ...
 * @returns 返回一个输出字段值
*/
export async function vViewpoint_func(strInFldName:string , strOutFldName:string , strInValue:string , strid_CurrEduCls_C: string)
{
...
strInFldName, clsvViewpointEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function vViewpoint_GetFirstObjAsync(strWhereCond: string): Promise<clsvViewpointEN>  
{
...
clsvViewpointEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsvViewpointEN._CurrTabName);
