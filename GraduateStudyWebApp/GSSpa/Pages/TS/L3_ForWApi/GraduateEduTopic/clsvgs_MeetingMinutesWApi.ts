 ...
 * @returns 对象
 **/
export async function vgs_MeetingMinutes_GetObjByMeetingIdAsync(strMeetingId: string): Promise<clsvgs_MeetingMinutesEN>  
{
const strThisFuncName = "GetObjByMeetingIdAsync";

if (IsNullOrEmpty(strMeetingId) == true)
{
  const strMsg = Format("参数:[strMeetingId]不能为空！(In GetObjByMeetingIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strMeetingId.length != 10)
{
const strMsg = Format("缓存分类变量:[strMeetingId]的长度:[{0}]不正确！", strMeetingId.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(strMeetingId) == true)
{
  const strMsg = Format("参数:[strMeetingId]不能为空！(In GetObjByMeetingId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strMeetingId.length != 10)
...
if (IsNullOrEmpty(strMeetingId) == true)
{
  const strMsg = Format("参数:[strMeetingId]不能为空！(In GetObjByMeetingId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
if (strMeetingId.length != 10)
...
strInFldName, clsvgs_MeetingMinutesEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function vgs_MeetingMinutes_GetFirstObjAsync(strWhereCond: string): Promise<clsvgs_MeetingMinutesEN>  
{
...
clsvgs_MeetingMinutesEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsvgs_MeetingMinutesEN._CurrTabName);
