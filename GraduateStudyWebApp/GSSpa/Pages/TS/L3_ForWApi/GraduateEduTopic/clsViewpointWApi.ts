 ...
 * @returns 对象
 **/
export async function Viewpoint_GetObjByViewpointIdAsync(strViewpointId: string): Promise<clsViewpointEN>  
{
 ...
 * @returns 返回一个输出字段值
*/
export async function Viewpoint_func(strInFldName:string , strOutFldName:string , strInValue:string , strid_CurrEduCls_C: string)
{
...
strInFldName, clsViewpointEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function Viewpoint_GetFirstObjAsync(strWhereCond: string): Promise<clsViewpointEN>  
{
...
clsViewpointEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsViewpointEN._CurrTabName);
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In clsViewpointWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
