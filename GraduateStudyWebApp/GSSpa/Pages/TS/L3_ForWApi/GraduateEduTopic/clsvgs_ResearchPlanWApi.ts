 ...
 * @returns 对象
 **/
export async function vgs_ResearchPlan_GetObjByPlanIdAsync(strPlanId: string): Promise<clsvgs_ResearchPlanEN>  
{
...
if (IsNullOrEmpty(strPlanId) == true)
{
  const strMsg = Format("参数:[strPlanId]不能为空！(In GetNameByPlanId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strPlanId.length != 8)
 ...
 * @returns 返回一个输出字段值
*/
export async function vgs_ResearchPlan_func(strInFldName:string , strOutFldName:string , strInValue:string , strid_CurrEduCls_C: string)
{
...
strInFldName, clsvgs_ResearchPlanEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function vgs_ResearchPlan_GetFirstObjAsync(strWhereCond: string): Promise<clsvgs_ResearchPlanEN>  
{
...
clsvgs_ResearchPlanEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsvgs_ResearchPlanEN._CurrTabName);
