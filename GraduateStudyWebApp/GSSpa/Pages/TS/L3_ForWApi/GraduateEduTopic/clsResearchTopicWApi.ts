 ...
 * @returns 对象
 **/
export async function ResearchTopic_GetObjByTopicIdAsync(strTopicId: string): Promise<clsResearchTopicEN>  
{
 ...
 * @returns 返回一个输出字段值
*/
export async function ResearchTopic_func(strInFldName:string , strOutFldName:string , strInValue:string , strid_CurrEduCls_C: string)
{
...
strInFldName, clsResearchTopicEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function ResearchTopic_GetFirstObjAsync(strWhereCond: string): Promise<clsResearchTopicEN>  
{
...
clsResearchTopicEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsResearchTopicEN._CurrTabName);
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In clsResearchTopicWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
