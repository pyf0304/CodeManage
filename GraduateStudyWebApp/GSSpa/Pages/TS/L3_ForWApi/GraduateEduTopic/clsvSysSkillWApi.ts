 ...
 * @returns 对象
 **/
export async function vSysSkill_GetObjBySkillIdAsync(strSkillId: string): Promise<clsvSysSkillEN>  
{
...
if (IsNullOrEmpty(strSkillId) == true)
{
  const strMsg = Format("参数:[strSkillId]不能为空！(In GetNameBySkillId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strSkillId.length != 10)
 ...
 * @returns 返回一个输出字段值
*/
export async function vSysSkill_func(strInFldName:string , strOutFldName:string , strInValue:string , strid_CurrEduCls_C: string)
{
...
strInFldName, clsvSysSkillEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function vSysSkill_GetFirstObjAsync(strWhereCond: string): Promise<clsvSysSkillEN>  
{
...
clsvSysSkillEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsvSysSkillEN._CurrTabName);
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In BindDdl_SkillIdInDiv)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
