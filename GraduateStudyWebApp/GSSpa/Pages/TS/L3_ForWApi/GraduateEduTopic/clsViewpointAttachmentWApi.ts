 ...
 * @returns 对象
 **/
export async function ViewpointAttachment_GetObjByViewpointAttachmentIdAsync(lngViewpointAttachmentId: number): Promise<clsViewpointAttachmentEN>  
{
...
if (lngViewpointAttachmentId == 0)
{
  const strMsg = Format("参数:[lngViewpointAttachmentId]不能为空！(In GetObjByViewpointAttachmentId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
if (lngViewpointAttachmentId == 0)
{
  const strMsg = Format("参数:[lngViewpointAttachmentId]不能为空！(In GetObjByViewpointAttachmentId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
 ...
 * @returns 返回一个输出字段值
*/
export async function ViewpointAttachment_func(strInFldName:string , strOutFldName:string , strInValue:number , strid_CurrEduCls_C: string)
{
...
strInFldName, clsViewpointAttachmentEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function ViewpointAttachment_GetFirstObjAsync(strWhereCond: string): Promise<clsViewpointAttachmentEN>  
{
...
clsViewpointAttachmentEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsViewpointAttachmentEN._CurrTabName);
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In clsViewpointAttachmentWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
