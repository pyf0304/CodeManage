 ...
 * @returns 对象
 **/
export async function gs_ResearchPlan_GetObjByPlanIdAsync(strPlanId: string): Promise<clsgs_ResearchPlanEN>  
{
const strThisFuncName = "GetObjByPlanIdAsync";

if (IsNullOrEmpty(strPlanId) == true)
{
  const strMsg = Format("参数:[strPlanId]不能为空！(In GetObjByPlanIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strPlanId.length != 8)
{
const strMsg = Format("缓存分类变量:[strPlanId]的长度:[{0}]不正确！", strPlanId.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(strPlanId) == true)
{
  const strMsg = Format("参数:[strPlanId]不能为空！(In GetObjByPlanId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strPlanId.length != 8)
...
if (IsNullOrEmpty(strPlanId) == true)
{
  const strMsg = Format("参数:[strPlanId]不能为空！(In GetObjByPlanId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
if (strPlanId.length != 8)
 ...
 * @returns 返回一个输出字段值
*/
export async function gs_ResearchPlan_func(strInFldName:string , strOutFldName:string , strInValue:string , strid_CurrEduCls_C: string)
{
...
strInFldName, clsgs_ResearchPlanEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function gs_ResearchPlan_GetFirstObjAsync(strWhereCond: string): Promise<clsgs_ResearchPlanEN>  
{
...
clsgs_ResearchPlanEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsgs_ResearchPlanEN._CurrTabName);
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In clsgs_ResearchPlanWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
