 ...
 * @returns 对象
 **/
export async function SysSocialRelations_GetObjBySocialIdAsync(strSocialId: string): Promise<clsSysSocialRelationsEN>  
{
 ...
 * @returns 返回一个输出字段值
*/
export async function SysSocialRelations_func(strInFldName:string , strOutFldName:string , strInValue:string , strid_CurrEduCls_C: string)
{
...
strInFldName, clsSysSocialRelationsEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function SysSocialRelations_GetFirstObjAsync(strWhereCond: string): Promise<clsSysSocialRelationsEN>  
{
...
clsSysSocialRelationsEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsSysSocialRelationsEN._CurrTabName);
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In clsSysSocialRelationsWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
