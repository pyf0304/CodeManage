 ...
 * @returns 对象
 **/
export async function LoginLog_GetObjByLoginLogIdAsync(lngLoginLogId: number): Promise<clsLoginLogEN>  
{
const strThisFuncName = "GetObjByLoginLogIdAsync";

if (lngLoginLogId == 0)
{
  const strMsg = Format("参数:[lngLoginLogId]不能为空！(In GetObjByLoginLogIdAsync)");
...
if (lngLoginLogId == 0)
{
  const strMsg = Format("参数:[lngLoginLogId]不能为空！(In GetObjByLoginLogId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
if (lngLoginLogId == 0)
{
  const strMsg = Format("参数:[lngLoginLogId]不能为空！(In GetObjByLoginLogId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
...
strInFldName, clsLoginLogEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function LoginLog_GetFirstObjAsync(strWhereCond: string): Promise<clsLoginLogEN>  
{
...
clsLoginLogEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsLoginLogEN._CurrTabName);
