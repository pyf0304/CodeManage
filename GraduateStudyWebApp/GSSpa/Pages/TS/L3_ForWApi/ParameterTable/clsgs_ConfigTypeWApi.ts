 ...
 * @returns 对象
 **/
export async function gs_ConfigType_GetObjByConfigTypeIdAsync(strConfigTypeId: string): Promise<clsgs_ConfigTypeEN>  
{
const strThisFuncName = "GetObjByConfigTypeIdAsync";

if (IsNullOrEmpty(strConfigTypeId) == true)
{
  const strMsg = Format("参数:[strConfigTypeId]不能为空！(In GetObjByConfigTypeIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strConfigTypeId.length != 2)
{
const strMsg = Format("缓存分类变量:[strConfigTypeId]的长度:[{0}]不正确！", strConfigTypeId.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(strConfigTypeId) == true)
{
  const strMsg = Format("参数:[strConfigTypeId]不能为空！(In GetObjByConfigTypeId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strConfigTypeId.length != 2)
...
if (IsNullOrEmpty(strConfigTypeId) == true)
{
  const strMsg = Format("参数:[strConfigTypeId]不能为空！(In GetObjByConfigTypeId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
if (strConfigTypeId.length != 2)
...
strInFldName, clsgs_ConfigTypeEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function gs_ConfigType_GetFirstObjAsync(strWhereCond: string): Promise<clsgs_ConfigTypeEN>  
{
...
clsgs_ConfigTypeEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsgs_ConfigTypeEN._CurrTabName);
