 ...
 * @returns 对象
 **/
export async function gs_Color_GetObjByColorIdAsync(strColorId: string): Promise<clsgs_ColorEN>  
{
const strThisFuncName = "GetObjByColorIdAsync";

if (IsNullOrEmpty(strColorId) == true)
{
  const strMsg = Format("参数:[strColorId]不能为空！(In GetObjByColorIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strColorId.length != 2)
{
const strMsg = Format("缓存分类变量:[strColorId]的长度:[{0}]不正确！", strColorId.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(strColorId) == true)
{
  const strMsg = Format("参数:[strColorId]不能为空！(In GetObjByColorId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strColorId.length != 2)
...
if (IsNullOrEmpty(strColorId) == true)
{
  const strMsg = Format("参数:[strColorId]不能为空！(In GetObjByColorId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
if (strColorId.length != 2)
...
strInFldName, clsgs_ColorEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function gs_Color_GetFirstObjAsync(strWhereCond: string): Promise<clsgs_ColorEN>  
{
...
clsgs_ColorEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsgs_ColorEN._CurrTabName);
