 ...
 * @returns 对象
 **/
export async function SysScoreType_GetObjByScoreTypeIdAsync(strScoreTypeId: string): Promise<clsSysScoreTypeEN>  
{
const strThisFuncName = "GetObjByScoreTypeIdAsync";

if (IsNullOrEmpty(strScoreTypeId) == true)
{
  const strMsg = Format("参数:[strScoreTypeId]不能为空！(In GetObjByScoreTypeIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strScoreTypeId.length != 4)
{
const strMsg = Format("缓存分类变量:[strScoreTypeId]的长度:[{0}]不正确！", strScoreTypeId.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(strScoreTypeId) == true)
{
  const strMsg = Format("参数:[strScoreTypeId]不能为空！(In GetObjByScoreTypeId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strScoreTypeId.length != 4)
...
if (IsNullOrEmpty(strScoreTypeId) == true)
{
  const strMsg = Format("参数:[strScoreTypeId]不能为空！(In GetObjByScoreTypeId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
if (strScoreTypeId.length != 4)
...
if (IsNullOrEmpty(strScoreTypeId) == true)
{
  const strMsg = Format("参数:[strScoreTypeId]不能为空！(In GetNameByScoreTypeId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strScoreTypeId.length != 4)
...
strInFldName, clsSysScoreTypeEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function SysScoreType_GetFirstObjAsync(strWhereCond: string): Promise<clsSysScoreTypeEN>  
{
...
clsSysScoreTypeEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsSysScoreTypeEN._CurrTabName);
