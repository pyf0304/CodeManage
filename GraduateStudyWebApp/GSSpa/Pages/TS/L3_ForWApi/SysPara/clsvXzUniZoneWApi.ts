 ...
 * @returns 对象
 **/
export async function vXzUniZone_GetObjByid_UniZoneAsync(strid_UniZone: string): Promise<clsvXzUniZoneEN>  
{
const strThisFuncName = "GetObjByid_UniZoneAsync";

if (IsNullOrEmpty(strid_UniZone) == true)
{
  const strMsg = Format("参数:[strid_UniZone]不能为空！(In GetObjByid_UniZoneAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_UniZone.length != 4)
{
const strMsg = Format("缓存分类变量:[strid_UniZone]的长度:[{0}]不正确！", strid_UniZone.length);
console.error(strMsg);
throw (strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function vXzUniZone_GetFirstObjAsync(strWhereCond: string): Promise<clsvXzUniZoneEN>  
{
