 ...
 * @returns 对象
 **/
export async function qa_Paper_GetObjByqa_PaperIdAsync(strqa_PaperId: string): Promise<clsqa_PaperEN>  
{
 ...
 * @returns 返回一个输出字段值
*/
export async function qa_Paper_func(strInFldName:string , strOutFldName:string , strInValue:string , strid_CurrEduCls_C: string)
{
...
strInFldName, clsqa_PaperEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function qa_Paper_GetFirstObjAsync(strWhereCond: string): Promise<clsqa_PaperEN>  
{
...
clsqa_PaperEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsqa_PaperEN._CurrTabName);
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In clsqa_PaperWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
