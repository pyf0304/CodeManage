 ...
 * @returns 对象
 **/
export async function qa_Answer_GetObjByAnswerIdAsync(strAnswerId: string): Promise<clsqa_AnswerEN>  
{
const strThisFuncName = "GetObjByAnswerIdAsync";

if (IsNullOrEmpty(strAnswerId) == true)
{
  const strMsg = Format("参数:[strAnswerId]不能为空！(In GetObjByAnswerIdAsync)");
console.error(strMsg);
 throw (strMsg);
}
if (strAnswerId.length != 10)
{
const strMsg = Format("缓存分类变量:[strAnswerId]的长度:[{0}]不正确！", strAnswerId.length);
console.error(strMsg);
throw (strMsg);
}
...
if (IsNullOrEmpty(strAnswerId) == true)
{
  const strMsg = Format("参数:[strAnswerId]不能为空！(In GetObjByAnswerId_Cache)");
console.error(strMsg);
 throw (strMsg);
}
if (strAnswerId.length != 10)
...
if (IsNullOrEmpty(strAnswerId) == true)
{
  const strMsg = Format("参数:[strAnswerId]不能为空！(In GetObjByAnswerId_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
if (strAnswerId.length != 10)
 ...
 * @returns 返回一个输出字段值
*/
export async function qa_Answer_func(strInFldName:string , strOutFldName:string , strInValue:string , strTopicId_C: string)
{
...
strInFldName, clsqa_AnswerEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function qa_Answer_GetFirstObjAsync(strWhereCond: string): Promise<clsqa_AnswerEN>  
{
...
clsqa_AnswerEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clsqa_AnswerEN._CurrTabName);
...
if (IsNullOrEmpty(strTopicId) == true)
{
  const strMsg = Format("参数:[strTopicId]不能为空！(In clsqa_AnswerWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strTopicId.length != 8)
