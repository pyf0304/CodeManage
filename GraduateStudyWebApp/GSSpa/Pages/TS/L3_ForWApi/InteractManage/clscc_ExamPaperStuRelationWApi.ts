 ...
 * @returns 对象
 **/
export async function cc_ExamPaperStuRelation_GetObjById_ExamPaperStuRelationAsync(lngId_ExamPaperStuRelation: number): Promise<clscc_ExamPaperStuRelationEN>  
{
const strThisFuncName = "GetObjById_ExamPaperStuRelationAsync";

if (lngId_ExamPaperStuRelation == 0)
{
  const strMsg = Format("参数:[lngId_ExamPaperStuRelation]不能为空！(In GetObjById_ExamPaperStuRelationAsync)");
...
if (lngId_ExamPaperStuRelation == 0)
{
  const strMsg = Format("参数:[lngId_ExamPaperStuRelation]不能为空！(In GetObjById_ExamPaperStuRelation_Cache)");
console.error(strMsg);
 throw (strMsg);
}
...
if (lngId_ExamPaperStuRelation == 0)
{
  const strMsg = Format("参数:[lngId_ExamPaperStuRelation]不能为空！(In GetObjById_ExamPaperStuRelation_localStorage)");
console.error(strMsg);
 throw (strMsg);
}
 ...
 * @returns 返回一个输出字段值
*/
export async function cc_ExamPaperStuRelation_func(strInFldName:string , strOutFldName:string , strInValue:number , strid_CurrEduCls_C: string)
{
...
strInFldName, clscc_ExamPaperStuRelationEN.AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 第一条记录对象
 **/
export async function cc_ExamPaperStuRelation_GetFirstObjAsync(strWhereCond: string): Promise<clscc_ExamPaperStuRelationEN>  
{
...
clscc_ExamPaperStuRelationEN.CacheAddiCondition = GetSession_CacheAdditionCondition(clscc_ExamPaperStuRelationEN._CurrTabName);
...
if (IsNullOrEmpty(strid_CurrEduCls) == true)
{
  const strMsg = Format("参数:[strid_CurrEduCls]不能为空！(In clscc_ExamPaperStuRelationWApi.ReFreshCache)");
console.error(strMsg);
 throw (strMsg);
}
if (strid_CurrEduCls.length != 8)
