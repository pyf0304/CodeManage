import { GetLst_TabCacheAdditionCondition } from "../FunClass/clsPubVar4Web.js";
...
export class CacheHelper {
    ...
    public static CacheLength(): number {let intCount = 0;
    ...
    /// 获取缓存
    /// </summary>
    public static Key(index: number) {let rlt = "";
    ...
    /// <returns></returns>
    public static GetCache(key: string, func: (strKey: string) => any) {let t: any;
        if (CacheHelper.Exsits(key)) {
            //缓存存在，直接获取原数据
    ...
    /// <param name="func">委托方法 传入操作对象</param>
    /// <returns></returns>
    public static GetCacheByCondition(key: string, strCondition: string, func: (strCondition: string) => any) {let t: any;
        if (CacheHelper.Exsits(key)) {
            //缓存存在，直接获取原数据
            t = CacheHelper.Get(key);
        }
    ...
    public static GetCacheByConditionAsync(key: string, strCondition: string, func: (strCondition: string) => any): Promise<any> {let t: any;
        if (CacheHelper.Exsits(key)) {
            //缓存存在，直接获取原数据
            t = CacheHelper.Get(key);
        }
    ...
    public static getBytes(str: string): number {
        const len = str.length;let bytes = len;
        for (let i = 0; i < len; i++) {
    ...
    public static localStorageSubTotals(): number {
        let strKey; let myCache; let strCacheKeyLst = "";let intTotals = 0;
    ...
    public static sessionStorageSubTotals(): number{let strKey;let myCache;let strCacheKeyLst = "";let intTotals = 0;
