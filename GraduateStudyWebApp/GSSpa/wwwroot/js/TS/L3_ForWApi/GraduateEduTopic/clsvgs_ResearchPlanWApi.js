...
(function (factory) {
        ...
        try {
            ...
            if (arrvgs_ResearchPlan_Sel.length > 0) {
                ...
                if (bolTryAsyncOnce == true) {
                    objvgs_ResearchPlan = await vgs_ResearchPlan_GetObjByPlanIdAsync(strPlanId);
    ...
    */
    async function vgs_ResearchPlan_GetNameByPlanId_Cache(strPlanId, strid_CurrEduCls) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strPlanId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strPlanId]不能为空！(In GetNameByPlanId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strPlanId.length != 8) {
    ...
    */
    async function vgs_ResearchPlan_func(strInFldName, strOutFldName, strInValue, strid_CurrEduCls_C) {
        ...
        if (clsvgs_ResearchPlanEN_js_1.clsvgs_ResearchPlanEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_1.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strInFldName, clsvgs_ResearchPlanEN_js_1.clsvgs_ResearchPlanEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
    ...
    **/
    async function vgs_ResearchPlan_GetObjLst_ClientCache(strid_CurrEduCls) {
        ...
        clsvgs_ResearchPlanEN_js_1.clsvgs_ResearchPlanEN.CacheAddiCondition = (0, CacheHelper_js_1.GetSession_CacheAdditionCondition)(clsvgs_ResearchPlanEN_js_1.clsvgs_ResearchPlanEN._CurrTabName);
        ...
        if (CacheHelper_js_2.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrvgs_ResearchPlanExObjLst_Cache = CacheHelper_js_2.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_2.CacheHelper.Add(strKey, arrvgs_ResearchPlanExObjLst);
