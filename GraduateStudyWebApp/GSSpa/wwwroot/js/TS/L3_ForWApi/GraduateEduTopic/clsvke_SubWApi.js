...
(function (factory) {
    ...
    */
    async function vke_Sub_GetNameByke_SubId_Cache(strke_SubId) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strke_SubId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strke_SubId]不能为空！(In GetNameByke_SubId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strke_SubId.length != 4) {
    ...
    */
    async function vke_Sub_func(strInFldName, strOutFldName, strInValue) {
        ...
        if (clsvke_SubEN_js_1.clsvke_SubEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_1.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strInFldName, clsvke_SubEN_js_1.clsvke_SubEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
    ...
    **/
    async function vke_Sub_GetObjLst_ClientCache() {
        ...
        clsvke_SubEN_js_1.clsvke_SubEN.CacheAddiCondition = (0, CacheHelper_js_1.GetSession_CacheAdditionCondition)(clsvke_SubEN_js_1.clsvke_SubEN._CurrTabName);
        ...
        if (CacheHelper_js_2.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrvke_SubExObjLst_Cache = CacheHelper_js_2.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_2.CacheHelper.Add(strKey, arrvke_SubExObjLst);
