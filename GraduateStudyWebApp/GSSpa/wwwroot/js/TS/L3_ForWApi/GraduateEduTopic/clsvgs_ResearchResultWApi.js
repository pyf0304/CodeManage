...
(function (factory) {
    ...
    */
    async function vgs_ResearchResult_GetObjByResultId_Cache(strResultId, bolTryAsyncOnce = true) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strResultId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strResultId]不能为空！(In GetObjByResultId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strResultId.length != 10) {
    ...
    */
    async function vgs_ResearchResult_GetObjByResultId_localStorage(strResultId) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strResultId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strResultId]不能为空！(In GetObjByResultId_localStorage)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strResultId.length != 10) {
    ...
    */
    async function vgs_ResearchResult_func(strInFldName, strOutFldName, strInValue) {
        ...
        if (clsvgs_ResearchResultEN_js_1.clsvgs_ResearchResultEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_1.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strInFldName, clsvgs_ResearchResultEN_js_1.clsvgs_ResearchResultEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
    ...
    **/
    async function vgs_ResearchResult_GetObjLst_ClientCache() {
        ...
        clsvgs_ResearchResultEN_js_1.clsvgs_ResearchResultEN.CacheAddiCondition = (0, CacheHelper_js_1.GetSession_CacheAdditionCondition)(clsvgs_ResearchResultEN_js_1.clsvgs_ResearchResultEN._CurrTabName);
        ...
        if (CacheHelper_js_2.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrvgs_ResearchResultExObjLst_Cache = CacheHelper_js_2.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_2.CacheHelper.Add(strKey, arrvgs_ResearchResultExObjLst);
