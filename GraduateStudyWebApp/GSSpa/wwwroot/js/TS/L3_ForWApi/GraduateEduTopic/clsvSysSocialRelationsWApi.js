...
(function (factory) {
    ...
    */
    async function vSysSocialRelations_GetNameBySocialId_Cache(strSocialId, strid_CurrEduCls) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strSocialId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strSocialId]不能为空！(In GetNameBySocialId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strSocialId.length != 10) {
    ...
    */
    async function vSysSocialRelations_func(strInFldName, strOutFldName, strInValue, strid_CurrEduCls_C) {
        ...
        if (clsvSysSocialRelationsEN_js_1.clsvSysSocialRelationsEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_1.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strInFldName, clsvSysSocialRelationsEN_js_1.clsvSysSocialRelationsEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
    ...
    **/
    async function vSysSocialRelations_GetObjLst_ClientCache(strid_CurrEduCls) {
        ...
        clsvSysSocialRelationsEN_js_1.clsvSysSocialRelationsEN.CacheAddiCondition = (0, CacheHelper_js_1.GetSession_CacheAdditionCondition)(clsvSysSocialRelationsEN_js_1.clsvSysSocialRelationsEN._CurrTabName);
        ...
        if (CacheHelper_js_2.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrvSysSocialRelationsExObjLst_Cache = CacheHelper_js_2.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_2.CacheHelper.Add(strKey, arrvSysSocialRelationsExObjLst);
    ...
    */
    async function vSysSocialRelations__Cache(strDivName, strDdlName, strid_CurrEduCls) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strid_CurrEduCls) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strid_CurrEduCls]不能为空！(In )");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strid_CurrEduCls.length != 8) {
