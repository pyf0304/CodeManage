...
(function (factory) {
        ...
        try {
            ...
            if (arrvSysSkill_Sel.length > 0) {
                ...
                if (bolTryAsyncOnce == true) {
                    objvSysSkill = await vSysSkill_GetObjBySkillIdAsync(strSkillId);
    ...
    */
    async function vSysSkill_GetNameBySkillId_Cache(strSkillId, strid_CurrEduCls) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strSkillId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strSkillId]不能为空！(In GetNameBySkillId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strSkillId.length != 10) {
    ...
    */
    async function vSysSkill_func(strInFldName, strOutFldName, strInValue, strid_CurrEduCls_C) {
        ...
        if (clsvSysSkillEN_js_1.clsvSysSkillEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_1.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strInFldName, clsvSysSkillEN_js_1.clsvSysSkillEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
    ...
    **/
    async function vSysSkill_GetObjLst_ClientCache(strid_CurrEduCls) {
        ...
        clsvSysSkillEN_js_1.clsvSysSkillEN.CacheAddiCondition = (0, CacheHelper_js_1.GetSession_CacheAdditionCondition)(clsvSysSkillEN_js_1.clsvSysSkillEN._CurrTabName);
        ...
        if (CacheHelper_js_2.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrvSysSkillExObjLst_Cache = CacheHelper_js_2.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_2.CacheHelper.Add(strKey, arrvSysSkillExObjLst);
    ...
    */
    async function vSysSkill_BindDdl_SkillIdInDiv_Cache(strDivName, strDdlName, strid_CurrEduCls) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strid_CurrEduCls) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strid_CurrEduCls]不能为空！(In BindDdl_SkillIdInDiv)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strid_CurrEduCls.length != 8) {
