...
(function (factory) {
    ...
    */
    async function vSysScoreWeight_GetObjByScoreWeightId_Cache(strScoreWeightId, bolTryAsyncOnce = true) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strScoreWeightId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strScoreWeightId]不能为空！(In GetObjByScoreWeightId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strScoreWeightId.length != 1) {
    ...
    */
    async function vSysScoreWeight_GetObjByScoreWeightId_localStorage(strScoreWeightId) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strScoreWeightId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strScoreWeightId]不能为空！(In GetObjByScoreWeightId_localStorage)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strScoreWeightId.length != 1) {
    ...
    */
    async function vSysScoreWeight_func(strInFldName, strOutFldName, strInValue) {
        ...
        if (clsvSysScoreWeightEN_js_1.clsvSysScoreWeightEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_1.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strInFldName, clsvSysScoreWeightEN_js_1.clsvSysScoreWeightEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
    ...
    **/
    async function vSysScoreWeight_GetObjLst_ClientCache() {
        ...
        clsvSysScoreWeightEN_js_1.clsvSysScoreWeightEN.CacheAddiCondition = (0, CacheHelper_js_1.GetSession_CacheAdditionCondition)(clsvSysScoreWeightEN_js_1.clsvSysScoreWeightEN._CurrTabName);
        ...
        if (CacheHelper_js_2.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrvSysScoreWeightExObjLst_Cache = CacheHelper_js_2.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_2.CacheHelper.Add(strKey, arrvSysScoreWeightExObjLst);
