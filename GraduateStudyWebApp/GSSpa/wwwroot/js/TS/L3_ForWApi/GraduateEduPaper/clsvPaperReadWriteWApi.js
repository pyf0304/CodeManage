...
(function (factory) {
    ...
    */
    async function vPaperReadWrite_GetNameByPaperRWId_Cache(strPaperRWId, strid_CurrEduCls) {
        ...
        if ((0, clsString_js_1.IsNullOrEmpty)(strPaperRWId) == true) {
            const strMsg = (0, clsString_js_1.Format)("参数:[strPaperRWId]不能为空！(In GetNameByPaperRWId_Cache)");
            console.error(strMsg);
            throw (strMsg);
        }
        if (strPaperRWId.length != 8) {
    ...
    */
    async function vPaperReadWrite_func(strInFldName, strOutFldName, strInValue, strid_CurrEduCls_C) {
        ...
        if (clsvPaperReadWriteEN_js_1.clsvPaperReadWriteEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_1.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strInFldName, clsvPaperReadWriteEN_js_1.clsvPaperReadWriteEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
    ...
    **/
    async function vPaperReadWrite_GetObjLst_ClientCache(strid_CurrEduCls) {
        ...
        clsvPaperReadWriteEN_js_1.clsvPaperReadWriteEN.CacheAddiCondition = (0, CacheHelper_js_1.GetSession_CacheAdditionCondition)(clsvPaperReadWriteEN_js_1.clsvPaperReadWriteEN._CurrTabName);
        ...
        if (CacheHelper_js_2.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrvPaperReadWriteExObjLst_Cache = CacheHelper_js_2.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_2.CacheHelper.Add(strKey, arrvPaperReadWriteExObjLst);
