...
(function (factory) {
    ...
    * 注意:该类必须与调用界面处于同一个包，否则调用不成功!
    ...
    const clsvPaperSimEN_js_1 = require("../../L0_Entity/Other/clsvPaperSimEN.js");
        ...
        try {
            ...
            if (arrvPaperSim_Sel.length > 0) {
                ...
                if (bolTryAsyncOnce == true) {
                    objvPaperSim = await vPaperSim_GetObjByPaperIdAsync(strPaperId);
    ...
    */
    async function vPaperSim_func(strInFldName, strOutFldName, strInValue) {
        ...
        if (clsvPaperSimEN_js_1.clsvPaperSimEN.AttributeName.indexOf(strOutFldName) == -1) {
            const strMsg = (0, clsString_js_1.Format)("输出字段名:[{0}]不正确，不在输出字段范围之内!({1})", strInFldName, clsvPaperSimEN_js_1.clsvPaperSimEN.AttributeName.join(','));
            console.error(strMsg);
            throw new Error(strMsg);
        }
    ...
    function vPaperSim_SortFunByKey(strKey, AscOrDesc) {
        ...
        if (AscOrDesc == "Asc" || AscOrDesc == "") {
            switch (strKey) {
                ...
                case clsvPaperSimEN_js_1.clsvPaperSimEN.con_IsGSuser:
                    return (a, b) => {
                        if (a.isGSuser == true)
                            return 1;
                        else
    ...
    **/
    async function vPaperSim_GetObjLst_ClientCache() {
        ...
        clsvPaperSimEN_js_1.clsvPaperSimEN.CacheAddiCondition = (0, CacheHelper_js_1.GetSession_CacheAdditionCondition)(clsvPaperSimEN_js_1.clsvPaperSimEN._CurrTabName);
        ...
        if (CacheHelper_js_2.CacheHelper.Exsits(strKey)) {
            //缓存存在，直接返回
            const arrvPaperSimExObjLst_Cache = CacheHelper_js_2.CacheHelper.Get(strKey);
        ...
        try {
            ...
            CacheHelper_js_2.CacheHelper.Add(strKey, arrvPaperSimExObjLst);
    ...
    */
    function vPaperSim_GetCombineCondition(objvPaperSim_Cond) {
        ...
        if (Object.prototype.hasOwnProperty.call(objvPaperSim_Cond.dicFldComparisonOp, clsvPaperSimEN_js_1.clsvPaperSimEN.con_IsGSuser) == true) {
            if (objvPaperSim_Cond.isGSuser == true) {
                strWhereCond += (0, clsString_js_1.Format)(" And {0} = '1'", clsvPaperSimEN_js_1.clsvPaperSimEN.con_IsGSuser);
            }
            else {
                strWhereCond += (0, clsString_js_1.Format)(" And {0} = '0'", clsvPaperSimEN_js_1.clsvPaperSimEN.con_IsGSuser);
            }
        }
        return strWhereCond;
    }
