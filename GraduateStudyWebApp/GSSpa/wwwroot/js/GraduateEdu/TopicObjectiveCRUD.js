...
(function (factory) {
   ...
   */
    class TopicObjectiveCRUD extends clsOperateList2_js_1.clsOperateList {
        constructor() {
            super();
       ...
       */
        CombineData(arrvTopicObjectiveObjLst, arrDataColumn) {
            ...
            for (var i = 0; i < intRowNum; i++) {
                ...
                var objEN = arrvTopicObjectiveObjLst[i];
                for (var j = 0; j < intColNum; j++) {
            ...
            var strFileName = clsString2_js_1.clsString.Format("主题客观表({0})导出.xlsx", clsvTopicObjectiveEN_js_1.clsvTopicObjectiveEN._CurrTabName);
       ...
       */
        async ExportExcel_vTopicObjective_Cache() {
            ...
            objvTopicObjective_Cond.SetCondFldValue(clsvTopicObjectiveEN_js_1.clsvTopicObjectiveEN.con_id_CurrEduCls, TopicObjectiveCRUD.strid_CurrEduCls_Cache, "=");
            var strWhereCond = JSON.stringify(objvTopicObjective_Cond);
            var intCurrPageIndex = this.getCurrPageIndex(this.divName4Pager); //获取当前页
            var arrvTopicObjectiveObjLst = [];
            try {
                this.RecCount = await (0, clsvTopicObjectiveWApi_js_1.vTopicObjective_GetRecCountByCond_Cache)(objvTopicObjective_Cond, TopicObjectiveCRUD.strid_CurrEduCls_Cache);
                if (this.RecCount == 0) {
                    var lblMsg = document.createElement("span");
                    lblMsg.innerHTML = clsString2_js_1.clsString.Format("根据条件:[{0}]获取的对象列表数为0！", objvTopicObjective_Cond.whereCond);
                    var divDataLst = document.getElementById("divDataLst");
                    if (divDataLst != null) {
                        divDataLst.innerText = "";
                        divDataLst.appendChild(lblMsg);
                    }
                    var strMsg = `在绑定Gv_Cache过程中，根据条件:[${objvTopicObjective_Cond.whereCond}]获取的对象列表数为0！`;
                    console.error("Error: ", strMsg);
                    //console.trace();
                    alert(strMsg);
                    return;
                }
                var arrvTopicObjectiveObjLst = await (0, clsvTopicObjectiveWApi_js_1.vTopicObjective_GetSubObjLst_Cache)(objvTopicObjective_Cond, TopicObjectiveCRUD.strid_CurrEduCls_Cache);
            }
            catch (e) {
            ...
            if (arrvTopicObjectiveObjLst.length == 0) {
                var strKey = `${clsTopicObjectiveEN_js_1.clsTopicObjectiveEN._CurrTabName}_${TopicObjectiveCRUD.strid_CurrEduCls_Cache}`;
       ...
       */
        async ExportExcel_vTopicObjective() {
            ...
            try {
                ...
                var arrvTopicObjectiveObjLst = await (0, clsvTopicObjectiveWApi_js_1.vTopicObjective_GetObjLstAsync)(strWhereCond);
            }
            catch (e) {
                var strMsg = `绑定GridView不成功,${e}.`;
                console.error(strMsg);
       ...
       */
        async DelRecord(strTopicObjectiveId) {
            try {
                ...
                if (returnInt > 0) {
                    (0, clsTopicObjectiveWApi_js_1.TopicObjective_ReFreshCache)(TopicObjectiveCRUD.strid_CurrEduCls_Cache);
       ...
       */
        CombinevTopicObjectiveCondition() {
            ...
            try {
                ...
                var strMsg = `(errid:WiTsCs0009)在组合查询条件(CombineTopicObjectiveCondition)时出错!请联系管理员!${objException}`;
                throw strMsg;
            }
            return strWhereCond;
        }
       ...
       */
        CombinevTopicObjectiveConditionObj() {
            ...
            try {
                ...
                var strMsg = `(errid:WiTsCs0010)在组合查询条件对象(CombineTopicObjectiveConditionObj)时出错!请联系管理员!${objException}`;
                throw strMsg;
            }
        ...
        (AutoGCLib.WA_ViewScriptCS_TS4TypeScript:Gen_WApi_Ts_CombineConditionObj4ExportExcel)
         <returns>条件串(strWhereCond)</returns>
       */
        CombinevTopicObjectiveConditionObj4ExportExcel() {
            //使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
            //例如 1 = 1 && userName = '张三'
            var strWhereCond = " 1 = 1 ";
            var objvTopicObjective_Cond = new clsvTopicObjectiveEN_js_1.clsvTopicObjectiveEN();
            ...
            try {
                ...
                var strMsg = `(errid:WiTsCs0013)在组合导出Excel条件对象(CombineTopicObjectiveConditionObj4ExportExcel)时出错!请联系管理员!${objException}`;
                throw strMsg;
            }
            objvTopicObjective_Cond.whereCond = strWhereCond;
            return objvTopicObjective_Cond;
       ...
       */
        BindTab_vTopicObjective(divContainer, arrvTopicObjectiveObjLst) {
            ...
            clsCommonFunc4Web2_js_1.clsCommonFunc4Web.BindTabV4(o, arrvTopicObjectiveObjLst, arrDataColumn, "topicObjectiveId", this);
        ...
        async BindGv_vTopicObjective_Cache() {
            ...
            try {
                ...
                var objPagerPara = {
                    ...
                    whereCond: strWhereCond,
                    orderBy: this.hidSortvTopicObjectiveBy,
                    sortFun: clsvTopicObjectiveWApi_js_1.vTopicObjective_SortFun_Defa
                };
                var arrvTopicObjectiveObjLst = await (0, clsvTopicObjectiveWApi_js_1.vTopicObjective_GetObjLstByPager_Cache)(objPagerPara, TopicObjectiveCRUD.strid_CurrEduCls_Cache);
            }
            catch (e) {
                var strMsg = `绑定GridView不成功,${e}.`;
                console.error(strMsg);
       ...
       */
        async CopyRecord(arrTopicObjectiveId) {
            try {
                const responseText = await (0, clsTopicObjectiveWApi_js_1.TopicObjective_GetObjLstByTopicObjectiveIdLstAsync)(arrTopicObjectiveId);
                console.log('responseText=');
                console.log(responseText);
                var intCount = 0;
                var arrTopicObjectiveObjLst = responseText;
                for (let objInFor of arrTopicObjectiveObjLst) {
                    const responseText2 = await (0, clsTopicObjectiveWApi_js_1.TopicObjective_AddNewRecordAsync)(objInFor);
    ...
    TopicObjectiveCRUD.strid_CurrEduCls_Cache = "99999991"; //缓存分类字段
});
