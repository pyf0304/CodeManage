(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "../TS/L0_Entity/GraduateEduTopic/clske_SuperEN.js", "../TS/PubFun/clsCommonFunc4Web2.js", "../TS/FunClass/clsPubFun4Web.js", "../TS/PubFun/clsOperateList2.js", "../TS/PubFun/clsPubSessionStorage.js", "../TS/PubFun/clsString2.js", "../TS/L3_ForWApi/GraduateEduTopic/clske_SuperWApi.js"], factory);
    }
})(function (require, exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.ke_SuperCRUD = void 0;
    /*-- -- -- -- -- -- -- -- -- -- --
    类名:ke_SuperCRUD
   ...
   */
    class ke_SuperCRUD extends clsOperateList2_js_1.clsOperateList {
        constructor() {
            super(...arguments);
        ...
        async btnQuery_Click() {
            ...
            try {
                ...
                const responseObjLst = await (0, clske_SuperWApi_js_1.ke_Super_GetObjLstByPagerAsync)(objPagerPara).then((jsonData) => {
                    var arrke_SuperObjLst = jsonData;
       ...
       */
        async DelRecord(strke_SuperId) {
            try {
                const responseText = await (0, clske_SuperWApi_js_1.ke_Super_DelRecordAsync)(strke_SuperId);
       ...
       */
        async SelectRecord(strke_SuperId) {
            try {
                const responseText = await (0, clske_SuperWApi_js_1.ke_Super_GetObjByke_SuperIdAsync)(strke_SuperId);
                var objke_SuperEN = responseText;
                console.log("完成SelectRecord!");
                clsCommonFunc4Web2_js_1.clsCommonFunc4Web.Redirect("/Index/Main_ke_Super");
            }
            catch (e) {
                console.error(e);
       ...
       */
        async btnExportExcel_Click() {
            ...
            try {
                const responseText = await (0, clske_SuperWApi_js_1.ke_Super_GetObjLstAsync)(strWhereCond).then((jsonData) => {
                    var arrke_SuperObjLst = jsonData;
                    this.BindTab_ke_Super(this.mstrListDiv, arrke_SuperObjLst);
                });
            }
       ...
       */
        Combineke_SuperCondition() {
            ...
            try {
                ...
                var strMsg = `(errid:WiTsCs0002)在组合查询条件(Combineke_SuperCondition)时出错!请联系管理员!${objException}`;
                throw strMsg;
            }
            return strWhereCond;
        }
        ...
        (AutoGCLib.WA_ViewScriptCS_TS4TypeScript:Gen_WApi_Ts_CombineConditionObj)
         <returns>条件串(strWhereCond)</returns>
       */
        Combineke_SuperConditionObj() {
            //使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
            //例如 1 = 1 && userName = '张三'
            var strWhereCond = " 1 = 1 ";
            var objke_Super_Cond = new clske_SuperEN_js_1.clske_SuperEN();
            //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
            try {
            }
            catch (objException) {
                var strMsg = `(errid:WiTsCs0008)在组合查询条件对象(Combineke_SuperConditionObj)时出错!请联系管理员!${objException}`;
                throw strMsg;
            }
       ...
       */
        async BindGv_ke_Super_Cache() {
            ...
            var arrke_SuperObjLst = [];
            try {
                this.RecCount = await (0, clske_SuperWApi_js_1.ke_Super_GetRecCountByCond_Cache)(objke_Super_Cond);
                var objPagerPara = {
                    pageIndex: intCurrPageIndex,
                    pageSize: this.pageSize,
                    whereCond: strWhereCond,
                ...
                var arrke_SuperObjLst = await (0, clske_SuperWApi_js_1.ke_Super_GetObjLstByPager_Cache)(objPagerPara);
            }
            catch (e) {
                console.error('catch(e)=');
                console.error(e);
            ...
            if (arrke_SuperObjLst.length == 0) {
                var strMsg = `在BindGv_Cache过程中，根据条件对象获取的对象列表数为0！(Key=ke_Super)`;
                alert(strMsg);
                return;
            }
            try {
       ...
       */
        PutDataToke_SuperClass(pobjke_SuperEN) {
            ...
            pobjke_SuperEN.updUser = clsPubSessionStorage_js_1.clsPubSessionStorage.GetSession_UserId(); // 修改人
            pobjke_SuperEN.updDate = clsPubFun4Web_js_1.clsPubFun4Web.getNowDate(); // 修改日期
        }
        /* 添加新记录，保存函数
       ...
       */
        async AddNewRecordSave() {
            ...
            var objke_SuperEN = new clske_SuperEN_js_1.clske_SuperEN();
            this.PutDataToke_SuperClass(objke_SuperEN);
            try {
                const responseText = await (0, clske_SuperWApi_js_1.ke_Super_IsExistAsync)(objke_SuperEN.ke_SuperId);
                var bolIsExist = responseText;
                if (bolIsExist == true) {
                    var strMsg = `添加记录时，关键字：${objke_SuperEN.ke_SuperId}已经存在！`;
                    alert(strMsg);
                    return responseText; //一定要有一个返回值，否则会出错！
                }
                const responseText2 = await (0, clske_SuperWApi_js_1.ke_Super_AddNewRecordAsync)(objke_SuperEN);
       ...
       */
        async AddNewRecordWithMaxIdSave() {
            ...
            this.PutDataToke_SuperClass(objke_SuperEN);
            try {
                const responseText2 = await (0, clske_SuperWApi_js_1.ke_Super_AddNewRecordWithMaxIdAsync)(objke_SuperEN);
                var returnBool = !!responseText2;
                if (returnBool == true) {
                    var strInfo = `添加记录成功!`;
       ...
       */
        async ShowData(strke_SuperId) {
            ...
            var objke_SuperEN = new clske_SuperEN_js_1.clske_SuperEN();
            try {
                const responseText = await (0, clske_SuperWApi_js_1.ke_Super_IsExistAsync)(strke_SuperId);
                var returnBool = responseText;
                if (returnBool == false) {
                    var strInfo = `关键字:[${strke_SuperId}] 的记录不存在!`;
       ...
       */
        async UpdateRecordSave() {
            ...
            try {
                const responseText = await (0, clske_SuperWApi_js_1.ke_Super_UpdateRecordAsync)(objke_SuperEN);
                var returnBool = !!responseText;
                if (returnBool == true) {
                }
                return returnBool;
       ...
       */
        async DelMultiRecord(arrke_SuperId) {
            try {
                const responseText = await (0, clske_SuperWApi_js_1.ke_Super_Delke_SupersAsync)(arrke_SuperId);
                var returnInt = responseText;
                if (returnInt > 0) {
                    var strInfo = `删除记录成功,共删除${returnInt}条记录!`;
                    //显示信息框
       ...
       */
        Showke_SuperObj(divContainer, objke_Super) {
            var o = document.getElementById(divContainer);
            if (o == null) {
                alert(`${divContainer}不存在！`);
                return;
            }
            var sstrKeys = clsCommonFunc4Web2_js_1.clsCommonFunc4Web.GetObjKeys(objke_Super);
       ...
       */
        set hidSortke_SuperBy(value) {
            $("#hidSortke_SuperBy").val(value);
        }
        /*
        * 设置排序字段
       */
        get hidSortke_SuperBy() {
    ...
    exports.ke_SuperCRUD = ke_SuperCRUD;
});
