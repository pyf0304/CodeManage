     ...
     */
      ...
      */
        PutDataToViewpointClass(pobjViewpointEN) {
            ...
            pobjViewpointEN.citationId = this.paperId; //引用Id
            //判断用户类型，然后添加数据
            var strhidUserTypeId = $("#hidUserTypeId").val();
            if (strhidUserTypeId != "") {
      ...
      */
        async btnDelRecordInTab_Click(strKeyId) {
            try {
                ...
                //查询和id相关的评论评分是否存在
                var strWhere2 = " 1=1 And tableKey ='" + strKeyId + "' and commentTypeId='04'";
                var commentCount = 0;
                ...
                if (RTVCount > 0) {
                    ...
                    const responseText7 = await (0, clsgs_TotalDataStatisticsExWApi_js_1.gs_TotalDataStatisticsEx_AddAndUpdateAndDeleteTotalStatis)(strSubViewpointId, "05", "3", this.Id_CurrEduCls);
                    var returnBool7 = !!responseText7;
                    if (returnBool7 == true) {
                        console.log("专家观点数据总表删除成功；");
                    }
                    else {
                        console.log("专家观点数据总表删除失败；");
                    }
      ...
      */
        async BindGv_vViewpoint() {
            ...
            try {
                ...
                //strhtml += '<div style="float:right; margin-right:20px;"><button style="color:#666" title="添加专家观点" class="layui-btn layui-btn-warm layui-btn-xs" onclick = btnAddNewRecord_Click();> <i class="layui-icon" ></i>添加专家观点</button></div>';
                ...
                //        //修改个人观点
                //        strhtml += '&nbsp;&nbsp;<button title="修改专家观点" class="layui-btn layui-btn layui-btn-xs" onclick=btnUpdateRecordInTab_Click("' + arrvViewpointObjLst[i].viewpointId + '")> <i class="layui-icon" >&#xe642;</i>修改</button>';
                //        //删除个人观点
                //        strhtml += '&nbsp;&nbsp;<button title="删除专家观点" class="layui-btn layui-btn-danger layui-btn-xs" onclick=btnDelRecordInTab_Click("' + arrvViewpointObjLst[i].viewpointId + '")> <i class="layui-icon" >&#xe640;</i>删除</button>';
                //        //观点提交
                ...
                //            strhtml += "&nbsp;&nbsp;<button class=\"layui-btn layui-btn-warm layui-btn-xs\" onclick=\"xadmin.open('专家观点历史版本', '../GraduateEduEx/HistoricalVersion?Key=" + arrvViewpointObjLst[i].viewpointId + "&Type=04')\"> <i class=\"layui-icon\" >&#xe642;</i>历史版本</button >";
                //        }
                //        strhtml += "&nbsp;&nbsp;<button id=\"AddViewRela\" class=\"layui-btn-xs layui-btn layui-btn-normal\" onclick=\" xadmin.open('建立观点关系', '../GraduateEduTools/gs_TotalDataRela?KeyId=" + strViewpointId + "&TypeId=05','','',true)\"> <i class=\"layui-icon\" >&#xe642;</i>建立观点关系</button >";
                //    } else {
      ...
      */
        async btnAddNewRecord_Click() {
            ...
            try {
                ...
                //获取分享Id
                const responseText1 = await (0, clsgs_UserConfigExWApi_js_1.gs_UserConfigEx_GetNewReturnShareIdEx)("05", clsPubSessionStorage_js_1.clsPubSessionStorage.GetSession_UserId());
                ...
                if (clsPubSessionStorage_js_1.clsPubSessionStorage.GetSession_UserId() != "") {
                    switch (strCommandText) {
                        ...
                        case "确认添加":
                            ...
                            const responseText6 = await (0, clsgs_TotalDataStatisticsExWApi_js_1.gs_TotalDataStatisticsEx_AddAndUpdateAndDeleteTotalStatis)(strTopicObjectiveId, "05", "1", this.Id_CurrEduCls);
                        ...
                        case "确认修改":
                            ...
                            //更新总表3个参数 主键、子表类型、页面操作类型；
                            var strTopicObjectiveId = $('#hidViewpointId').val();
                            const responseText7 = await (0, clsgs_TotalDataStatisticsExWApi_js_1.gs_TotalDataStatisticsEx_AddAndUpdateAndDeleteTotalStatis)(strTopicObjectiveId, "05", "2", this.Id_CurrEduCls);
                            var returnBool7 = !!responseText7;
                            if (returnBool7 == true) {
