(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "../TS/L0_Entity/GraduateEduTopic/clsSysSkillEN.js", "../TS/L0_Entity/GraduateEduTopic/clsvSysSkillEN.js", "../TS/L0_Entity/ParameterTable/clsSysOperationTypeEN.js", "../TS/L0_Entity/ParameterTable/clsSysSecurityLevelEN.js", "../TS/PubFun/clsCommonFunc4Web2.js", "../TS/PubFun/clsOperateList2.js", "../TS/L3_ForWApi/GraduateEduTopic/clsvSysSkillWApi.js", "../TS/L3_ForWApi/GraduateEduTopic/clsSysSkillWApi.js", "../TS/L3_ForWApi/ParameterTable/clsSysOperationTypeWApi.js", "../TS/L3_ForWApi/ParameterTable/clsSysSecurityLevelWApi.js"], factory);
    }
    ...
    */
    class SysSkillCRUD extends clsOperateList2_js_1.clsOperateList {
        constructor() {
            super(...arguments);
       ...
       */
        async DelRecord(strSkillId) {
            try {
                const responseText = await (0, clsSysSkillWApi_js_1.SysSkill_DelRecordAsync)(strSkillId);
       ...
       */
        async SelectRecord(strSkillId) {
            try {
                ...
                clsCommonFunc4Web2_js_1.clsCommonFunc4Web.Redirect("/Index/Main_vSysSkill");
            }
            catch (e) {
                console.error(e);
                var strMsg = `根据关键字获取相应的记录的对象不成功,${e}.`;
       ...
       */
        async btnExportExcel_Click() {
            ...
            try {
                const responseText = await (0, clsvSysSkillWApi_js_1.vSysSkill_GetObjLstAsync)(strWhereCond).then((jsonData) => {
                    var arrvSysSkillObjLst = jsonData;
                    this.BindTab_vSysSkill(this.mstrListDiv, arrvSysSkillObjLst);
                });
            }
            catch (e) {
       ...
       */
        async BindGv_vSysSkill_Cache() {
            ...
            objvSysSkill_Cond.SetCondFldValue(clsvSysSkillEN_js_1.clsvSysSkillEN.con_id_CurrEduCls, SysSkillCRUD.strid_CurrEduCls_Cache, "=");
            var strWhereCond = JSON.stringify(objvSysSkill_Cond);
            var intCurrPageIndex = this.getCurrPageIndex(this.divName4Pager); //获取当前页
            var arrvSysSkillObjLst = [];
            try {
                this.RecCount = await (0, clsvSysSkillWApi_js_1.vSysSkill_GetRecCountByCond_Cache)(objvSysSkill_Cond, SysSkillCRUD.strid_CurrEduCls_Cache);
                var objPagerPara = {
                    pageIndex: intCurrPageIndex,
                    pageSize: this.pageSize,
                    whereCond: strWhereCond,
                ...
                var arrvSysSkillObjLst = await (0, clsvSysSkillWApi_js_1.vSysSkill_GetObjLstByPager_Cache)(objPagerPara, SysSkillCRUD.strid_CurrEduCls_Cache);
            }
            catch (e) {
                console.error('catch(e)=');
                console.error(e);
            ...
            if (arrvSysSkillObjLst.length == 0) {
                var strMsg = `在BindGv_Cache过程中，根据条件对象获取的对象列表数为0！(Key=SysSkill_${SysSkillCRUD.strid_CurrEduCls_Cache})`;
                alert(strMsg);
                return;
            }
            try {
        ...
        async GetMaxStrId(strKeyCtrlName) {
            ...
            try {
                ...
                if (returnString == "") {
                    ...
                    var strInfo = `获取表SysSkill的最大关键字为：${returnString}!`;
                    $('#lblResult40').val(strInfo);
                    //显示信息框
                    alert(strInfo);
                    $(strKeyCtrlName).val(returnString);
       ...
       */
        async AddNewRecordSave() {
            ...
            try {
                const responseText = await (0, clsSysSkillWApi_js_1.SysSkill_IsExistAsync)(objSysSkillEN.skillId);
                var bolIsExist = responseText;
                if (bolIsExist == true) {
                    var strMsg = `添加记录时，关键字：${objSysSkillEN.skillId}已经存在！`;
                    alert(strMsg);
                    return responseText; //一定要有一个返回值，否则会出错！
                }
                const responseText2 = await (0, clsSysSkillWApi_js_1.SysSkill_AddNewRecordAsync)(objSysSkillEN);
       ...
       */
        async AddNewRecordWithMaxIdSave() {
            ...
            this.PutDataToSysSkillClass(objSysSkillEN);
            try {
                const responseText2 = await (0, clsSysSkillWApi_js_1.SysSkill_AddNewRecordWithMaxIdAsync)(objSysSkillEN);
                var returnBool = !!responseText2;
                if (returnBool == true) {
                    var strInfo = `添加记录成功!`;
       ...
       */
        async ShowData(strSkillId) {
            ...
            var objSysSkillEN = new clsSysSkillEN_js_1.clsSysSkillEN();
            try {
                const responseText = await (0, clsSysSkillWApi_js_1.SysSkill_IsExistAsync)(strSkillId);
                var returnBool = responseText;
                if (returnBool == false) {
                    var strInfo = `关键字:[${strSkillId}] 的记录不存在!`;
       ...
       */
        async DelMultiRecord(arrSkillId) {
            try {
                const responseText = await (0, clsSysSkillWApi_js_1.SysSkill_DelSysSkillsAsync)(arrSkillId);
                var returnInt = responseText;
                if (returnInt > 0) {
                    var strInfo = `删除记录成功,共删除${returnInt}条记录!`;
                    //显示信息框
       ...
       */
        ShowSysSkillObj(divContainer, objSysSkill) {
            ...
            var sstrKeys = clsCommonFunc4Web2_js_1.clsCommonFunc4Web.GetObjKeys(objSysSkill);
    ...
    SysSkillCRUD.strid_CurrEduCls_Cache = "00000001";
});
