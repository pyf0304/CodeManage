(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "../TS/L0_Entity/InteractManage/clsqa_AnswerEN.js", "../TS/L0_Entity/InteractManage/clsvqa_AnswerEN.js", "../TS/L3_ForWApi/InteractManage/clsqa_AnswerWApi.js", "../TS/L3_ForWApi/InteractManage/clsvqa_AnswerWApi.js", "../TS/PubFun/clsCommonFunc4Web2.js", "../TS/PubFun/clsOperateList2.js", "../TS/PubFun/clsPager.js", "../TS/PubFun/clsPublocalStorage.js", "../TS/PubFun/clsString2.js"], factory);
    }
    ...
    */
    class qa_AnswerCRUD extends clsOperateList2_js_1.clsOperateList {
        constructor() {
            super(...arguments);
       ...
       */
        ...
        async AddDPV_Detail(divName4Detail) {
            var strUrl = "./WA_qa_Answer_Detail/";
        ...
        async btnQuery_Click() {
            ...
            try {
                ...
                const responseObjLst = await (0, clsvqa_AnswerWApi_js_1.vqa_Answer_GetObjLstByPagerAsync)(objPagerPara).then((jsonData) => {
                    var arrvqa_AnswerObjLst = jsonData;
       ...
       */
        async DetailRecord(strAnswerId) {
            ...
            this.btnCancel = "关闭";
            try {
                const responseText = await (0, clsvqa_AnswerWApi_js_1.vqa_Answer_GetObjByAnswerIdAsync)(strAnswerId);
                var objvqa_AnswerEN = responseText;
       ...
       */
        async SelectRecord(strAnswerId) {
            try {
                ...
                clsCommonFunc4Web2_js_1.clsCommonFunc4Web.Redirect("/Index/Main_vqa_Answer");
            }
            catch (e) {
                console.error(e);
                var strMsg = `根据关键字获取相应的记录的对象不成功,${e}.`;
       ...
       */
        async btnExportExcel_Click() {
            ...
            try {
                const responseText = await (0, clsvqa_AnswerWApi_js_1.vqa_Answer_GetObjLstAsync)(strWhereCond).then((jsonData) => {
                    var arrvqa_AnswerObjLst = jsonData;
                    this.BindTab_vqa_Answer(this.mstrListDiv, arrvqa_AnswerObjLst);
                });
            }
       ...
       */
        Combinevqa_AnswerCondition() {
            ...
            try {
                if (this.QuestionsId_q != "") {
                    strWhereCond += ` And ${clsvqa_AnswerEN_js_1.clsvqa_AnswerEN.con_QuestionsId} like '% ${this.QuestionsId_q}%'`;
                }
            }
            catch (objException) {
        ...
        (AutoGCLib.WA_ViewScriptCS_TS4TypeScript:Gen_WApi_Ts_CombineConditionObj)
         <returns>条件串(strWhereCond)</returns>
       */
        Combinevqa_AnswerConditionObj() {
            //使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
            //例如 1 = 1 && userName = '张三'
            var strWhereCond = " 1 = 1 ";
            var objvqa_Answer_Cond = new clsvqa_AnswerEN_js_1.clsvqa_AnswerEN();
            //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
            try {
                if (this.QuestionsId_q != "") {
                    strWhereCond += ` And ${clsvqa_AnswerEN_js_1.clsvqa_AnswerEN.con_QuestionsId} like '% ${this.QuestionsId_q}%'`;
                ...
                var strMsg = `(errid:WiTsCs0005)在组合查询条件对象(Combineqa_AnswerConditionObj)时出错!请联系管理员!${objException}`;
                throw strMsg;
            }
       ...
       */
        async BindGv_vqa_Answer_Cache() {
            ...
            var arrvqa_AnswerObjLst = [];
            try {
                this.RecCount = await (0, clsvqa_AnswerWApi_js_1.vqa_Answer_GetRecCountByCond_Cache)(objvqa_Answer_Cond, strid_CurrEduCls);
                var objPagerPara = {
                    pageIndex: intCurrPageIndex,
                    pageSize: this.pageSize,
                    whereCond: strWhereCond,
                ...
                var arrvqa_AnswerObjLst = await (0, clsvqa_AnswerWApi_js_1.vqa_Answer_GetObjLstByPager_Cache)(objPagerPara, strid_CurrEduCls);
            }
            catch (e) {
                console.error('catch(e)=');
                console.error(e);
            ...
            if (arrvqa_AnswerObjLst.length == 0) {
                var strMsg = `在BindGv_Cache过程中，根据条件对象获取的对象列表数为0！(Key=qa_Answer)`;
                alert(strMsg);
                return;
            }
            try {
       ...
       */
        async GetMaxStrId(strKeyCtrlName) {
            ...
            try {
                const responseText = await (0, clsqa_AnswerWApi_js_1.qa_Answer_GetMaxStrIdAsync)();
                var returnString = responseText.toString();
                if (returnString == "") {
                    var strInfo = `获取表qa_Answer的最大关键字为空，不成功，请检查!`;
                    ...
                    var strInfo = `获取表qa_Answer的最大关键字为：${returnString}!`;
                    $('#lblResult40').val(strInfo);
                    //显示信息框
                    alert(strInfo);
                    $(strKeyCtrlName).val(returnString);
       ...
       */
        async AddNewRecordSave() {
            ...
            try {
                const responseText = await (0, clsqa_AnswerWApi_js_1.qa_Answer_IsExistAsync)(objqa_AnswerEN.answerId);
                var bolIsExist = responseText;
                if (bolIsExist == true) {
                    var strMsg = `添加记录时，关键字：${objqa_AnswerEN.answerId}已经存在！`;
                    alert(strMsg);
                    return responseText; //一定要有一个返回值，否则会出错！
                }
                const responseText2 = await (0, clsqa_AnswerWApi_js_1.qa_Answer_AddNewRecordAsync)(objqa_AnswerEN);
       ...
       */
        async ShowData(strAnswerId) {
            ...
            var objqa_AnswerEN = new clsqa_AnswerEN_js_1.clsqa_AnswerEN();
            try {
                const responseText = await (0, clsqa_AnswerWApi_js_1.qa_Answer_IsExistAsync)(strAnswerId);
                var returnBool = responseText;
                if (returnBool == false) {
                    var strInfo = `关键字:[${strAnswerId}] 的记录不存在!`;
       ...
       */
        async DelMultiRecord(arrAnswerId) {
            try {
                const responseText = await (0, clsqa_AnswerWApi_js_1.qa_Answer_Delqa_AnswersAsync)(arrAnswerId);
                var returnInt = responseText;
                if (returnInt > 0) {
                    var strInfo = `删除记录成功,共删除${returnInt}条记录!`;
                    //显示信息框
       ...
       */
        Showqa_AnswerObj(divContainer, objqa_Answer) {
            ...
            var sstrKeys = clsCommonFunc4Web2_js_1.clsCommonFunc4Web.GetObjKeys(objqa_Answer);
       ...
       */
        set hidSortvqa_AnswerBy(value) {
            $("#hidSortvqa_AnswerBy").val(value);
        }
        /*
        * 设置排序字段
       */
        get hidSortvqa_AnswerBy() {
