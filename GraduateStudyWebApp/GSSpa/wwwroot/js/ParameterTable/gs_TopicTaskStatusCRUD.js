(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "../TS/L0_Entity/ParameterTable/clsgs_TopicTaskStatusEN.js", "../TS/PubFun/clsCommonFunc4Web2.js", "../TS/PubFun/clsOperateList2.js", "../TS/PubFun/clsPubSessionStorage.js", "../TS/PubFun/clsString2.js", "../TS/L3_ForWApi/ParameterTable/clsgs_TopicTaskStatusWApi.js"], factory);
    }
   ...
   */
    class gs_TopicTaskStatusCRUD extends clsOperateList2_js_1.clsOperateList {
        constructor() {
            super(...arguments);
       ...
       */
        ...
        async AddDPV_Edit(divName4Edit) {
            var strUrl = "./gs_TopicTaskStatus_Edit/";
        ...
        async AddDPV_Detail(divName4Detail) {
            var strUrl = "./gs_TopicTaskStatus_Detail/";
        ...
        async btnQuery_Click() {
            ...
            try {
                ...
                const responseObjLst = await (0, clsgs_TopicTaskStatusWApi_js_1.gs_TopicTaskStatus_GetObjLstByPagerAsync)(objPagerPara).then((jsonData) => {
                    var arrgs_TopicTaskStatusObjLst = jsonData;
       ...
       */
        async DelRecord(strStatusId) {
            try {
                const responseText = await (0, clsgs_TopicTaskStatusWApi_js_1.gs_TopicTaskStatus_DelRecordAsync)(strStatusId);
       ...
       */
        async SelectRecord(strStatusId) {
            try {
                const responseText = await (0, clsgs_TopicTaskStatusWApi_js_1.gs_TopicTaskStatus_GetObjByStatusIdAsync)(strStatusId);
                var objgs_TopicTaskStatusEN = responseText;
                console.log("完成SelectRecord!");
                clsCommonFunc4Web2_js_1.clsCommonFunc4Web.Redirect("/Index/Main_gs_TopicTaskStatus");
            }
            catch (e) {
                console.error(e);
       ...
       */
        async btnExportExcel_Click() {
            ...
            try {
                const responseText = await (0, clsgs_TopicTaskStatusWApi_js_1.gs_TopicTaskStatus_GetObjLstAsync)(strWhereCond).then((jsonData) => {
                    var arrgs_TopicTaskStatusObjLst = jsonData;
                    this.BindTab_gs_TopicTaskStatus(this.mstrListDiv, arrgs_TopicTaskStatusObjLst);
                });
            }
       ...
       */
        Combinegs_TopicTaskStatusCondition() {
            ...
            try {
                if (this.StatusName_q != "") {
                    strWhereCond += ` And ${clsgs_TopicTaskStatusEN_js_1.clsgs_TopicTaskStatusEN.con_StatusName} like '% ${this.StatusName_q}%'`;
                }
            }
            catch (objException) {
                var strMsg = `(errid:WiTsCs0002)在组合查询条件(Combinegs_TopicTaskStatusCondition)时出错!请联系管理员!${objException}`;
                throw strMsg;
            }
            return strWhereCond;
        }
        ...
        (AutoGCLib.WA_ViewScriptCS_TS4TypeScript:Gen_WApi_Ts_CombineConditionObj)
         <returns>条件串(strWhereCond)</returns>
       */
        Combinegs_TopicTaskStatusConditionObj() {
            //使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
            //例如 1 = 1 && userName = '张三'
            var strWhereCond = " 1 = 1 ";
            var objgs_TopicTaskStatus_Cond = new clsgs_TopicTaskStatusEN_js_1.clsgs_TopicTaskStatusEN();
            //如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
            try {
                if (this.StatusName_q != "") {
                    strWhereCond += ` And ${clsgs_TopicTaskStatusEN_js_1.clsgs_TopicTaskStatusEN.con_StatusName} like '% ${this.StatusName_q}%'`;
                ...
                var strMsg = `(errid:WiTsCs0008)在组合查询条件对象(Combinegs_TopicTaskStatusConditionObj)时出错!请联系管理员!${objException}`;
                throw strMsg;
            }
       ...
       */
        async BindGv_gs_TopicTaskStatus_Cache() {
            ...
            var arrgs_TopicTaskStatusObjLst = [];
            try {
                this.RecCount = await (0, clsgs_TopicTaskStatusWApi_js_1.gs_TopicTaskStatus_GetRecCountByCond_Cache)(objgs_TopicTaskStatus_Cond);
                var objPagerPara = {
                    pageIndex: intCurrPageIndex,
                    pageSize: this.pageSize,
                    whereCond: strWhereCond,
                ...
                var arrgs_TopicTaskStatusObjLst = await (0, clsgs_TopicTaskStatusWApi_js_1.gs_TopicTaskStatus_GetObjLstByPager_Cache)(objPagerPara);
            }
            catch (e) {
                console.error('catch(e)=');
                console.error(e);
            ...
            if (arrgs_TopicTaskStatusObjLst.length == 0) {
                var strMsg = `在BindGv_Cache过程中，根据条件对象获取的对象列表数为0！(Key=gs_TopicTaskStatus)`;
                alert(strMsg);
                return;
            }
            try {
       ...
       */
        async AddNewRecordSave() {
            ...
            var objgs_TopicTaskStatusEN = new clsgs_TopicTaskStatusEN_js_1.clsgs_TopicTaskStatusEN();
            this.PutDataTogs_TopicTaskStatusClass(objgs_TopicTaskStatusEN);
            try {
                const responseText = await (0, clsgs_TopicTaskStatusWApi_js_1.gs_TopicTaskStatus_IsExistAsync)(objgs_TopicTaskStatusEN.statusId);
                var bolIsExist = responseText;
                if (bolIsExist == true) {
                    var strMsg = `添加记录时，关键字：${objgs_TopicTaskStatusEN.statusId}已经存在！`;
                    alert(strMsg);
                    return responseText; //一定要有一个返回值，否则会出错！
                }
                const responseText2 = await (0, clsgs_TopicTaskStatusWApi_js_1.gs_TopicTaskStatus_AddNewRecordAsync)(objgs_TopicTaskStatusEN);
       ...
       */
        async AddNewRecordWithMaxIdSave() {
            ...
            this.PutDataTogs_TopicTaskStatusClass(objgs_TopicTaskStatusEN);
            try {
                const responseText2 = await (0, clsgs_TopicTaskStatusWApi_js_1.gs_TopicTaskStatus_AddNewRecordWithMaxIdAsync)(objgs_TopicTaskStatusEN);
                var returnBool = !!responseText2;
                if (returnBool == true) {
                    var strInfo = `添加记录成功!`;
       ...
       */
        async ShowData(strStatusId) {
            ...
            var objgs_TopicTaskStatusEN = new clsgs_TopicTaskStatusEN_js_1.clsgs_TopicTaskStatusEN();
            try {
                const responseText = await (0, clsgs_TopicTaskStatusWApi_js_1.gs_TopicTaskStatus_IsExistAsync)(strStatusId);
                var returnBool = responseText;
                if (returnBool == false) {
                    var strInfo = `关键字:[${strStatusId}] 的记录不存在!`;
            ...
            try {
                const responseText = await (0, clsgs_TopicTaskStatusWApi_js_1.gs_TopicTaskStatus_GetObjByStatusIdAsync)(strStatusId);
                objgs_TopicTaskStatusEN = responseText;
            }
            catch (e) {
                var strMsg = `根据关键字获取相应的记录的对象不成功,${e}.`;
                alert(strMsg);
       ...
       */
        async UpdateRecordSave() {
            ...
            try {
                const responseText = await (0, clsgs_TopicTaskStatusWApi_js_1.gs_TopicTaskStatus_UpdateRecordAsync)(objgs_TopicTaskStatusEN);
                var returnBool = !!responseText;
                if (returnBool == true) {
                }
                return returnBool;
       ...
       */
        async DelMultiRecord(arrStatusId) {
            try {
                const responseText = await (0, clsgs_TopicTaskStatusWApi_js_1.gs_TopicTaskStatus_Delgs_TopicTaskStatussAsync)(arrStatusId);
                var returnInt = responseText;
                if (returnInt > 0) {
                    var strInfo = `删除记录成功,共删除${returnInt}条记录!`;
                    //显示信息框
       ...
       */
        Showgs_TopicTaskStatusObj(divContainer, objgs_TopicTaskStatus) {
            ...
            var sstrKeys = clsCommonFunc4Web2_js_1.clsCommonFunc4Web.GetObjKeys(objgs_TopicTaskStatus);
       ...
       */
        set hidSortgs_TopicTaskStatusBy(value) {
            $("#hidSortgs_TopicTaskStatusBy").val(value);
        }
        /*
        * 设置排序字段
       */
        get hidSortgs_TopicTaskStatusBy() {
