...
try
{
 objvCacheUseStateEN.mId = TransNullToInt(objDT.Rows[0][convCacheUseState.mId].ToString().Trim()); //mId(字段类型:bigint,字段长度:8,是否可空:True)
 ...
 objvCacheUseStateEN.Memo = objDT.Rows[0][convCacheUseState.Memo].ToString().Trim(); //备注(字段类型:varchar,字段长度:1000,是否可空:True)
}
 catch(Exception objException)
{
...
try
{
 objvCacheUseStateEN.mId = Int32.Parse(objRow[convCacheUseState.mId].ToString().Trim()); //mId(字段类型:bigint,字段长度:8,是否可空:True)
 ...
 objvCacheUseStateEN.Memo = objRow[convCacheUseState.Memo] == DBNull.Value ? null : objRow[convCacheUseState.Memo].ToString().Trim(); //备注(字段类型:varchar,字段长度:1000,是否可空:True)
}
 catch(Exception objException)
{
...
Memo = objRow[convCacheUseState.Memo] == DBNull.Value ? null : objRow[convCacheUseState.Memo].ToString().Trim() //备注
};
objvCacheUseStateEN.ClearUpdateState();//清除修改状态,即清除脏字段信息
return objvCacheUseStateEN;
}
...
//检查外键字段长度
 ...
 /// </summary>
 /// <param name = "objvCacheUseStateEN">表对象</param>
 /// <returns>返回唯一性条件串</returns>
public string GetUniCondStr(clsvCacheUseStateEN objvCacheUseStateEN)
{
