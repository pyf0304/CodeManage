...
try
{
 objUserStateEN.UserStateId = objDT.Rows[0][conUserState.UserStateId].ToString().Trim(); //用户状态Id(字段类型:char,字段长度:2,是否可空:True)
 objUserStateEN.UserStateName = objDT.Rows[0][conUserState.UserStateName].ToString().Trim(); //用户状态名(字段类型:varchar,字段长度:20,是否可空:True)
...
try
{
 objUserStateEN.UserStateId = objRow[conUserState.UserStateId].ToString().Trim(); //用户状态Id(字段类型:char,字段长度:2,是否可空:True)
 objUserStateEN.UserStateName = objRow[conUserState.UserStateName].ToString().Trim(); //用户状态名(字段类型:varchar,字段长度:20,是否可空:True)
...
Memo = objRow[conUserState.Memo] == DBNull.Value ? null : objRow[conUserState.Memo].ToString().Trim() //备注
};
objUserStateEN.ClearUpdateState();//清除修改状态,即清除脏字段信息
return objUserStateEN;
}
...
sbSQL.AppendFormat("Update UserState Set ");
 ...
 {
 var strMemo = objUserStateEN.Memo.Replace("'", "''"); //转换值串中的单撇"'",使之成为双撇"''"
 sbSQL.AppendFormat("{1} = '{0}',", strMemo, conUserState.Memo); //备注
 }
 else
 {
