...
objQxRolesEN.IsInUse = TransNullToBool(objRow[conQxRoles.IsInUse].ToString().Trim()); //IsInUse
...
try
{
 ...
 objQxRolesEN.RoleName = objDT.Rows[0][conQxRoles.RoleName].ToString().Trim(); //角色名(字段类型:varchar,字段长度:100,是否可空:True)
 ...
 objQxRolesEN.QxPrjId = objDT.Rows[0][conQxRoles.QxPrjId].ToString().Trim(); //QxPrjId(字段类型:char,字段长度:4,是否可空:False)
 objQxRolesEN.IsInUse = TransNullToBool(objDT.Rows[0][conQxRoles.IsInUse].ToString().Trim()); //IsInUse(字段类型:bit,字段长度:1,是否可空:True)
 objQxRolesEN.UpdDate = objDT.Rows[0][conQxRoles.UpdDate].ToString().Trim(); //修改日期(字段类型:varchar,字段长度:20,是否可空:True)
 objQxRolesEN.UpdUserId = objDT.Rows[0][conQxRoles.UpdUserId].ToString().Trim(); //修改用户Id(字段类型:varchar,字段长度:20,是否可空:True)
...
try
{
 ...
 objQxRolesEN.RoleName = objRow[conQxRoles.RoleName].ToString().Trim(); //角色名(字段类型:varchar,字段长度:100,是否可空:True)
 ...
 objQxRolesEN.QxPrjId = objRow[conQxRoles.QxPrjId].ToString().Trim(); //QxPrjId(字段类型:char,字段长度:4,是否可空:False)
 objQxRolesEN.IsInUse = clsEntityBase2.TransNullToBool_S(objRow[conQxRoles.IsInUse].ToString().Trim()); //IsInUse(字段类型:bit,字段长度:1,是否可空:True)
 objQxRolesEN.UpdDate = objRow[conQxRoles.UpdDate] == DBNull.Value ? null : objRow[conQxRoles.UpdDate].ToString().Trim(); //修改日期(字段类型:varchar,字段长度:20,是否可空:True)
 objQxRolesEN.UpdUserId = objRow[conQxRoles.UpdUserId] == DBNull.Value ? null : objRow[conQxRoles.UpdUserId].ToString().Trim(); //修改用户Id(字段类型:varchar,字段长度:20,是否可空:True)
...
RoleName = objRow[conQxRoles.RoleName].ToString().Trim(), //角色名
...
QxPrjId = objRow[conQxRoles.QxPrjId].ToString().Trim(), //QxPrjId
IsInUse = TransNullToBool(objRow[conQxRoles.IsInUse].ToString().Trim()), //IsInUse
...
objRow[conQxRoles.RoleName] = objQxRolesEN.RoleName; //角色名
...
objRow[conQxRoles.QxPrjId] = objQxRolesEN.QxPrjId; //QxPrjId
objRow[conQxRoles.IsInUse] = objQxRolesEN.IsInUse; //IsInUse
...
return objQxRolesEN.RoleId;
 ...
 /// <returns>如果插入成功则返回新插入记录的关键字的值,否则就报错</returns>
public string AddNewRecordBySQL2WithReturnKey(clsQxRolesEN objQxRolesEN, SqlConnection objSqlConnection, SqlTransaction objSqlTransaction)
{
 objQxRolesEN.UpdDate = clsDateTime.getTodayDateTimeStr(1);
 if (objQxRolesEN._IsCheckProperty  ==  false)
 {
...
objRow[conQxRoles.RoleName] = oRow[conQxRoles.RoleName].ToString().Trim(); //角色名
...
objRow[conQxRoles.QxPrjId] = oRow[conQxRoles.QxPrjId].ToString().Trim(); //QxPrjId
objRow[conQxRoles.IsInUse] = oRow[conQxRoles.IsInUse].ToString().Trim(); //IsInUse
...
sbSQL.AppendFormat("Update QxRoles Set ");
 ...
 {
 var strRoleName = objQxRolesEN.RoleName.Replace("'", "''"); //转换值串中的单撇"'",使之成为双撇"''"
 sbSQL.AppendFormat("{1} = '{0}',", strRoleName, conQxRoles.RoleName); //角色名
 }
 else
 {
 sbSQL.AppendFormat("{0} = null,",conQxRoles.RoleName); //角色名
 }
 }
 ...
 {
 var strQxPrjId = objQxRolesEN.QxPrjId.Replace("'", "''"); //转换值串中的单撇"'",使之成为双撇"''"
 sbSQL.AppendFormat("{1} = '{0}',", strQxPrjId, conQxRoles.QxPrjId); //QxPrjId
 }
 else
 {
 sbSQL.AppendFormat("{0} = null,",conQxRoles.QxPrjId); //QxPrjId
 }
 }
 
 if (objQxRolesEN.IsUpdated(conQxRoles.IsInUse))
 {
 sbSQL.AppendFormat(" {1} = '{0}',", objQxRolesEN.IsInUse == true?"1":"0", conQxRoles.IsInUse); //IsInUse
 }
...
sbSQL.AppendFormat("Update QxRoles Set ");
 ...
 if (objQxRolesEN.IsUpdated(conQxRoles.IsInUse))
 {
 sbSQL.AppendFormat(" IsInUse = '{0}',", objQxRolesEN.IsInUse == true?"1":"0"); //IsInUse
 }
...
clsCheckSql.CheckFieldLen(objQxRolesEN.RoleName, 100, conQxRoles.RoleName);
