...
clsCheckSql.CheckFieldLen(strUserId, 20, conCacheUseState.UserId);
...
objCacheUseStateEN.Memo = strMemo; //备注
if (string.IsNullOrEmpty(strComparisonOp) == false)
{
...
public static string GetCombineCondition(this clsCacheUseStateEN objCacheUseState_Cond)
{
//使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
//例如 1 = 1 && UserName = '张三'
string strWhereCond = " 1 = 1 ";
//如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
if (objCacheUseState_Cond.IsUpdated(conCacheUseState.mId) == true)
{
string strComparisonOp_mId = objCacheUseState_Cond.dicFldComparisonOp[conCacheUseState.mId];
strWhereCond += string.Format(" And {0} {2} {1}", conCacheUseState.mId, objCacheUseState_Cond.mId, strComparisonOp_mId);
}
if (objCacheUseState_Cond.IsUpdated(conCacheUseState.UserId) == true)
{
string strComparisonOp_UserId = objCacheUseState_Cond.dicFldComparisonOp[conCacheUseState.UserId];
strWhereCond += string.Format(" And {0} {2} '{1}'", conCacheUseState.UserId, objCacheUseState_Cond.UserId, strComparisonOp_UserId);
}
if (objCacheUseState_Cond.IsUpdated(conCacheUseState.CacheModeId) == true)
{
string strComparisonOp_CacheModeId = objCacheUseState_Cond.dicFldComparisonOp[conCacheUseState.CacheModeId];
strWhereCond += string.Format(" And {0} {2} '{1}'", conCacheUseState.CacheModeId, objCacheUseState_Cond.CacheModeId, strComparisonOp_CacheModeId);
}
if (objCacheUseState_Cond.IsUpdated(conCacheUseState.CacheKey) == true)
{
string strComparisonOp_CacheKey = objCacheUseState_Cond.dicFldComparisonOp[conCacheUseState.CacheKey];
strWhereCond += string.Format(" And {0} {2} '{1}'", conCacheUseState.CacheKey, objCacheUseState_Cond.CacheKey, strComparisonOp_CacheKey);
}
if (objCacheUseState_Cond.IsUpdated(conCacheUseState.CacheSize) == true)
{
string strComparisonOp_CacheSize = objCacheUseState_Cond.dicFldComparisonOp[conCacheUseState.CacheSize];
strWhereCond += string.Format(" And {0} {2} {1}", conCacheUseState.CacheSize, objCacheUseState_Cond.CacheSize, strComparisonOp_CacheSize);
}
if (objCacheUseState_Cond.IsUpdated(conCacheUseState.Memo) == true)
{
string strComparisonOp_Memo = objCacheUseState_Cond.dicFldComparisonOp[conCacheUseState.Memo];
strWhereCond += string.Format(" And {0} {2} '{1}'", conCacheUseState.Memo, objCacheUseState_Cond.Memo, strComparisonOp_Memo);
}
 return strWhereCond;
}
...
try
{
objCacheUseStateEN.sf_UpdFldSetStr = objCacheUseStateEN.getsf_UpdFldSetStr();
...
throw new Exception(strMsg); 
 ...
 /// <returns></returns>
public static string GetUniConditionStr_mId(this clsCacheUseStateEN objCacheUseStateEN)
{
//检测记录是否存在
...
sbCondition.AppendFormat("1 = 1");
 sbCondition.AppendFormat(" and mId = '{0}'", objCacheUseStateEN.mId);
...
{
if (!Object.Equals(null, objCacheUseStateEN.UserId) && GetStrLen(objCacheUseStateEN.UserId) > 20)
{
...
string strResult = "";
clsCacheUseStateEN objCacheUseStateEN = null;
...
{
objCacheUseStateEN = JsonConvert.DeserializeObject<clsCacheUseStateEN>((string)jobjReturn["ReturnObj"]);
return objCacheUseStateEN;
}
else
{
...
throw new Exception(strMsg);
 ...
 /// <param name = "lngmId">表关键字</param>
 /// <returns>表对象</returns>
public static clsCacheUseStateEN GetObjBymId_WA_Cache(long lngmId, string strUserId)
{
...
public static clsCacheUseStateEN GetObjBymId_Cache(long lngmId, string strUserId)
{
//初始化列表缓存
string strKey = string.Format("{0}_{1}", clsCacheUseStateEN._CurrTabName_S, strUserId);
List<clsCacheUseStateEN> arrCacheUseStateObjLst_Cache = GetObjLst_Cache(strUserId);
IEnumerable <clsCacheUseStateEN> arrCacheUseStateObjLst_Sel =
from objCacheUseStateEN in arrCacheUseStateObjLst_Cache
...
public static string GetUserIdBymId_Cache(long lngmId, string strUserId)
{
//初始化列表缓存
string strKey = string.Format("{0}_{1}", clsCacheUseStateEN._CurrTabName_S, strUserId);
List<clsCacheUseStateEN> arrCacheUseStateObjLst_Cache = GetObjLst_Cache(strUserId);
...
throw new Exception(strErrMsgForGetObjById);
 ...
 /// <param name = "lngmId">所给的关键字</param>
 /// <returns>根据关键字获取的名称</returns>
public static string GetNameBymId_Cache(long lngmId, string strUserId)
{
//初始化列表缓存
string strKey = string.Format("{0}_{1}", clsCacheUseStateEN._CurrTabName_S, strUserId);
List<clsCacheUseStateEN> arrCacheUseStateObjLst_Cache = GetObjLst_Cache(strUserId);
...
{
 List<clsCacheUseStateEN> arrObjLst = null; 
...
if ((int)jobjReturn["ErrorId"] == 0)
{
arrObjLst = JsonConvert.DeserializeObject<List<clsCacheUseStateEN>>((string)jobjReturn["ReturnObjLst"]);
return arrObjLst;
}
else
{
...
throw new Exception(strMsg);
 ...
 /// <param name = "lngmId">所给的关键字</param>
 /// <returns>根据关键字获取的对象</returns>
public static IEnumerable<clsCacheUseStateEN> GetObjLstByMIdLst_Cache(List<long> arrMId, string strUserId)
{
//初始化列表缓存
string strKey = string.Format("{0}_{1}", clsCacheUseStateEN._CurrTabName_S, strUserId);
List<clsCacheUseStateEN> arrCacheUseStateObjLst_Cache = GetObjLst_Cache(strUserId);
...
return arrCacheUseStateObjLst_Sel;
 ...
 /// <param name = "strWhereCond">给定条件</param>
 /// <returns>返回对象列表</returns>
public static List<clsCacheUseStateEN> GetObjLstByMIdLst_WA_Cache(List<long> arrMId, string strUserId)
{
 List<clsCacheUseStateEN> arrObjLst = null; 
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
public static List<clsCacheUseStateEN> GetObjLstByPager_Cache(stuPagerPara objPagerPara)
{
 List<clsCacheUseStateEN> arrObjLst = null; 
...
{
if (string.IsNullOrEmpty(objCacheUseStateEN.sf_UpdFldSetStr) == true)
{
...
//{
//arrCacheUseStateObjLst_Cache = await clsCacheUseStateWApi.GetObjLst(strWhereCond);
//}
}
...
string strKey = string.Format("{0}_{1}", clsCacheUseStateEN._CurrTabName_S, strUserId);
CacheHelper.Remove(strKey);
clsCacheUseStateWApi.objCommFun4BL.ReFreshCache(strUserId);
}
}
...
public static List<clsCacheUseStateEN> GetObjLst_Cache(string strUserId)
{
//初始化列表缓存
InitListCache(); 
string strKey = string.Format("{0}_{1}", clsCacheUseStateEN._CurrTabName_S, strUserId);
List<clsCacheUseStateEN> arrCacheUseStateObjLst_Cache = CacheHelper.GetCache(strKey, () => { return GetObjLst(string.Format("UserId='{0}'", strUserId)); });
