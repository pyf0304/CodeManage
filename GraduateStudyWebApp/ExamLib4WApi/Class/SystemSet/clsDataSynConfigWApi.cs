...
{
if (!Object.Equals(null, objDataSynConfigEN.TabName) && GetStrLen(objDataSynConfigEN.TabName) > 50)
{
...
{
//初始化列表缓存
string strKey = string.Format("{0}", clsDataSynConfigEN._CurrTabName_S);
...
//检查缓存刷新机制
string strMsg = "";
if (clsDataSynConfigWApi.objCommFun4BL == null)
{
strMsg = string.Format("类clsDataSynConfigWApi没有刷新缓存机制(clsDataSynConfigWApi.objCommFun4BL == null), 请联系程序员！(from {0})", clsStackTrace.GetCurrClassFunction());
throw new Exception(strMsg);
}
//初始化列表缓存
...
string strKey = string.Format("{0}", clsDataSynConfigEN._CurrTabName_S);
CacheHelper.Remove(strKey);
clsDataSynConfigWApi.objCommFun4BL.ReFreshCache();
}
}
