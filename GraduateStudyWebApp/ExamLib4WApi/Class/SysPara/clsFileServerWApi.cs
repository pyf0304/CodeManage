...
public static clsFileServerEN Setid_File(this clsFileServerEN objFileServerEN, string strid_File, string strComparisonOp="")
	{
clsCheckSql.CheckFieldLen(strid_File, 8, conFileServer.id_File);
clsCheckSql.CheckFieldForeignKey(strid_File, 8, conFileServer.id_File);
objFileServerEN.id_File = strid_File; //文件流水号
if (string.IsNullOrEmpty(strComparisonOp) == false)
{
if (objFileServerEN.dicFldComparisonOp.ContainsKey(conFileServer.id_File) == false)
{
objFileServerEN.dicFldComparisonOp.Add(conFileServer.id_File, strComparisonOp);
}
else
{
objFileServerEN.dicFldComparisonOp[conFileServer.id_File] = strComparisonOp;
}
}
...
if (objFileServer_Cond.IsUpdated(conFileServer.id_File) == true)
{
string strComparisonOp_id_File = objFileServer_Cond.dicFldComparisonOp[conFileServer.id_File];
strWhereCond += string.Format(" And {0} {2} '{1}'", conFileServer.id_File, objFileServer_Cond.id_File, strComparisonOp_id_File);
}
...
if (objFileServer_Cond.IsUpdated(conFileServer.SaveTime) == true)
{
string strComparisonOp_SaveTime = objFileServer_Cond.dicFldComparisonOp[conFileServer.SaveTime];
strWhereCond += string.Format(" And {0} {2} '{1}'", conFileServer.SaveTime, objFileServer_Cond.SaveTime, strComparisonOp_SaveTime);
}
if (objFileServer_Cond.IsUpdated(conFileServer.ResourceOwner) == true)
{
string strComparisonOp_ResourceOwner = objFileServer_Cond.dicFldComparisonOp[conFileServer.ResourceOwner];
strWhereCond += string.Format(" And {0} {2} '{1}'", conFileServer.ResourceOwner, objFileServer_Cond.ResourceOwner, strComparisonOp_ResourceOwner);
}
if (objFileServer_Cond.IsUpdated(conFileServer.FileOriginName) == true)
{
string strComparisonOp_FileOriginName = objFileServer_Cond.dicFldComparisonOp[conFileServer.FileOriginName];
strWhereCond += string.Format(" And {0} {2} '{1}'", conFileServer.FileOriginName, objFileServer_Cond.FileOriginName, strComparisonOp_FileOriginName);
}
 return strWhereCond;
}
...
{
 if (string.IsNullOrEmpty(objFileServerEN.id_File) == true)
 {
...
try
{
objFileServerEN.sf_UpdFldSetStr = objFileServerEN.getsf_UpdFldSetStr();
...
clsStackTrace.GetCurrClassFunction()); 
 ...
 if (clsFileServerWApi.IsExist(objFileServerEN.id_File) == true)
 {
string strMsg = string.Format("添加记录时，关键字:[{0}]已经存在!(from {1})\r\n", objFileServerEN.id_File, 
clsStackTrace.GetCurrClassFunction()); 
 throw new Exception(strMsg);
 }
try
...
throw new Exception(strMsg); 
 ...
 /// <param name = "objFileServerEN">需要添加的实体对象</param>
 /// <returns>如果插入成功则返回TRUE,否则为FALSE</returns>
public static string AddNewRecordWithMaxId(this clsFileServerEN objFileServerEN)
{
try
{
clsFileServerWApi.CheckPropertyNew(objFileServerEN); 
string strid_File = clsFileServerWApi.AddNewRecordWithMaxId(objFileServerEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsFileServerWApi.ReFreshCache();
return strid_File;
}
...
public static void BindDdl_id_File(System.Web.UI.WebControls.DropDownList objDDL)
{
//为数据源于表的下拉框设置内容
System.Web.UI.WebControls.ListItem li = new System.Web.UI.WebControls.ListItem("选[文件服务]...","0");
List<clsFileServerEN> arrObjLst = GetObjLst("1=1");
objDDL.DataValueField="id_File";
...
public static void BindCbo_id_File(System.Windows.Forms.ComboBox objComboBox )
{
//为数据源为表的下拉框设置内容
string strCondition = string.Format("1 =1 Order By {0}", conFileServer.id_File); 
List<clsFileServerEN> arrObjLst = clsFileServerWApi.GetObjLst(strCondition);
//初始化一个对象列表
...
arrObjLst.Insert(0, objFileServerEN);
//设置下拉框的数据源、以及设置值项、显示项
objComboBox.ValueMember = conFileServer.id_File;
objComboBox.DisplayMember = conFileServer.FileName;
...
{
if (!Object.Equals(null, objFileServerEN.id_File) && GetStrLen(objFileServerEN.id_File) > 8)
{
 throw new Exception("字段[文件流水号]的长度不能超过8!");
}
if (!Object.Equals(null, objFileServerEN.FileName) && GetStrLen(objFileServerEN.FileName) > 500)
{
...
if (!Object.Equals(null, objFileServerEN.SaveDate) && GetStrLen(objFileServerEN.SaveDate) > 8)
{
 throw new Exception("字段[创建日期]的长度不能超过8!");
}
if (!Object.Equals(null, objFileServerEN.FileSize) && GetStrLen(objFileServerEN.FileSize) > 50)
{
 throw new Exception("字段[文件大小]的长度不能超过50!");
}
if (!Object.Equals(null, objFileServerEN.SaveTime) && GetStrLen(objFileServerEN.SaveTime) > 6)
{
 throw new Exception("字段[创建时间]的长度不能超过6!");
}
if (!Object.Equals(null, objFileServerEN.ResourceOwner) && GetStrLen(objFileServerEN.ResourceOwner) > 50)
{
 throw new Exception("字段[上传者]的长度不能超过50!");
}
if (!Object.Equals(null, objFileServerEN.FileOriginName) && GetStrLen(objFileServerEN.FileOriginName) > 500)
{
 throw new Exception("字段[原文件名]的长度不能超过500!");
}
 ...
 /// <returns>表对象</returns>
public static clsFileServerEN GetObjByid_File(string strid_File)
{
string strAction = "GetObjByid_File";
...
throw new Exception(strMsg);
 ...
 /// <param name = "strid_File">表关键字</param>
 /// <returns>表对象</returns>
public static clsFileServerEN GetObjByid_File_WA_Cache(string strid_File)
{
string strAction = "GetObjByid_File_Cache";
string strErrMsg = string.Empty;
string strResult = "";
clsFileServerEN objFileServerEN = null;
Dictionary<string, string> dictParam = new Dictionary<string, string>()
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <param name = "strid_File">所给的关键字</param>
 /// <returns>根据关键字获取的对象</returns>
public static clsFileServerEN GetObjByid_File_Cache(string strid_File)
{
if (string.IsNullOrEmpty(strid_File) == true) return null;
//初始化列表缓存
string strKey = string.Format("{0}", clsFileServerEN._CurrTabName_S);
...
where objFileServerEN.id_File == strid_File
select objFileServerEN;
if (arrFileServerObjLst_Sel.Count() == 0)
{
   clsFileServerEN obj = clsFileServerWApi.GetObjByid_File(strid_File);
   if (obj != null)
 {
...
public static string GetFileNameByid_File_Cache(string strid_File)
{
if (string.IsNullOrEmpty(strid_File) == true) return "";
//初始化列表缓存
string strKey = string.Format("{0}", clsFileServerEN._CurrTabName_S);
List<clsFileServerEN> arrFileServerObjLst_Cache = GetObjLst_Cache();
IEnumerable <clsFileServerEN> arrFileServerObjLst_Sel1 =
from objFileServerEN in arrFileServerObjLst_Cache
where objFileServerEN.id_File == strid_File
select objFileServerEN;
List <clsFileServerEN> arrFileServerObjLst_Sel = new List<clsFileServerEN>();
foreach (clsFileServerEN obj in arrFileServerObjLst_Sel1)
{
arrFileServerObjLst_Sel.Add(obj);
}
if (arrFileServerObjLst_Sel.Count > 0)
{
return arrFileServerObjLst_Sel[0].FileName;
}
string strErrMsgForGetObjById = string.Format("在FileServer对象缓存列表中,找不到记录[id_File = {0}](函数:{1})", strid_File, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsFileServerBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
throw new Exception(strErrMsgForGetObjById);
 ...
 /// <param name = "strid_File">所给的关键字</param>
 /// <returns>根据关键字获取的名称</returns>
public static string GetNameByid_File_Cache(string strid_File)
{
if (string.IsNullOrEmpty(strid_File) == true) return "";
//初始化列表缓存
string strKey = string.Format("{0}", clsFileServerEN._CurrTabName_S);
...
string strErrMsgForGetObjById = string.Format("在FileServer对象缓存列表中,找不到记录的相关名称[id_File = {0}](函数:{1})", strid_File, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsFileServerBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
throw new Exception(strErrMsgForGetObjById);
}
...
throw new Exception(strMsg);
 ...
 /// <param name = "strWhereCond">给定条件</param>
 /// <returns>返回对象列表</returns>
public static List<clsFileServerEN> GetObjLstById_FileLst(List<string> arrId_File)
{
 List<clsFileServerEN> arrObjLst = null; 
string strAction = "GetObjLstById_FileLst";
string strErrMsg = string.Empty;
string strResult = "";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrId_File);
...
throw new Exception(strMsg);
 ...
 /// <param name = "strid_File">所给的关键字</param>
 /// <returns>根据关键字获取的对象</returns>
public static IEnumerable<clsFileServerEN> GetObjLstById_FileLst_Cache(List<string> arrId_File)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsFileServerEN._CurrTabName_S);
List<clsFileServerEN> arrFileServerObjLst_Cache = GetObjLst_Cache();
IEnumerable <clsFileServerEN> arrFileServerObjLst_Sel =
from objFileServerEN in arrFileServerObjLst_Cache
where arrId_File.Contains(objFileServerEN.id_File)
select objFileServerEN;
return arrFileServerObjLst_Sel;
 ...
 /// <param name = "strWhereCond">给定条件</param>
 /// <returns>返回对象列表</returns>
public static List<clsFileServerEN> GetObjLstById_FileLst_WA_Cache(List<string> arrId_File)
{
 List<clsFileServerEN> arrObjLst = null; 
string strAction = "GetObjLstById_FileLst_Cache";
string strErrMsg = string.Empty;
string strResult = "";
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
...
public static int DelRecord(string strid_File)
{
...
try
{
 clsFileServerEN objFileServerEN = clsFileServerWApi.GetObjByid_File(strid_File);
if (clsPubFun4WApi.Delete(mstrApiControllerName, strAction, strid_File.ToString(), out strResult, out strErrMsg) == true)
{
JObject jobjReturn = JObject.Parse(strResult);
if ((int)jobjReturn["ErrorId"] == 0)
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>实际删除记录的个数</returns>
public static int DelFileServers(List<string> arrid_File)
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>是否成功?</returns>
public static string AddNewRecordWithMaxId(clsFileServerEN objFileServerEN)
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsFileServerWApi.ReFreshCache();
var strid_File = (string)jobjReturn["ReturnStr"];
return strid_File;
}
else
{
...
{
if (string.IsNullOrEmpty(objFileServerEN.sf_UpdFldSetStr) == true)
{
string strMsg = string.Format("修改时，修改标志串为空，请联系管理员.对象关键字:{0}.(from {1}).",
objFileServerEN.id_File, clsStackTrace.GetCurrClassFunction());
throw new Exception(strMsg);
}
...
throw new Exception(strMsg);
 ...
 /// </summary>
 /// <returns>是否存在?存在返回True</returns>
public static bool IsExist(string strid_File)
{
//检测记录是否存在
...
try
{
objFileServerENT.id_File = objFileServerENS.id_File; //文件流水号
...
//初始化列表缓存
//string strWhereCond = string.Format("1 = 1 order by id_File");
...
string strKey = string.Format("{0}", clsFileServerEN._CurrTabName_S);
CacheHelper.Remove(strKey);
clsFileServerWApi.objCommFun4BL.ReFreshCache();
}
}
...
objDT.Columns.Add(conFileServer.id_File, Type.GetType("System.String"));
...
objDT.Columns.Add(conFileServer.FileSize, Type.GetType("System.String"));
objDT.Columns.Add(conFileServer.SaveTime, Type.GetType("System.String"));
objDT.Columns.Add(conFileServer.ResourceOwner, Type.GetType("System.String"));
objDT.Columns.Add(conFileServer.FileOriginName, Type.GetType("System.String"));
foreach (clsFileServerEN objInFor in arrObjLst)
{
DataRow objDR = objDT.NewRow();
objDR[conFileServer.id_File] = objInFor[conFileServer.id_File];
...
objDR[conFileServer.SaveTime] = objInFor[conFileServer.SaveTime];
objDR[conFileServer.ResourceOwner] = objInFor[conFileServer.ResourceOwner];
objDR[conFileServer.FileOriginName] = objInFor[conFileServer.FileOriginName];
