...
public static clsCaseTypeEN SetId_CaseType(this clsCaseTypeEN objCaseTypeEN, string strId_CaseType, string strComparisonOp="")
	{
clsCheckSql.CheckFieldLen(strId_CaseType, 4, conCaseType.Id_CaseType);
clsCheckSql.CheckFieldForeignKey(strId_CaseType, 4, conCaseType.Id_CaseType);
objCaseTypeEN.Id_CaseType = strId_CaseType; //案例类型流水号
if (string.IsNullOrEmpty(strComparisonOp) == false)
{
if (objCaseTypeEN.dicFldComparisonOp.ContainsKey(conCaseType.Id_CaseType) == false)
{
objCaseTypeEN.dicFldComparisonOp.Add(conCaseType.Id_CaseType, strComparisonOp);
}
else
{
objCaseTypeEN.dicFldComparisonOp[conCaseType.Id_CaseType] = strComparisonOp;
}
}
...
if (objCaseType_Cond.IsUpdated(conCaseType.Id_CaseType) == true)
{
string strComparisonOp_Id_CaseType = objCaseType_Cond.dicFldComparisonOp[conCaseType.Id_CaseType];
strWhereCond += string.Format(" And {0} {2} '{1}'", conCaseType.Id_CaseType, objCaseType_Cond.Id_CaseType, strComparisonOp_Id_CaseType);
}
...
{
 if (string.IsNullOrEmpty(objCaseTypeEN.Id_CaseType) == true)
 {
...
try
{
objCaseTypeEN.sf_UpdFldSetStr = objCaseTypeEN.getsf_UpdFldSetStr();
...
throw new Exception(strMsg); 
 ...
 /// <returns></returns>
public static string GetUniConditionStr_CaseTypeID(this clsCaseTypeEN objCaseTypeEN)
{
//检测记录是否存在
StringBuilder sbCondition = new StringBuilder();
if (objCaseTypeEN == null) return "";
if (objCaseTypeEN.Id_CaseType == null || objCaseTypeEN.Id_CaseType == "")
{
...
sbCondition.AppendFormat("Id_CaseType !=  '{0}'", objCaseTypeEN.Id_CaseType);
 sbCondition.AppendFormat(" and CaseTypeID = '{0}'", objCaseTypeEN.CaseTypeID);
 return sbCondition.ToString();
}
}
...
clsStackTrace.GetCurrClassFunction()); 
 ...
 if (clsCaseTypeWApi.IsExist(objCaseTypeEN.Id_CaseType) == true)
 {
string strMsg = string.Format("添加记录时，关键字:[{0}]已经存在!(from {1})\r\n", objCaseTypeEN.Id_CaseType, 
clsStackTrace.GetCurrClassFunction()); 
 throw new Exception(strMsg);
 }
try
...
{
clsCaseTypeWApi.CheckPropertyNew(objCaseTypeEN); 
string strId_CaseType = clsCaseTypeWApi.AddNewRecordWithMaxId(objCaseTypeEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsCaseTypeWApi.ReFreshCache();
return strId_CaseType;
}
catch (Exception objException)
{
...
public static void BindDdl_Id_CaseType(System.Web.UI.WebControls.DropDownList objDDL)
{
//为数据源于表的下拉框设置内容
...
objDDL.DataValueField="Id_CaseType";
...
public static void BindCbo_Id_CaseType(System.Windows.Forms.ComboBox objComboBox )
{
//为数据源为表的下拉框设置内容
string strCondition = string.Format("1 =1 Order By {0}", conCaseType.Id_CaseType); 
...
objComboBox.ValueMember = conCaseType.Id_CaseType;
...
{
if (!Object.Equals(null, objCaseTypeEN.Id_CaseType) && GetStrLen(objCaseTypeEN.Id_CaseType) > 4)
{
...
{
 throw new Exception("字段[案例类型ID]的长度不能超过4!");
}
if (!Object.Equals(null, objCaseTypeEN.CaseTypeName) && GetStrLen(objCaseTypeEN.CaseTypeName) > 50)
{
 throw new Exception("字段[案例类型名称]的长度不能超过50!");
}
if (!Object.Equals(null, objCaseTypeEN.Memo) && GetStrLen(objCaseTypeEN.Memo) > 1000)
{
 ...
 /// <returns>表对象</returns>
public static clsCaseTypeEN GetObjById_CaseType(string strId_CaseType)
{
string strAction = "GetObjById_CaseType";
string strErrMsg = string.Empty;
string strResult = "";
clsCaseTypeEN objCaseTypeEN = null;
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["Id_CaseType"] = strId_CaseType
};
try
{
...
throw new Exception(strMsg);
 ...
 /// <param name = "strId_CaseType">表关键字</param>
 /// <returns>表对象</returns>
public static clsCaseTypeEN GetObjById_CaseType_WA_Cache(string strId_CaseType)
{
string strAction = "GetObjById_CaseType_Cache";
string strErrMsg = string.Empty;
string strResult = "";
clsCaseTypeEN objCaseTypeEN = null;
Dictionary<string, string> dictParam = new Dictionary<string, string>()
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <param name = "strId_CaseType">所给的关键字</param>
 /// <returns>根据关键字获取的对象</returns>
public static clsCaseTypeEN GetObjById_CaseType_Cache(string strId_CaseType)
{
if (string.IsNullOrEmpty(strId_CaseType) == true) return null;
//初始化列表缓存
string strKey = string.Format("{0}", clsCaseTypeEN._CurrTabName_S);
...
where objCaseTypeEN.Id_CaseType == strId_CaseType
select objCaseTypeEN;
if (arrCaseTypeObjLst_Sel.Count() == 0)
{
   clsCaseTypeEN obj = clsCaseTypeWApi.GetObjById_CaseType(strId_CaseType);
   if (obj != null)
 {
...
public static string GetCaseTypeNameById_CaseType_Cache(string strId_CaseType)
{
if (string.IsNullOrEmpty(strId_CaseType) == true) return "";
//初始化列表缓存
string strKey = string.Format("{0}", clsCaseTypeEN._CurrTabName_S);
List<clsCaseTypeEN> arrCaseTypeObjLst_Cache = GetObjLst_Cache();
...
string strErrMsgForGetObjById = string.Format("在CaseType对象缓存列表中,找不到记录[Id_CaseType = {0}](函数:{1})", strId_CaseType, clsStackTrace.GetCurrFunction());
...
throw new Exception(strErrMsgForGetObjById);
 ...
 /// <param name = "strId_CaseType">所给的关键字</param>
 /// <returns>根据关键字获取的名称</returns>
public static string GetNameById_CaseType_Cache(string strId_CaseType)
{
if (string.IsNullOrEmpty(strId_CaseType) == true) return "";
//初始化列表缓存
string strKey = string.Format("{0}", clsCaseTypeEN._CurrTabName_S);
...
string strErrMsgForGetObjById = string.Format("在CaseType对象缓存列表中,找不到记录的相关名称[Id_CaseType = {0}](函数:{1})", strId_CaseType, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsCaseTypeBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
throw new Exception(strErrMsgForGetObjById);
}
...
throw new Exception(strMsg);
 ...
 /// <param name = "strWhereCond">给定条件</param>
 /// <returns>返回对象列表</returns>
public static List<clsCaseTypeEN> GetObjLstById_CaseTypeLst(List<string> arrId_CaseType)
{
 List<clsCaseTypeEN> arrObjLst = null; 
string strAction = "GetObjLstById_CaseTypeLst";
string strErrMsg = string.Empty;
string strResult = "";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrId_CaseType);
...
throw new Exception(strMsg);
 ...
 /// <param name = "strId_CaseType">所给的关键字</param>
 /// <returns>根据关键字获取的对象</returns>
public static IEnumerable<clsCaseTypeEN> GetObjLstById_CaseTypeLst_Cache(List<string> arrId_CaseType)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsCaseTypeEN._CurrTabName_S);
List<clsCaseTypeEN> arrCaseTypeObjLst_Cache = GetObjLst_Cache();
IEnumerable <clsCaseTypeEN> arrCaseTypeObjLst_Sel =
from objCaseTypeEN in arrCaseTypeObjLst_Cache
where arrId_CaseType.Contains(objCaseTypeEN.Id_CaseType)
select objCaseTypeEN;
return arrCaseTypeObjLst_Sel;
 ...
 /// <param name = "strWhereCond">给定条件</param>
 /// <returns>返回对象列表</returns>
public static List<clsCaseTypeEN> GetObjLstById_CaseTypeLst_WA_Cache(List<string> arrId_CaseType)
{
 List<clsCaseTypeEN> arrObjLst = null; 
string strAction = "GetObjLstById_CaseTypeLst_Cache";
string strErrMsg = string.Empty;
string strResult = "";
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
...
public static int DelRecord(string strId_CaseType)
{
...
try
{
 clsCaseTypeEN objCaseTypeEN = clsCaseTypeWApi.GetObjById_CaseType(strId_CaseType);
if (clsPubFun4WApi.Delete(mstrApiControllerName, strAction, strId_CaseType.ToString(), out strResult, out strErrMsg) == true)
{
JObject jobjReturn = JObject.Parse(strResult);
if ((int)jobjReturn["ErrorId"] == 0)
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>实际删除记录的个数</returns>
public static int DelCaseTypes(List<string> arrId_CaseType)
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsCaseTypeWApi.ReFreshCache();
var strId_CaseType = (string)jobjReturn["ReturnStr"];
return strId_CaseType;
}
else
{
...
{
if (string.IsNullOrEmpty(objCaseTypeEN.sf_UpdFldSetStr) == true)
{
string strMsg = string.Format("修改时，修改标志串为空，请联系管理员.对象关键字:{0}.(from {1}).",
objCaseTypeEN.Id_CaseType, clsStackTrace.GetCurrClassFunction());
throw new Exception(strMsg);
}
...
throw new Exception(strMsg);
 ...
 /// </summary>
 /// <returns>是否存在?存在返回True</returns>
public static bool IsExist(string strId_CaseType)
{
//检测记录是否存在
...
try
{
objCaseTypeENT.Id_CaseType = objCaseTypeENS.Id_CaseType; //案例类型流水号
...
//初始化列表缓存
//string strWhereCond = string.Format("1 = 1 order by Id_CaseType");
//if (arrCaseTypeObjLst_Cache == null)
//{
//arrCaseTypeObjLst_Cache = await clsCaseTypeWApi.GetObjLst(strWhereCond);
//}
}
...
string strKey = string.Format("{0}", clsCaseTypeEN._CurrTabName_S);
CacheHelper.Remove(strKey);
clsCaseTypeWApi.objCommFun4BL.ReFreshCache();
}
}
...
objDT.Columns.Add(conCaseType.Id_CaseType, Type.GetType("System.String"));
...
objDR[conCaseType.Id_CaseType] = objInFor[conCaseType.Id_CaseType];
