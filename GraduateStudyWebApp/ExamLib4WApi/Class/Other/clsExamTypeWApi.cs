...
{
public static class clsExamTypeWApi_Static
{
...
try
{
objExamTypeEN.sf_UpdFldSetStr = objExamTypeEN.getsf_UpdFldSetStr();
clsExamTypeWApi.CheckPropertyNew(objExamTypeEN); 
bool bolResult = clsExamTypeWApi.UpdateRecord(objExamTypeEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsExamTypeWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// <returns></returns>
public static string GetUniConditionStr_ExamTypeName(this clsExamTypeEN objExamTypeEN)
{
//检测记录是否存在
...
try
{
 if (string.IsNullOrEmpty(objExamTypeEN.ExamTypeId) == true || clsExamTypeWApi.IsExist(objExamTypeEN.ExamTypeId) == true)
 {
     objExamTypeEN.ExamTypeId = clsExamTypeWApi.GetMaxStrId();
 }
clsExamTypeWApi.CheckPropertyNew(objExamTypeEN); 
bool bolResult = clsExamTypeWApi.AddNewRecord(objExamTypeEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsExamTypeWApi.ReFreshCache();
return bolResult;
}
...
{
clsExamTypeWApi.CheckPropertyNew(objExamTypeEN); 
string strExamTypeId = clsExamTypeWApi.AddNewRecordWithMaxId(objExamTypeEN);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsExamTypeWApi.ReFreshCache();
...
{
clsExamTypeWApi.CheckPropertyNew(objExamTypeEN); 
bool bolResult = clsExamTypeWApi.UpdateWithCondition(objExamTypeEN, strWhereCond);
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsExamTypeWApi.ReFreshCache();
return bolResult;
}
...
throw new Exception(strMsg); 
 ...
 /// </summary>
public class clsExamTypeWApi
{
private static readonly string mstrApiControllerName = "ExamTypeApi";
...
List<clsExamTypeEN> arrObjLst = GetObjLst("1=1");
objDDL.DataValueField="ExamTypeId";
...
List<clsExamTypeEN> arrObjLst = clsExamTypeWApi.GetObjLst(strCondition);
//初始化一个对象列表
...
{
if (!Object.Equals(null, objExamTypeEN.ExamTypeId) && GetStrLen(objExamTypeEN.ExamTypeId) > 8)
{
 throw new Exception("字段[题目类型ID]的长度不能超过8!");
}
if (!Object.Equals(null, objExamTypeEN.ExamTypeName) && GetStrLen(objExamTypeEN.ExamTypeName) > 50)
{
 throw new Exception("字段[题库类型名称]的长度不能超过50!");
}
if (!Object.Equals(null, objExamTypeEN._DeletedDate) && GetStrLen(objExamTypeEN._DeletedDate) > 50)
{
 throw new Exception("字段[删除日期]的长度不能超过50!");
}
if (!Object.Equals(null, objExamTypeEN.Memo) && GetStrLen(objExamTypeEN.Memo) > 1000)
{
...
{
string strAction = "GetObjByExamTypeId";
string strErrMsg = string.Empty;
string strResult = "";
clsExamTypeEN objExamTypeEN = null;
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
["ExamTypeId"] = strExamTypeId
};
try
{
...
{
objExamTypeEN = JsonConvert.DeserializeObject<clsExamTypeEN>((string)jobjReturn["ReturnObj"]);
return objExamTypeEN;
}
else
{
...
throw new Exception(strMsg);
 ...
 /// <param name = "strExamTypeId">表关键字</param>
 /// <returns>表对象</returns>
public static clsExamTypeEN GetObjByExamTypeId_WA_Cache(string strExamTypeId)
{
string strAction = "GetObjByExamTypeId_Cache";
string strErrMsg = string.Empty;
string strResult = "";
clsExamTypeEN objExamTypeEN = null;
Dictionary<string, string> dictParam = new Dictionary<string, string>()
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static clsExamTypeEN GetFirstObj(string strWhereCond)
{
...
{
if (string.IsNullOrEmpty(strExamTypeId) == true) return null;
//初始化列表缓存
...
from objExamTypeEN in arrExamTypeObjLst_Cache
where objExamTypeEN.ExamTypeId == strExamTypeId
select objExamTypeEN;
if (arrExamTypeObjLst_Sel.Count() == 0)
{
   clsExamTypeEN obj = clsExamTypeWApi.GetObjByExamTypeId(strExamTypeId);
   if (obj != null)
 {
...
string strKey = string.Format("{0}", clsExamTypeEN._CurrTabName_S);
List<clsExamTypeEN> arrExamTypeObjLst_Cache = GetObjLst_Cache();
IEnumerable <clsExamTypeEN> arrExamTypeObjLst_Sel1 =
from objExamTypeEN in arrExamTypeObjLst_Cache
where objExamTypeEN.ExamTypeId == strExamTypeId
select objExamTypeEN;
List <clsExamTypeEN> arrExamTypeObjLst_Sel = new List<clsExamTypeEN>();
foreach (clsExamTypeEN obj in arrExamTypeObjLst_Sel1)
{
arrExamTypeObjLst_Sel.Add(obj);
}
if (arrExamTypeObjLst_Sel.Count > 0)
{
return arrExamTypeObjLst_Sel[0].ExamTypeName;
}
string strErrMsgForGetObjById = string.Format("在ExamType对象缓存列表中,找不到记录[ExamTypeId = {0}](函数:{1})", strExamTypeId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsExamTypeBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
...
string strErrMsgForGetObjById = string.Format("在ExamType对象缓存列表中,找不到记录的相关名称[ExamTypeId = {0}](函数:{1})", strExamTypeId, clsStackTrace.GetCurrFunction());
clsLog.LogErrorS2("clsExamTypeBL", clsStackTrace.GetCurrClassFunction(), strErrMsgForGetObjById, "", "");
throw new Exception(strErrMsgForGetObjById);
}
...
{
 List<clsExamTypeEN> arrObjLst = null; 
...
if ((int)jobjReturn["ErrorId"] == 0)
{
arrObjLst = JsonConvert.DeserializeObject<List<clsExamTypeEN>>((string)jobjReturn["ReturnObjLst"]);
return arrObjLst;
}
else
{
...
throw new Exception(strMsg);
 ...
 /// <param name = "strWhereCond">给定条件</param>
 /// <returns>返回对象列表</returns>
public static List<clsExamTypeEN> GetObjLstByExamTypeIdLst(List<string> arrExamTypeId)
{
 List<clsExamTypeEN> arrObjLst = null; 
string strAction = "GetObjLstByExamTypeIdLst";
string strErrMsg = string.Empty;
string strResult = "";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
{
string strJSON = clsJSON.GetJsonFromObjLst(arrExamTypeId);
...
throw new Exception(strMsg);
 ...
 /// <param name = "strExamTypeId">所给的关键字</param>
 /// <returns>根据关键字获取的对象</returns>
public static IEnumerable<clsExamTypeEN> GetObjLstByExamTypeIdLst_Cache(List<string> arrExamTypeId)
{
//初始化列表缓存
string strKey = string.Format("{0}", clsExamTypeEN._CurrTabName_S);
List<clsExamTypeEN> arrExamTypeObjLst_Cache = GetObjLst_Cache();
IEnumerable <clsExamTypeEN> arrExamTypeObjLst_Sel =
from objExamTypeEN in arrExamTypeObjLst_Cache
where arrExamTypeId.Contains(objExamTypeEN.ExamTypeId)
select objExamTypeEN;
return arrExamTypeObjLst_Sel;
 ...
 /// <param name = "strWhereCond">给定条件</param>
 /// <returns>返回对象列表</returns>
public static List<clsExamTypeEN> GetObjLstByExamTypeIdLst_WA_Cache(List<string> arrExamTypeId)
{
 List<clsExamTypeEN> arrObjLst = null; 
string strAction = "GetObjLstByExamTypeIdLst_Cache";
string strErrMsg = string.Empty;
string strResult = "";
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
...
throw new Exception(strMsg);
 ...
 /// <returns>返回对象列表</returns>
public static List<clsExamTypeEN> GetObjLstByRange(stuRangePara objRangePara)
{
 List<clsExamTypeEN> arrObjLst = null; 
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "objPagerPara">分页获取记录的参数对象</param>
 /// <returns>返回JSON对象列表</returns>
public static List<clsExamTypeEN> GetObjLstByPager_Cache(stuPagerPara objPagerPara)
{
 List<clsExamTypeEN> arrObjLst = null; 
...
try
{
 clsExamTypeEN objExamTypeEN = clsExamTypeWApi.GetObjByExamTypeId(strExamTypeId);
if (clsPubFun4WApi.Delete(mstrApiControllerName, strAction, strExamTypeId.ToString(), out strResult, out strErrMsg) == true)
{
JObject jobjReturn = JObject.Parse(strResult);
if ((int)jobjReturn["ErrorId"] == 0)
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>实际删除记录的个数</returns>
public static int DelExamTypes(List<string> arrExamTypeId)
{
string strAction = "DelExamTypes";
string strErrMsg = string.Empty;
string strResult = "";
Dictionary<string, string> dictParam = new Dictionary<string, string>();
try
...
{
string strAction = "DelExamTypesByCond";
string strErrMsg = string.Empty;
string strResult = "";
Dictionary<string, string> dictParam = new Dictionary<string, string>()
{
...
public static bool AddNewRecord(clsExamTypeEN objExamTypeEN)
{
...
try
{
string strJson = clsJSON.GetJsonFromObj<clsExamTypeEN>(objExamTypeEN);
if (clsPubFun4WApi.Post(mstrApiControllerName, strAction, dictParam, strJson, out strResult, out strErrMsg) == true)
{
JObject jobjReturn = JObject.Parse(strResult);
if ((int)jobjReturn["ErrorId"] == 0)
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>是否成功?</returns>
public static string AddNewRecordWithMaxId(clsExamTypeEN objExamTypeEN)
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsExamTypeWApi.ReFreshCache();
var strExamTypeId = (string)jobjReturn["ReturnStr"];
return strExamTypeId;
}
else
{
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// </summary>
 /// <returns>是否成功?</returns>
public static bool UpdateRecord(clsExamTypeEN objExamTypeEN)
{
if (string.IsNullOrEmpty(objExamTypeEN.sf_UpdFldSetStr) == true)
{
string strMsg = string.Format("修改时，修改标志串为空，请联系管理员.对象关键字:{0}.(from {1}).",
objExamTypeEN.ExamTypeId, clsStackTrace.GetCurrClassFunction());
throw new Exception(strMsg);
}
...
clsPubFun4WApi.GetWebApiUrl(mstrApiControllerName, strAction));
 ...
 /// <param name = "strWhereCond">条件串</param>
 /// <returns>返回的第一条记录的关键字值</returns>
public static bool UpdateWithCondition(clsExamTypeEN objExamTypeEN, string strWhereCond)
{
if (string.IsNullOrEmpty(objExamTypeEN.sf_UpdFldSetStr) == true)
{
string strMsg = string.Format("修改时，修改标志串为空，请联系管理员.对象关键字:{0}.(from {1}).",
...
public static DataTable ToDataTable(List<clsExamTypeEN> arrObj)
{
DataTable dataTable = new DataTable(); //实例化
DataTable result;
if (arrObj.Count == 0) return null;
if (clsExamTypeEN.AttributeName.Length == 0)
{
result = dataTable;
return result;
}
Type type = typeof(clsExamTypeEN);
PropertyInfo[] arrPropertyInfo = type.GetProperties();
try
{
//Columns
foreach (string strAttrName in clsExamTypeEN.AttributeName)
{
PropertyInfo proprety_Curr = arrPropertyInfo.Where(x => x.Name == strAttrName).First();
dataTable.Columns.Add(strAttrName, proprety_Curr.PropertyType);
}
foreach (clsExamTypeEN objInFor in arrObj)
{
//Rows
...
//检查缓存刷新机制
string strMsg = "";
if (clsExamTypeWApi.objCommFun4BL == null)
{
strMsg = string.Format("类clsExamTypeWApi没有刷新缓存机制(clsExamTypeWApi.objCommFun4BL == null), 请联系程序员！(from {0})", clsStackTrace.GetCurrClassFunction());
throw new Exception(strMsg);
}
//初始化列表缓存
...
//{
//arrExamTypeObjLst_Cache = await clsExamTypeWApi.GetObjLst(strWhereCond);
//}
}
...
string strKey = string.Format("{0}", clsExamTypeEN._CurrTabName_S);
CacheHelper.Remove(strKey);
clsExamTypeWApi.objCommFun4BL.ReFreshCache();
}
}
...
public static DataTable GetDataTableByObjLst(List<clsExamTypeEN> arrObjLst)
{
DataTable objDT = new DataTable();
objDT.Columns.Add(conExamType.ExamTypeId, Type.GetType("System.String"));
objDT.Columns.Add(conExamType.ExamTypeName, Type.GetType("System.String"));
objDT.Columns.Add(conExamType._IsDeleted, Type.GetType("System.Boolean"));
objDT.Columns.Add(conExamType._DeletedDate, Type.GetType("System.String"));
objDT.Columns.Add(conExamType.Memo, Type.GetType("System.String"));
foreach (clsExamTypeEN objInFor in arrObjLst)
{
DataRow objDR = objDT.NewRow();
objDR[conExamType.ExamTypeId] = objInFor[conExamType.ExamTypeId];
objDR[conExamType.ExamTypeName] = objInFor[conExamType.ExamTypeName];
objDR[conExamType._IsDeleted] = objInFor[conExamType._IsDeleted];
objDR[conExamType._DeletedDate] = objInFor[conExamType._DeletedDate];
objDR[conExamType.Memo] = objInFor[conExamType.Memo];
...
return objDT;
 ...
 /// </summary>
public class clsCommFun4WA4ExamType : clsCommFun4BL
{
...
// 静态的对象列表,用于清空相关缓存,针对记录较少,作为参数表可以使用
clsExamTypeWApi.ReFreshThisCache();
}
}
