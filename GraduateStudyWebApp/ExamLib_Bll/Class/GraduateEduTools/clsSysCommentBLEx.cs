...
{
    public static class clsSysCommentBLEx
    {
        ...
        public static bool AddViewpointTotal(string strTypeUserId, string strCurrEduId, string strUserId)
        {
            ...
            try
            {
                ...
                //查询用户各个类型的分数；
                List<clsSysScoreSummaryEN> objList = clsSysScoreSummaryBL.GetObjLst("UserId= '" + strTypeUserId + "' And IdCurrEduCls='" + strCurrEduId + "' And ScoreTypeId in('0001','0002','0003','0004','0005','0006','0007','0021','0023','0025')");
        ...
        public static float GetScorebyScoreTypeId(string strUserId, string strScoreTypeId, string strCurrEduId, string strScoreWeightValue)
        {
            ...
            try
            {
                ...
                switch (strScoreTypeId)
                {
                    ...
                    case "0022":
                        //论文子观点
                        strNewScoreTypeId = "0011";
                        strNewScoreWeightValue = (Convert.ToDouble(strScoreWeightValue) * 0.1).ToString();
                        break;
                    case "0024":
                        //论文子观点
                        strNewScoreTypeId = "0012";
                        strNewScoreWeightValue = (Convert.ToDouble(strScoreWeightValue) * 0.1).ToString();
                        break;
                    case "0026":
                        //论文子观点
                        strNewScoreTypeId = "0013";
                        strNewScoreWeightValue = (Convert.ToDouble(strScoreWeightValue) * 0.1).ToString();
                        break;
                    default:
                        string strMsg = string.Format("没有获得类型数据");
            ...
            try
            {
                ...
                switch (strCommentTypeId)
                {
                    ...
                    case "11":
                        ...
                        //调用论文修改；
                        bool cAnswer = UpdateRecordqa_Answer(objqa_Answer);
                        if (cAnswer == true)
                        {
                            bool csi = GetSysCountData(objcountEnEx);
                            if (csi == true)
                            {
                                ...
                                if (i == true)
                                {
                                    string msg = string.Format("论文回答讨论更新完成，统计数据完成");
                                    result = true;
                                }
                                else
                                {
                                    string msg = string.Format("论文回答讨论更新失败");
                                    result = false;
                                }
                        ...
                        #endregion
                        WriteTextLog("101.251.68.133", "论文回答讨论", "SysCommentLog");
                        break;
