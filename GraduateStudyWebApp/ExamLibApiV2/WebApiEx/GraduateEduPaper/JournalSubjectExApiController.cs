...
/*-- -- -- -- -- -- -- -- -- -- --
类名:JournalSubjectExApiController
表名:JournalSubject(01120930)
生成代码版本:2021.06.19.1
生成日期:2021/06/20 00:37:45
生成者:pyf
...
{
    /// <summary>
    /// JournalSubjectExApiController 的摘要说明
    /// (AutoGCLib.WA_SrvEx4CSharp:GeneCode)
    /// </summary>
    public class JournalSubjectExApiController : JournalSubjectApiController
    {
        ...
        public ActionResult ImportDataFromCsvBak([FromBody]string strCsv, string strUserId)
        {
            ...
            try
            {
                var varResult = clsJournalSubjectBLEx.ImportDataFromCsv(strCsv, strUserId);
        ...
        public ActionResult ImportDataFromCsv([FromBody]stuExcelData objExcelData)
        {
            ...
            try
            {
                var varResult = clsJournalSubjectBLEx.ImportDataFromCsv(objExcelData.Csv, objExcelData.UserId);
                return Ok(new { errorId = 0, errorMsg = "", returnInt = varResult });
            }
            catch (Exception objException)
            {
