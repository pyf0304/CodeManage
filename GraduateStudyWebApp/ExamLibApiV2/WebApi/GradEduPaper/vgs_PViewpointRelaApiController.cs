 ...
 /*-- -- -- -- -- -- -- -- -- -- --
 类名:vgs_PViewpointRelaApiController
...
{
 /// <summary>
 /// vgs_PViewpointRelaController 的摘要说明
 /// (AutoGCLib.WA_Srv4CSharp:GeneCode)
 /// </summary>
[ApiController]
[Route("[controller]")]
[ApiExplorerSettings(IgnoreApi = true)]
public class  vgs_PViewpointRelaApiController : ControllerBase
{ 

 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/vgs_PViewpointRelaApi/GetObjBymId?mId=value
...
return Ok(new { errorId = 0, errorMsg = "", returnObj = objvgs_PViewpointRelaEN });
 }
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取对象列表
 /// 调用方法: GET /api/vgs_PViewpointRelaApi/GetObjLst?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
List<clsvgs_PViewpointRelaEN> arrvgs_PViewpointRelaObjLst = clsvgs_PViewpointRelaBL.GetObjLst(strWhereCond);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvgs_PViewpointRelaObjLst });
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取对象列表
 /// 调用方法: GET /api/vgs_PViewpointRelaApi/GetObjLst_Cache?
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
List<clsvgs_PViewpointRelaEN> arrvgs_PViewpointRelaObjLst = clsvgs_PViewpointRelaBL.GetObjLstCache();
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvgs_PViewpointRelaObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据关键字列表获取相关对象列表
 /// 调用方法: POST /api/vgs_PViewpointRelaapi/GetObjLstByMIdLst
...
List<long> lstMId = lstMId_Str.Select(x=>long.Parse(x)).ToList();
 try
 {
List<clsvgs_PViewpointRelaEN> arrvgs_PViewpointRelaObjLst = clsvgs_PViewpointRelaBL.GetObjLstByMIdLst(lstMId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvgs_PViewpointRelaObjLst });
 }
 catch (Exception objException)
 {
...
List<long> lstMId = lstMId_Str.Select(x=>long.Parse(x)).ToList();
 try
 {
IEnumerable<clsvgs_PViewpointRelaEN> arrvgs_PViewpointRelaObjLst = clsvgs_PViewpointRelaBL.GetObjLstByMIdLstCache(lstMId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvgs_PViewpointRelaObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取顶部对象列表
 /// 调用方法: GET /api/vgs_PViewpointRelaApi/GetTopObjLst?intTopSize=v1&strOrderBy=v2&strWhereCond=v3&
...
clsPubFun_WebApi.AccessException(strMsg, HttpStatusCode.NotFound);
 ...
 try
 {
List<clsvgs_PViewpointRelaEN> arrvgs_PViewpointRelaObjLst = clsvgs_PViewpointRelaBL.GetTopObjLst(objTopPara);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvgs_PViewpointRelaObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: GET /api/vgs_PViewpointRelaApi/GetFirstID?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 lngmId = clsvgs_PViewpointRelaBL.GetFirstID_S(strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前表满足条件的第一条记录的对象
 /// 调用方法: GET /api/vgs_PViewpointRelaApi/GetFirstObj?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 clsvgs_PViewpointRelaEN objvgs_PViewpointRelaEN = clsvgs_PViewpointRelaBL.GetFirstObj_S(strWhereCond);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objvgs_PViewpointRelaEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/vgs_PViewpointRelaApi/GetObjLstByPager?intPageIndex=v1&intPageSize=v2&strOrderBy=v3&strWhereCond=v4&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 List<clsvgs_PViewpointRelaEN> arrvgs_PViewpointRelaObjLst;
 try
 {
 arrvgs_PViewpointRelaObjLst = clsvgs_PViewpointRelaBL.GetObjLstByPager(objPagerPara);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvgs_PViewpointRelaObjLst });
 ...
 /// <summary>
 /// 功能:获取某一条件的记录数
 /// 调用方法: GET /api/vgs_PViewpointRelaApi/GetRecCountByCond?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
int intCount = clsvgs_PViewpointRelaBL.GetRecCountByCond(strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: GET /api/vgs_PViewpointRelaApi/GetFldValue?strWhereCond=v1&strFldName=v2&strTabName=v3&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
List<string> arrList = clsvgs_PViewpointRelaBL.GetFldValue(strFldName, strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 功能:判断是否存在某一条件的记录
 /// 调用方法: GET /api/vgs_PViewpointRelaApi/IsExistRecord?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
bool bolIsExist = clsvgs_PViewpointRelaBL.IsExistRecord(strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 判断当前表中是否存在给定关键字值的记录
 /// 调用方法: GET /api/vgs_PViewpointRelaApi/IsExist?mId=value
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
bool bolIsExist = clsvgs_PViewpointRelaBL.IsExist(lngmId);
return Ok(new { errorId = 0, errorMsg = "", returnBool = bolIsExist });
 }
 catch (Exception objException)
 {
