 ...
 /*-- -- -- -- -- -- -- -- -- -- --
 类名:vcc_CourseApiController
 表名:vcc_Course(01120058)
 * 版本:2024.02.19.1(服务器:WIN-SRV103-116)
 日期:2024/02/27 16:08:47
 生成者:pyf
 生成服务器IP:
 工程名称:问卷调查(0112)
...
{
 /// <summary>
 /// vcc_CourseController 的摘要说明
 /// (AutoGCLib.WA_Srv4CSharp:GeneCode)
 /// </summary>
[ApiController]
[Route("[controller]")]
[ApiExplorerSettings(IgnoreApi = true)]
public class  vcc_CourseApiController : ControllerBase
{ 

 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/vcc_CourseApi/GetObjByCourseId?CourseId=value
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsvcc_CourseEN objvcc_CourseEN = clsvcc_CourseBL.GetObjByCourseId(strCourseId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objvcc_CourseEN });
 }
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取对象列表
 /// 调用方法: GET /api/vcc_CourseApi/GetObjLst?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
List<clsvcc_CourseEN> arrvcc_CourseObjLst = clsvcc_CourseBL.GetObjLst(strWhereCond);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvcc_CourseObjLst });
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取对象列表
 /// 调用方法: GET /api/vcc_CourseApi/GetObjLst_Cache?
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
List<clsvcc_CourseEN> arrvcc_CourseObjLst = clsvcc_CourseBL.GetObjLstCache();
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvcc_CourseObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据关键字列表获取相关对象列表
 /// 调用方法: POST /api/vcc_Courseapi/GetObjLstByCourseIdLst
...
List<string> lstCourseId = new(arrCourseId);
 try
 {
List<clsvcc_CourseEN> arrvcc_CourseObjLst = clsvcc_CourseBL.GetObjLstByCourseIdLst(lstCourseId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvcc_CourseObjLst });
 }
 catch (Exception objException)
 {
...
List<string> lstCourseId = new(arrCourseId);
 try
 {
IEnumerable<clsvcc_CourseEN> arrvcc_CourseObjLst = clsvcc_CourseBL.GetObjLstByCourseIdLstCache(lstCourseId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvcc_CourseObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取顶部对象列表
 /// 调用方法: GET /api/vcc_CourseApi/GetTopObjLst?intTopSize=v1&strOrderBy=v2&strWhereCond=v3&
...
clsPubFun_WebApi.AccessException(strMsg, HttpStatusCode.NotFound);
 ...
 try
 {
List<clsvcc_CourseEN> arrvcc_CourseObjLst = clsvcc_CourseBL.GetTopObjLst(objTopPara);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvcc_CourseObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: GET /api/vcc_CourseApi/GetFirstID?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 strCourseId = clsvcc_CourseBL.GetFirstID_S(strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前表满足条件的第一条记录的对象
 /// 调用方法: GET /api/vcc_CourseApi/GetFirstObj?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 clsvcc_CourseEN objvcc_CourseEN = clsvcc_CourseBL.GetFirstObj_S(strWhereCond);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objvcc_CourseEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/vcc_CourseApi/GetObjLstByPager?intPageIndex=v1&intPageSize=v2&strOrderBy=v3&strWhereCond=v4&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 List<clsvcc_CourseEN> arrvcc_CourseObjLst;
 try
 {
 arrvcc_CourseObjLst = clsvcc_CourseBL.GetObjLstByPager(objPagerPara);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvcc_CourseObjLst });
 ...
 /// <summary>
 /// 功能:获取某一条件的记录数
 /// 调用方法: GET /api/vcc_CourseApi/GetRecCountByCond?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
int intCount = clsvcc_CourseBL.GetRecCountByCond(strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: GET /api/vcc_CourseApi/GetFldValue?strWhereCond=v1&strFldName=v2&strTabName=v3&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
List<string> arrList = clsvcc_CourseBL.GetFldValue(strFldName, strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 功能:判断是否存在某一条件的记录
 /// 调用方法: GET /api/vcc_CourseApi/IsExistRecord?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
bool bolIsExist = clsvcc_CourseBL.IsExistRecord(strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 判断当前表中是否存在给定关键字值的记录
 /// 调用方法: GET /api/vcc_CourseApi/IsExist?CourseId=value
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
bool bolIsExist = clsvcc_CourseBL.IsExist(strCourseId);
return Ok(new { errorId = 0, errorMsg = "", returnBool = bolIsExist });
 }
 catch (Exception objException)
 {
