...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: Post /api/UserDefaValueApi/EditRecordEx
 /// 在Body区传输objUserDefaValueEN的JSON对象
...
public ActionResult EditRecordEx([FromBody]clsUserDefaValueEN objUserDefaValueEN)
{
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strUserDefaValueJSONObj = clsJSON.GetJsonFromObj(objUserDefaValueEN);
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
    clsUserDefaValueBL.SetUpdFlag(objUserDefaValueEN);
    clsUserDefaValueBL.AccessFldValueNull(objUserDefaValueEN);
bool bolResult = objUserDefaValueEN.EditRecordEx();
return Ok(new { errorId = 0, errorMsg = "", returnBool = bolResult });
 }
 catch (Exception objException)
 {
