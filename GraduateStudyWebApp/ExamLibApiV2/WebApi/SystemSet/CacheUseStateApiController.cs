...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取对象列表
 /// 调用方法: GET /api/CacheUseStateApi/GetObjLst_Cache?
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
clsCacheUseStateEN objCacheUseStateCond = new();
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: Post /api/CacheUseStateApi/EditRecordEx
 /// 在Body区传输objCacheUseStateEN的JSON对象
...
public ActionResult EditRecordEx([FromBody]clsCacheUseStateEN objCacheUseStateEN)
{
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strCacheUseStateJSONObj = clsJSON.GetJsonFromObj(objCacheUseStateEN);
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
    clsCacheUseStateBL.SetUpdFlag(objCacheUseStateEN);
    clsCacheUseStateBL.AccessFldValueNull(objCacheUseStateEN);
bool bolResult = objCacheUseStateEN.EditRecordEx();
return Ok(new { errorId = 0, errorMsg = "", returnBool = bolResult });
 }
 catch (Exception objException)
 {
