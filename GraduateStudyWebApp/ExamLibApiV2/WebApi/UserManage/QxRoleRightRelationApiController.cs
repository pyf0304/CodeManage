...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: Post /api/QxRoleRightRelationApi/EditRecordEx
 /// 在Body区传输objQxRoleRightRelationEN的JSON对象
...
public ActionResult EditRecordEx([FromBody]clsQxRoleRightRelationEN objQxRoleRightRelationEN)
{
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strQxRoleRightRelationJSONObj = clsJSON.GetJsonFromObj(objQxRoleRightRelationEN);
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
    clsQxRoleRightRelationBL.SetUpdFlag(objQxRoleRightRelationEN);
    clsQxRoleRightRelationBL.AccessFldValueNull(objQxRoleRightRelationEN);
bool bolResult = objQxRoleRightRelationEN.EditRecordEx();
return Ok(new { errorId = 0, errorMsg = "", returnBool = bolResult });
 }
 catch (Exception objException)
 {
