 ...
 /*-- -- -- -- -- -- -- -- -- -- --
 类名:vRTUserRelaApiController
 表名:vRTUserRela(01120584)
 * 版本:2024.02.19.1(服务器:WIN-SRV103-116)
 日期:2024/02/27 16:08:20
 生成者:pyf
 生成服务器IP:
 工程名称:问卷调查(0112)
...
{
 /// <summary>
 /// vRTUserRelaController 的摘要说明
 /// (AutoGCLib.WA_Srv4CSharp:GeneCode)
 /// </summary>
[ApiController]
[Route("[controller]")]
[ApiExplorerSettings(IgnoreApi = true)]
public class  vRTUserRelaApiController : ControllerBase
{ 

 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/vRTUserRelaApi/GetObjBymId?mId=value
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
clsvRTUserRelaEN objvRTUserRelaEN = clsvRTUserRelaBL.GetObjBymId(lngmId);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objvRTUserRelaEN });
 }
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取对象列表
 /// 调用方法: GET /api/vRTUserRelaApi/GetObjLst?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
List<clsvRTUserRelaEN> arrvRTUserRelaObjLst = clsvRTUserRelaBL.GetObjLst(strWhereCond);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvRTUserRelaObjLst });
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取对象列表
 /// 调用方法: GET /api/vRTUserRelaApi/GetObjLst_Cache?
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
List<clsvRTUserRelaEN> arrvRTUserRelaObjLst = clsvRTUserRelaBL.GetObjLstCache(strUserId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvRTUserRelaObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据关键字列表获取相关对象列表
 /// 调用方法: POST /api/vRTUserRelaapi/GetObjLstByMIdLst
...
List<long> lstMId = lstMId_Str.Select(x=>long.Parse(x)).ToList();
 try
 {
List<clsvRTUserRelaEN> arrvRTUserRelaObjLst = clsvRTUserRelaBL.GetObjLstByMIdLst(lstMId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvRTUserRelaObjLst });
 }
 catch (Exception objException)
 {
...
List<long> lstMId = lstMId_Str.Select(x=>long.Parse(x)).ToList();
 try
 {
IEnumerable<clsvRTUserRelaEN> arrvRTUserRelaObjLst = clsvRTUserRelaBL.GetObjLstByMIdLstCache(lstMId, UserId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvRTUserRelaObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取顶部对象列表
 /// 调用方法: GET /api/vRTUserRelaApi/GetTopObjLst?intTopSize=v1&strOrderBy=v2&strWhereCond=v3&
...
clsPubFun_WebApi.AccessException(strMsg, HttpStatusCode.NotFound);
 ...
 try
 {
List<clsvRTUserRelaEN> arrvRTUserRelaObjLst = clsvRTUserRelaBL.GetTopObjLst(objTopPara);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvRTUserRelaObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: GET /api/vRTUserRelaApi/GetFirstID?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 lngmId = clsvRTUserRelaBL.GetFirstID_S(strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前表满足条件的第一条记录的对象
 /// 调用方法: GET /api/vRTUserRelaApi/GetFirstObj?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 clsvRTUserRelaEN objvRTUserRelaEN = clsvRTUserRelaBL.GetFirstObj_S(strWhereCond);
return Ok(new { errorId = 0, errorMsg = "", returnObj = objvRTUserRelaEN });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/vRTUserRelaApi/GetObjLstByPager?intPageIndex=v1&intPageSize=v2&strOrderBy=v3&strWhereCond=v4&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 List<clsvRTUserRelaEN> arrvRTUserRelaObjLst;
 try
 {
 arrvRTUserRelaObjLst = clsvRTUserRelaBL.GetObjLstByPager(objPagerPara);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvRTUserRelaObjLst });
 ...
 /// <summary>
 /// 功能:获取某一条件的记录数
 /// 调用方法: GET /api/vRTUserRelaApi/GetRecCountByCond?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
int intCount = clsvRTUserRelaBL.GetRecCountByCond(strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: GET /api/vRTUserRelaApi/GetFldValue?strWhereCond=v1&strFldName=v2&strTabName=v3&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
List<string> arrList = clsvRTUserRelaBL.GetFldValue(strFldName, strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 功能:判断是否存在某一条件的记录
 /// 调用方法: GET /api/vRTUserRelaApi/IsExistRecord?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
bool bolIsExist = clsvRTUserRelaBL.IsExistRecord(strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 判断当前表中是否存在给定关键字值的记录
 /// 调用方法: GET /api/vRTUserRelaApi/IsExist?mId=value
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
bool bolIsExist = clsvRTUserRelaBL.IsExist(lngmId);
return Ok(new { errorId = 0, errorMsg = "", returnBool = bolIsExist });
 }
 catch (Exception objException)
 {
