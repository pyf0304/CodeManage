...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
List<clsQxUserRoleRelationEN> arrQxUserRoleRelationObjLst = clsQxUserRoleRelationBL.GetObjLstCache();
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrQxUserRoleRelationObjLst });
 }
 catch (Exception objException)
 {
...
List<long> lstMId = lstMId_Str.Select(x=>long.Parse(x)).ToList();
 try
 {
IEnumerable<clsQxUserRoleRelationEN> arrQxUserRoleRelationObjLst = clsQxUserRoleRelationBL.GetObjLstByMIdLstCache(lstMId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrQxUserRoleRelationObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: Post /api/QxUserRoleRelationApi/EditRecordEx
 /// 在Body区传输objQxUserRoleRelationEN的JSON对象
...
public ActionResult EditRecordEx([FromBody]clsQxUserRoleRelationEN objQxUserRoleRelationEN)
{
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strQxUserRoleRelationJSONObj = clsJSON.GetJsonFromObj(objQxUserRoleRelationEN);
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
    clsQxUserRoleRelationBL.SetUpdFlag(objQxUserRoleRelationEN);
    clsQxUserRoleRelationBL.AccessFldValueNull(objQxUserRoleRelationEN);
bool bolResult = objQxUserRoleRelationEN.EditRecordEx();
return Ok(new { errorId = 0, errorMsg = "", returnBool = bolResult });
 }
 catch (Exception objException)
 {
