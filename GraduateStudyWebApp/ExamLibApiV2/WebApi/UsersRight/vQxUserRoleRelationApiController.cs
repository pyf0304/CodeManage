...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
List<clsvQxUserRoleRelationEN> arrvQxUserRoleRelationObjLst = clsvQxUserRoleRelationBL.GetObjLstCache();
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvQxUserRoleRelationObjLst });
 }
 catch (Exception objException)
 {
...
List<long> lstMId = lstMId_Str.Select(x=>long.Parse(x)).ToList();
 try
 {
IEnumerable<clsvQxUserRoleRelationEN> arrvQxUserRoleRelationObjLst = clsvQxUserRoleRelationBL.GetObjLstByMIdLstCache(lstMId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvQxUserRoleRelationObjLst });
 }
 catch (Exception objException)
 {
