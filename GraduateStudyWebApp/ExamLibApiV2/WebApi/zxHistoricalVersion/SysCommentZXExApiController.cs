...
namespace ExamLib.WebApi
{
    [ApiController]
    [Route("[controller]")]
    public class SysCommentZXExApiController : ControllerBase
    {
        ...
        public ActionResult AddNewRecordEx([FromBody]clsSysCommentEN objSysCommentEN)
        {
            ...
            try
            {
                ...
                bool i = clsSysCommentZxBLEx.PersonalScoreStatistics(strUserId, strTableKeyId, strCommentTypeId, strParentId, stridCurrEduCls);
                if (i == true)
                {
                    bolResult = true;
                }
        ...
        public ActionResult DelRecordEx(string Id)
        {
            ...
            try
            {
                int intRecNum = clsSysCommentZxBLEx.DelRecord(strCommentId);
        ...
        #region  用于总分核算的方法 UpdUserId用于记录当前是谁在核算操作；
        [HttpGet("GetTotalRevalidation")]
        public ActionResult GetTotalRevalidation(string UpdUserId)
        {
            string strFunctionName = clsStackTrace.GetCurrFunction();
            Dictionary<string, string> dictParam = new Dictionary<string, string>();
            ...
            try
            {
                #region 获取教学班数据
                //1.获取评分教学班数据
                bool i = clsSysCommentZxBLEx.GetTotalRevalidation(UpdUserId);
                if (i == true)
                {
                    varResult = true;
        ...
        public ActionResult UpdateCommentWeek(string strIdCurrEduCls, string strUpdUser)
        {
            ...
            try
            {
                ...
                bool i = clsSysCommentZxBLEx.UpdateCommentWeek(strIdCurrEduCls, strUpdUser);
                if (i == true)
                {
                    varResult = true;
