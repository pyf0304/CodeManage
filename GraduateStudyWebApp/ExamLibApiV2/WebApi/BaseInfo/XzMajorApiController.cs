...
{ 
 ...
 /// 调用方法: GET /api/XzMajorApi/GetObjByIdXzMajor?IdXzMajor=value
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据关键字列表获取相关对象列表
 /// 调用方法: POST /api/XzMajorapi/GetObjLstByIdXzMajorLst
...
List<string> lstIdXzMajor = new(arrIdXzMajor);
 try
 {
List<clsXzMajorEN> arrXzMajorObjLst = clsXzMajorBL.GetObjLstByIdXzMajorLst(lstIdXzMajor);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrXzMajorObjLst });
 }
 catch (Exception objException)
 {
...
List<string> lstIdXzMajor = new(arrIdXzMajor);
 try
 {
IEnumerable<clsXzMajorEN> arrXzMajorObjLst = clsXzMajorBL.GetObjLstByIdXzMajorLstCache(lstIdXzMajor);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrXzMajorObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 strIdXzMajor = clsXzMajorBL.GetFirstID_S(strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 判断当前表中是否存在给定关键字值的记录
 /// 调用方法: GET /api/XzMajorApi/IsExist?IdXzMajor=value
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
bool bolIsExist = clsXzMajorBL.IsExist(strIdXzMajor);
return Ok(new { errorId = 0, errorMsg = "", returnBool = bolIsExist });
 }
 catch (Exception objException)
 {
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
string strMaxstrIdXzMajor = clsXzMajorBL.GetMaxStrId_S();
return Ok(new { errorId = 0, errorMsg = "", returnStr =  strMaxstrIdXzMajor });
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 ...
 {
    clsXzMajorBL.AccessFldValueNull(objXzMajorEN);
string strIdXzMajor = objXzMajorEN.AddNewRecordWithMaxId();
return Ok(new { errorId = 0, errorMsg = "", returnStr =  strIdXzMajor });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: Post /api/XzMajorApi/EditRecordEx
 /// 在Body区传输objXzMajorEN的JSON对象
...
public ActionResult EditRecordEx([FromBody]clsXzMajorEN objXzMajorEN)
{
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
string strXzMajorJSONObj = clsJSON.GetJsonFromObj(objXzMajorEN);
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
    clsXzMajorBL.SetUpdFlag(objXzMajorEN);
    clsXzMajorBL.AccessFldValueNull(objXzMajorEN);
bool bolResult = objXzMajorEN.EditRecordEx();
return Ok(new { errorId = 0, errorMsg = "", returnBool = bolResult });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
int intRecNum = clsXzMajorBL.DelRecord(strIdXzMajor);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 在Body区传输arrIdXzMajor(关键字值列表)的JSON串
 /// (AutoGCLib.WA_Srv4CSharp:Gen_DelMultiRecord)
 /// </summary>
 /// <param name = "arrIdXzMajor">给定的关键字值列表</param>
 /// <returns>返回删除的记录数</returns>
[HttpPost("DelXzMajors")]
public ActionResult DelXzMajors([FromBody]string[] arrIdXzMajor)
{
string strFunctionName = clsStackTrace.GetCurrFunction();
Dictionary<string, string> dictParam = new();
List<string> arrLst = new(arrIdXzMajor);
