 ...
 /*-- -- -- -- -- -- -- -- -- -- --
 类名:vQxUsersApiController
 表名:vQxUsers(01120564)
 * 版本:2024.02.19.1(服务器:WIN-SRV103-116)
 日期:2024/02/27 16:07:48
 生成者:pyf
 生成服务器IP:
 工程名称:问卷调查(0112)
...
{
 /// <summary>
 /// vQxUsersController 的摘要说明
 /// (AutoGCLib.WA_Srv4CSharp:GeneCode)
 /// </summary>
[ApiController]
[Route("[controller]")]
[ApiExplorerSettings(IgnoreApi = true)]
public class  vQxUsersApiController : ControllerBase
{ 

 /// <summary>
 /// 获取当前关键字的记录对象,用对象的形式表示.
 /// 调用方法: GET /api/vQxUsersApi/GetObjByUserId?UserId=value
...
return Ok(new { errorId = 0, errorMsg = "", returnObj = objvQxUsersEN });
 }
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取对象列表
 /// 调用方法: GET /api/vQxUsersApi/GetObjLst?strWhereCond=v1&
...
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvQxUsersObjLst });
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取对象列表
 /// 调用方法: GET /api/vQxUsersApi/GetObjLst_Cache?
...
clsPubFun_WebApi.Log4Debug(this, strFunctionName, dictParam);
 try
 {
List<clsvQxUsersEN> arrvQxUsersObjLst = clsvQxUsersBL.GetObjLstCache();
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvQxUsersObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据关键字列表获取相关对象列表
 /// 调用方法: POST /api/vQxUsersapi/GetObjLstByUserIdLst
...
List<string> lstUserId = new(arrUserId);
 try
 {
List<clsvQxUsersEN> arrvQxUsersObjLst = clsvQxUsersBL.GetObjLstByUserIdLst(lstUserId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvQxUsersObjLst });
 }
 catch (Exception objException)
 {
...
List<string> lstUserId = new(arrUserId);
 try
 {
IEnumerable<clsvQxUsersEN> arrvQxUsersObjLst = clsvQxUsersBL.GetObjLstByUserIdLstCache(lstUserId);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvQxUsersObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件获取顶部对象列表
 /// 调用方法: GET /api/vQxUsersApi/GetTopObjLst?intTopSize=v1&strOrderBy=v2&strWhereCond=v3&
...
clsPubFun_WebApi.AccessException(strMsg, HttpStatusCode.NotFound);
 ...
 try
 {
List<clsvQxUsersEN> arrvQxUsersObjLst = clsvQxUsersBL.GetTopObjLst(objTopPara);
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvQxUsersObjLst });
 }
 catch (Exception objException)
 {
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: GET /api/vQxUsersApi/GetFirstID?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 获取当前表满足条件的第一条记录的对象
 /// 调用方法: GET /api/vQxUsersApi/GetFirstObj?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 根据条件分页获取JSON对象列表
 /// 调用方法: GET /api/vQxUsersApi/GetObjLstByPager?intPageIndex=v1&intPageSize=v2&strOrderBy=v3&strWhereCond=v4&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
 arrvQxUsersObjLst = clsvQxUsersBL.GetObjLstByPager(objPagerPara);
 }
 catch (Exception objException)
 {
string strMsg = string.Format("{0}.(from {1})", objException.Message,  clsStackTrace.GetCurrClassFunction());
...
return Ok(new { errorId = 0, errorMsg = "", returnObjLst = arrvQxUsersObjLst });
 ...
 /// <summary>
 /// 功能:获取某一条件的记录数
 /// 调用方法: GET /api/vQxUsersApi/GetRecCountByCond?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// 调用方法: GET /api/vQxUsersApi/GetFldValue?strWhereCond=v1&strFldName=v2&strTabName=v3&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 try
 {
List<string> arrList = clsvQxUsersBL.GetFldValue(strFldName, strWhereCond);
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 功能:判断是否存在某一条件的记录
 /// 调用方法: GET /api/vQxUsersApi/IsExistRecord?strWhereCond=v1&
...
return Ok(new { errorId = 1, errorMsg = strMsg });
 ...
 /// <summary>
 /// 判断当前表中是否存在给定关键字值的记录
 /// 调用方法: GET /api/vQxUsersApi/IsExist?UserId=value
